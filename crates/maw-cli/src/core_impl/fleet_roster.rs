...
struct FleetRosterMemberView {
    member: NativeFleetMember,
    node: Option<String>,
    is_live: Option<bool>,
}
...
fn fleet_roster_remove_run(argv: &[String]) -> Result<(i32, String), String> {
    ...
    let (Some(group), Some(handle), None) =
...
fn fleet_roster_positionals(argv: &[String], command: &str) -> Result<Vec<String>, String> {
    ...
    for arg in argv {
        if arg.starts_with('-') {
            return Err(format!(
                "fleet {command}: unknown argument {arg}\n{FLEET_ROSTER_USAGE}"
            ));
        }
...
fn fleet_roster_remove_member(
    ...
    let text = std::fs::read_to_string(&entry.path)
        .map_err(|error| format!("fleet {verb}: read {}: {error}", entry.path.display()))?;
    let mut value: serde_json::Value = serde_json::from_str(&text)
        .map_err(|error| format!("fleet {verb}: parse {}: {error}", entry.path.display()))?;
    let object = value
        .as_object_mut()
        .ok_or_else(|| format!("fleet {verb}: fleet file is not a JSON object"))?;
    let members = object
        .get_mut("members")
        .and_then(serde_json::Value::as_array_mut)
        .ok_or_else(|| format!("fleet {verb}: squad {group} has no members roster"))?;
    let before = members.len();
    members
        .retain(|member| member.get("handle").and_then(serde_json::Value::as_str) != Some(handle));
    ...
    Ok((
        0,
        format!("fleet {verb} {group}: {handle} removed ({count} members)\n"),
    ))
}

fn fleet_roster_find_entry(
    env: &MawXdgEnv,
    group: &str,
    verb: &str,
) -> Result<NativeFleetEntry, String> {
    fleet_load_entries_result_for_env(env, &format!("fleet {verb}"))?
        .into_iter()
        .find(|entry| fleet_roster_entry_matches(entry, group))
        .ok_or_else(|| {
            format!("fleet {verb}: no squad named {group} — try: maw fleet create {group}")
        })
}
...
fn fleet_roster_run(argv: &[String]) -> Result<(i32, String), String> {
    ...
    while idx < argv.len() {
        match argv[idx].as_str() {
            ...
            "--token" => {
                idx += 1;
                let Some(value) = argv.get(idx) else {
                    return Err("fleet create: --token requires a value".to_owned());
                };
...
fn fleet_roster_create_with_token(
    env: &MawXdgEnv,
    group: &str,
    token: Option<&str>,
) -> Result<(i32, String), String> {
    ...
    if entries
        .iter()
        .any(|entry| fleet_roster_entry_matches(entry, group))
    {
    ...
    let dir = maw_state_path(env, &["fleet"])
        .join("squads")
        .join(format!("{nn:02}-{group}"));
    std::fs::create_dir_all(&dir)
        .map_err(|error| format!("fleet create: create {}: {error}", dir.display()))?;
    let path = dir.join("squad.json");
    let body = fleet_roster_new_file_json_with_token(nn, group, &fleet_registry_now_iso(), token)?;
    std::fs::write(&path, body)
        .map_err(|error| format!("fleet create: write {}: {error}", path.display()))?;
    Ok((
        0,
        format!("fleet create {group}: created {}\n", path.display()),
    ))
}
...
fn fleet_roster_new_file_json_with_token(
    nn: u32,
    group: &str,
    created_at: &str,
    token: Option<&str>,
        ...
        if let Ok(entries) = std::fs::read_dir(&dir) {
            used.extend(
                entries.flatten().filter_map(|entry| {
                    fleet_roster_nn_prefix(&entry.file_name().to_string_lossy())
                }),
            );
        }
...
fn fleet_roster_squad_name(entry: &NativeFleetEntry) -> Option<String> {
    ...
    if !entry.session.squad_name.is_empty() {
        return Some(entry.session.squad_name.clone());
    }
    Some(fleet_roster_unnumbered_stem(entry).to_owned())
}

fn fleet_roster_show(
    env: &MawXdgEnv,
    group: &str,
    json: bool,
    live: Option<&[TmuxSession]>,
) -> Result<(i32, String), String> {
    ...
    let members = entry
        ...
        .collect::<Vec<_>>();
    if json {
        return fleet_roster_json(group, &entry, &members);
    }
    Ok((0, fleet_roster_render(group, &entry, &members)))
}

fn fleet_roster_member_view(
    member: NativeFleetMember,
    cache: Option<&LocateRegistryCache>,
    live: Option<&[TmuxSession]>,
) -> FleetRosterMemberView {
    let cached = cache.and_then(|cache| {
        cache
            .oracles
            .iter()
            .find(|oracle| oracle.name == member.handle)
    });
    let node = member
        .node
        .clone()
        .or_else(|| cached.and_then(|oracle| oracle.federation_node.clone()));
...
fn fleet_roster_json(
    group: &str,
    entry: &NativeFleetEntry,
    members: &[FleetRosterMemberView],
) -> Result<(i32, String), String> {
...
fn fleet_roster_member_json(view: &FleetRosterMemberView) -> serde_json::Value {
    ...
    if let Some(node) = &view.node {
        value["node"] = serde_json::json!(node);
    }
    if let Some(is_live) = view.is_live {
        value["is_live"] = serde_json::json!(is_live);
    }
    value
}

fn fleet_roster_render(
    group: &str,
    entry: &NativeFleetEntry,
    members: &[FleetRosterMemberView],
) -> String {
    ...
    for view in members {
        ...
        if let Some(role) = &view.member.role {
            let _ = write!(out, " [{role}]");
        }
        if let Some(node) = &view.node {
            let _ = write!(out, " node={node}");
        }
        if let Some(live) = view.is_live {
            let _ = write!(out, " live={}", if live { "yes" } else { "no" });
        }
        out.push('\n');
    }
    out
...
mod fleet_roster_tests {
    ...
    const ROSTER_LEGACY_FIXTURE: &str =
        include_str!("../../tests/fixtures/native-fleet-roster/legacy-03-alpha.json");
    const ROSTER_SQUAD_FIXTURE: &str =
        include_str!("../../tests/fixtures/native-fleet-roster/squads/01-3e/squad.json");

    fn roster_args(values: &[&str]) -> Vec<String> {
        values.iter().map(|value| (*value).to_owned()).collect()
    }

    fn roster_env(name: &str) -> (std::path::PathBuf, Vec<EnvVarRestore>) {
        let root =
            std::env::temp_dir().join(format!("maw-rs-fleet-roster-{name}-{}", std::process::id()));
    ...
    fn fleet_roster_legacy_fixture_parses_without_members() {
        let session: NativeFleetSession =
            serde_json::from_str(ROSTER_LEGACY_FIXTURE).expect("legacy parse");
    ...
    fn fleet_roster_new_file_json_is_deterministic_and_round_trips() {
        let first =
            fleet_roster_new_file_json(1, "3e", "2026-07-08T00:00:00.000Z").expect("render");
        assert_eq!(
            first,
            fleet_roster_new_file_json(1, "3e", "2026-07-08T00:00:00.000Z").expect("render")
        );
    ...
    fn fleet_create_show_round_trip_cache_resolution_and_legacy_files() {
        ...
        assert_eq!(
            run_fleet_command(&roster_args(&["create", "3e"])).code,
            1,
            "duplicate squad refused"
        );
        ...
        assert_eq!(
            session.members.expect("members")[0].token.as_deref(),
            Some("dd2")
        );
        ...
        std::fs::write(
            root.join("state/fleet/squads/05-ccdc/squad.json"),
            roster_json,
        )
        .expect("squad file");
        std::fs::write(
            root.join("state/fleet/03-alpha.json"),
            ROSTER_LEGACY_FIXTURE,
        )
        .expect("legacy file");
        ...
        assert_eq!(
            roster["members"][0]["node"], "white",
            "resolved via oracles.json cache"
        );
        assert!(
            roster["members"][0].get("is_live").is_none(),
            "show has no liveness"
        );
        assert_eq!(roster["members"][1]["node"], "mba", "explicit node wins");
        let legacy = run_fleet_command(&roster_args(&["show", "alpha", "--json"]));
        assert_eq!(
            legacy.code, 1,
            "flat session snapshots are not squad rosters"
        );
    }
    ...
    fn fleet_roster_squads_fixture_and_flat_roster_migrates() {
        ...
        std::fs::write(
            fleet_dir.join("groups/01-3e/group.json"),
            ROSTER_SQUAD_FIXTURE,
        )
        .expect("legacy group fixture");
        std::fs::write(
            fleet_dir.join("01-3e.json"),
            r#"{"name":"01-3e","groupName":"3e","windows":[],"members":[]}"#,
        )
        .expect("duplicate flat roster");
        let shown: serde_json::Value = serde_json::from_str(
            &run_fleet_command(&roster_args(&["show", "3e", "--json"])).stdout,
        )
        .expect("json");
        ...
        std::fs::write(
            fleet_dir.join("02-flat.json"),
            r#"{"name":"02-flat","groupName":"flat","windows":[],"members":[{"handle":"one"}]}"#,
        )
        .expect("flat roster");
        ...
        let migrated_file = std::fs::read_to_string(fleet_dir.join("squads/02-flat/squad.json"))
            .expect("migrated flat");
        assert!(
            migrated_file.contains(r#""squadName": "flat""#),
            "{migrated_file}"
        );
        assert!(!migrated_file.contains("groupName"), "{migrated_file}");
        assert_eq!(
            serde_json::from_str::<serde_json::Value>(&migrated.stdout).expect("json")
                ["memberCount"],
            1
        );
    }
    ...
    fn fleet_remove_and_leave_round_trip_roster_members() {
        ...
        assert!(removed
            .stdout
            .contains("fleet remove ccdc: ghost removed (2 members)"));
        let missing = run_fleet_command(&roster_args(&["remove", "ccdc", "ghost"]));
        assert_eq!(missing.code, 1);
        assert!(
            missing.stderr.contains("unknown member ghost"),
            "{}",
            missing.stderr
        );
        ...
        assert!(left
            .stdout
            .contains("fleet leave ccdc: drift removed (1 members)"));
        ...
        let file: serde_json::Value = serde_json::from_str(
            &std::fs::read_to_string(root.join("state/fleet/squads/05-ccdc/squad.json"))
                .expect("roster file"),
        )
        .expect("roster json");
    ...
    fn fleet_status_marks_live_members_from_tmux_inventory() {
        let sessions = vec![TmuxSession {
            name: "12-atlas".to_owned(),
            windows: Vec::new(),
        }];
        let atlas = NativeFleetMember {
            handle: "atlas".to_owned(),
            ..NativeFleetMember::default()
        };
        let view = fleet_roster_member_view(atlas, None, Some(&sessions));
        assert_eq!((view.is_live, view.node), (Some(true), None));
        let ghost = NativeFleetMember {
            handle: "ghost".to_owned(),
            ..NativeFleetMember::default()
        };
        assert_eq!(
            fleet_roster_member_view(ghost, None, Some(&sessions)).is_live,
            Some(false)
        );
        assert_eq!(
            fleet_roster_member_view(NativeFleetMember::default(), None, None).is_live,
            None
        );
    }
}
