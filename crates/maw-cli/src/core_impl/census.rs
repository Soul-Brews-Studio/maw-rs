const DISPATCH_380: &[DispatcherEntry] = &[DispatcherEntry {
    command: "census",
    handler: Handler::Sync(census_run_command),
}];
...
struct CensusSpace {
    display: String,
    space: String,
    oracles: Vec<CensusOracle>,
}

fn census_run_command(argv: &[String]) -> CliOutput {
    let json = match census_parse_args(argv) {
        Ok(json) => json,
    ...
    let options = LsPlanOptions {
        ...
        now: Some(current_epoch_seconds()),
        panes,
    ...
    CliOutput {
        code: 0,
        stdout: if json {
            census_render_json(&spaces)
        } else {
            census_render_text(&spaces)
        },
        stderr: String::new(),
    }
}
        ...
        match arg.as_str() {
            ...
            _ => {
                return Err(CliOutput {
                    code: 2,
                    stdout: String::new(),
                    stderr: format!("census: unknown argument {arg}\nusage: maw census [--json]\n"),
                })
            }
        }
    }
...
fn census_default_pins_path() -> std::path::PathBuf {
    std::env::var_os("HOME").map_or_else(
        || std::path::PathBuf::from(".config/window-arranger-oracle/pins.json"),
        |home| std::path::PathBuf::from(home).join(".config/window-arranger-oracle/pins.json"),
    )
}
...
fn census_parse_pins_value(value: &serde_json::Value) -> Vec<CensusPin> {
    let rows = value
        .as_array()
        .or_else(|| value.get("pins").and_then(serde_json::Value::as_array));
    rows.into_iter()
        .flatten()
        .filter_map(census_parse_pin)
        .collect()
}
...
fn census_parse_pin(value: &serde_json::Value) -> Option<CensusPin> {
    ...
    Some(CensusPin {
        display: census_json_str(value, &["displayName", "display", "displayId"])
            .unwrap_or("unassigned")
            .to_owned(),
        space: census_json_str(value, &["space", "spaceName", "grid"])
            .unwrap_or("default")
            .to_owned(),
...
fn census_join(
    ...
    for pane in panes.iter().filter(|pane| {
        pane.agent
            || pane.session.ends_with("-oracle")
            || is_default_ls_oracle_session(&pane.session)
    }) {
        let oracle = census_oracle_name(&pane.session);
        let pin = pins.iter().find(|pin| {
            census_same_oracle(&pin.oracle, &oracle)
                || census_same_oracle(&pin.oracle, &pane.session)
        });
        let key = pin.map_or_else(
            || ("unassigned".to_owned(), "live".to_owned()),
            |pin| {
                pinned.insert(census_norm(&pin.oracle));
                (pin.display.clone(), pin.space.clone())
            },
        );
        groups.entry(key).or_default().push(CensusOracle {
            oracle,
            session: Some(pane.session.clone()),
            pane: Some(pane.id.clone()),
            model_tier: census_model_tier(&pane.command),
            status: pane.status.to_owned(),
            idle_sec: pane.age_sec,
            annotation: ls_pane_annotation(pane, annotations),
            pinned: pin.is_some(),
        });
    }
    for pin in pins
        .iter()
        .filter(|pin| !pinned.contains(&census_norm(&pin.oracle)))
    {
        groups
            .entry((pin.display.clone(), pin.space.clone()))
            .or_default()
            .push(CensusOracle {
                oracle: pin.oracle.clone(),
                session: None,
                pane: None,
                model_tier: "unknown".to_owned(),
                status: "pinned".to_owned(),
                idle_sec: None,
                annotation: pin.note.clone().unwrap_or_default(),
                pinned: true,
            });
    }
    groups
        .into_iter()
        .map(|((display, space), mut oracles)| {
            oracles.sort_by(|a, b| a.oracle.cmp(&b.oracle));
            CensusSpace {
                display,
                space,
                oracles,
...
fn census_oracle_name(session: &str) -> String {
    let stem = session
        ...
        .map_or(session, |(_, stem)| stem);
    stem.strip_suffix("-oracle").unwrap_or(stem).to_owned()
}

fn census_norm(value: &str) -> String {
    census_oracle_name(value)
        .to_lowercase()
        .replace(['_', ' '], "-")
}
fn census_same_oracle(left: &str, right: &str) -> bool {
    census_norm(left) == census_norm(right)
}

fn census_model_tier(command: &str) -> String {
    let command = command.to_lowercase();
    if command.contains("opus") {
        "opus"
    } else if command.contains("sonnet") {
        "sonnet"
    } else if command.contains("haiku") {
        "haiku"
    } else if command.contains("gpt-5") {
        "gpt-5"
...
fn census_render_text(spaces: &[CensusSpace]) -> String {
    if spaces.is_empty() {
        return "maw census: no oracle panes or pins found\n".to_owned();
    }
    let mut out = "maw census\n".to_owned();
    let mut current = "";
    for space in spaces {
        if current != space.display {
        ...
        for oracle in &space.oracles {
            ...
            let idle = oracle
                .idle_sec
                .map_or("-".to_owned(), |age| format_ls_age(Some(age)));
            let pin = if oracle.pinned { "📌" } else { " " };
            let _ = writeln!(
                out,
                "    {pin} {:<22} {:<8} {:<6} {:<8} {}",
                oracle.oracle, oracle.status, idle, pane, oracle.annotation
            );
        }
    }
    out
...
fn census_render_json(spaces: &[CensusSpace]) -> String {
    ...
    for space in spaces {
        by_display.entry(&space.display).or_default().push(space);
    }
    let displays = by_display
        .into_iter()
        .map(|(display, spaces)| {
            let spaces = spaces
                .iter()
                .map(|space| {
                    format!(
                        ...
                        census_oracles_json(&space.oracles)
                    )
                })
                .collect::<Vec<_>>();
            format!(
...
mod census_tests {
    ...
    fn fixture_panes() -> Vec<LsPanePlan> {
        let value: serde_json::Value =
            serde_json::from_str(include_str!("../../tests/fixtures/census/ls-panes.json"))
                .expect("ls fixture");
        value
            .as_array()
            .expect("rows")
            ...
            .map(|row| {
                ...
                LsPanePlan {
                    ...
                    command: row["command"].as_str().unwrap_or_default().to_owned(),
                    title: String::new(),
                    source: None,
                    last_activity: None,
    ...
    fn census_join_places_live_and_pinned_oracles() {
        let pins = census_parse_pins_value(
            &serde_json::from_str(include_str!("../../tests/fixtures/census/pins.json"))
                .expect("pins"),
        );
        ...
        assert!(spaces[1]
            .oracles
            .iter()
            .any(|oracle| oracle.oracle == "zai" && oracle.session.is_some()));
        assert!(spaces[1]
            .oracles
            .iter()
            .any(|oracle| oracle.oracle == "ghost" && oracle.status == "pinned"));
    }

    #[test]
    fn census_json_renders_contract_schema() {
        let spaces = vec![CensusSpace {
            display: "D1".to_owned(),
            space: "S1".to_owned(),
            oracles: vec![CensusOracle {
                oracle: "athena".to_owned(),
                session: Some("01-athena".to_owned()),
                pane: Some("%1".to_owned()),
                model_tier: "opus".to_owned(),
                status: "active".to_owned(),
                idle_sec: Some(1),
                annotation: "fleet: athena".to_owned(),
                pinned: true,
            }],
        }];
        let value: serde_json::Value =
            serde_json::from_str(&census_render_json(&spaces)).expect("json");
        assert_eq!(value["schema"], "maw.census.v1");
        assert_eq!(
            value["displays"][0]["spaces"][0]["oracles"][0]["oracle"],
            "athena"
        );
    }
}
