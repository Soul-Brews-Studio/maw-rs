const DISPATCH_103: &[DispatcherEntry] = &[
    DispatcherEntry {
        command: "scout",
        handler: Handler::Async(zenohscout_async_native),
    },
    DispatcherEntry {
        command: "zenoh-scout",
        handler: Handler::Async(zenohscout_async_native),
...
enum ZenohScoutTransportChoice {
    Zenoh,
    Scout,
    Both,
}
...
fn zenohscout_parse_args(argv: &[String]) -> Result<ZenohScoutArgs, String> {
    ...
    while index < argv.len() {
        match argv[index].as_str() {
            ...
            "--transport" => {
                parsed.transport = zenohscout_parse_transport(&zenohscout_take_value(
                    argv,
                    index,
                    "--transport",
                )?)?;
            ...
            "--locator" => {
                parsed.locator = Some(zenohscout_validate_locator(&zenohscout_take_value(
                    argv,
                    index,
                    "--locator",
                )?)?);
            ...
            "--timeout" => {
                parsed.timeout_ms = Some(zenohscout_parse_timeout(&zenohscout_take_value(
                    argv,
                    index,
                    "--timeout",
                )?)?);
            ...
            "--limit" => {
                parsed.limit = Some(zenohscout_parse_limit(&zenohscout_take_value(
                    argv, index, "--limit",
                )?)?);
                index += 1;
            }
            value if value.starts_with("--transport=") => {
                parsed.transport = zenohscout_parse_transport(&value["--transport=".len()..])?;
            }
            value if value.starts_with("--locator=") => {
                parsed.locator = Some(zenohscout_validate_locator(&value["--locator=".len()..])?);
            }
            value if value.starts_with("--timeout=") => {
                parsed.timeout_ms = Some(zenohscout_parse_timeout(&value["--timeout=".len()..])?);
            }
            value if value.starts_with("--limit=") => {
                parsed.limit = Some(zenohscout_parse_limit(&value["--limit=".len()..])?);
            }
            value if value.starts_with('-') => {
                return Err(format!("zenoh-scout: unknown argument {value}"))
            }
...
fn zenohscout_take_value(argv: &[String], index: usize, flag: &str) -> Result<String, String> {
    let Some(value) = argv.get(index + 1) else {
        return Err(format!("zenoh-scout: missing {flag} value"));
    };
...
fn zenohscout_parse_transport(value: &str) -> Result<ZenohScoutTransportChoice, String> {
    if value.starts_with('-') || value.chars().any(char::is_control) {
        return Err("zenoh-scout: invalid transport".to_owned());
    }
    match value {
...
fn zenohscout_validate_locator(value: &str) -> Result<String, String> {
    ...
    if trimmed.is_empty() {
        return Err("zenoh-scout: locator is required".to_owned());
    }
    ...
    if trimmed.len() > 2048 {
        return Err("zenoh-scout: locator is too long".to_owned());
    }
    if !trimmed.contains("://") {
        return Err("zenoh-scout: locator must include a scheme".to_owned());
    }
    Ok(trimmed.to_owned())
}
...
fn zenohscout_parse_timeout(value: &str) -> Result<u64, String> {
    let timeout = value
        .parse::<u64>()
        .map_err(|_| "zenoh-scout: timeout must be a positive number".to_owned())?;
    if timeout == 0 {
        return Err("zenoh-scout: timeout must be a positive number".to_owned());
    }
...
fn zenohscout_parse_limit(value: &str) -> Result<usize, String> {
    let limit = value
        .parse::<usize>()
        .map_err(|_| "zenoh-scout: limit must be a positive number".to_owned())?;
    if limit == 0 {
        return Err("zenoh-scout: limit must be a positive number".to_owned());
    }
    Ok(limit.min(500))
}
...
fn zenohscout_read_config() -> ZenohScoutConfigNative {
    ...
    ZenohScoutConfigNative {
        enabled: scout
            ...
            == Some(true),
        locator: zenohscout_config_string(scout, "locator")
            .or_else(|| zenohscout_config_string(zenoh, "locator"))
            .unwrap_or_else(|| ZENOHSCOUT_DEFAULT_LOCATOR.to_owned()),
        timeout_ms: zenohscout_config_u64(scout, "timeoutMs")
            .unwrap_or(ZENOHSCOUT_DEFAULT_TIMEOUT_MS)
            .max(1),
        key_prefix: zenohscout_config_string(scout, "keyPrefix")
            .map(|value| value.trim_end_matches('/').to_owned())
            .filter(|value| !value.is_empty())
            .unwrap_or_else(|| ZENOHSCOUT_DEFAULT_KEY_PREFIX.to_owned()),
        node: node.to_owned(),
...
fn zenohscout_status_result(
    base: &ZenohScoutConfigNative,
    locator: &str,
) -> ZenohScoutResultNative {
    let hint = if base.enabled {
...
fn zenohscout_run_zenoh(
    ...
    if advertise {
        let _key = zenohscout_discovery_key(&config);
    }
...
fn zenohscout_fake_result(config: &ZenohScoutConfigNative, raw: &str) -> ZenohScoutResultNative {
    ...
    let mut peers = raw
        .lines()
        .filter_map(|line| zenohscout_parse_discovery_key(line.trim(), &config.key_prefix, now))
        .filter(|peer| peer.node != config.node || peer.oracle != config.oracle)
        .collect::<Vec<_>>();
    peers.sort_by(|left, right| {
        left.node
            .cmp(&right.node)
            .then(left.oracle.cmp(&right.oracle))
    });
    ZenohScoutResultNative {
        ok: true,
        enabled: true,
...
async fn zenohscout_execute_scout(args: &ZenohScoutArgs, timeout_ms: u64) -> CliOutput {
    let scout = zenohscout_fetch_discoveries(args.all, args.limit, timeout_ms).await;
    if args.json {
        return zenohscout_ok(&zenohscout_json(&scout));
    }
    match scout {
        ZenohScoutDiscoveryResult::Ok(response) => {
            zenohscout_ok(&zenohscout_format_discoveries(&response))
        }
        ZenohScoutDiscoveryResult::Err(error) => {
            zenohscout_ok(&zenohscout_format_discovery_error(&error))
        }
    }
}
...
#[derive(Debug, Clone, serde::Serialize, serde::Deserialize, PartialEq, Eq)]
#[serde(untagged)]
enum ZenohScoutDiscoveryResult {
    Ok(ZenohScoutDiscoveryResponse),
    Err(ZenohScoutDiscoveryError),
}
...
async fn zenohscout_fetch_discoveries(
    all: bool,
    limit: Option<usize>,
    timeout_ms: u64,
) -> ZenohScoutDiscoveryResult {
    ...
    tokio::task::spawn_blocking(move || zenohscout_fetch_discoveries_blocking(&request, timeout_ms))
        ...
        .unwrap_or_else(|error| {
            zenohscout_discovery_err(
                "daemon_unreachable",
                Some(format!("{error} — is `maw serve` running?")),
                None,
            )
        })
}
...
fn zenohscout_discoveries_request(all: bool, limit: Option<usize>) -> ZenohScoutHttpRequest {
    ...
    ZenohScoutHttpRequest {
        port,
        path: format!("/api/peers/discoveries{suffix}"),
    }
}

fn zenohscout_fetch_discoveries_blocking(
    request: &ZenohScoutHttpRequest,
    timeout_ms: u64,
) -> ZenohScoutDiscoveryResult {
    ...
    let mut stream = match std::net::TcpStream::connect_timeout(&addr, timeout) {
        Ok(stream) => stream,
        Err(error) => {
            return zenohscout_discovery_err(
                "daemon_unreachable",
                Some(format!("{error} — is `maw serve` running?")),
                None,
            )
...
",
        ...
        return zenohscout_discovery_err(
            "parse_error",
            Some("invalid http response".to_owned()),
            None,
        );
    };
    ...
    if (200..300).contains(&status_code) {
        return match serde_json::from_str::<ZenohScoutDiscoveryResponse>(body) {
            ...
            Err(error) => {
                zenohscout_discovery_err("parse_error", Some(error.to_string()), Some(status_code))
            }
        };
    }
    let body = serde_json::from_str::<serde_json::Value>(body).ok();
    if status_code == 404 {
        return zenohscout_discovery_err("discovery_endpoint_missing", Some("daemon doesn't expose /api/peers/discoveries — restart `maw serve` after upgrading".to_owned()), Some(404));
    }
    let error = body
        .as_ref()
        .and_then(|value| value.get("error"))
        .and_then(serde_json::Value::as_str)
        .map_or_else(|| format!("http_{status_code}"), ToOwned::to_owned);
    let hint = body
        .as_ref()
...
fn zenohscout_discovery_err(
    error: &str,
    hint: Option<String>,
    status: Option<u16>,
) -> ZenohScoutDiscoveryResult {
    ZenohScoutDiscoveryResult::Err(ZenohScoutDiscoveryError {
        ok: false,
...
fn zenohscout_render_both(
    zenoh: &ZenohScoutResultNative,
    scout: &ZenohScoutDiscoveryResult,
    json: bool,
) -> CliOutput {
    ...
    zenohscout_ok(&format!(
        "zenoh:\n{}\n\nscout:\n{}",
        zenohscout_indent(&zenohscout_format_result(zenoh)),
        zenohscout_indent(&zenohscout_format_discovery_any(scout))
    ))
}

fn zenohscout_render_result(result: &ZenohScoutResultNative, json: bool) -> CliOutput {
    if json {
        zenohscout_ok(&zenohscout_json(result))
    } else {
        zenohscout_ok(&zenohscout_format_result(result))
    }
}
...
fn zenohscout_format_result(result: &ZenohScoutResultNative) -> String {
    if !result.enabled {
        return format!(
            "zenoh-scout disabled\n  locator: {}\n  hint: {}",
            result.locator,
            result
                .hint
                .as_deref()
                .unwrap_or("set zenoh.scout.enabled=true")
        );
    }
    if !result.ok {
        return format!(
            "zenoh-scout unavailable\n  locator: {}\n  error: {}\n  hint: {}",
            result.locator,
            result.error.as_deref().unwrap_or("unknown"),
            result.hint.as_deref().unwrap_or("check zenohd remote-api")
        );
    }
    if result.peers.is_empty() {
        return format!(
            "no zenoh discoveries\n  locator: {}\n  key: {}/**",
            result.locator, result.key_prefix
        );
    }
...
fn zenohscout_format_discoveries(response: &ZenohScoutDiscoveryResponse) -> String {
    if response.peers.is_empty() {
        return if response.filtered {
            "no unpaired discoveries (pass --all to include already-paired)".to_owned()
        } else {
            "no discoveries".to_owned()
        };
    }
    ...
    let rows = response
        ...
        .map(|peer| {
            vec![
                format!("{}…", peer.zid.chars().take(8).collect::<String>()),
                peer.node.clone(),
                peer.oracle.clone(),
                peer.host.clone(),
                peer.seen_rel.clone(),
...
fn zenohscout_format_discovery_error(error: &ZenohScoutDiscoveryError) -> String {
    match &error.hint {
        Some(hint) => format!("{} — {hint}", error.error),
...
fn zenohscout_table(header: &[&str], rows: &[Vec<String>]) -> String {
    ...
    lines.push(zenohscout_table_row(
        &header
            .iter()
...
fn zenohscout_table_row(cols: &[String], widths: &[usize]) -> String {
    cols.iter()
        .enumerate()
        .map(|(index, col)| format!("{col:<width$}", width = widths[index]))
        .collect::<Vec<_>>()
        .join("  ")
}
...
fn zenohscout_discovery_key(config: &ZenohScoutConfigNative) -> String {
    [
        ...
        zenohscout_base64url(config.node.as_bytes()),
        zenohscout_base64url(config.oracle.as_bytes()),
        zenohscout_base64url(config.api_url.as_bytes()),
        zenohscout_base64url(config.capabilities.join(",").as_bytes()),
        "alive".to_owned(),
    ]
    .join("/")
}

fn zenohscout_parse_discovery_key(
    key: &str,
    prefix: &str,
    now_ms: u64,
) -> Option<ZenohScoutPeerNative> {
    let prefix = prefix.trim_end_matches('/');
    let rest = key
        .strip_prefix(&format!("{prefix}/"))?
        .split('/')
        .collect::<Vec<_>>();
    if rest.len() != 5 || rest[4] != "alive" {
        return None;
    }
    ...
    Some(ZenohScoutPeerNative {
        zid: format!("zenoh:{:016x}", zenohscout_stable_hash(key)),
        node,
        oracle: oracle.clone(),
        host: zenohscout_host_from_url(&url),
        locators: vec![url],
        capabilities: caps
            .split(',')
            .filter(|value| !value.is_empty())
        ...
        last_seen: iso,
        seen_rel: "now".to_owned(),
        paired: false,
        transport: "zenoh".to_owned(),
    })
}
...
fn zenohscout_now_millis() -> u64 {
    std::env::var(ZENOHSCOUT_FAKE_NOW_ENV)
        .ok()
...
fn zenohscout_indent(value: &str) -> String {
    value
        .split('\n')
...
fn zenohscout_json<T: serde::Serialize>(value: &T) -> String {
    match serde_json::to_string_pretty(value) {
        Ok(json) => format!("{json}\n"),
        Err(error) => format!(
            "{{\"ok\":false,\"error\":\"json_encode\",\"hint\":{}}}\n",
...
fn zenohscout_ok(text: &str) -> CliOutput {
    CliOutput {
        code: 0,
        stdout: format!("{}{}", text, if text.ends_with('\n') { "" } else { "\n" }),
        stderr: String::new(),
    }
}

fn zenohscout_error(message: &str) -> CliOutput {
    CliOutput {
        code: 2,
        stdout: String::new(),
...
mod zenohscout_tests {
    ...
    fn zenohscout_args(values: &[&str]) -> Vec<String> {
    ...
    fn zenohscout_dispatch_registers_aliases_and_parser_guards() {
        ...
        assert!(zenohscout_parse_args(&zenohscout_args(&[
            "--transport",
            "both",
            "--force",
            "--all",
        ...
        .is_ok());
        assert!(
            zenohscout_parse_args(&zenohscout_args(&["--transport", "bad"]))
                .unwrap_err()
                .contains("zenoh|scout|both")
        );
        assert!(
            zenohscout_parse_args(&zenohscout_args(&["--locator", "-secret"]))
                .unwrap_err()
                .contains("not allowed")
        );
        assert!(zenohscout_parse_args(&zenohscout_args(&["--timeout", "0"]))
            .unwrap_err()
            .contains("positive"));
    }
    ...
    fn zenohscout_key_roundtrip_and_fake_result_are_hermetic() {
        let config = ZenohScoutConfigNative {
            enabled: true,
            locator: "ws://127.0.0.1:10000".to_owned(),
            timeout_ms: 750,
            key_prefix: ZENOHSCOUT_DEFAULT_KEY_PREFIX.to_owned(),
            node: "alpha".to_owned(),
            oracle: "bo".to_owned(),
            api_url: "http://alpha:3456".to_owned(),
            capabilities: vec!["pair".to_owned(), "feed".to_owned(), "send".to_owned()],
        };
        let peer = ZenohScoutConfigNative {
            node: "beta".to_owned(),
            oracle: "nova".to_owned(),
            api_url: "http://beta:3456".to_owned(),
            ..config.clone()
        };
        let key = zenohscout_discovery_key(&peer);
        let parsed = zenohscout_parse_discovery_key(&key, &config.key_prefix, 1_782_277_200_000)
            .expect("peer");
        assert_eq!(parsed.node, "beta");
        assert_eq!(parsed.oracle, "nova");
        assert_eq!(parsed.host, "beta:3456");
        let result = zenohscout_fake_result(
            &config,
