const DISPATCH_113: &[DispatcherEntry] = &[DispatcherEntry {
    command: "split",
    handler: Handler::Sync(split_run_command),
}];

const SPLIT_USAGE: &str =
    "usage: maw split <target> [-v|--vertical] [--pct N] [--cmd <cmd>] [--dry-run]";
...
fn split_run_with_runner<R: maw_tmux::TmuxRunner>(
    ...
    if let Some(command) = opts.command.as_deref() {
        split_validate_command_text(command).map_err(|message| (1, message))?;
    }
    if opts.dry_run {
        return Ok(split_render_dry_run(&opts));
    }
    let tmux_args = split_tmux_args(&opts).map_err(|message| (1, message))?;
    runner
        .run("split-window", &tmux_args)
        .map_err(|error| (1, error.message))?;
    Ok(format!("split → {}\n", opts.target))
}
...
fn split_parse_args(argv: &[String]) -> Result<SplitOptions, (i32, String)> {
        ...
        match argv[index].as_str() {
            ...
            "--pct" => {
                let Some(value) = argv.get(index + 1) else {
                    return Err((2, "split: missing --pct value".to_owned()));
                };
            ...
            "--cmd" => {
                let Some(value) = argv.get(index + 1) else {
                    return Err((2, "split: missing --cmd value".to_owned()));
                };
            ...
            arg if arg.starts_with('-') => {
                return Err((2, format!("split: unknown argument {arg}")))
            }
            value => {
                if target.is_some() {
                    return Err((2, "split: target already provided".to_owned()));
                }
    ...
    Ok(SplitOptions {
        target: target.ok_or_else(|| (2, SPLIT_USAGE.to_owned()))?,
        vertical,
        pct,
        command,
        dry_run,
...
fn split_parse_pct(value: &str) -> Result<f64, (i32, String)> {
    value
        .parse::<f64>()
        .map_err(|_| (2, format!("split: invalid --pct value {value}")))
}
...
fn split_render_dry_run(opts: &SplitOptions) -> String {
    ...
    let command = opts
        ...
        .unwrap_or_default();
    format!(
        "tmux split-window {flag} -l {}% -t {}{command}\n",
...
mod split_tests {
    ...
    fn split_strings(values: &[&str]) -> Vec<String> {
    ...
    fn split_uses_tmux_runner_and_argv_vec() {
        ...
        let out = split_run_with_runner(
            &split_strings(&["%1", "--vertical", "--pct", "25", "--cmd", "echo hi"]),
            &mut runner,
        )
        .unwrap();
