...
#[derive(Debug, Clone)]
struct TeamReassignOptions129 {
    selector: String,
    issue: u64,
}

#[derive(Debug, Clone)]
struct TeamReassignSelection129 {
...
fn reassign_parse(argv: &[String]) -> Result<TeamReassignOptions129, String> {
    ...
    if argv.len() != 3 {
        return Err(reassign_usage());
    }
    reassign_validate_selector(&selector)?;
    let issue = issue_raw
        .parse::<u64>()
        .map_err(|_| "new-issue must be a positive integer".to_owned())?;
    if issue == 0 {
        return Err("new-issue must be a positive integer".to_owned());
    }
    Ok(TeamReassignOptions129 { selector, issue })
}

fn reassign_usage() -> String {
    "usage: maw team reassign <member> <new-issue>".to_owned()
}

fn reassign_session(charter: &TeamCharter122) -> Result<String, String> {
    if let Some(session) = charter.session.as_ref().filter(|value| !value.is_empty()) {
    ...
    let raw = maw_tmux::TmuxRunner::run(&mut runner, "display-message", &args)
        .map_err(|_| "session required: pass charter session or run inside tmux".to_owned())?;
...
fn reassign_select(
    ...
    session: &str,
    panes: &[TeamPane124],
) -> Result<TeamReassignSelection129, String> {
    ...
    for member in &charter.members {
        ...
        if team_t3_matches_selectors(member, &[selector.to_owned()], &identity, &worktree) {
            if member
                .target
                .as_deref()
                .is_some_and(|target| target != "auto")
            {
                return Err(format!(
                    "member '{selector}' is unavailable: target {}",
                    member.target.as_deref().unwrap_or_default()
                ));
            }
    ...
    if matches.is_empty() {
        return Err(format!("member not found: {selector}"));
    }
    if matches.len() > 1 {
        return Err(format!(
            "member '{selector}' is ambiguous across: {}",
            reassign_describe_matches(&matches)
        ));
    }
    let mut item = matches.remove(0);
    reassign_prefer_live_derived_window(&mut item, session, panes);
    if item.state == "skipped" {
        return Err(format!("member '{selector}' is unavailable"));
    }
    if item.state != "live" {
        return Err(format!("member '{selector}' is not live: {}", item.state));
    }
    let target = item
        .pane
        .as_ref()
        .map_or_else(|| item.identity.clone(), |pane| pane.window.clone());
    Ok(TeamReassignSelection129 { item, target })
}

fn reassign_prefer_live_derived_window(
    item: &mut TeamRosterItem124,
    session: &str,
    panes: &[TeamPane124],
) {
    if item.state == "live" {
    ...
    let target = format!(
        "{}-{}",
        item.identity,
        workon_sanitize_task_slug(&item.identity)
    );
    if let Some(pane) = panes.iter().find(|pane| {
        pane.session == session && pane.window == target && team_t3_is_live_command(&pane.command)
    }) {
...
fn reassign_describe_matches(items: &[TeamRosterItem124]) -> String {
    items
        .iter()
        .map(|item| {
            if item.identity == item.role {
                item.role.clone()
            } else {
                format!("{} ({})", item.role, item.identity)
            }
        })
        .collect::<Vec<_>>()
        .join(", ")
...
fn reassign_validate_target(
    ...
    match matches {
        1 => Ok(()),
        0 => Err(format!(
            "team reassign refuse missing target before done: {session}:{target}"
        )),
        _ => Err(format!(
            "team reassign refuse ambiguous target before done: {session}:{target}"
        )),
    }
}

fn reassign_repo_slug() -> Result<String, String> {
    if let Ok(repo) = std::env::var("MAW_RS_TEAM_REASSIGN_REPO") {
    ...
    let out = std::process::Command::new("git")
        ...
        .output()
        .map_err(|_| "team reassign: repo detect failed".to_owned())?;
    if !out.status.success() {
        return Err("team reassign: repo detect failed".to_owned());
    }
    let remote = String::from_utf8_lossy(&out.stdout);
    let repo = reassign_parse_remote(&remote)
        .ok_or_else(|| "team reassign: repo detect failed".to_owned())?;
    reassign_validate_repo(&repo)?;
    Ok(repo)
}
...
fn reassign_fetch_prompt(issue: u64, repo: &str) -> Result<String, String> {
    ...
    if std::env::var_os("MAW_RS_TEAM_FAKE_GH_FAIL").is_some() {
        return Err("team reassign: gh issue fetch failed".to_owned());
    }
    let json = if let Ok(path) = std::env::var("MAW_RS_TEAM_FAKE_GH_JSON") {
        std::fs::read_to_string(path)
            .map_err(|_| "team reassign: gh issue fetch failed".to_owned())?
    } else {
        reassign_fetch_prompt_real(issue, repo)?
    };
...
fn reassign_prompt_from_json(issue: u64, repo: &str, json: &str) -> Result<String, String> {
    ...
    let value: serde_json::Value = serde_json::from_str(json)
        .map_err(|_| "team reassign: gh issue parse failed".to_owned())?;
    ...
    let _ = write!(raw, "\n{body}");
    Ok(reassign_wrap_external(
        &format!("GitHub issue #{issue} ({repo})"),
        &raw,
    ))
}
...
fn reassign_wake_plan(
    selected: &TeamReassignSelection129,
    prompt: &str,
    session: &str,
    repo_slug: &str,
...
fn reassign_validate_selector(value: &str) -> Result<(), String> {
    team_t5b_validate_member(value)
}

fn reassign_validate_repo(value: &str) -> Result<(), String> {
    if value.is_empty() || value.starts_with('-') || value.contains("..") {
        return Err("team reassign: invalid repo".to_owned());
    }
    if !value
        .chars()
...
fn reassign_record_fake(kind: &str, value: &str) -> Result<(), String> {
    let Some(path) = std::env::var_os("MAW_RS_TEAM_REASSIGN_FAKE_LOG") else {
        return Ok(());
    };
