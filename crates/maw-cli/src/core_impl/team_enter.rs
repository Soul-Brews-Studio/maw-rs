const DISPATCH_240: &[DispatcherEntry] = &[
    DispatcherEntry {
        command: "team",
        handler: Handler::Sync(team_run_command),
    },
    DispatcherEntry {
        command: "t",
        handler: Handler::Sync(team_run_command),
...
impl Default for TeamEnterTmux240 {
    fn default() -> Self {
        Self {
            runner: maw_tmux::CommandTmuxRunner::default(),
            fake_log: std::env::var_os("MAW_RS_TEAM_ENTER_FAKE_TMUX_LOG")
...
impl TeamEnterTmux240 {
    ...
    fn team_send_enter(&mut self, pane_id: &str) -> Result<(), String> {
        team_validate_pane_id240(pane_id)?;
        self.team_tmux_run240("send-keys", &maw_tmux::tmux_send_enter_args(pane_id))
            .map(|_| ())
    }
    ...
    fn team_probe_pane(&mut self, pane_id: &str) -> Result<(String, String), String> {
        ...
        if std::env::var_os("MAW_RS_TEAM_TMUX_PANES").is_some() {
            ...
            return match matches.as_slice() {
                ...
                [] => Err(format!(
                    "team enter refuse missing pane before send: {pane_id}"
                )),
                _ => Err(format!(
                    "team enter refuse ambiguous pane before send: {pane_id}"
                )),
            };
        }
        let raw = self.team_tmux_run240(
            "display-message",
            &[
                "-t".to_owned(),
                pane_id.to_owned(),
                "-p".to_owned(),
                "#{window_name}|#{pane_current_command}".to_owned(),
            ],
        )?;
    ...
    fn team_tmux_run240(&mut self, command: &str, args: &[String]) -> Result<String, String> {
        if let Some(path) = &self.fake_log {
            ...
            let pending = command == "capture-pane"
                && std::env::var("MAW_RS_TEAM_ENTER_FAKE_PENDING_PANE")
                    .ok()
                    .as_deref()
                    == target;
            let different = command == "capture-pane"
                && std::env::var("MAW_RS_TEAM_ENTER_FAKE_DIFFERENT_PANE")
                    .ok()
                    .as_deref()
                    == target
                && target.is_some_and(|pane| {
                    body.contains(&format!("\"args\":[\"-t\",\"{pane}\",\"-l\","))
                });
            body.push_str(
                &(serde_json::json!({"program":"tmux","command":command,"args":args}).to_string()
                    + "\n"),
            );
            team_atomic_write_0600(path, &body)?;
            if pending {
                return Ok("$ existing input".to_owned());
            }
            if different {
                return Ok("$ different queued input".to_owned());
            }
...
impl maw_tmux::TmuxRunner for TeamEnterTmux240 {
    fn run(&mut self, command: &str, args: &[String]) -> Result<String, maw_tmux::TmuxError> {
        self.team_tmux_run240(command, args)
...
fn team_enter_send_enter(argv: &[String]) -> Result<String, String> {
    ...
    let config = team_read_json::<TeamConfig122>(&team_paths(&opts.team).tool_config)
        .ok_or_else(|| format!("\x1b[33m⚠\x1b[0m team '{}' not found", opts.team))?;
...
fn team_enter_parse240(argv: &[String]) -> Result<TeamEnterOptions240, String> {
    let subcommand = argv
        .first()
        .ok_or_else(|| "usage: maw team enter <agent|all>".to_owned())?;
    if subcommand != "enter" && subcommand != "send-enter" {
        return Err("usage: maw team enter <agent|all>".to_owned());
    }
    let selector = argv
        .get(1)
        .ok_or_else(|| "usage: maw team enter <agent|all>".to_owned())?
    ...
    let text = if subcommand == "send-enter" && argv.len() > 2 {
        ...
        if subcommand == "enter" && argv.len() > 2 {
            return Err("usage: maw team enter <agent|all>".to_owned());
        }
        None
    };
    Ok(TeamEnterOptions240 {
        subcommand: subcommand.to_owned(),
        selector,
        text,
        team: team_resolve_context240()?,
    })
}
...
fn team_enter_members240(
    config: &TeamConfig122,
    opts: &TeamEnterOptions240,
) -> Result<Vec<TeamEnterMember240>, String> {
    let mut out = Vec::new();
    ...
    {
        let Some(pane_id) = member.tmux_pane_id.as_deref() else {
            continue;
        };
        if opts.selector != "all" && !team_member_matches240(member, &opts.selector, &opts.team) {
            continue;
        }
        team_validate_member_name240(&member.name)?;
        team_validate_pane_id240(pane_id)?;
        out.push(TeamEnterMember240 {
    ...
    if out.is_empty() {
        let available = config
            .members
            .iter()
            .filter(|member| member.tmux_pane_id.is_some())
            .map(|member| member.name.as_str())
            .collect::<Vec<_>>()
            .join(", ");
        return Err(format!(
            "\x1b[33m⚠\x1b[0m agent '{}' not found or no pane ID\nAvailable: {}",
            opts.selector,
...
fn team_member_matches240(member: &TeamMember122, selector: &str, team: &str) -> bool {
    member.name == selector
        || member.agent_id.as_deref() == Some(selector)
        || member.agent_id.as_deref() == Some(&format!("{selector}@{team}"))
}

fn team_enter_run240(
    opts: &TeamEnterOptions240,
    members: &[TeamEnterMember240],
    tmux: &mut TeamEnterTmux240,
) -> Result<String, String> {
    ...
    for member in members {
        let result = if let Some(text) = &opts.text {
            tmux.team_send_text(&member.pane_id, text)
        } else {
        ...
        if let Err(error) = result {
            ...
            let (failure, progress) = if opts.text.is_some() {
                (
                    "submission not confirmed for",
                    "prior submissions confirmed",
                )
            } else {
                (
                    "Enter key not accepted by tmux for",
                    "prior Enter keys accepted by tmux",
                )
            };
            return Err(format!(
                "team {} {failure} {}; {} of {} {progress} ({names}): {error}",
                opts.subcommand,
                member.display,
                completed.len(),
                members.len()
        ...
        if opts.text.is_some() {
            let _ = writeln!(
                out,
                "\x1b[36m↵\x1b[0m submission confirmed for {}",
                member.display
            );
        } else {
            let _ = writeln!(
                out,
                "\x1b[36m↵\x1b[0m tmux accepted Enter key for {} (pane state unconfirmed)",
                member.display
            );
        }
    }
...
fn team_validate_member_pane_belongs240(
    team: &str,
    member: &TeamEnterMember240,
    tmux: &mut TeamEnterTmux240,
) -> Result<(), String> {
    team_validate_name(team)?;
    team_validate_member_name240(&member.name)?;
    ...
    if window != member.name {
        return Err(format!(
            "team enter refuse pane mismatch before send: {team}:{} != {}",
            window, member.name
        ));
    }
    if !team_t3_is_live_command(&command) {
        return Err(format!(
            "team enter refuse dead pane before send: {team}:{}",
            member.pane_id
        ));
    }
    Ok(())
...
fn team_validate_member_name240(value: &str) -> Result<(), String> {
    ...
    if value.chars().any(|ch| ch.is_control() || ch == '\0') {
        return Err("invalid team member: control character rejected".to_owned());
    }
...
fn team_validate_text240(value: &str) -> Result<(), String> {
    if value.is_empty() {
        return Err("team text is empty".to_owned());
    }
    if value.starts_with('-') {
        return Err("invalid team text: leading dash rejected".to_owned());
    }
    if value.chars().any(|ch| ch.is_control() || ch == '\0') {
        return Err("invalid team text: control character rejected".to_owned());
    }
    Ok(())
}
...
fn team_validate_pane_id240(value: &str) -> Result<(), String> {
    ...
    {
        return Err(format!("invalid pane id {value:?}"));
    }
    if !value.starts_with('%') {
        return Err(format!("invalid pane id {value:?}: expected tmux pane id"));
    }
    Ok(())
}
...
mod team_enter_tests240 {
    ...
    fn team_enter_rejects_injection_values() {
        assert!(
            team_enter_parse240(&team_enter_strings240(&["enter", "-bad"]))
                .expect_err("dash")
                .contains("leading dash")
        );
        assert!(
            team_enter_parse240(&team_enter_strings240(&["send-enter", "builder", "-bad"]))
                .expect_err("dash")
                .contains("team text")
        );
        assert!(team_validate_pane_id240("pane")
            .expect_err("pane")
            .contains("expected tmux pane id"));
    }

    fn team_enter_strings240(args: &[&str]) -> Vec<String> {
