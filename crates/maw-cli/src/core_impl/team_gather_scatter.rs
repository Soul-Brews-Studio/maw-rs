const DISPATCH_329: &[DispatcherEntry] = &[
    DispatcherEntry {
        command: "gather",
        handler: Handler::Sync(run_team_gather_command),
    },
    DispatcherEntry {
        command: "scatter",
        handler: Handler::Sync(run_team_scatter_command),
    },
];
...
struct TeamGatherOptions329 {
    team: Option<String>,
    members: Vec<String>,
}

#[derive(Debug, Clone, PartialEq, Eq)]
struct TeamGatherTarget329 {
    session: String,
    window: String,
    pane_id: String,
}

fn run_team_gather_command(argv: &[String]) -> CliOutput {
    team_gather_output(team_gather_with_runner(
        argv,
        &mut maw_tmux::CommandTmuxRunner::new(),
    ))
}
fn run_team_scatter_command(argv: &[String]) -> CliOutput {
    team_gather_output(team_scatter_with_runner(
        argv,
        &mut maw_tmux::CommandTmuxRunner::new(),
    ))
}
...
fn team_gather_output(result: Result<String, String>) -> CliOutput {
    match result {
        ...
        Err(message) if message == TEAM_GATHER_USAGE || message == TEAM_SCATTER_USAGE => {
            CliOutput {
                code: 2,
                stdout: String::new(),
...
fn team_gather_with_runner<R: maw_tmux::TmuxRunner>(
    ...
    for item in &roster {
        ...
        match (&item.state[..], &item.pane) {
            ("live", Some(pane))
                if pane.session == target.session && pane.window == target.window =>
            {
                rows.push(team_gather_row(item, "skip already gathered"));
            }
            ("live", Some(pane)) => {
                ...
                .map_err(|(_, message)| format!("maw gather: {message}"))?;
                joined += 1;
    ...
    if joined > 0 {
        tmux_layout_with_runner(
            &[target.pane_id.clone(), "main-vertical".to_owned()],
            runner,
        )
        .map_err(|(_, message)| format!("maw gather: {message}"))?;
    }
    Ok(team_gather_render(
        "maw gather",
        &team,
        &session,
        &target,
...
fn team_scatter_with_runner<R: maw_tmux::TmuxRunner>(
    argv: &[String],
    runner: &mut R,
) -> Result<String, String> {
    ...
    for item in &roster {
        ...
        match (&item.state[..], &item.pane) {
            ("live", Some(pane))
                if pane.session != target.session || pane.window != target.window =>
            {
                rows.push(team_gather_row(item, "skip already scattered"));
            }
            ("live", Some(pane))
                if pane.pane_id == target.pane_id
                    || matches!(item.role.as_str(), "lead" | "bridge") =>
            {
                rows.push(team_gather_row(item, "skip lead"));
            }
            ("live", Some(pane)) => {
                team_t5b_validate_pane_id(&pane.pane_id)?;
                ...
                runner.run("break-pane", &break_args).map_err(|error| {
                    format!(
                        "maw scatter: break-pane {} failed: {}",
                        pane.pane_id, error.message
                    )
                })?;
...
fn team_gather_parse(argv: &[String], usage: &str) -> Result<TeamGatherOptions329, String> {
    ...
    while index < argv.len() {
        match argv[index].as_str() {
            ...
            "--team" => {
                index += 1;
                opts.team = Some(team_t3_next(argv, index, "--team")?);
            }
            value if value.starts_with('-') => {
                return Err(format!("maw gather: unknown argument {value}"))
            }
    ...
    for member in &opts.members {
        team_t3_validate_token(member, "member selector")?;
    }
    Ok(opts)
}
...
fn team_gather_load(opts: &TeamGatherOptions329) -> Result<(String, TeamCharter122), String> {
    let team = opts
        .team
        .clone()
        .or_else(team_gather_current_team)
        .ok_or_else(|| "maw gather: --team required (no current team)".to_owned())?;
    team_validate_name(&team)?;
    let path = team_t3_resolve_charter_path(&team, None)?;
    Ok((team, team_read_charter_path(&path)?))
}

fn team_gather_current_team() -> Option<String> {
    std::env::var("MAW_TEAM")
        .ok()
        .filter(|team| !team.is_empty())
        .or_else(team_gather_single_local_charter)
}
...
fn team_gather_current_target<R: maw_tmux::TmuxRunner>(
    runner: &mut R,
) -> Result<TeamGatherTarget329, String> {
    let args = vec![
        "-p".to_owned(),
        "#{session_name}|#{window_name}|#{pane_id}".to_owned(),
    ];
    let raw = runner.run("display-message", &args).map_err(|error| {
        format!(
            "maw gather: current tmux pane unavailable: {}",
            error.message
        )
    })?;
    let mut parts = raw.trim().split('|');
    let target = TeamGatherTarget329 {
        session: parts.next().unwrap_or_default().to_owned(),
        window: parts.next().unwrap_or_default().to_owned(),
        pane_id: parts.next().unwrap_or_default().to_owned(),
    };
    if target.session.is_empty() || target.window.is_empty() || target.pane_id.is_empty() {
        return Err("maw gather: current tmux pane unavailable".to_owned());
    }
    Ok(target)
}

fn team_gather_roster(
    charter: &TeamCharter122,
    opts: &TeamGatherOptions329,
    team: &str,
    session: &str,
) -> Result<Vec<TeamRosterItem124>, String> {
    let panes = team_down_panes()?;
    let t3_opts = TeamT3Options124 {
        team: Some(team.to_owned()),
        only: opts.members.clone(),
        ..Default::default()
    };
    Ok(charter
        .members
        .iter()
        .map(|member| team_gather_classify(member, &t3_opts, session, &panes))
        .collect())
}

fn team_gather_classify(
    ...
    let Some(pane) = panes
        .iter()
        .find(|pane| pane.session == session && team_gather_path_matches(&pane.path, &item))
        .cloned()
    else {
        return item;
    };
    let state = if team_t3_is_live_command(&pane.command) {
        "live"
    } else {
        "dead"
    };
...
fn team_gather_path_matches(path: &str, item: &TeamRosterItem124) -> bool {
    let candidates = [
        ...
        item.worktree.as_str(),
        std::path::Path::new(&item.worktree)
            .file_name()
...
fn team_gather_row(item: &TeamRosterItem124, action: &str) -> String {
    format!("{}\t{}\t{}", item.role, item.state, action)
}

fn team_gather_render(
    kind: &str,
    team: &str,
    session: &str,
    target: &TeamGatherTarget329,
    rows: &[String],
) -> String {
    let mut out = format!(
        "{kind}: {team} ({session}) -> {}:{}\nrole\tstate\taction\n",
        ...
        fn run(
            ...
            Ok(
                if subcommand == "display-message"
                    && args.iter().any(|arg| arg.contains("session_name"))
                {
                    "s|lead|%1\n".to_owned()
                } else {
                    String::new()
