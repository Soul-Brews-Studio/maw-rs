const DISPATCH_60: &[DispatcherEntry] = &[DispatcherEntry {
    command: "absorb",
    handler: Handler::Sync(run_absorb_command),
}];

const ABSORB_USAGE: &str = "usage: maw absorb <donor> --into <receiver> [--dry-run]";
const ABSORB_SYNC_DIRS: &[&str] = &[
    "memory/learnings",
...
struct AbsorbSyncResult {
    synced: Vec<(String, usize)>,
    total: usize,
}

#[derive(Default)]
struct AbsorbLocalTmux {
...
fn absorb_run(argv: &[String], tmux: &mut AbsorbLocalTmux) -> Result<String, String> {
    ...
    let donor = absorb_find_fleet_entry(&entries, &options.donor)
        .ok_or_else(|| format!("donor oracle '{}' not found in fleet config", options.donor))?;
    let receiver = absorb_find_fleet_entry(&entries, &options.receiver).ok_or_else(|| {
        format!(
            "receiver oracle '{}' not found in fleet config",
            options.receiver
        )
    })?;
...
fn absorb_parse_args(argv: &[String]) -> Result<AbsorbOptions, String> {
    ...
    {
        return Err(format!(
            "{ABSORB_USAGE} — absorb donor knowledge, archive donor, and switch to receiver"
        ));
    }
        ...
        match argv[index].as_str() {
            ...
            "--into" => {
                let Some(value) = argv.get(index + 1) else {
                    return Err(ABSORB_USAGE.to_owned());
                };
                absorb_validate_name(value, "receiver")?;
                receiver = Some(value.clone());
                index += 2;
            }
            value if value.starts_with('-') => {
                return Err(format!("absorb: unknown argument {value}"))
            }
            value => {
                if donor.is_some() {
                    return Err(ABSORB_USAGE.to_owned());
                }
                absorb_validate_name(value, "donor")?;
                donor = Some(value.to_owned());
                index += 1;
            }
        }
    }
...
fn absorb_run_entries(
    options: &AbsorbOptions,
    donor: &AbsorbFleetEntry,
    receiver: &AbsorbFleetEntry,
    tmux: &mut AbsorbLocalTmux,
    ...
    if donor.file == receiver.file
        || absorb_normalize_name(&donor_name) == absorb_normalize_name(&receiver_name)
    {
        return Err("donor and receiver must be different oracles".to_owned());
    }
    let donor_path = absorb_resolve_entry_path(donor, &donor_name)
        .ok_or_else(|| format!("could not resolve donor oracle path for '{donor_name}'"))?;
    let receiver_path = absorb_resolve_entry_path(receiver, &receiver_name)
        .ok_or_else(|| format!("could not resolve receiver oracle path for '{receiver_name}'"))?;
    let mut out = String::new();
    let _ = writeln!(out, "\n  Absorbing {donor_name} -> {receiver_name}\n");
    if options.dry_run {
        absorb_render_dry_run(&mut out, &donor_path, &receiver_path, &donor_name);
    } else {
        absorb_run_sync_and_archive(
            &mut out,
            &donor_path,
            &receiver_path,
            &donor_name,
    ...
    if options.dry_run {
        ...
        absorb_render_complete(&mut out, donor, &donor_name, &receiver_name);
    }
    Ok(out)
}

fn absorb_render_dry_run(
    out: &mut String,
    donor_path: &std::path::Path,
    receiver_path: &std::path::Path,
    donor_name: &str,
) {
    let _ = writeln!(
        out,
        "  [dry-run] would sync psi memory: {} -> {}",
        donor_path.display(),
        receiver_path.display()
    );
    let _ = writeln!(
        out,
        "  [dry-run] would archive donor via: maw archive {donor_name}"
    );
}

fn absorb_run_sync_and_archive(
    out: &mut String,
    donor_path: &std::path::Path,
    receiver_path: &std::path::Path,
    donor_name: &str,
    ...
    if result.total == 0 {
        out.push_str("  [ok] psi memory sync complete: nothing new\n");
    } else {
        let _ = writeln!(
            out,
            "  [ok] psi memory sync complete: {}",
            absorb_format_sync_summary(&result)
        );
    }
...
fn absorb_render_complete(
    ...
    let archived = if disabled.exists() {
        "archived"
    } else {
        "archive attempted"
    };
...
fn absorb_find_fleet_entry(entries: &[AbsorbFleetEntry], query: &str) -> Option<AbsorbFleetEntry> {
    ...
    entries
        .iter()
        .find(|entry| {
            absorb_entry_names(entry).iter().any(|name| {
                let candidate = name.to_lowercase();
                candidate == raw || absorb_normalize_name(name) == normalized
            })
        })
        .cloned()
}
...
fn absorb_entry_names(entry: &AbsorbFleetEntry) -> Vec<String> {
    ...
    for window in &entry.session.windows {
        names.push(window.name.clone());
        names.push(absorb_repo_name(&window.repo));
    }
    names.into_iter().filter(|name| !name.is_empty()).collect()
}

fn absorb_resolve_entry_path(
    entry: &AbsorbFleetEntry,
    display_name: &str,
    ...
    let repo = entry
        ...
        .windows
        .first()
        .map(|window| window.repo.as_str())
        .unwrap_or_default();
    let fallback = repos_root.join(repo);
    if !repo.is_empty() && fallback.exists() {
...
fn absorb_find_oracle_repo(repos_root: &std::path::Path, stem: &str) -> Option<std::path::PathBuf> {
    ...
    for org in orgs.flatten().filter(|entry| entry.path().is_dir()) {
        ...
        for repo in repos.flatten().filter(|entry| entry.path().is_dir()) {
            ...
            if bare_match.is_none() && name.to_string_lossy().eq_ignore_ascii_case(stem) {
                bare_match = Some(repo.path());
            }
        }
    }
    bare_match
...
fn absorb_switch_to_receiver(
    ...
    if dry_run {
        let _ = writeln!(out, "  [dry-run] would switch client: {command}");
        return;
    }
    if std::env::var_os("TMUX").is_none() {
        let _ = writeln!(out, "  [info] not inside tmux; run manually: {command}");
        return;
    }
    match tmux.absorb_switch_client(session_name) {
        Ok(()) => {
            let _ = writeln!(out, "  [ok] switched client to {session_name}");
        }
        Err(error) => {
            let _ = writeln!(out, "  [warn] could not switch to receiver: {error}");
            let _ = writeln!(out, "  [hint] run manually: {command}");
        }
    }
}
...
impl AbsorbLocalTmux {
    fn absorb_switch_client(&mut self, session_name: &str) -> Result<(), String> {
        ...
        maw_tmux::TmuxRunner::run(
            &mut self.runner,
            "switch-client",
            &["-t".to_owned(), session_name.to_owned()],
        )
        .map(|_| ())
...
fn absorb_sync_oracle_vaults(
    ...
    for dir in ABSORB_SYNC_DIRS {
        ...
        if count > 0 {
            result.synced.push(((*dir).to_owned(), count));
            result.total += count;
        }
    }
    if result.total > 0 {
        absorb_append_sync_log(to_path, from_name, to_name, &result);
    }
    result
}
...
fn absorb_sync_dir(src: &std::path::Path, dst: &std::path::Path) -> usize {
    ...
    for entry in entries.flatten() {
        ...
        if src_path.is_dir() {
            count += absorb_sync_dir(&src_path, &dst_path);
        } else if !dst_path.exists() {
            count += absorb_copy_new_file(&src_path, &dst_path);
        }
    }
    count
}
...
fn absorb_append_sync_log(
    to_path: &std::path::Path,
    from_name: &str,
    to_name: &str,
    result: &AbsorbSyncResult,
) {
...
fn absorb_strip_session_prefix(name: &str) -> String {
    name.split_once('-')
...
fn absorb_strip_oracle_suffix(name: &str) -> String {
    name.strip_suffix("-oracle").unwrap_or(name).to_owned()
}
fn absorb_normalize_name(name: &str) -> String {
    absorb_strip_oracle_suffix(&absorb_strip_session_prefix(name)).to_lowercase()
}
fn absorb_repo_name(repo_slug: &str) -> String {
    repo_slug
        .split('/')
...
fn absorb_shell_quote(value: &str) -> String {
...
fn absorb_validate_name(value: &str, label: &str) -> Result<(), String> {
    if value.is_empty() || value.trim() != value || value.starts_with('-') || value.contains('/') {
        Err(format!(
            "absorb: {label} must be non-empty, unpadded, not start with '-', and not contain '/'"
        ))
    } else {
        Ok(())
    }
...
fn absorb_validate_tmux_target(value: &str) -> Result<(), String> {
    if value.is_empty() || value.trim() != value || value.starts_with('-') {
        Err(format!("absorb: invalid tmux target '{value}'"))
    } else {
        Ok(())
    }
}
...
mod absorb_tests {
    ...
    fn absorb_strings(values: &[&str]) -> Vec<String> {
        values.iter().map(|value| (*value).to_owned()).collect()
    }

    #[test]
    fn absorb_parse_matches_flags_and_blocks_injection() {
        let parsed = absorb_parse_args(&absorb_strings(&[
            "donor",
            "--into",
            "receiver",
            "--dry-run",
        ...
        assert_eq!(
            absorb_parse_args(&absorb_strings(&["-Sbad", "--into", "receiver"])).unwrap_err(),
            "absorb: unknown argument -Sbad"
        );
        assert!(
            absorb_parse_args(&absorb_strings(&["donor", "--into", "-Rbad"]))
                .unwrap_err()
                .contains("receiver must")
        );
    }
        ...
        let entry = AbsorbFleetEntry {
            ...
            session: NativeFleetSession {
                ...
                windows: vec![NativeFleetWindow {
                    name: "neo-oracle".to_owned(),
                    repo: "org/neo-oracle".to_owned(),
                    kind: None,
                }],
    ...
    fn absorb_repo_lookup_prefers_oracle_then_falls_back_to_bare() {
        ...
        assert_eq!(
            absorb_find_oracle_repo(&root, "display-census"),
            Some(budded)
        );
        let _ = std::fs::remove_dir_all(root);
    }
    ...
    fn absorb_sync_copies_only_new_memory_files() {
        ...
        std::fs::write(donor.join("ψ/memory/learnings/nested/b.md"), "new2")
            .expect("write donor nested");
        ...
        assert_eq!(
            std::fs::read_to_string(receiver.join("ψ/memory/learnings/a.md")).expect("existing"),
            "old"
        );
        assert_eq!(
            std::fs::read_to_string(receiver.join("ψ/memory/learnings/nested/b.md"))
                .expect("copied"),
            "new2"
        );
        let _ = std::fs::remove_dir_all(root);
