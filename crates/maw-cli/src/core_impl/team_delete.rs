...
fn team_delete(argv: &[String]) -> Result<String, String> {
    let team = argv
        .get(1)
        .ok_or_else(|| "usage: maw team delete <team-name>".to_owned())?;
    if argv.len() > 2 {
        return Err(format!("team delete: unexpected argument {}", argv[2]));
    }
    team_validate_name(team)?;
    let mut removed = Vec::new();
    team_delete_archive_team(team)?;
    for path in team_delete_paths(team) {
        if team_bounded_remove_dir_all(&path.root, &path.target, &format!("team delete {team}"))? {
...
fn team_prune(argv: &[String]) -> Result<String, String> {
    if argv.len() > 1 {
        return Err(format!("team prune: unexpected argument {}", argv[1]));
    }
    let active = team_prune_active_sessions();
    let root = team_home_dir().join(".claude").join("teams");
    let mut pruned = Vec::new();
    let Ok(entries) = std::fs::read_dir(&root) else {
        return Ok("team prune: no teams pruned\n".to_owned());
    };
    for entry in entries.flatten() {
        let name = entry.file_name().to_string_lossy().into_owned();
        if team_validate_name(&name).is_err()
            || team_prune_is_active(&name, &active)
            || !team_prune_is_empty_team(&name)
        {
            continue;
        }
        team_delete_archive_team(&name)?;
        if team_bounded_remove_dir_all(&root, &entry.path(), &format!("team prune {name}"))? {
            pruned.push(name);
        }
    }
    pruned.sort();
...
fn team_gc(argv: &[String]) -> Result<String, String> {
    ...
    if options.confirm {
        for candidate in &candidates {
            ...
            for path in team_delete_paths(&candidate.name) {
                team_bounded_remove_dir_all(
                    &path.root,
                    &path.target,
                    &format!("team gc {}", candidate.name),
                )?;
            }
        }
    }
...
fn team_delete_paths(team: &str) -> Vec<TeamDeletePath131> {
    ...
    vec![
        TeamDeletePath131 {
            label: "team dir".to_owned(),
            root: team_home_dir().join(".claude").join("teams"),
            target: paths.tool_dir,
        },
        TeamDeletePath131 {
            label: "tasks".to_owned(),
            root: tasks_root.clone(),
            target: tasks_root.join(team).join("tasks"),
        },
    ]
}
...
fn team_delete_archive_team(team: &str) -> Result<(), String> {
    ...
    let archive = team_psi_dir()
        ...
        .join("teams")
        .join(team)
        .join(format!("delete-archive-{stamp}"));
    std::fs::create_dir_all(&archive)
        .map_err(|error| format!("team delete archive mkdir failed: {error}"))?;
    let paths = team_paths(team);
    ...
    if paths.vault_manifest.exists() {
        std::fs::copy(&paths.vault_manifest, archive.join("manifest.json"))
            .map_err(|error| format!("team delete archive manifest failed: {error}"))?;
    }
    Ok(())
}
...
fn team_copy_tree_if_exists(src: &std::path::Path, dst: &std::path::Path) -> Result<(), String> {
    ...
    let meta = std::fs::symlink_metadata(src)
        .map_err(|error| format!("team delete archive stat failed: {error}"))?;
    if meta.is_file() {
        if let Some(parent) = dst.parent() {
            std::fs::create_dir_all(parent)
                .map_err(|error| format!("team delete archive parent failed: {error}"))?;
        }
        std::fs::copy(src, dst)
            .map_err(|error| format!("team delete archive copy failed: {error}"))?;
        return Ok(());
    }
    if !meta.is_dir() {
        return Ok(());
    ...
    std::fs::create_dir_all(dst)
        .map_err(|error| format!("team delete archive dir failed: {error}"))?;
    for entry in std::fs::read_dir(src)
        .map_err(|error| format!("team delete archive read failed: {error}"))?
        .flatten()
    {
...
fn team_bounded_remove_dir_all(
    root: &std::path::Path,
    ...
    if target_canon == root_canon || !target_canon.starts_with(&root_canon) {
        return Err(format!(
            "{label}: refuse unbounded remove_dir_all {} outside {}",
    ...
    std::fs::remove_dir_all(&target_canon).map_err(|error| {
        format!(
            "{label}: remove_dir_all {} failed: {error}",
            target_canon.display()
        )
    })?;
    Ok(true)
...
fn team_prune_active_sessions() -> Vec<String> {
    if let Ok(raw) = std::env::var("MAW_RS_TEAM_TMUX_SESSIONS") {
        return raw
            .lines()
            .map(str::trim)
...
fn team_prune_is_active(team: &str, sessions: &[String]) -> bool {
    sessions.iter().any(|session| {
        session == team
            || session
                .split_once('-')
                .is_some_and(|(_, tail)| tail == team)
    })
}

fn team_prune_is_empty_team(team: &str) -> bool {
    let Some(config) = team_read_json::<TeamConfig122>(&team_paths(team).tool_config) else {
        return false;
    };
...
fn team_gc_parse(argv: &[String]) -> Result<TeamGcOptions251, String> {
    let mut options = TeamGcOptions251 {
        confirm: false,
        keep_empty: false,
        age_ms: 7 * 24 * 60 * 60 * 1000,
    };
    let mut index = 1;
        ...
        match argv[index].as_str() {
            ...
            "--age" => {
                ...
                let value = argv.get(index).ok_or_else(|| {
                    "usage: maw team gc [--confirm] [--age 7d] [--keep-empty]".to_owned()
                })?;
                options.age_ms = team_gc_parse_age_ms(value)?;
            }
            value if value.starts_with("--age=") => {
                options.age_ms = team_gc_parse_age_ms(&value["--age=".len()..])?;
            }
            value if value.starts_with('-') => {
                return Err(format!("team gc: unknown argument {value}"))
            }
...
fn team_gc_parse_age_ms(value: &str) -> Result<u64, String> {
    ...
    let amount = number
        .parse::<u64>()
        .map_err(|_| format!("team gc: invalid --age {value:?}"))?;
    Ok(amount.saturating_mul(multiplier))
}

fn team_gc_candidates(options: &TeamGcOptions251) -> Vec<TeamGcCandidate251> {
    let root = team_home_dir().join(".claude").join("teams");
    team_read_tool_teams(&root)
        .into_iter()
        .filter_map(|(name, config)| team_gc_candidate(&name, &config, options))
        .collect()
}

fn team_gc_candidate(
    name: &str,
    config: &TeamConfig122,
    options: &TeamGcOptions251,
) -> Option<TeamGcCandidate251> {
    if team_validate_name(name).is_err() || config.members.iter().any(|member| member.is_active) {
        return None;
    }
...
fn team_gc_render(candidates: &[TeamGcCandidate251], confirm: bool) -> String {
    ...
    if candidates.is_empty() {
        return if confirm {
            "team gc: no candidates removed\n".to_owned()
        } else {
            "team gc dry-run: no candidates\n".to_owned()
        };
    }
    let mut out = if confirm {
        format!("team gc: {} removed\n", candidates.len())
    } else {
        format!("team gc dry-run: {} candidate(s)\n", candidates.len())
    };
    for candidate in candidates {
        let _ = writeln!(
            out,
            "  - {} [{}] members={}",
            candidate.name, candidate.reason, candidate.members
        );
    }
    if !confirm {
        out.push_str("  rerun with --confirm to remove\n");
    }
    out
}
...
mod team_delete_prune_tests {
    ...
    fn team_delete_bounded_remove_refuses_outside_root() {
        ...
        let err =
            team_bounded_remove_dir_all(&root, &outside, "unit").expect_err("outside rejected");
