const DISPATCH_106: &[DispatcherEntry] = &[
    DispatcherEntry {
        command: "token",
        handler: Handler::Sync(token_run_command),
    },
    DispatcherEntry {
        command: "tokens",
        handler: Handler::Sync(token_tokens_alias_command),
    },
];
...
struct TokenSystemApplyTmux {
    client: maw_tmux::TmuxClient<maw_tmux::CommandTmuxRunner>,
}

impl TokenSystemApplyTmux {
    fn new() -> Self {
        Self {
            client: maw_tmux::TmuxClient::local(),
        }
    }
}
...
impl TokenFakeRunner {
    fn token_new_from_env() -> Option<Self> {
        ...
        let fail = std::env::var(TOKEN_FAKE_FAIL_ENV)
            .ok()
    ...
    fn token_should_fail(&self, op: &str) -> bool {
        self.fail
            .as_deref()
            .is_some_and(|value| value == op || value == "all")
    }
}

impl TokenRunner for TokenFakeRunner {
    fn token_pass_show(&mut self, name: &str) -> Result<String, ()> {
        if self.token_should_fail("show") {
            return Err(());
        }
    ...
    fn token_pass_insert_multiline(&mut self, name: &str, stdin: &str) -> Result<(), ()> {
        if self.token_should_fail("insert") {
            return Err(());
        }
    ...
    fn token_direnv_allow(&mut self, cwd: &std::path::Path) -> Result<(), ()> {
        if self.token_should_fail("direnv") {
            return Err(());
        }
    ...
    fn token_confirm(&mut self, _prompt: &str) -> bool {
        false
    }
}
...
fn token_runner_from_env() -> Box<dyn TokenRunner> {
    if let Some(fake) = TokenFakeRunner::token_new_from_env() {
        Box::new(fake)
    } else {
        Box::new(TokenSystemRunner)
    }
}

fn token_dispatch(
    argv: &[String],
    runner: &mut dyn TokenRunner,
) -> Result<TokenCommandResult, String> {
    ...
    match sub {
        ...
        "current" => Ok(token_ok(
            token_current().map_or_else(String::new, |name| format!("{name}\n")),
        )),
        ...
        match value.as_str() {
            ...
            "--session" => {
                ...
                let Some(next) = argv.get(index) else {
                    return Err("maw token apply: --session requires a value".to_owned());
                };
            ...
            "--squad" => {
                index += 1;
                let Some(next) = argv.get(index) else {
                    return Err("maw token apply: --squad requires a value".to_owned());
                };
...
fn token_cmd_list(runner: &mut dyn TokenRunner) -> Result<String, String> {
    let cwd = std::env::current_dir()
        .map_err(|_| "maw token: current directory unavailable".to_owned())?;
    let dir = cwd
        .file_name()
    ...
    if let Some(active) = active.as_deref() {
        out.push(format!("Here ({dir}): {active}"));
    } else if envrc_present {
        out.push(format!(
            "Here ({dir}): .envrc present, no CLAUDE_TOKEN_NAME"
        ));
    } else {
        out.push(format!("Here ({dir}): no .envrc"));
    }
    out.push(String::new());
    if !tokens.is_empty() {
    ...
    if !envrcs.is_empty() {
        ...
        for (idx, name) in envrcs.iter().enumerate() {
            out.push(format!("  {}. {name}", idx + 1));
        }
        out.push(String::new());
    }
    if tokens.is_empty() && envrcs.is_empty() {
        out.push("Empty vault. Add tokens: pass insert claude/token-<name>".to_owned());
    }
    Ok(format!("{}\n", out.join("\n")))
}
...
fn token_cmd_use(
    ...
    let Some(name) = args.positionals.get(1).map(String::as_str) else {
        ...
        return Ok(token_ok(format!(
            "{list}Usage: maw token use <name> [--no-team]\n"
        )));
    };
    token_validate_name("token name", name)?;
    let pass_path = format!("{TOKEN_TOKEN_PREFIX}{name}");
    if !token_pass_exists(runner, &pass_path) {
        return Err(format!("token \"{name}\" not found in pass ({pass_path})"));
    }
    let cwd = std::env::current_dir()
        .map_err(|_| "maw token use: current directory unavailable".to_owned())?;
    let envrc_path = cwd.join(".envrc");
    ...
    token_atomic_write(&envrc_path, &content)
        .map_err(|_| "maw token use: failed to write .envrc".to_owned())?;
    let direnv_ok = runner.token_direnv_allow(&cwd).is_ok();
    let mut stdout = format!("Now using: {name}\n");
    if !direnv_ok {
        stdout.push_str("warning: direnv allow failed — run `direnv allow .` manually\n");
    }
    Ok(token_ok(stdout))
}
...
fn token_cmd_apply(
    args: &TokenArgs,
    runner: &mut dyn TokenRunner,
    tmux: &mut dyn TokenApplyTmux,
) -> Result<TokenCommandResult, String> {
    let Some(name) = args.positionals.get(1).map(String::as_str) else {
        return Err(
            "usage: maw token apply <name> [--session X|--squad S|--all] [--dry-run]".to_owned(),
        );
    };
    token_validate_name("token name", name)?;
    let pass_path = format!("{TOKEN_TOKEN_PREFIX}{name}");
    ...
    let scopes = usize::from(args.all)
        + usize::from(args.session.is_some())
        + usize::from(args.squad.is_some());
    if scopes > 1 {
        return Err("maw token apply: choose only one of --session, --squad, or --all".to_owned());
    }
    ...
    for pane in panes {
        ...
        if pane.cwd.as_deref().unwrap_or_default().is_empty() {
            let _ =
                std::fmt::Write::write_fmt(&mut out, format_args!("skip {target}: cwd unknown\n"));
        } else if token_apply_pane_idle(tmux, &target) {
            let _ = std::fmt::Write::write_fmt(
                &mut out,
                format_args!("plan {target}: /exit; direnv reload 2>/dev/null; claude -c\n"),
            );
            idle.push(target);
        } else {
    ...
    if args.dry_run {
        return Ok(token_ok(out));
    }
    out.insert_str(0, &token_cmd_use(args, runner)?.stdout);
    for target in idle {
        tmux.apply_send_text(&target, "/exit")
            .map_err(|error| format!("token apply: {target}: send /exit failed: {error}"))?;
        tmux.apply_send_enter(&target)
            .map_err(|error| format!("token apply: {target}: send enter failed: {error}"))?;
        tmux.apply_sleep_ms(1_500);
        tmux.apply_send_text(&target, "direnv reload 2>/dev/null; claude -c")
            .map_err(|error| format!("token apply: {target}: restart failed: {error}"))?;
        tmux.apply_send_enter(&target)
            .map_err(|error| format!("token apply: {target}: restart enter failed: {error}"))?;
...
fn token_cmd_save(
    ...
    let cwd = std::env::current_dir()
        .map_err(|_| "maw token save: current directory unavailable".to_owned())?;
    ...
    let content = std::fs::read_to_string(&envrc_path)
        .map_err(|_| "no .envrc in current directory".to_owned())?;
    if token_pass_exists(runner, &path)
        && !args.force
        && !token_confirm_overwrite(runner, &format!("Overwrite {path}?"))
    {
        return Ok(token_ok(format!("Skipped (would overwrite {path})\n")));
    }
    runner
        .token_pass_insert_multiline(&path, &content)
        .map_err(|()| "pass insert failed".to_owned())?;
...
fn token_cmd_load(
    ...
    let cwd = std::env::current_dir()
        .map_err(|_| "maw token load: current directory unavailable".to_owned())?;
    let name = token_default_name(args.positionals.get(1).map(String::as_str), &cwd)?;
    token_validate_name("envrc name", &name)?;
    let path = format!("{TOKEN_PASS_PREFIX}/{name}");
    let envrc_path = cwd.join(".envrc");
    if !token_pass_exists(runner, &path) {
        return Err(format!("{path} not found in pass"));
    }
    if envrc_path.exists() && !args.force && !token_confirm_overwrite(runner, "Overwrite .envrc?") {
        return Ok(token_ok(format!(
            "Skipped (would overwrite .envrc; {path})\n"
        )));
    }
    let content = runner
        .token_pass_show(&path)
        .map_err(|()| "pass show failed".to_owned())?;
    token_reject_literal_oauth_value(&content)?;
    token_atomic_write(&envrc_path, &content)
        .map_err(|_| "maw token load: failed to write .envrc".to_owned())?;
    let direnv_ok = runner.token_direnv_allow(&cwd).is_ok();
...
fn token_cmd_scan(runner: &mut dyn TokenRunner) -> Result<String, String> {
    ...
    for (label, path) in files {
        ...
        if let Some(name) = token_detect_active_token(&content) {
            rows.push((label, name, "named".to_owned()));
            continue;
        }
        if let Some(name) = token_match_fingerprint(&content, &fingerprints) {
            rows.push((label, name, "matched".to_owned()));
        } else if content.contains("CLAUDE_CODE_OAUTH_TOKEN") {
            rows.push((label, "unknown".to_owned(), "unmatched".to_owned()));
        }
    }
...
fn token_cmd_resolve() -> Result<String, String> {
    let cwd = std::env::current_dir()
        .map_err(|_| "maw token resolve: current directory unavailable".to_owned())?;
    if let Some(oracle) = token_resolve_oracle_from_cwd(&cwd) {
        if let Some(name) = token_resolve_assigned_token(&oracle)? {
            return Ok(format!("{name}\n"));
        }
    }
    if let Some(name) = token_current() {
        return Ok(format!("{name}\n"));
    }
    if let Some(name) = token_current_file() {
        return Ok(format!("{name}\n"));
    }
...
fn token_resolve_assigned_token(oracle: &str) -> Result<Option<String>, String> {
    for entry in fleet_load_entries_result("token resolve")? {
        let Some(members) = entry.session.members.as_ref() else {
            continue;
        };
        let Some(member) = members
            .iter()
            .find(|member| token_oracle_matches(&member.handle, oracle))
        else {
            continue;
        };
...
fn token_current_file() -> Option<String> {
    let value =
        std::fs::read_to_string(maw_state_path(&current_xdg_env(), &["token-current"])).ok()?;
    let name = value.trim();
...
"
    .to_owned();
    for (oracle, assigned, running) in rows {
        let _ =
            std::fmt::Write::write_fmt(&mut out, format_args!("{oracle}\t{assigned}\t{running}\n"));
    }
    Ok(out)
}
...
fn token_running_by_oracle(panes: &[maw_tmux::TmuxPane]) -> BTreeMap<String, String> {
    ...
    for pane in panes {
        ...
        let Some(token) = token_extract_running_marker(&pane.title) else {
            continue;
        };
...
fn token_apply_scope_sessions(args: &TokenArgs) -> Result<Option<Vec<String>>, String> {
    if let Some(session) = args.session.as_ref() {
        return Ok(Some(vec![session.clone()]));
    }
    if let Some(squad) = args.squad.as_ref() {
        ...
        if sessions.is_empty() {
            return Err(format!("no squad named {squad}"));
        }
...
fn token_apply_targets(
    panes: Vec<maw_tmux::TmuxPane>,
    sessions: Option<&[String]>,
) -> Vec<maw_tmux::TmuxPane> {
    let mut out = panes
        .into_iter()
        .filter(token_apply_is_claude_pane)
        .filter(|pane| {
            sessions.is_none_or(|names| {
                names
                    .iter()
                    .any(|name| token_apply_pane_in_session(pane, name))
            })
        })
        .collect::<Vec<_>>();
...
fn token_apply_is_claude_pane(pane: &maw_tmux::TmuxPane) -> bool {
    maw_tmux::is_claude_like_pane(Some(&pane.command))
        || pane.title.to_ascii_lowercase().contains("claude")
}
...
fn token_apply_pane_idle(tmux: &mut dyn TokenApplyTmux, target: &str) -> bool {
    let Ok(first) = tmux.apply_capture(target) else {
        return false;
    };
    tmux.apply_sleep_ms(600);
    let Ok(second) = tmux.apply_capture(target) else {
        return false;
    };
    normalize_activity_snapshot(&first) == normalize_activity_snapshot(&second)
}

fn token_ok(stdout: String) -> TokenCommandResult {
    TokenCommandResult { ok: true, stdout }
}
...
fn token_build_envrc_content(existing: &str, name: &str, no_team: bool) -> String {
    ...
    for line in existing.split('\n') {
        let s = line.trim();
        if s.starts_with("export CLAUDE_TOKEN_NAME=") || s.starts_with("CLAUDE_TOKEN_NAME=") {
            continue;
        }
        if s.starts_with("export CLAUDE_CODE_OAUTH_TOKEN=")
            || s.starts_with("CLAUDE_CODE_OAUTH_TOKEN=")
        {
            continue;
        }
        if s.starts_with("export CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=") {
            continue;
        }
        if token_is_legacy_token_line(s) {
            continue;
        }
...
fn token_list_token_names(runner: &mut dyn TokenRunner) -> Vec<String> {
    let Ok(raw) = runner.token_pass_ls("claude") else {
        return Vec::new();
    };
    let mut out = raw
        .lines()
        .filter_map(token_parse_token_ls_line)
        .collect::<Vec<_>>();
    out.sort();
    out.dedup();
    out
...
fn token_list_envrc_names(runner: &mut dyn TokenRunner) -> Vec<String> {
    let Ok(raw) = runner.token_pass_ls(TOKEN_PASS_PREFIX) else {
        return Vec::new();
    };
    let mut out = raw
        .lines()
        .filter_map(token_parse_envrc_ls_line)
        .collect::<Vec<_>>();
    out.sort();
    out.dedup();
    out
...
fn token_parse_envrc_ls_line(raw: &str) -> Option<String> {
    ...
    if clean.is_empty() || clean.ends_with('/') || clean.contains("Password Store") {
        return None;
    }
...
fn token_match_fingerprint(content: &str, fingerprints: &[(String, String)]) -> Option<String> {
    fingerprints
        .iter()
        .find_map(|(value, name)| content.contains(value).then(|| name.clone()))
}

fn token_format_scan(rows: &[(String, String, String)]) -> String {
    if rows.is_empty() {
        return "No .envrc files with Claude tokens found\n".to_owned();
    }
    let mut by_token = std::collections::BTreeMap::<String, Vec<(String, String)>>::new();
    for (label, name, method) in rows {
        by_token
            .entry(name.clone())
            .or_default()
            .push((label.clone(), method.clone()));
    }
    let mut out = format!(
        "  {} oracles using {} tokens:\n\n",
        rows.len(),
        by_token.len()
    );
    ...
    if rows.iter().any(|(_, _, method)| method == "unmatched") {
        out.push_str("  * = token not in pass vault (unknown)\n");
    }
    out
}
...
fn token_resolve_ghq_root() -> Result<std::path::PathBuf, String> {
    ...
    if github.is_dir() {
        ...
        Err("scan: ghq root unavailable — install ghq or set up ~/ghq/github.com (no hardcoded fallback)".to_owned())
    }
}
...
fn token_find_envrc_files(ghq_root: &std::path::Path) -> Vec<(String, std::path::PathBuf)> {
    ...
    let Ok(orgs) = std::fs::read_dir(ghq_root) else {
        return out;
    };
    ...
    for org in orgs {
        ...
        for repo in repos {
            ...
            if envrc.is_file() {
                out.push((format!("{org_name}/{repo_name}"), envrc));
            }
        }
    }
    out
...
fn token_reject_literal_oauth_value(content: &str) -> Result<(), String> {
    for line in content.lines() {
        ...
        if !trimmed.contains("CLAUDE_CODE_OAUTH_TOKEN") {
            continue;
        }
        if trimmed.contains("$(pass show claude/token-") || trimmed.starts_with('#') {
            continue;
        }
...
mod token_apply_tests {
    ...
    impl TokenApplyTmux for FakeApply {
        ...
        fn apply_capture(&mut self, _target: &str) -> Result<String, String> {
            Ok(self.captures.remove(0))
        }
        fn apply_send_text(&mut self, target: &str, text: &str) -> Result<(), String> {
            self.actions.push(format!("text {target} {text}"));
            Ok(())
        }
        fn apply_send_enter(&mut self, target: &str) -> Result<(), String> {
            self.actions.push(format!("enter {target}"));
            Ok(())
        }
        fn apply_sleep_ms(&mut self, ms: u64) {
            self.actions.push(format!("sleep {ms}"));
        }
    }

    fn args(values: &[&str]) -> TokenArgs {
        token_parse_args(
            &values
                .iter()
    ...
    fn temp(name: &str) -> std::path::PathBuf {
        let root =
            std::env::temp_dir().join(format!("maw-rs-token-apply-{name}-{}", std::process::id()));
    ...
    fn token_status_reports_assigned_and_running_tokens() {
        ...
        assert_eq!(
            token_extract_running_marker("x 🔐dd2*"),
            Some("dd2".to_owned())
        );
    }
    ...
    fn token_apply_dry_run_plans_idle_and_skips_busy() {
        ...
        let mut tmux = FakeApply {
            panes: vec![pane("%1", "s1"), pane("%2", "s2")],
            captures: vec!["same".into(), "same".into(), "old".into(), "new".into()],
            actions: Vec::new(),
        };
        let out = token_cmd_apply(
            &args(&["apply", "blue", "--all", "--dry-run"]),
            &mut runner,
            &mut tmux,
        )
        .expect("apply");
        ...
        assert!(out
            .stdout
            .contains("plan %1: /exit; direnv reload 2>/dev/null; claude -c"));
        assert!(out.stdout.contains("skip %2: busy"));
        assert!(tmux
            .actions
            .iter()
            .all(|action| action.starts_with("sleep ")));
    }
    ...
    fn token_apply_live_writes_envrc_then_restarts_idle_pane() {
        ...
        let mut tmux = FakeApply {
            panes: vec![pane("%3", "s1")],
            captures: vec!["idle".into(), "idle".into()],
            actions: Vec::new(),
        };
        let out = token_cmd_apply(
            &args(&["apply", "green", "--session", "s1"]),
            &mut runner,
            &mut tmux,
        )
        .expect("apply");
        std::env::set_current_dir(old).expect("restore cwd");
        assert!(std::fs::read_to_string(cwd.join(".envrc"))
            .expect("envrc")
            .contains("CLAUDE_TOKEN_NAME=\"green\""));
        ...
        assert_eq!(
            ...
            vec![
                ...
                "enter %3",
                "sleep 1500",
                "text %3 direnv reload 2>/dev/null; claude -c",
                "enter %3"
            ]
        );
    }
