const DISPATCH_290: &[DispatcherEntry] = &[DispatcherEntry {
    command: "reindex-gpu",
    handler: Handler::Async(reindex_async_native),
}];

const REINDEX_USAGE: &str = "usage: maw reindex-gpu [root] [--json] [--timeout-ms <ms>]";
const REINDEX_MAX_BYTES_PER_FILE: u64 = 256 * 1024;
const REINDEX_ALLOWED_EXTENSIONS: &[&str] =
...
trait ReindexGateway290 {
    fn post<'a>(
        ...
        Box<dyn std::future::Future<Output = Result<ReindexPostOutcome290, String>> + Send + 'a>,
    >;
}
...
impl ReindexGateway290 for ReindexReqwestGateway290 {
    ...
    > {
        Box::pin(
            async move { reindex_reqwest_post_once(endpoint, payload, timeout_ms, used_alt).await },
        )
    }
}

fn reindex_async_native(
    args: Vec<String>,
) -> std::pin::Pin<Box<dyn std::future::Future<Output = CliOutput> + Send>> {
    Box::pin(async move {
...
async fn reindex_run_async_with<G: ReindexGateway290 + Send>(
    argv: &[String],
    gateway: &mut G,
) -> CliOutput {
    match reindex_execute_with(argv, gateway).await {
        Ok(output) => output,
        Err(message) if message == REINDEX_USAGE => CliOutput {
            code: 0,
            stdout: format!("{REINDEX_USAGE}\n"),
            stderr: String::new(),
        },
...
async fn reindex_execute_with<G: ReindexGateway290 + Send>(
    argv: &[String],
    gateway: &mut G,
...
fn reindex_parse_args(argv: &[String]) -> Result<ReindexArgs290, String> {
    ...
    let mut json = false;
    let mut timeout_ms = std::env::var("MAW_REINDEX_GPU_TIMEOUT_MS")
        .ok()
    ...
    while index < argv.len() {
        ...
        match arg.as_str() {
            ...
            "--timeout-ms" => {
                ...
                let raw = argv
                    .get(index)
                    .ok_or_else(|| "reindex-gpu: --timeout-ms requires a value".to_owned())?;
                reindex_validate_token(raw, "timeout")?;
                timeout_ms = raw
                    .parse::<u64>()
                    .map_err(|_| "reindex-gpu: --timeout-ms must be numeric".to_owned())?;
                if !(100..=120_000).contains(&timeout_ms) {
                    return Err(
                        "reindex-gpu: --timeout-ms must be between 100 and 120000".to_owned()
                    );
                }
            }
            "--endpoint" | "--alt-endpoint" => {
                return Err(
                    "reindex-gpu: gateway endpoint must come from config/env, not argv".to_owned(),
                )
            }
            _ if arg.starts_with('-') => {
                return Err(format!("reindex-gpu: leading dash rejected: {arg}"))
            }
            _ => {
                if root.is_some() {
                    return Err(REINDEX_USAGE.to_owned());
                }
    ...
    if let Some(alt) = &alt_endpoint {
        reindex_validate_endpoint(alt, "alt endpoint")?;
    }
...
fn reindex_endpoint_from_env_or_config() -> Result<String, String> {
    if let Ok(value) = std::env::var("MAW_REINDEX_GPU_ENDPOINT") {
        if !value.trim().is_empty() {
            return Ok(value);
        }
    }
    if let Some(value) = reindex_config_string("reindexGpuEndpoint")
        .or_else(|| reindex_config_nested_string("reindexGpu", "endpoint"))
    {
        return Ok(value);
    }
...
fn reindex_alt_endpoint_from_env_or_config() -> Option<String> {
    if let Ok(value) = std::env::var("MAW_REINDEX_GPU_ALT_ENDPOINT") {
        return (!value.trim().is_empty()).then_some(value);
    }
    reindex_config_string("reindexGpuAltEndpoint")
        .or_else(|| reindex_config_nested_string("reindexGpu", "altEndpoint"))
}

fn reindex_index_path_from_env_or_config() -> Result<std::path::PathBuf, String> {
    let path = std::env::var("MAW_REINDEX_GPU_INDEX_PATH")
        .ok()
        .filter(|value| !value.trim().is_empty())
        .or_else(|| {
            reindex_config_string("reindexGpuIndexPath")
                .or_else(|| reindex_config_nested_string("reindexGpu", "indexPath"))
        });
...
fn reindex_build_payload(args: &ReindexArgs290) -> Result<ReindexPayload290, String> {
    let root = std::fs::canonicalize(&args.root)
        .map_err(|error| format!("reindex-gpu: root not readable: {error}"))?;
    if !root.is_dir() {
        return Err("reindex-gpu: root must be a directory".to_owned());
    }
    let mut documents = Vec::new();
    reindex_collect_documents(&root, &root, &mut documents)?;
    documents.sort_by(|a, b| a.path.cmp(&b.path));
    if documents.is_empty() {
        return Err("reindex-gpu: no indexable documents found".to_owned());
    }
    Ok(ReindexPayload290 {
        client: "maw-rs",
        mode: "remote-gpu-embed",
        root: root.to_string_lossy().into_owned(),
        documents,
    })
}

fn reindex_collect_documents(
    root: &std::path::Path,
    dir: &std::path::Path,
    out: &mut Vec<ReindexDocument290>,
) -> Result<(), String> {
    let entries = std::fs::read_dir(dir)
        .map_err(|error| format!("reindex-gpu: read {} failed: {error}", dir.display()))?;
    for entry in entries.flatten() {
        ...
        if !file_type.is_file() || !reindex_is_indexable(&path) {
            continue;
        }
        let Ok(meta) = entry.metadata() else { continue };
        if meta.len() > REINDEX_MAX_BYTES_PER_FILE {
            continue;
        }
...
async fn reindex_post_with_fallback<G: ReindexGateway290 + Send>(
    args: &ReindexArgs290,
    payload: &ReindexPayload290,
    gateway: &mut G,
) -> Result<ReindexPostOutcome290, String> {
    let primary = gateway
        .post(&args.endpoint, payload, args.timeout_ms, false)
        .await;
    match primary {
        Ok(outcome) => Ok(outcome),
        Err(primary_error) => {
            let Some(alt) = &args.alt_endpoint else {
                return Err(primary_error);
            };
            gateway
                .post(alt, payload, args.timeout_ms, true)
                .await
                .map_err(|alt_error| {
                    format!("{primary_error}; alternate endpoint failed: {alt_error}")
                })
        }
    }
}

async fn reindex_reqwest_post_once(
    endpoint: &str,
    payload: &ReindexPayload290,
    timeout_ms: u64,
    used_alt: bool,
    ...
    if !status.is_success() {
        return Err(format!(
            "reindex-gpu: remote gpu-embed gateway returned HTTP {}",
            status.as_u16()
        ));
    }
    Ok(ReindexPostOutcome290 {
        endpoint: endpoint.to_owned(),
        status: status.as_u16(),
        body: reindex_redact_body(&body),
        used_alt,
    })
}

fn reindex_build_index(
    payload: &ReindexPayload290,
    outcome: &ReindexPostOutcome290,
) -> ReindexIndex290 {
    ReindexIndex290 {
        command: "reindex-gpu",
        ...
        documents: payload
            .documents
            .iter()
            .map(|doc| ReindexIndexDocument290 {
                path: doc.path.clone(),
                bytes: doc.bytes,
            })
            .collect(),
...
fn reindex_write_index_atomic(
    path: &std::path::Path,
    index: &ReindexIndex290,
) -> Result<(), String> {
    ...
    std::fs::create_dir_all(parent).map_err(|error| {
        format!(
            "reindex-gpu: create index dir {} failed: {error}",
            parent.display()
        )
    })?;
    let body = serde_json::to_string_pretty(index)
        .map_err(|error| format!("reindex-gpu: encode index failed: {error}"))?
        + "\n";
    ...
    #[cfg(unix)]
    opts.mode(0o600);
    let mut file = opts.open(&tmp).map_err(|error| {
        format!(
            "reindex-gpu: open temp index {} failed: {error}",
            tmp.display()
        )
    })?;
    file.write_all(body.as_bytes()).map_err(|error| {
        format!(
            "reindex-gpu: write temp index {} failed: {error}",
            tmp.display()
        )
    })?;
    file.sync_all().map_err(|error| {
        format!(
            "reindex-gpu: sync temp index {} failed: {error}",
            tmp.display()
        )
    })?;
    drop(file);
    std::fs::rename(&tmp, path).map_err(|error| {
        format!(
            "reindex-gpu: rename index {} -> {} failed: {error}",
            tmp.display(),
            path.display()
        )
    })
...
fn reindex_validate_endpoint(value: &str, label: &str) -> Result<(), String> {
    ...
    let parsed = reqwest::Url::parse(value)
        .map_err(|error| format!("reindex-gpu: invalid {label}: {error}"))?;
    if parsed.username() != "" || parsed.password().is_some() {
        return Err(format!(
            "reindex-gpu: invalid {label}: credentials in URL rejected"
        ));
    }
    if parsed.path() != "/api/embed" {
        return Err(format!(
            "reindex-gpu: invalid {label}: path must be /api/embed"
        ));
    }
    match parsed.scheme() {
        "https" => Ok(()),
...
fn reindex_validate_path_arg(value: &str) -> Result<(), String> {
    ...
    {
        return Err("reindex-gpu: path traversal rejected".to_owned());
    }
    Ok(())
}
...
fn reindex_validate_token(value: &str, label: &str) -> Result<(), String> {
    if value.trim().is_empty() {
        return Err(format!("reindex-gpu: {label} is empty"));
    }
    if value.starts_with('-') {
        return Err(format!("reindex-gpu: {label} leading dash rejected"));
    }
    if value.chars().any(|ch| ch == '\0' || ch.is_control()) {
        return Err(format!("reindex-gpu: {label} control character rejected"));
    }
    Ok(())
}
...
mod reindex_tests290 {
    ...
    impl ReindexGateway290 for MockGateway290 {
        fn post<'a>(
            ...
            Box<
                dyn std::future::Future<Output = Result<ReindexPostOutcome290, String>> + Send + 'a,
            >,
        > {
            Box::pin(async move {
                self.calls
                    .push((endpoint.to_owned(), used_alt, payload.documents.len()));
                if self.fail_first && !used_alt {
                    return Err(
                        "reindex-gpu: remote gpu-embed gateway unreachable: mock".to_owned()
                    );
                }
                Ok(ReindexPostOutcome290 {
                    endpoint: endpoint.to_owned(),
    ...
    fn reindex_parse_rejects_endpoint_and_path_injection() {
        ...
        std::env::set_var(
            "MAW_REINDEX_GPU_ENDPOINT",
            "http://user:pw@example.test/api/embed",
        );
        assert!(reindex_parse_args(&[])
            .expect_err("creds")
            .contains("credentials"));
        std::env::set_var("MAW_REINDEX_GPU_ENDPOINT", "http://example.test/api/embed");
        assert!(reindex_parse_args(&[])
            .expect_err("ssrf")
            .contains("http endpoint must be loopback"));
        std::env::set_var("MAW_REINDEX_GPU_ENDPOINT", "https://example.test/not-embed");
        assert!(reindex_parse_args(&[])
            .expect_err("path")
            .contains("/api/embed"));
        assert!(reindex_parse_args(&["../brain".to_owned()])
            .expect_err("traversal")
            .contains("traversal"));
        assert!(reindex_parse_args(&["-bad".to_owned()])
            .expect_err("dash")
            .contains("leading dash"));
    }
    ...
    fn reindex_mock_gateway_writes_atomic_index_and_matches_golden() {
        ...
        assert_eq!(
            normalized,
            include_str!("../../tests/fixtures/native-reindex-gpu/reindex-mock.stdout")
        );
        assert_eq!(
            gateway.calls,
            vec![("https://gpu.example.test/api/embed".to_owned(), false, 2)]
        );
    ...
    fn reindex_unreachable_does_not_write_partial_index_and_alt_is_remote_only() {
        ...
        std::env::set_var(
            "MAW_REINDEX_GPU_ENDPOINT",
            "https://primary.example.test/api/embed",
        );
        std::env::set_var(
            "MAW_REINDEX_GPU_ALT_ENDPOINT",
            "https://alt.example.test/api/embed",
        );
        std::env::set_var("MAW_REINDEX_GPU_INDEX_PATH", &index_path);
        let mut gateway = MockGateway290 {
            fail_first: true,
            ..MockGateway290::default()
        };
        let output = reindex_test_block_on(reindex_run_async_with(
            &[docs.to_string_lossy().into_owned()],
            &mut gateway,
        ...
        assert_eq!(
            ...
            vec![
                (
                    "https://primary.example.test/api/embed".to_owned(),
                    false,
                    1
                ),
        ...
        assert!(std::fs::read_to_string(&index_path)
            .expect("index")
            .contains("alt.example.test"));

        std::fs::remove_file(&index_path).expect("reset index");
        std::env::remove_var("MAW_REINDEX_GPU_ALT_ENDPOINT");
        let mut failing = MockGateway290 {
            fail_first: true,
            ..MockGateway290::default()
        };
        let failed = reindex_test_block_on(reindex_run_async_with(
            &[docs.to_string_lossy().into_owned()],
            &mut failing,
        ));
        ...
        assert!(
            !index_path.exists(),
            "unreachable gateway must not write partial index"
        );
        drop(restores);
    }
    ...
    fn reindex_redacts_secret_shaped_gateway_body() {
        assert_eq!(
            reindex_redact_body("ok ghp_secret github_pat_secret sk-secret short"),
            "ok [redacted] [redacted] [redacted] short"
        );
    }

    fn reindex_temp_dir(label: &str) -> std::path::PathBuf {
        let path =
            std::env::temp_dir().join(format!("maw-rs-reindex-gpu-{label}-{}", random_hex(6)));
