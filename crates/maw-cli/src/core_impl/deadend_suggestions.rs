...
fn deadend_oracle_candidates() -> Vec<maw_matcher::ResolveTypedCandidate> {
    ...
    if let Some(cache) = locate_load_registry_cache() {
        for oracle in cache.oracles {
            if oracle.name.is_empty() || !seen.insert(oracle.name.to_lowercase()) {
                continue;
            }
            let aliases = [oracle.repo, oracle.local_path]
                .into_iter()
...
fn deadend_closest_matches(
    ...
    for candidate in candidates {
        let names = std::iter::once(candidate.name.as_str())
            .chain(candidate.aliases.iter().map(String::as_str))
            .flat_map(maw_matcher::normalized_match_names)
            .collect::<Vec<_>>();
        let Some(distance) = targets
            .iter()
            .flat_map(|target| names.iter().map(|name| deadend_edit_distance(target, name)))
            .min()
        else {
            continue;
        };
    ...
    scored
        .into_iter()
        .filter_map(|(_, name, candidate)| {
            seen.insert(name).then_some(maw_matcher::ResolveMatch {
                rank: maw_matcher::ResolveMatchRank::Fuzzy,
                candidate,
            })
        })
        .take(5)
    ...
    if !candidates.is_empty() {
        ...
        for matched in candidates {
            let action = match (command, matched.candidate.kind) {
                ...
                ) => format!("maw attach {}", matched.candidate.name),
                ("attach", maw_matcher::ResolveCandidateKind::FleetSquad) => {
