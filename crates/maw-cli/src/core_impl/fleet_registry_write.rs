...
fn fleet_registry_upsert_session_for_env(
    ...
    std::fs::create_dir_all(&dir)
        .map_err(|error| format!("fleet registry: create {}: {error}", dir.display()))?;
        ...
        .or_else(|| {
            entries.iter().find(|entry| {
                fleet_entry_is_session(entry)
                    ...
                    && entry.session.windows.iter().any(|window| {
                        windows_by_repo.contains(&fleet_repo_canonical_key(&window.repo))
                    })
            })
        })
        .map_or_else(
    ...
    let body = serde_json::to_string_pretty(&value)
        .map_err(|error| format!("fleet registry: render json: {error}"))?
        + "\n";
    std::fs::write(&path, body)
        .map_err(|error| format!("fleet registry: write {}: {error}", path.display()))?;
    let window_count = value
        .get("windows")
        .and_then(serde_json::Value::as_array)
        .map_or(0, Vec::len);
    Ok(FleetRegistryWrite {
        path,
        created,
        window_count,
    })
...
fn fleet_fix_duplicate_windows(
    entries: &[NativeFleetEntry],
    live: &[TmuxSession],
) -> Result<Vec<FleetWindowRepair>, String> {
    let mut repairs = Vec::new();
    for entry in entries.iter().filter(|entry| fleet_entry_is_session(entry)) {
        let text = std::fs::read_to_string(&entry.path)
            .map_err(|error| format!("fleet doctor: read {}: {error}", entry.path.display()))?;
        let mut value: serde_json::Value = serde_json::from_str(&text)
            .map_err(|error| format!("fleet doctor: parse {}: {error}", entry.path.display()))?;
        ...
        repairs.push(FleetWindowRepair {
...
fn fleet_dedupe_window_values(
    ...
    for (index, window) in windows.iter().enumerate() {
        ...
        {
            by_repo
                .entry(fleet_repo_canonical_key(repo))
                .or_default()
                .push(index);
        }
    }
    let mut remove = BTreeSet::new();
    for indices in by_repo
        .iter()
        .filter(|(repo, indices)| mergeable.contains(*repo) && indices.len() > 1)
        ...
        let kept = indices
            ...
            .find(|&&index| {
                windows[index]
                    .get("kind")
                    .and_then(serde_json::Value::as_str)
                    .and_then(native_repo_kind_from_role)
                    .is_some()
            })
            .copied()
...
fn fleet_write_json_atomic(
    ...
    let body = serde_json::to_string_pretty(value)
        .map_err(|error| format!("fleet doctor: render json: {error}"))?
        + "\n";
    std::fs::write(&tmp, body)
        .map_err(|error| format!("fleet doctor: write {}: {error}", tmp.display()))?;
    if let Ok(metadata) = std::fs::metadata(path) {
        if let Err(error) = std::fs::set_permissions(&tmp, metadata.permissions()) {
            let _ = std::fs::remove_file(&tmp);
            return Err(format!(
                "fleet doctor: set permissions {}: {error}",
                tmp.display()
            ));
        }
    }
...
fn fleet_registry_merge_windows(
    ...
    let existing_repo_counts =
        windows
            .iter()
            .fold(BTreeMap::<String, usize>::new(), |mut counts, window| {
                *counts
                    .entry(fleet_repo_canonical_key(&window.repo))
                    .or_default() += 1;
                counts
            });
        ...
        .filter(|window| !window.name.trim().is_empty())
    {
        ...
        let lone_repo_alias = existing_repo_counts.get(&repo_key) == Some(&1)
            && update_repo_counts.get(&repo_key) == Some(&1);
...
fn fleet_registry_windows_from_tmux(
    ...
    for window in windows {
        let Some(cwd) = window
            .cwd
            .as_deref()
            .and_then(|cwd| fleet_repo_slug_from_path(std::path::Path::new(cwd), repos_root))
        else {
            continue;
        };
        ...
        if seen.insert(name.clone()) {
            ...
            result.push(FleetWindowSummary {
                name,
                repo: cwd,
                kind,
            });
