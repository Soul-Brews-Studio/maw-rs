...
fn tmux_close_with_runner<R: maw_tmux::TmuxRunner>(
    ...
    if let Some(target) = opts.target.as_deref() {
        ...
        if target == current_pane {
            return Err((
                1,
                "tmux close: refusing to close current pane (would lose this shell)".to_owned(),
            ));
        }
    ...
    Ok(format!(
        "✓ closed {hidden} pane{} (hidden — still alive)\n",
        if hidden == 1 { "" } else { "s" }
    ))
}
...
fn tmux_close_list_current_window_panes<R: maw_tmux::TmuxRunner>(
    runner: &mut R,
) -> Result<Vec<String>, (i32, String)> {
    let args = vec!["-F".to_owned(), "#{pane_id}".to_owned()];
    let raw = runner.run("list-panes", &args).map_err(|error| {
        (
            1,
            format!("tmux close: list-panes failed: {}", error.message),
        )
    })?;
    Ok(raw
        .lines()
...
fn tmux_close_break_pane_detached<R: maw_tmux::TmuxRunner>(
    target: &str,
    runner: &mut R,
) -> Result<(), (i32, String)> {
    let args = vec!["-d".to_owned(), "-s".to_owned(), target.to_owned()];
    runner.run("break-pane", &args).map_err(|error| {
        (
            1,
            format!(
                "tmux close: break-pane failed for '{target}': {}",
                error.message
            ),
        )
    })?;
...
fn tmux_close_validate_target(value: &str) -> Result<(), String> {
    if value.is_empty() || value.trim() != value || value == "--" || value.starts_with('-') {
        return Err(
            "tmux close: target must be non-empty, unpadded, not '--', and not start with '-'"
                .to_owned(),
        );
    }
...
mod tmux_close_tests281 {
    ...
    fn tmux_close_explicit_target_breaks_detached_with_arg_vector() {
        ...
        assert_eq!(
            runner.calls,
            vec![("break-pane".to_owned(), strings(&["-d", "-s", "%42"]))]
        );
    }
    ...
    fn tmux_close_no_target_hides_non_current_panes() {
        ...
        let mut runner = CloseFakeRunner {
            panes: strings(&["%1", "%2", "%3"]),
            ..CloseFakeRunner::default()
        };
    ...
    fn tmux_close_refuses_current_pane_before_runner() {
        ...
        let mut runner = CloseFakeRunner::default();
        let err =
            tmux_close_with_runner(&strings(&["%7"]), &mut runner).expect_err("current pane guard");
        assert_eq!(err.0, 1);
        assert!(err.1.contains("refusing to close current pane"));
        assert!(
            runner.calls.is_empty(),
            "current pane reached tmux: {:?}",
            runner.calls
        );
    }
    ...
    fn tmux_close_rejects_leading_dash_before_runner() {
        ...
        let _env = EnvGuard::set("/tmp/tmux-1/default,1,0", "%1");
        let mut runner = CloseFakeRunner::default();
        let err = tmux_close_with_runner(&strings(&["-oProxyCommand=bad"]), &mut runner)
            .expect_err("guard");
        assert_eq!(err.0, 2);
