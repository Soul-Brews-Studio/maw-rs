...
fn team_remove(argv: &[String]) -> Result<String, String> {
    ...
    let charter_text = std::fs::read_to_string(&charter_path)
        .map_err(|error| format!("team remove: read charter failed: {error}"))?;
    ...
    let target = selected
        .pane
        .as_ref()
        .map_or_else(|| selected.identity.clone(), |pane| pane.window.clone());
    team_t3_validate_token(&target, "remove target")?;
    let has_worktree = team_remove_has_worktree(&charter, &selected.role);
    let mut actions = Vec::new();
    if !has_worktree {
        actions.push(format!(
            "teardown: no worktree for {target} (worktree: false)"
        ));
    } else if opts.dry_run {
        let suffix = if opts.keep_branch {
            " (keep branch)"
        } else {
            ""
        };
    ...
    if opts.dry_run {
        actions.push(format!(
            "charter: would remove '{}' from {charter_path}",
            selected.role
        ));
    } else {
        team_atomic_write_0600(std::path::Path::new(&charter_path), &updated)?;
        actions.push(format!(
            "charter: removed '{}' from {charter_path}",
            selected.role
        ));
    }
    Ok(team_remove_render(
        &team,
        &selected.role,
        &target,
        &selected.state,
    ...
    while index < argv.len() {
        match argv[index].as_str() {
            ...
            value if value.starts_with('-') && selector.is_none() => {
                return Err(format!(
                    "unsafe member selector '{value}': leading dash rejected"
                ))
            }
            value if value.starts_with('-') => {
                return Err(format!("team remove: unknown argument {value}"))
            }
...
fn team_remove_select(
    charter: &TeamCharter122,
    selector: &str,
    session: &str,
) -> Result<TeamRosterItem124, String> {
    ...
    let matches = charter
        ...
        .filter_map(|member| {
            ...
            if team_remove_member_matches(member, selector, &item.identity, &item.worktree) {
                Some(item)
            } else {
                None
            }
    ...
    match matches.len() {
        ...
        _ => Err(format!(
            "member '{selector}' is ambiguous across: {}",
            matches
                .iter()
...
fn team_remove_member_matches(
    ...
    selector == member.role
        || selector == identity
        || selector == worktree
        || member.name.as_deref() == Some(selector)
        || member.cwd.as_deref() == Some(selector)
}
...
fn team_remove_reject_adopted(charter: &TeamCharter122, role: &str) -> Result<(), String> {
    if charter
        .members
        .iter()
        .any(|member| member.role == role && member.adopted)
    {
...
fn team_remove_validate_live_target(session: &str, target: &str) -> Result<(), String> {
    ...
    match matches {
        1 => Ok(()),
        0 => Err(format!(
            "team remove refuse missing live target before teardown: {session}:{target}"
        )),
        _ => Err(format!(
            "team remove refuse ambiguous live target before teardown: {session}:{target}"
        )),
    }
}

fn team_remove_archive_before_done(
    team: &str,
    member: &str,
    charter_path: &str,
) -> Result<(), String> {
    let archive_dir = team_psi_dir()
        .join("memory")
        .join("mailbox")
        .join(member)
        .join(format!("team-{team}-remove-archive"));
    std::fs::create_dir_all(&archive_dir)
        .map_err(|error| format!("team remove archive mkdir failed: {error}"))?;
    ...
    if inbox.exists() {
        std::fs::copy(&inbox, archive_dir.join("inbox.json"))
            .map_err(|error| format!("team remove archive inbox failed: {error}"))?;
    }
    if std::path::Path::new(charter_path).exists() {
        std::fs::copy(charter_path, archive_dir.join("charter.yaml"))
            .map_err(|error| format!("team remove archive charter failed: {error}"))?;
    }
    if paths.tool_config.exists() {
        std::fs::copy(&paths.tool_config, archive_dir.join("config.json"))
            .map_err(|error| format!("team remove archive config failed: {error}"))?;
    }
    team_remove_record_fake("archive", member)
}

fn team_remove_done(session: &str, target: &str, keep_branch: bool) -> Result<(), String> {
    if team_remove_fake_mode() {
        return team_remove_record_fake(
            "done",
            &format!("{session}:{target}:keep_branch={keep_branch}"),
        );
    }
...
fn team_remove_fake_mode() -> bool {
    std::env::var_os("MAW_RS_TEAM_REMOVE_FAKE_LOG").is_some()
}

fn team_remove_record_fake(kind: &str, value: &str) -> Result<(), String> {
    use std::io::Write as _;
    let Some(path) = std::env::var_os("MAW_RS_TEAM_REMOVE_FAKE_LOG") else {
        return Ok(());
    };
    let path = std::path::PathBuf::from(path);
    if let Some(parent) = path.parent() {
        std::fs::create_dir_all(parent)
            .map_err(|error| format!("team remove fake log mkdir failed: {error}"))?;
    }
    let mut file = std::fs::OpenOptions::new()
        .create(true)
        .append(true)
        .open(&path)
        .map_err(|error| format!("team remove fake log open failed: {error}"))?;
    writeln!(file, "{kind}\t{value}")
        .map_err(|error| format!("team remove fake log write failed: {error}"))
}

fn team_remove_render(
    team: &str,
    member: &str,
    target: &str,
    state: &str,
...
fn team_remove_charter_text(text: &str, selector: &str) -> Result<String, String> {
    if text.trim_start().starts_with('{') {
        return team_remove_json_charter_text(text, selector);
    }
    ...
    let matches = blocks
        .iter()
        .filter(|block| team_remove_block_matches(block, selector))
        .collect::<Vec<_>>();
    if matches.is_empty() {
        return Err(format!("member not found in charter: {selector}"));
    }
    if matches.len() > 1 {
        return Err(format!(
            "member '{selector}' is ambiguous in charter ({} matches)",
            matches.len()
        ));
    }
    if blocks.len() == 1 {
        return Err(format!("refusing to remove the last member '{selector}'; a team charter requires at least one member"));
    }
    let block = matches[0];
...
fn team_remove_member_blocks(lines: &[String]) -> Vec<TeamRemoveBlock130> {
    let Some(members_idx) = lines.iter().position(|line| line.trim() == "members:") else {
        return Vec::new();
    };
    ...
    while index < lines.len() {
        ...
        if dash_indent.is_none() && is_dash {
            dash_indent = Some(indent);
        }
        if !is_dash || Some(indent) != dash_indent {
            index += 1;
            continue;
        }
        let start = index;
        index += 1;
        while index < lines.len()
            && (lines[index].trim().is_empty() || team_remove_indent(&lines[index]) > indent)
        {
            index += 1;
        }
        let end = index;
        ...
        if !member.role.is_empty() {
            blocks.push(TeamRemoveBlock130 { start, end, member });
        }
    }
    blocks
}

fn team_remove_indent(line: &str) -> usize {
...
fn team_remove_parse_block(lines: &[String]) -> TeamCharterMember122 {
    ...
    for raw in lines {
        ...
        for (key, slot) in [
            ("role:", &mut member.role),
            ("name:", member.name.get_or_insert_with(String::new)),
            ("cwd:", member.cwd.get_or_insert_with(String::new)),
            ("worktree:", member.worktree.get_or_insert_with(String::new)),
        ] {
            if let Some(rest) = line.strip_prefix(key) {
                *slot = team_unquote(rest);
            }
        }
    }
    member
...
fn team_remove_block_matches(block: &TeamRemoveBlock130, selector: &str) -> bool {
    selector == block.member.role
        || block.member.name.as_deref() == Some(selector)
        || block.member.cwd.as_deref() == Some(selector)
        || block.member.worktree.as_deref() == Some(selector)
}

fn team_remove_json_charter_text(text: &str, selector: &str) -> Result<String, String> {
    let mut value: serde_json::Value = serde_json::from_str(text)
        .map_err(|error| format!("team remove json charter parse failed: {error}"))?;
    let members = value["members"]
        .as_array_mut()
        .ok_or_else(|| "team remove json charter missing members".to_owned())?;
    let matches = members
        .iter()
        .enumerate()
        .filter(|(_, member)| {
            ["role", "name", "cwd", "worktree"]
                .iter()
                .any(|key| member[*key].as_str() == Some(selector))
        })
        .map(|(index, _)| index)
        .collect::<Vec<_>>();
    if matches.is_empty() {
