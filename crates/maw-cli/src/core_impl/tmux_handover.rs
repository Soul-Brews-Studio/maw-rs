const DISPATCH_81: &[DispatcherEntry] = &[
    DispatcherEntry {
        command: "take",
        handler: Handler::Sync(take_run_command),
    },
    DispatcherEntry {
        command: "handover",
        handler: Handler::Sync(take_run_command),
    ...
    DispatcherEntry {
        command: "mv",
        handler: Handler::Sync(mv_run_command),
    },
];
...
fn take_with_runner<R: maw_tmux::TmuxRunner>(
    ...
    if split {
        take_create_session(runner, &target)?;
    }
    if target == source.session {
        return Ok(take_same_session_output());
    }
    let sessions = take_list_sessions(runner)?;
    let src_session = take_find_session(&sessions, &source.session)?;
    let src_window =
        take_resolve_source_window(&src_session.windows, &source.window).ok_or_else(|| {
            format!(
                "window '{}' not found in session '{}'",
                source.window, source.session
            )
        })?;
    ...
    if split {
        take_kill_default_window(runner, &target);
    }
    Ok(take_success_output(
        &src_session.name,
        &src_window.name,
        &target,
        split,
...
fn take_parse_args(argv: &[String]) -> Result<TakeOptions, String> {
    if argv.is_empty() {
        return Err(take_usage());
    }
    if argv.iter().any(|arg| arg == "--") {
        return Err("take does not accept -- separator".to_owned());
    }
    if argv.len() > 2 {
        return Err(take_usage());
    }
    ...
    let target = argv
        .get(1)
        .map(|value| take_validate_cli_value(value, "target-session"))
        .transpose()?;
...
fn take_validate_cli_value(value: &str, label: &str) -> Result<String, String> {
    if value.is_empty() || value.trim() != value || value.starts_with('-') || value == "--" {
        return Err(format!(
            "take {label} must be non-empty, unpadded, and not start with '-'"
        ));
    }
    if value
        .chars()
    ...
    {
        return Err(format!(
            "take {label} must not contain whitespace or control characters"
        ));
    }
    Ok(value.to_owned())
}
...
fn take_parse_source(source: &str) -> Result<TakeSource, String> {
    let Some((session, window)) = source.split_once(':') else {
        return Err(take_usage_with_example());
    };
...
fn take_default_split_target_name(window: &str) -> String {
    take_strip_tmux_display_suffix(window)
        .unwrap_or(window)
        .to_owned()
}

fn take_create_session<R: maw_tmux::TmuxRunner>(
    runner: &mut R,
    target: &str,
) -> Result<(), String> {
    take_validate_tmux_target_part(target, "target session")?;
    match runner.run(
        "new-session",
        &["-d".to_owned(), "-s".to_owned(), target.to_owned()],
    ) {
        Ok(_) => Ok(()),
        Err(error) if error.message.contains("duplicate") => Ok(()),
        Err(error) => Err(format!(
            "could not create session '{target}': {}",
            error.message
        )),
    }
}
...
fn take_find_session<'a>(
    sessions: &'a [TakeSession],
    requested: &str,
) -> Result<&'a TakeSession, String> {
    sessions
        .iter()
...
fn take_exact_window(windows: &[TakeWindow], requested: &str) -> Option<TakeWindow> {
    windows
        .iter()
        .find(|window| {
            window.name.eq_ignore_ascii_case(requested) || window.index.to_string() == requested
        })
        .cloned()
}
...
fn take_pane_cwd<R: maw_tmux::TmuxRunner>(
    runner: &mut R,
    source_target: &str,
...
fn take_kill_default_window<R: maw_tmux::TmuxRunner>(runner: &mut R, target_session: &str) {
    if take_validate_tmux_target_part(target_session, "target session").is_err() {
        return;
    }
    let _ = runner.run(
        "kill-window",
        &["-t".to_owned(), format!("{target_session}:1")],
    );
}
...
fn take_same_session_output() -> CliOutput {
    CliOutput {
        code: 0,
        stdout: "  \x1b[33m⚠\x1b[0m source and target are the same session\n".to_owned(),
        stderr: String::new(),
    }
}
...
fn take_success_output(
    ...
    let mut stdout =
        format!("  \x1b[32m✓\x1b[0m {source_session}:{source_window} → {target}{suffix}\n");
    if !pane_cwd.is_empty() {
        let _ = writeln!(stdout, "  \x1b[90m  cwd: {pane_cwd}\x1b[0m");
    }
    CliOutput {
        code: 0,
        stdout,
...
fn mv_run_command(argv: &[String]) -> CliOutput {
    match mv_with_runner(argv, &mut maw_tmux::CommandTmuxRunner::new()) {
        ...
        Err(message) if message == MV_USAGE => CliOutput {
            code: 2,
            stdout: String::new(),
            stderr: format!("{message}\n"),
        },
...
fn mv_with_runner<R: maw_tmux::TmuxRunner>(
    argv: &[String],
    runner: &mut R,
) -> Result<String, String> {
    ...
    let [source_raw, index_raw] = argv else {
        return Err(MV_USAGE.to_owned());
    };
    ...
    Ok(format!(
        "  \x1b[32m✓\x1b[0m moved {}:{} → {dest_target}\n",
...
fn mv_parse_index(value: &str) -> Result<u32, String> {
    ...
    value
        .parse::<u32>()
        .map_err(|_| "mv index is too large".to_owned())
}
...
mod take_tests {
    ...
    fn take_strings(values: &[&str]) -> Vec<String> {
    ...
    fn mv_rejects_bad_index_before_move_window() {
        ...
        let err =
            mv_with_runner(&take_strings(&["neo:work", "-1"]), &mut tmux).expect_err("bad index");
    ...
    fn take_moves_to_explicit_target_and_prints_cwd() {
        ...
        let mut tmux = TakeMockTmux {
            windows: "neo\t2\tneo-skills\n".to_owned(),
            cwd: "/repo\n".to_owned(),
            ..Default::default()
        };

        let output =
            take_with_runner(&take_strings(&["neo:neo-skills", "pulse"]), &mut tmux).expect("take");

        assert_eq!(
            output.stdout,
            "  \x1b[32m✓\x1b[0m neo:neo-skills → pulse\n  \x1b[90m  cwd: /repo\x1b[0m\n"
        );
        assert_eq!(
            tmux.calls[0],
            (
        ...
        assert_eq!(
            ...
            (
                "move-window".to_owned(),
                take_strings(&["-s", "neo:neo-skills", "-t", "pulse:"])
            )
        );
    }
    ...
    fn take_split_creates_session_and_kills_default_window() {
        ...
        assert_eq!(
            output.stdout,
            "  \x1b[32m✓\x1b[0m neo:skills → skills (new session)\n"
        );
        assert_eq!(
            tmux.calls[0],
            (
                "new-session".to_owned(),
                take_strings(&["-d", "-s", "skills"])
            )
        );
        assert_eq!(
            tmux.calls[4],
            ("kill-window".to_owned(), take_strings(&["-t", "skills:1"]))
        );
    }
    ...
    fn take_same_session_returns_warning_after_split_creation() {
        ...
        let output =
            take_with_runner(&take_strings(&["neo:work", "neo"]), &mut tmux).expect("same");

        assert_eq!(
            output.stdout,
            "  \x1b[33m⚠\x1b[0m source and target are the same session\n"
        );
        assert!(tmux.calls.is_empty());
    }
    ...
    fn take_duplicate_new_session_is_ignored_but_move_failure_is_reported() {
        ...
        let mut tmux = TakeMockTmux {
            windows: "neo\t0\twork\n".to_owned(),
            fail_new: Some("duplicate session".to_owned()),
            fail_move: true,
            ..Default::default()
        };
