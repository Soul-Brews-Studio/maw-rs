const DISPATCH_119: &[DispatcherEntry] = &[DispatcherEntry {
    command: "tonk",
    handler: Handler::Sync(tonk_run_command),
}];
...
fn tonk_run_with_runner<R: TonkGhRunner>(argv: &[String], runner: &mut R) -> CliOutput {
    match tonk_render(argv, runner) {
        ...
        Err(message) => CliOutput {
            code: 1,
            stdout: format!("🌿 gh error: {}\n", tonk_redact_error(&message)),
            stderr: String::new(),
        },
    }
}
...
fn tonk_gh_whoami<R: TonkGhRunner>(runner: &mut R) -> Result<String, String> {
    let out = runner.tonk_gh(&tonk_vec(&[
        "api",
        "user",
        "--jq",
        ".login + \" (\" + (.name // \"?\") + \")\"",
...
fn tonk_discuss_create<R: TonkGhRunner>(
    runner: &mut R,
    args: &[String],
    owner: &str,
    name: &str,
) -> Result<String, String> {
    let title = tonk_flag_value(args, "--title")?
        .ok_or_else(|| "need --title \"<thread title>\"".to_owned())?;
    tonk_validate_field("title", &title)?;
    let category = tonk_flag_value(args, "--category")?;
    if let Some(value) = category.as_deref() {
        tonk_validate_field("category", value)?;
    }
    ...
    let out = runner.tonk_gh(&tonk_graphql_args(&[
        ("query", mutation),
        ("r", &repo.id),
        ("c", &cat.id),
        ("t", &title),
    ...
    let json: serde_json::Value =
        serde_json::from_str(&out.stdout).map_err(|error| error.to_string())?;
    let discussion = &json["data"]["createDiscussion"]["discussion"];
    Ok(format!(
        "✅ new thread #{} in [{}] → {}\n",
        tonk_json_display(&discussion["number"]),
        cat.name,
        tonk_json_str(&discussion["url"])?
    ))
}

fn tonk_discuss_read<R: TonkGhRunner>(
    runner: &mut R,
    args: &[String],
    owner: &str,
    name: &str,
    ...
    if disc.is_null() {
        return Err(format!("discussion #{num} not found (private? no access?)"));
    }
    tonk_render_discussion(disc)
}

fn tonk_discuss_post<R: TonkGhRunner>(
    runner: &mut R,
    args: &[String],
    owner: &str,
    name: &str,
...
fn tonk_discuss_reply<R: TonkGhRunner>(
    runner: &mut R,
    args: &[String],
    owner: &str,
    name: &str,
    ...
    let num = tonk_discussion_number(args.get(4).map_or("", String::as_str))?;
    let reply_to = args.get(5).ok_or_else(|| {
        "need <commentId> to reply to (get it from `discuss read --json`)".to_owned()
    })?;
...
#[derive(Debug, Clone, PartialEq, Eq)]
struct TonkRepoCategories {
    id: String,
    categories: Vec<TonkCategory>,
}
...
fn tonk_repo_with_categories<R: TonkGhRunner>(
    runner: &mut R,
    owner: &str,
    name: &str,
) -> Result<TonkRepoCategories, String> {
    ...
    if repo.is_null() || repo["id"].is_null() {
        return Err(format!(
            "repo {owner}/{name} not found (private? no access?)"
        ));
    }
    Ok(TonkRepoCategories {
        id: tonk_json_str(&repo["id"])?.to_owned(),
        categories: tonk_categories(repo)?,
    })
}
...
fn tonk_categories(repo: &serde_json::Value) -> Result<Vec<TonkCategory>, String> {
    let nodes = repo["discussionCategories"]["nodes"]
        .as_array()
        .ok_or_else(|| "repo has no discussion categories (Discussions enabled?)".to_owned())?;
    if nodes.is_empty() {
        return Err("repo has no discussion categories (Discussions enabled?)".to_owned());
    }
    nodes
        .iter()
        .map(|node| {
            Ok(TonkCategory {
                id: tonk_json_str(&node["id"])?.to_owned(),
                name: tonk_json_str(&node["name"])?.to_owned(),
            })
        })
        .collect()
}

fn tonk_pick_category(
    categories: &[TonkCategory],
    want: Option<&str>,
) -> Result<TonkCategory, String> {
    if let Some(want) = want.filter(|value| !value.is_empty()) {
        return categories
            .iter()
            .find(|cat| cat.name.eq_ignore_ascii_case(want))
            .cloned()
            .ok_or_else(|| {
                format!(
                    "category \"{want}\" not found · available: {}",
                    categories
                        .iter()
...
fn tonk_discussion_id<R: TonkGhRunner>(
    runner: &mut R,
    owner: &str,
    name: &str,
    num: u64,
    ...
    if discussion.is_null() || discussion["id"].is_null() {
        return Err(format!(
            "discussion #{num} not found in {owner}/{name} (private? no access?)"
        ));
    }
    Ok((
        tonk_json_str(&discussion["id"])?.to_owned(),
        tonk_json_str(&discussion["title"])?.to_owned(),
    ))
}
...
fn tonk_render_discussion(disc: &serde_json::Value) -> Result<String, String> {
    ...
    let comments = disc["comments"]["nodes"]
        .as_array()
        .cloned()
        .unwrap_or_default();
...
fn tonk_body_from_flags(args: &[String]) -> Result<String, String> {
    ...
    let mut body = if let Some(index) = file_idx {
        let path = args
            .get(index + 1)
            .ok_or_else(|| "need --file <path> or --text \"<body>\"".to_owned())?;
    ...
    if !args.iter().any(|arg| arg == "--raw") && !body.contains("Tonk 🌿") {
        body.push_str(TONK_SIG);
    }
    Ok(body)
}
...
fn tonk_discussion_number(raw: &str) -> Result<u64, String> {
    ...
    raw.parse::<u64>()
        ...
        .ok_or_else(|| "need discussion number".to_owned())
}
...
fn tonk_validate_repo_part(label: &str, value: &str) -> Result<(), String> {
    if !tonk_safe_atom(value) {
        return Err(format!("tonk gh: invalid {label} '{value}'"));
    }
    Ok(())
}
...
fn tonk_validate_file_arg(value: &str) -> Result<(), String> {
    if tonk_empty_dash_control(value) || value.chars().any(char::is_whitespace) {
        return Err(format!("tonk gh: invalid --file '{value}'"));
    }
    Ok(())
}

fn tonk_validate_field(label: &str, value: &str) -> Result<(), String> {
    if tonk_empty_dash_control(value) {
        return Err(format!("tonk gh: invalid {label}"));
    }
    Ok(())
}

fn tonk_safe_atom(value: &str) -> bool {
    !tonk_empty_dash_control(value)
        && value
            .chars()
...
fn tonk_vec(values: &[&str]) -> Vec<String> {
...
fn tonk_json_str(value: &serde_json::Value) -> Result<&str, String> {
    value
        .as_str()
        .ok_or_else(|| "unexpected gh JSON shape".to_owned())
}
...
fn tonk_redact_error(message: &str) -> String {
    ...
    if lowered.contains("token") || lowered.contains("secret") || lowered.contains("authorization")
    {
        "gh failed (redacted)".to_owned()
    } else {
        message.to_owned()
    }
}
...
fn tonk_fake_gh(args: &[String]) -> Result<String, String> {
    ...
    {
        return Ok("tonk (Tonk Oracle)".to_owned());
    }
    let query = tonk_fake_field(args, "query").unwrap_or_default();
    if query.contains("discussionCategories") {
        return Ok(r#"{"data":{"repository":{"id":"R_repo","discussionCategories":{"nodes":[{"id":"CAT_general","name":"General"},{"id":"CAT_workshop","name":"Workshop"}]}}}}"#.to_owned());
    }
    if query.contains("discussion(number:$d){title body comments") {
        return Ok(r#"{"data":{"repository":{"discussion":{"title":"Tonk thread","body":"Welcome body","comments":{"nodes":[{"author":{"login":"alice"},"body":"first","url":"https://example.invalid/comment/1"}]}}}}}"#.to_owned());
    }
    if query.contains("discussion(number:$d){id title}") {
        return Ok(
            r#"{"data":{"repository":{"discussion":{"id":"D_thread","title":"Tonk thread"}}}}"#
                .to_owned(),
        );
    }
    if query.contains("createDiscussion") {
        return Ok(r#"{"data":{"createDiscussion":{"discussion":{"url":"https://example.invalid/discussions/7","number":7}}}}"#.to_owned());
    }
    if query.contains("addDiscussionComment") {
        return Ok(r#"{"data":{"addDiscussionComment":{"comment":{"url":"https://example.invalid/comment/9"}}}}"#.to_owned());
    }
...
mod tonk_tests {
    ...
    fn tonk_args(values: &[&str]) -> Vec<String> {
        values.iter().map(|value| (*value).to_owned()).collect()
    }

    #[test]
    fn tonk_dispatch_fragment_owns_tonk() {
        assert_eq!(DISPATCH_119[0].command, "tonk");
    }

    #[test]
    fn tonk_help_status_say_match_plugin_surface() {
        let mut fake = TonkFakeRunner::default();
        assert!(tonk_run_with_runner(&[], &mut fake)
            .stdout
            .contains("maw tonk gh ..."));
        assert!(tonk_run_with_runner(&tonk_args(&["status"]), &mut fake)
            .stdout
            .contains("Active Student"));
        assert!(tonk_run_with_runner(&tonk_args(&["say", "TK"]), &mut fake)
            .stdout
            .contains("Hello, TK!"));
    }
    ...
    fn tonk_mutations_are_hermetic_fake_runner_paths() {
        let mut fake = TonkFakeRunner::default();
        let create = tonk_run_with_runner(
            &tonk_args(&[
                "gh",
                "discuss",
                "create",
        ...
        let reply = tonk_run_with_runner(
            &tonk_args(&[
                "gh",
                "discuss",
                "reply",
        ...
        assert!(fake
            .calls
            .iter()
            .any(|call| call.iter().any(|arg| arg.contains("createDiscussion"))));
        assert_eq!(
            fake.calls
                .iter()
                .filter(|call| call.iter().any(|arg| arg.contains("addDiscussionComment")))
                .count(),
            2
        );
    }
    ...
    fn tonk_rejects_injected_repo_title_category_discussion() {
        let mut fake = TonkFakeRunner::default();
        assert_ne!(
            tonk_run_with_runner(
                &tonk_args(&["gh", "discuss", "read", "-bad/repo", "1"]),
                &mut fake
            )
            .code,
            0
        ...
        assert_ne!(
            tonk_run_with_runner(
                &tonk_args(&["gh", "discuss", "read", "org/repo", "-1"]),
                &mut fake
            )
            .code,
            0
        ...
        assert_ne!(
            tonk_run_with_runner(
                &tonk_args(&[
                    "gh", "discuss", "create", "org/repo", "--title", "-bad", "--text", "Body"
                ]),
                &mut fake
            )
            .code,
    ...
    fn tonk_redacts_secret_errors() {
        assert_eq!(
            tonk_redact_error("stderr had token ghp_123"),
            "gh failed (redacted)"
        );
    }
}
