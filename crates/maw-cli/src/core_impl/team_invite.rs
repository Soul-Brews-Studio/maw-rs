...
trait TeamInviteFs125 {
    ...
    fn team_invite_write_json_atomic_0600(
        &mut self,
        path: &std::path::Path,
...
trait TeamInviteConsentStore125 {
    fn team_invite_write_pending(
        &mut self,
        request: &TeamInviteConsentRequest125,
    ) -> Result<(), String>;
...
trait TeamInviteHttpClient125 {
    fn team_invite_post_consent(
        &mut self,
        base_url: &str,
        request: &TeamInviteConsentRequest125,
    ) -> TeamInvitePostResult125;
}
...
impl TeamInviteConsentStore125 for TeamInviteSystemConsent125 {
    fn team_invite_write_pending(
        ...
        let path = maw_state_path(
            &current_xdg_env(),
            &["consent-pending", &format!("{}.json", request.id)],
        );
        team_write_json_atomic_0600(&path, &team_invite_pending_json(request))
    }

    fn team_invite_next_id(&mut self) -> String {
        team_invite_generate_request_id()
    }

    fn team_invite_next_pin(&mut self) -> String {
        team_invite_generate_pin()
    }
}
...
impl TeamInviteHttpClient125 for TeamInviteSystemHttp125 {
    fn team_invite_post_consent(
        ...
        let Ok(url) = team_invite_consent_url(base_url) else {
            return TeamInvitePostResult125 {
                ok: false,
                status: 0,
                error: Some("invalid peer URL".to_owned()),
            };
        };
        let Ok(body) = serde_json::to_string(&team_invite_pending_json(request)) else {
            return TeamInvitePostResult125 {
                ok: false,
                status: 0,
...
fn team_invite_with(
    ...
    if !fs.team_invite_exists(&manifest_path) {
        return Err(format!(
            "\x1b[31m✗\x1b[0m team '{}' not found — run: maw team create {}",
            opts.team, opts.team
        ));
    }
    ...
    if !post.ok {
        let error = post
            .error
            .unwrap_or_else(|| format!("peer rejected request: HTTP {}", post.status));
    ...
    Err(format!(
        "__TEAM_INVITE_EXIT2__{}",
        team_invite_consent_required(&opts, &peer, &peer_id, &lead, &request)
    ))
}
...
fn team_invite_parse(argv: &[String]) -> Result<TeamInviteOptions125, String> {
    ...
    while index < argv.len() {
        match argv[index].as_str() {
            "--scope" => {
                index += 1;
                scope = Some(team_invite_next(argv, index, "--scope")?);
            }
            "--lead" => {
                index += 1;
                lead = Some(team_invite_next(argv, index, "--lead")?);
            }
    ...
    if positional.len() < 2 {
        return Err(
            "usage: maw team invite <team> <peer> [--scope <scope>] [--lead <lead>]".to_owned(),
        );
    }
    if positional.len() > 2 {
        return Err(format!(
            "team invite: unexpected argument {}",
            positional[2]
        ));
    }
    ...
    if let Some(value) = &lead {
        team_invite_validate_id(value, "lead")?;
    }
    Ok(TeamInviteOptions125 {
        team: positional[0].clone(),
        peer: positional[1].clone(),
        scope,
        lead,
    })
}
...
fn team_invite_next(argv: &[String], index: usize, flag: &str) -> Result<String, String> {
    ...
    if value.starts_with('-') {
        return Err(format!("{flag} value must not start with '-'"));
    }
    Ok(value.clone())
}

fn team_invite_record(
    path: &std::path::Path,
    ...
    {
        manifest["invitees"] = serde_json::json!([]);
    }
    let invitees = manifest["invitees"]
        .as_array_mut()
        .ok_or_else(|| "team invite: invalid invitees".to_owned())?;
    let entry = serde_json::to_value(TeamInviteEntry125 {
        name: peer.name.clone(),
        url: peer.url.clone(),
        node: peer.node.clone(),
        scope: scope.to_owned(),
        invited_at: team_timestamp(),
    })
    .map_err(|error| error.to_string())?;
    if let Some(existing) = invitees.iter().position(|entry| {
        entry.get("name").and_then(serde_json::Value::as_str) == Some(peer.name.as_str())
    }) {
        invitees[existing] = entry;
    } else {
        invitees.push(entry);
    }
...
fn team_invite_summary(
    opts: &TeamInviteOptions125,
    peer: &TeamInviteNamedPeer125,
    lead: &str,
) -> String {
    format!(
        "team-invite: team='{}' lead='{lead}' invitee='{}'{} url='{}' scope='{}'",
        opts.team,
        peer.name,
        peer.node
            .as_ref()
...
fn team_invite_consent_required(
    opts: &TeamInviteOptions125,
    peer: &TeamInviteNamedPeer125,
    peer_id: &str,
    lead: &str,
    ...
    [
        ...
        String::new(),
        format!(
            "   on {peer_id}: \x1b[36mmaw consent approve {} {pin}\x1b[0m",
            request.id
        ),
        format!(
            "   then re-run: \x1b[36mmaw team invite {} {}\x1b[0m",
            opts.team, peer.name
        ),
    ]
    .join("\n")
...
fn team_invite_success(opts: &TeamInviteOptions125) -> String {
    format!(
        "\x1b[32m✓\x1b[0m invited '{}' to team '{}' (scope: {})\n",
...
fn team_invite_trust_json_contains(
    value: &serde_json::Value,
    from: &str,
    to: &str,
    action: &str,
    ...
    value.as_array().is_some_and(|items| {
        items
            .iter()
            .any(|item| team_invite_trust_entry_matches(item, from, to, action))
    }) || value
        .get("trust")
        .is_some_and(|trust| team_invite_trust_json_contains(trust, from, to, action))
        || value
            .get("entries")
            .is_some_and(|entries| team_invite_trust_json_contains(entries, from, to, action))
}

fn team_invite_trust_entry_matches(
...
fn team_invite_validate_request_id(value: &str) -> Result<(), String> {
    if value.len() != 24 || !value.bytes().all(|b| b.is_ascii_hexdigit()) {
        return Err("team invite: invalid request id".to_owned());
    }
    Ok(())
}

fn team_invite_validate_pin(value: &str) -> Result<(), String> {
    if !maw_auth::is_valid_pair_code_shape(value) {
        return Err("team invite: invalid generated PIN".to_owned());
    }
    Ok(())
}
...
fn team_invite_split_url(value: &str) -> Result<(&str, &str, Option<u16>, &str), String> {
    ...
    let (scheme, rest) = value
        .split_once("://")
        .ok_or_else(|| format!("team invite: invalid peer URL {value:?}"))?;
    ...
    let (host, port) = if let Some((host, port_text)) = authority.rsplit_once(':') {
        ...
        let port = port_text
            .parse::<u16>()
            .map_err(|_| format!("team invite: invalid peer URL {value:?}"))?;
        (host, Some(port))
    } else {
        (authority, None)
    };
    if host.is_empty()
        || host.starts_with('-')
        || host.contains("..")
        || host.chars().any(|ch| ch.is_whitespace() || ch == '\\')
    {
        return Err(format!("team invite: invalid peer URL {value:?}"));
    }
...
fn team_invite_expiry() -> String {
    std::env::var("MAW_RS_TEAM_FIXED_EXPIRES")
        .unwrap_or_else(|_| (team_now_millis() + 600_000).to_string())
}
...
fn team_invite_http_post(url: &str, body: &str) -> TeamInvitePostResult125 {
    ...
    let Ok((scheme, host, port, path)) = team_invite_split_request_url(url) else {
        return TeamInvitePostResult125 {
            ok: false,
            status: 0,
            error: Some("invalid URL".to_owned()),
        };
    };
    if scheme == "https" {
        return TeamInvitePostResult125 {
            ok: false,
            status: 0,
            error: Some("https consent POST not available in native minimal client yet".to_owned()),
        };
    }
    ...
    let Ok(mut stream) = std::net::TcpStream::connect(addr) else {
        return TeamInvitePostResult125 {
            ok: false,
            status: 0,
            error: Some("network error contacting peer".to_owned()),
        };
    };
...
mod team_invite_tests125 {
    ...
    #[derive(Default)]
    struct FakeTrust125 {
        trusted: BTreeSet<(String, String, String)>,
    }
    impl TeamInviteTrustStore125 for FakeTrust125 {
        fn team_invite_is_trusted(&self, from: &str, to: &str, action: &str) -> bool {
            self.trusted
                .contains(&(from.to_owned(), to.to_owned(), action.to_owned()))
        }
    }

    #[derive(Default)]
    struct FakeConsent125 {
        writes: Vec<TeamInviteConsentRequest125>,
        ids: Vec<String>,
        pins: Vec<String>,
    }
    impl TeamInviteConsentStore125 for FakeConsent125 {
        ...
        fn team_invite_next_id(&mut self) -> String {
            self.ids
                .pop()
                .unwrap_or_else(|| "abcdefabcdefabcdefabcdef".to_owned())
        }
        fn team_invite_next_pin(&mut self) -> String {
            self.pins.pop().unwrap_or_else(|| "ABCDEF".to_owned())
        }
    }

    #[derive(Default)]
    struct FakeHttp125 {
        posts: Vec<(String, TeamInviteConsentRequest125)>,
        ok: bool,
    }
    impl TeamInviteHttpClient125 for FakeHttp125 {
        fn team_invite_post_consent(
            ...
            self.posts.push((base_url.to_owned(), request.clone()));
            TeamInvitePostResult125 {
                ok: self.ok,
                status: if self.ok { 201 } else { 503 },
                error: if self.ok {
                    None
                } else {
    ...
    fn config() -> TeamInviteConfig125 {
        TeamInviteConfig125 {
            node: Some("lead-node".to_owned()),
            named_peers: vec![TeamInviteNamedPeer125 {
                name: "scout".to_owned(),
                url: "http://scout.example:3456".to_owned(),
                node: Some("scout-node".to_owned()),
            }],
        }
    }
    fn manifest_path(team: &str) -> std::path::PathBuf {
        team_paths(team).vault_manifest
    }
    fn seed_manifest(fs: &mut FakeFs125) {
        fs.files.insert(manifest_path("alpha"), r#"{"name":"alpha","other":true,"invitees":[{"name":"builder","url":"http://builder","scope":"member","invitedAt":"keep"}]}"#.to_owned());
    }

    #[test]
    fn team_invite_missing_team_stops_before_peer_lookup_or_consent() {
        let mut fs = FakeFs125::default();
        let trust = FakeTrust125::default();
        let mut consent = FakeConsent125::default();
        let mut http = FakeHttp125::default();
        let err = team_invite_with(
            &args(&["invite", "missing", "scout"]),
            &config(),
            &mut fs,
            &trust,
            &mut consent,
        ...
        .unwrap_err();
        assert!(err.contains("team 'missing' not found"));
        assert_eq!(fs.writes, 0);
        assert!(consent.writes.is_empty());
    ...
    fn team_invite_unknown_peer_does_not_write() {
        ...
        let mut fs = FakeFs125::default();
        seed_manifest(&mut fs);
        let trust = FakeTrust125::default();
        let mut consent = FakeConsent125::default();
        let mut http = FakeHttp125::default();
        let err = team_invite_with(
            &args(&["invite", "alpha", "ghost"]),
            &config(),
            &mut fs,
            &trust,
            &mut consent,
        ...
        .unwrap_err();
        assert!(err.contains("unknown peer 'ghost'"), "{err}");
        assert_eq!(fs.writes, 0);
        assert!(http.posts.is_empty());
    }
    ...
    fn team_invite_consent_off_records_and_preserves_unrelated_manifest_fields() {
        let _guard = env_test_lock();
        let _restore = EnvVarRestore::capture("MAW_CONSENT");
        std::env::remove_var("MAW_CONSENT");
        let mut fs = FakeFs125::default();
        seed_manifest(&mut fs);
        let trust = FakeTrust125::default();
        let mut consent = FakeConsent125::default();
        let mut http = FakeHttp125::default();
        let out = team_invite_with(
            &args(&["invite", "alpha", "scout", "--scope", "reviewer"]),
            &config(),
            &mut fs,
            &trust,
            &mut consent,
        ...
        .unwrap();
        assert!(out.contains("invited 'scout'"));
        assert!(consent.writes.is_empty());
        assert!(http.posts.is_empty());
        let value: serde_json::Value =
            serde_json::from_str(fs.files.get(&manifest_path("alpha")).unwrap()).unwrap();
        assert_eq!(value["other"], serde_json::json!(true));
        assert_eq!(value["invitees"].as_array().unwrap().len(), 2);
    }
    ...
    fn team_invite_trusted_scope_records_without_requesting_consent() {
        let _guard = env_test_lock();
        let _restore = EnvVarRestore::capture("MAW_CONSENT");
        std::env::set_var("MAW_CONSENT", "1");
        let mut fs = FakeFs125::default();
        seed_manifest(&mut fs);
        let mut trust = FakeTrust125::default();
        trust.trusted.insert((
            "lead-node".to_owned(),
            "scout-node".to_owned(),
        ...
        .unwrap();
        assert!(out.contains("scope: member"));
        assert!(consent.writes.is_empty());
        assert!(http.posts.is_empty());
    }
    ...
    fn team_invite_not_trusted_exits_two_message_and_never_writes_manifest() {
        let _guard = env_test_lock();
        let _restore = EnvVarRestore::capture("MAW_CONSENT");
        let _time = EnvVarRestore::capture("MAW_RS_TEAM_FIXED_EXPIRES");
        std::env::set_var("MAW_CONSENT", "1");
        std::env::set_var("MAW_RS_TEAM_FIXED_EXPIRES", "2099-01-01T00:00:00.000Z");
        let mut fs = FakeFs125::default();
        seed_manifest(&mut fs);
        let trust = FakeTrust125::default();
        let mut consent = FakeConsent125 {
            ids: vec!["111111111111111111111111".to_owned()],
            pins: vec!["ABCDEF".to_owned()],
            ..Default::default()
        };
        let mut http = FakeHttp125 {
            ok: true,
            ..Default::default()
        };
        let err = team_invite_with(
        ...
        .unwrap_err();
        assert!(err.starts_with("__TEAM_INVITE_EXIT2__"));
        assert!(err.contains("request id: 111111111111111111111111"));
        assert!(err.contains("maw consent approve 111111111111111111111111 ABCDEF"));
        assert!(err.contains("then re-run:"));
        assert_eq!(fs.writes, 0);
        assert_eq!(consent.writes.len(), 1);
        assert_eq!(http.posts.len(), 1);
        assert!(!err.contains("pinHash"));
        let pending = team_invite_pending_json(&consent.writes[0]);
        assert!(pending.get("pinHash").is_some());
        assert!(pending.get("plaintextPin").is_none());
    }
    ...
    fn team_invite_request_failure_keeps_manifest_unchanged() {
        ...
        .unwrap_err();
        assert!(err.contains("consent request failed"));
        assert!(err.contains("request id (local mirror)"));
        assert_eq!(fs.writes, 0);
        assert_eq!(consent.writes.len(), 1);
    }
    ...
    fn team_invite_validates_url_before_http_and_args_before_writes() {
        ...
        .unwrap_err();
        assert!(err.contains("invalid peer URL"));
        assert_eq!(fs.writes, 0);
        assert!(http.posts.is_empty());
    ...
    fn team_invite_trust_json_is_action_scoped() {
        ...
        assert!(team_invite_trust_json_contains(
            &value,
            "lead",
            "peer",
            "team-invite"
    ...
    fn team_invite_sentinel_maps_to_exit_two() {
        let output =
            team_output_from_result(Err("__TEAM_INVITE_EXIT2__consent required".to_owned()));
