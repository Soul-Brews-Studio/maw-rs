...
trait SleepTmux {
    ...
    fn sleep_save_tab_order(
        &mut self,
        session: &str,
        windows: &[SleepWindow],
    ) -> Result<(), String>;
    fn sleep_run_lifecycle(
        &mut self,
        resolved: &SleepResolved,
        requested: &str,
    ) -> Result<(), String>;
...
impl SleepTmux for SleepSystemTmux {
    ...
    fn sleep_list_windows(&mut self, session: &str) -> Result<Vec<SleepWindow>, String> {
        ...
        let raw = sleep_tmux_run(
            &mut self.runner,
            "list-windows",
            &["-t", session, "-F", "#{window_index}|||#{window_name}"],
        )?;
...
fn sleep_run_command_with(
    argv: &[String],
    tmux: &mut impl SleepTmux,
...
fn sleep_run(
    ...
    let resolved = sleep_resolve_target(&options, &sessions, &fleet)
        .ok_or_else(|| sleep_missing_target(&options.target, &sessions))?;
    sleep_validate_before_destructive(&resolved, &sessions)?;
    let windows = tmux
        .sleep_list_windows(&resolved.session)
        .unwrap_or_else(|_| {
            sleep_find_session(&sessions, &resolved.session)
                .map_or_else(Vec::new, |s| s.windows.clone())
        });
    ...
    let still = tmux
        .sleep_list_windows(&resolved.session)
        .is_ok_and(|after| sleep_window_exists(&after, &resolved.window));
    if still {
        sleep_validate_before_destructive(&resolved, &sessions)?;
        tmux.sleep_kill_window(&target)?;
        let _ = writeln!(
            out,
            "  \x1b[33m!\x1b[0m force-killed {} (did not exit gracefully)",
            resolved.window
        );
    } else {
        let _ = writeln!(
            out,
            "  \x1b[32m✓\x1b[0m {} exited gracefully",
            resolved.window
        );
    }
    ...
    let _ = writeln!(
        out,
        "\x1b[32msleep\x1b[0m {} ({})",
        options.target, resolved.window
    );
    Ok(out)
}

fn sleep_run_all_done(
    tmux: &mut impl SleepTmux,
    options: &SleepAllDoneArgs,
) -> Result<String, String> {
    let session = tmux.sleep_current_session()?;
    let sessions = tmux.sleep_list_sessions()?;
    sleep_validate_current_session(&session, &sessions)?;
    let windows = tmux.sleep_list_windows(&session).unwrap_or_else(|_| {
        sleep_find_session(&sessions, &session).map_or_else(Vec::new, |s| s.windows.clone())
    });
...
fn sleep_parse_args(argv: &[String]) -> Result<SleepArgs, String> {
    if argv.is_empty() {
        return Err(SLEEP_USAGE.to_owned());
    }
    ...
    if all_done {
        if !words.is_empty() {
            return Err("sleep --all-done does not accept a target".to_owned());
        }
        return Ok(SleepArgs::AllDone(SleepAllDoneArgs { dry_run }));
    }
    if dry_run {
        return Err("sleep --dry-run is only supported with --all-done".to_owned());
    }
    match words.as_slice() {
        [target] => Ok(SleepArgs::One(SleepOneArgs {
            target: target.clone(),
            window: None,
        })),
        [target, window] => Ok(SleepArgs::One(SleepOneArgs {
            target: target.clone(),
...
fn sleep_resolve_target(
    args: &SleepOneArgs,
    sessions: &[SleepSession],
    fleet: &[NativeFleetSession],
) -> Option<SleepResolved> {
    if args.window.is_none() {
        if let Some(found) = sleep_resolve_by_window(&args.target, sessions) {
            return Some(found);
        }
    }
    if let Some(found) = sleep_resolve_by_session(args, sessions, fleet) {
        return Some(found);
    }
...
fn sleep_resolve_by_session(
    ...
    let session = sessions
        .iter()
        .find(|session| sleep_session_matches(&session.name, &args.target))?;
    let window = args
        .window
        .clone()
        .or_else(|| sleep_fleet_primary(&session.name, fleet))
        .or_else(|| session.windows.first().map(|item| item.name.clone()))?;
    Some(SleepResolved {
        session: session.name.clone(),
        window,
    })
...
fn sleep_detect_session(
    args: &SleepOneArgs,
    sessions: &[SleepSession],
    fleet: &[NativeFleetSession],
) -> Option<SleepResolved> {
    let fleet_entry = fleet
        .iter()
        .find(|entry| sleep_session_matches(&entry.name, &args.target))?;
    let session = fleet_entry.name.clone();
    let window = args
        .window
        .clone()
        .or_else(|| fleet_entry.windows.first().map(|item| item.name.clone()))
        .or_else(|| {
            sleep_find_session(sessions, &session)
                .and_then(|item| item.windows.first().map(|w| w.name.clone()))
        })?;
...
fn sleep_session_matches(session: &str, target: &str) -> bool {
    session == target
        ...
        || sleep_strip_dash(session) == sleep_strip_dash(target)
}
...
fn sleep_fleet_primary(session: &str, fleet: &[NativeFleetSession]) -> Option<String> {
    fleet
        ...
        .and_then(|entry| entry.windows.first())
        .map(|window| window.name.clone())
}

fn sleep_validate_before_destructive(
    resolved: &SleepResolved,
    sessions: &[SleepSession],
) -> Result<(), String> {
    sleep_validate_tmux_target(&resolved.session)?;
    sleep_validate_tmux_target(&resolved.window)?;
    let session = sleep_find_session(sessions, &resolved.session)
        .ok_or_else(|| format!("sleep: refusing missing session {}", resolved.session))?;
    if sleep_window_exists(&session.windows, &resolved.window) {
        Ok(())
    } else {
        Err(format!(
            "sleep: refusing missing window {}:{}",
...
fn sleep_window_exists(windows: &[SleepWindow], target: &str) -> bool {
    ...
    windows
        .iter()
        .any(|window| window.name == target || sleep_strip_dash(&window.name) == stripped)
}

fn sleep_tmux_window_target(resolved: &SleepResolved) -> String {
    format!("{}:{}", resolved.session, resolved.window)
}

fn sleep_parse_sessions(raw: &str) -> Vec<SleepSession> {
    let mut sessions = Vec::new();
    for line in raw.lines().filter(|line| !line.trim().is_empty()) {
        sleep_push_session_line(&mut sessions, line);
    }
    sessions
}
...
fn sleep_push_session_line(sessions: &mut Vec<SleepSession>, line: &str) {
    ...
    let window = SleepWindow {
        index: parts.get(1).and_then(|v| v.parse().ok()).unwrap_or(0),
...
fn sleep_validate_current_session(session: &str, sessions: &[SleepSession]) -> Result<(), String> {
    sleep_validate_tmux_target(session)?;
    if sleep_find_session(sessions, session).is_some() {
        Ok(())
    } else {
        Err(format!(
            "sleep --all-done: refusing missing current session {session}"
        ))
    }
}

fn sleep_collect_done_panes(
    tmux: &mut impl SleepTmux,
    session: &str,
) -> Result<Vec<SleepPane>, String> {
    let mut candidates = Vec::new();
    for pane in tmux.sleep_list_panes(session)? {
        if pane.session != session {
            continue;
        }
        if !sleep_is_parked_command(&pane.current_command) {
            continue;
        }
        sleep_validate_pane_ownership_fields(&pane, session)?;
        let capture = tmux.sleep_capture_pane(&pane.pane_id)?;
        if sleep_capture_has_done_marker(&capture) {
...
fn sleep_validate_all_done_ownership(
    tmux: &mut impl SleepTmux,
    session: &str,
    candidates: &[SleepPane],
) -> Result<Vec<SleepPane>, String> {
    ...
    for candidate in candidates {
        ...
        let Some(current) = refreshed
            ...
            return Err(format!(
                "sleep --all-done: refusing pane {} because ownership disappeared",
                candidate.pane_id
            ));
        };
        ...
        if !sleep_capture_has_done_marker(&capture)
            || !sleep_is_parked_command(&current.current_command)
        {
            return Err(format!(
                "sleep --all-done: refusing pane {} because it is no longer done",
                current.pane_id
            ));
        }
...
fn sleep_validate_one_done_pane_ownership(
    tmux: &mut impl SleepTmux,
    session: &str,
    expected: &SleepPane,
) -> Result<SleepPane, String> {
    let refreshed = tmux.sleep_list_panes(session)?;
    let Some(current) = refreshed
        .iter()
        .find(|pane| pane.pane_id == expected.pane_id)
    else {
        return Err(format!(
            "sleep --all-done: refusing pane {} because ownership disappeared",
            expected.pane_id
...
fn sleep_validate_same_owned_pane(
    expected: &SleepPane,
    current: &SleepPane,
    session: &str,
) -> Result<(), String> {
    sleep_validate_pane_ownership_fields(current, session)?;
    if current.pane_pid != expected.pane_pid
        || current.window_name != expected.window_name
        || current.window_index != expected.window_index
    {
        return Err(format!(
            "sleep --all-done: refusing pane {} because PID/window ownership changed",
            expected.pane_id
        ));
    }
    Ok(())
...
fn sleep_validate_pane_ownership_fields(pane: &SleepPane, session: &str) -> Result<(), String> {
    if pane.session != session {
        return Err(format!(
            "sleep --all-done: refusing pane {} outside current session {session}",
            pane.pane_id
        ));
    }
...
fn sleep_render_all_done_plan(session: &str, panes: &[SleepPane], dry_run: bool) -> String {
    let mut out = if dry_run {
        format!("\x1b[32msleep --all-done\x1b[0m {session} (dry-run)\n")
    } else {
        format!("\x1b[32msleep --all-done\x1b[0m {session}\n")
    };
    for pane in panes {
        let action = if dry_run { "would sleep" } else { "slept" };
        let _ = writeln!(
            out,
            "  {action} {} pid={} window={} command={}",
            pane.pane_id, pane.pane_pid, pane.window_name, pane.current_command
        );
    }
    out
}
...
fn sleep_flag_like_target(value: &str) -> String {
...
fn sleep_missing_target(target: &str, sessions: &[SleepSession]) -> String {
    ...
    let flat = sessions
        ...
        .flat_map(|s| {
            s.windows
                .iter()
                .map(move |w| format!("{}:{}", s.name, w.name))
        })
        .take(10)
        .collect::<Vec<_>>();
    if !flat.is_empty() {
...
fn sleep_strip_dash(value: &str) -> String {
    value.trim_end_matches('-').to_owned()
}

fn sleep_tmux_run<R: maw_tmux::TmuxRunner>(
    runner: &mut R,
    subcommand: &str,
    args: &[&str],
) -> Result<String, String> {
...
fn sleep_write_tab_order(session: &str, windows: &[SleepWindow]) -> Result<(), String> {
    ...
    if let Some(parent) = path.parent() {
        std::fs::create_dir_all(parent)
            .map_err(|error| format!("sleep: tab-order mkdir failed: {error}"))?;
    }
    let entries = windows
        .iter()
...
mod sleep_tests {
    ...
    impl SleepFakeTmux {
        ...
        fn sleep_all_done_fixture() -> Self {
            ...
            fake.panes = vec![
                SleepPane {
                    session: "01-nova".to_owned(),
                    window_index: 1,
                    window_name: "worktree-task-".to_owned(),
                    pane_index: 0,
                    pane_id: "%21".to_owned(),
                    pane_pid: "421".to_owned(),
                    current_command: "read".to_owned(),
                    title: "done".to_owned(),
                },
                SleepPane {
                    session: "01-nova".to_owned(),
                    window_index: 0,
                    window_name: "nova-oracle".to_owned(),
                    pane_index: 0,
                    pane_id: "%20".to_owned(),
                    pane_pid: "420".to_owned(),
                    current_command: "codex".to_owned(),
                    title: "busy".to_owned(),
                },
                SleepPane {
                    session: "02-other".to_owned(),
                    window_index: 0,
                    window_name: "other-done".to_owned(),
                    pane_index: 0,
                    pane_id: "%99".to_owned(),
                    pane_pid: "999".to_owned(),
                    current_command: "read".to_owned(),
                    title: "done".to_owned(),
                },
            ];
            fake.captures
                .insert("%21".to_owned(), "job output\n[done — exit 0]\n".to_owned());
            fake.captures
                .insert("%20".to_owned(), "still running\n".to_owned());
            fake.captures
                .insert("%99".to_owned(), "[done — exit 0]\n".to_owned());
            fake
        }
    }
    ...
    impl SleepTmux for SleepFakeTmux {
        ...
        fn sleep_list_windows(&mut self, session: &str) -> Result<Vec<SleepWindow>, String> {
            Ok(sleep_find_session(&self.sessions, session)
                .map_or_else(Vec::new, |s| s.windows.clone()))
        }
        fn sleep_current_session(&mut self) -> Result<String, String> {
        ...
        fn sleep_list_panes(&mut self, session: &str) -> Result<Vec<SleepPane>, String> {
            ...
            let mut panes = self
                ...
                .cloned()
                .collect::<Vec<_>>();
            if self.mutate_pid_on_refresh
                && self
                    .calls
                    .iter()
                    .filter(|call| call.as_str() == "list-panes 01-nova")
                    .count()
                    > 1
            {
                if let Some(pane) = panes.iter_mut().find(|pane| pane.pane_id == "%21") {
                    pane.pane_pid = "422".to_owned();
                }
            }
            Ok(panes)
        }
        ...
        fn sleep_send_exit(&mut self, target: &str) -> Result<(), String> {
            self.calls.push(format!("send-exit {target}"));
            Ok(())
        }
        fn sleep_wait_grace(&mut self) -> Result<(), String> {
            self.calls.push("wait3s".to_owned());
            Ok(())
        }
        fn sleep_kill_window(&mut self, target: &str) -> Result<(), String> {
            self.calls.push(format!("kill-window {target}"));
            for s in &mut self.sessions {
                s.windows
                    .retain(|w| format!("{}:{}", s.name, w.name) != target);
            }
            Ok(())
        }
        fn sleep_kill_pane(&mut self, pane_id: &str) -> Result<(), String> {
            self.calls.push(format!("kill-pane {pane_id}"));
        ...
        fn sleep_save_tab_order(
            &mut self,
            session: &str,
            _windows: &[SleepWindow],
        ) -> Result<(), String> {
            self.calls.push(format!("save-tab-order {session}"));
            Ok(())
        }
        fn sleep_run_lifecycle(
            &mut self,
        ...
        fn sleep_append_log(&mut self, requested: &str, window: &str) -> Result<(), String> {
            self.log.push(format!("{requested}:{window}"));
            Ok(())
        }
        fn sleep_take_snapshot(&mut self, _sessions: &[SleepSession]) -> Result<(), String> {
            self.calls.push("snapshot sleep".to_owned());
            Ok(())
        }
    }

    fn sleep_args(values: &[&str]) -> Vec<String> {
        values.iter().map(|value| (*value).to_owned()).collect()
    }

    fn sleep_fleet() -> Vec<NativeFleetSession> {
        vec![NativeFleetSession {
            name: "01-nova".to_owned(),
    ...
    fn sleep_preserves_destructive_flow_and_golden() {
        ...
        assert_eq!(
            out,
            include_str!("../../tests/fixtures/native-sleep/sleep-default.stdout")
        );
        assert_eq!(
            tmux.calls,
            [
                "save-tab-order 01-nova",
                "lifecycle nova 01-nova:nova-oracle",
                "send-exit 01-nova:nova-oracle",
                "wait3s",
                "kill-window 01-nova:nova-oracle",
                "snapshot sleep"
            ]
        );
    ...
    fn sleep_all_done_dry_run_is_hermetic_golden_and_non_mutating() {
        ...
        let out = sleep_run(
            &sleep_args(&["--all-done", "--dry-run"]),
            &mut tmux,
            sleep_fleet,
        )
        .expect("sleep all done dry-run");
        assert_eq!(
            out,
            include_str!("../../tests/fixtures/native-sleep/sleep-all-done-dry-run.stdout")
        );
    ...
    fn sleep_all_done_live_validates_ownership_before_kill_and_uses_golden() {
        ...
        std::env::set_var("MAW_JS_REF_DIR", "/nonexistent");
        let mut tmux = SleepFakeTmux::sleep_all_done_fixture();
        let out = sleep_run(&sleep_args(&["--all-done"]), &mut tmux, sleep_fleet)
            .expect("sleep all done");
        assert_eq!(
            out,
            include_str!("../../tests/fixtures/native-sleep/sleep-all-done-live.stdout")
        );
        assert_eq!(
            tmux.calls,
            [
    ...
    fn sleep_all_done_rejects_bad_pane_id_before_kill() {
        ...
        tmux.captures
            .insert("-bad".to_owned(), "[done — exit 0]\n".to_owned());
        let err = sleep_run(&sleep_args(&["--all-done"]), &mut tmux, sleep_fleet).unwrap_err();
    ...
    fn sleep_validates_before_destructive_and_rejects_flags() {
        ...
        assert!(
            missing.contains("refusing missing window") || missing.contains("could not resolve")
        );
