const DISPATCH_73: &[DispatcherEntry] = &[DispatcherEntry {
    command: "pane",
    handler: Handler::Sync(run_pane_command),
}];

const PANE_USAGE: &str =
...
fn pane_run_with_runner<R: maw_tmux::TmuxRunner>(
    argv: &[String],
    runner: &mut R,
) -> Result<String, (i32, String)> {
    if std::env::var_os("TMUX").is_none() {
        return Err((1, "\x1b[33m⚠\x1b[0m pane requires tmux".to_owned()));
    }
...
fn pane_parse_args(argv: &[String]) -> Result<PaneOptions, (i32, String)> {
    let Some(sub) = argv.first().map(String::as_str) else {
        return Ok(PaneOptions {
            action: PaneAction::Help,
            left: None,
            right: None,
        });
    };
    match sub.to_ascii_lowercase().as_str() {
        "--help" | "-h" => Ok(PaneOptions {
            action: PaneAction::Help,
            left: None,
            right: None,
        }),
...
fn pane_parse_swap_args(argv: &[String]) -> Result<PaneOptions, (i32, String)> {
    if argv.len() != 2 {
        return Err((1, format!("{PANE_USAGE}\ntwo pane targets required")));
    }
...
fn pane_validate_spec(value: &str) -> Result<String, String> {
    ...
    if matches!(trimmed, "top" | "bottom") || trimmed.chars().all(|ch| ch.is_ascii_digit()) {
    ...
    if trimmed.chars().all(pane_is_safe_title_char) {
        return Ok(trimmed.to_owned());
    }
    Err(format!("pane: invalid pane title prefix {value:?}"))
}

fn pane_is_safe_title_char(ch: char) -> bool {
    ch.is_ascii_alphanumeric() || matches!(ch, '_' | '-' | '.')
}

fn pane_swap<R: maw_tmux::TmuxRunner>(
    options: &PaneOptions,
    runner: &mut R,
...
fn pane_swap_in_window<R: maw_tmux::TmuxRunner>(
    options: &PaneOptions,
    window: &str,
    runner: &mut R,
) -> Result<String, String> {
    let left = options
        .left
        .as_deref()
        .ok_or_else(|| "two pane targets required".to_owned())?;
    let right = options
        .right
        .as_deref()
        .ok_or_else(|| "two pane targets required".to_owned())?;
    let rows = pane_list_rows(window, runner)?;
    let source = pane_resolve(left, &rows)
        .ok_or_else(|| format!("tile swap: could not resolve pane '{left}'"))?;
    let target = pane_resolve(right, &rows)
...
fn pane_list_rows<R: maw_tmux::TmuxRunner>(
    window: &str,
    runner: &mut R,
) -> Result<Vec<PaneRow>, String> {
    pane_validate_tmux_target(window)?;
    let args = vec![
        "-t".to_owned(),
        window.to_owned(),
        "-F".to_owned(),
        PANE_LIST_FORMAT.to_owned(),
    ];
    let raw = runner
        .run("list-panes", &args)
...
fn pane_resolve(spec: &str, rows: &[PaneRow]) -> Option<PaneRow> {
    match spec {
        ...
        value if value.chars().all(|ch| ch.is_ascii_digit()) => {
            rows.iter().find(|row| row.index == value).cloned()
        }
        value => rows
            .iter()
            .find(|row| row.title == value || row.title.starts_with(value))
            .cloned(),
    }
}
...
fn pane_top(rows: &[PaneRow], top_first: bool) -> Option<PaneRow> {
    ...
    if top_first {
        sorted.sort_by(|a, b| {
            a.top
                .cmp(&b.top)
                .then(pane_index_num(&a.index).cmp(&pane_index_num(&b.index)))
        });
    } else {
        sorted.sort_by(|a, b| {
            b.top
                .cmp(&a.top)
                .then(pane_index_num(&b.index).cmp(&pane_index_num(&a.index)))
        });
    }
    sorted.into_iter().next()
}

fn pane_index_num(value: &str) -> i64 {
    value.parse::<i64>().unwrap_or(0)
}

fn pane_resolve_pane_id(value: &str, rows: &[PaneRow]) -> Option<PaneRow> {
    rows.iter()
        .find(|row| row.pane_id == value)
        .cloned()
        .or_else(|| {
            Some(PaneRow {
                index: String::new(),
                pane_id: value.to_owned(),
...
fn pane_run_swap<R: maw_tmux::TmuxRunner>(
    source: &str,
    target: &str,
    runner: &mut R,
) -> Result<(), String> {
...
fn pane_display_name(row: &PaneRow) -> &str {
...
mod pane_tests {
    ...
    fn pane_strings(values: &[&str]) -> Vec<String> {
    ...
    fn pane_resolver_matches_indices_titles_and_edges() {
        let rows = vec![
            ...
            PaneRow {
                index: "1".to_owned(),
                pane_id: "%2".to_owned(),
                title: "tile-1".to_owned(),
                top: 40,
            },
            PaneRow {
                index: "2".to_owned(),
                pane_id: "%3".to_owned(),
                title: "tile-2".to_owned(),
                top: 10,
            },
        ];
