const DISPATCH_152: &[DispatcherEntry] = &[
    DispatcherEntry {
        command: "serve",
        handler: Handler::Async(run_serve_async),
    },
    DispatcherEntry {
        command: "messages",
        handler: Handler::Async(run_messages_async),
    },
];
...
impl MessagesLifecycleHost152 for MessagesSystemHost152 {
    fn messages_pid_path(&self) -> std::path::PathBuf {
        messages_pid_path152()
    }
    fn messages_log_path(&self) -> std::path::PathBuf {
        messages_log_path152()
    }
    fn messages_db_path(&self) -> std::path::PathBuf {
        messages_db_path152()
    }
    fn messages_read_pid(&self, path: &std::path::Path) -> Option<u32> {
        messages_read_pid_file152(path)
    }
    fn messages_pid_alive(&self, pid: u32) -> bool {
        messages_pid_alive152(pid)
    }
    fn messages_remove_pid(&mut self, path: &std::path::Path) -> Result<(), String> {
        messages_remove_file152(path)
    }
    fn messages_stop_pid(&mut self, pid: u32) -> Result<(), String> {
        messages_signal_term152(pid)
    }
    fn messages_serve_status(&self) -> ServeLifecycleStatus152 {
        serve_status_with_host152(&ServeSystemHost152)
    }
}
...
impl ServeLifecycleHost152 for ServeSystemHost152 {
    ...
    fn serve_process_is_maw(&self, pid: u32) -> bool {
        serve_process_is_maw152(pid)
    }
    fn serve_process_summary(&self, pid: u32) -> String {
        serve_process_summary152(pid)
    }
    fn serve_configured_port(&self) -> u16 {
        serve_configured_port152()
    }
    fn serve_probe_port(&self, port: u16) -> ServePortProbe152 {
        serve_probe_port152(port)
    }
    fn serve_listener_pid(&self, port: u16) -> Option<u32> {
        serve_listener_pid152(port)
    }
    fn serve_remove_pid(&mut self, path: &std::path::Path) -> Result<(), String> {
        messages_remove_file152(path)
    }
...
fn serve_lifecycle_subcommand152(raw_args: &[String]) -> Option<CliOutput> {
    ...
    if first != "status" && first != "--status" && first != "stop" {
        return None;
    }
...
fn messages_lifecycle_subcommand152(raw_args: &[String]) -> Option<CliOutput> {
    let first = raw_args.first()?.as_str();
    if first != "serve" && first != "status" && first != "stop" {
        return None;
    }
    Some(messages_lifecycle_run152(
        raw_args,
        &mut MessagesSystemHost152,
    ))
}

fn messages_lifecycle_run152(
    raw_args: &[String],
    host: &mut impl MessagesLifecycleHost152,
) -> CliOutput {
    match messages_parse_lifecycle152(raw_args) {
        Ok(MessagesLifecycleAction152::Serve {
            detach,
            engine,
            port,
        }) => messages_serve152(detach, &engine, port, host),
...
fn messages_parse_lifecycle152(raw_args: &[String]) -> Result<MessagesLifecycleAction152, String> {
    let Some(subcommand) = raw_args.first().map(String::as_str) else {
        return Err(messages_usage152());
    };
    match subcommand {
        "serve" => {
            ...
            while index < raw_args.len() {
                match raw_args[index].as_str() {
                    ...
                    "--engine" => {
                        let value = raw_args
                            .get(index + 1)
                            .ok_or_else(|| "messages serve: missing --engine value".to_owned())?;
                    ...
                    "--port" => {
                        let value = raw_args
                            .get(index + 1)
                            .ok_or_else(|| "messages serve: missing --port value".to_owned())?;
                        port = messages_parse_port152(value)?;
                        index += 1;
                    }
                    "--help" | "-h" => return Err(messages_usage152()),
                    value if value.starts_with('-') => {
                        return Err(format!("messages serve: unknown argument {value}"))
                    }
    ...
    while index < raw_args.len() {
        match raw_args[index].as_str() {
            "--engine" => {
                let value = raw_args
                    .get(index + 1)
                    .ok_or_else(|| format!("{label}: missing --engine value"))?;
                messages_validate_engine_url152(value)?;
                engine = messages_trim_url152(value);
                index += 1;
            }
            "--help" | "-h" => return Err(messages_usage152()),
            value if value.starts_with('-') => {
                return Err(format!("{label}: unknown argument {value}"))
            }
...
fn serve_parse_lifecycle152(raw_args: &[String]) -> Result<ServeLifecycleAction152, String> {
    let Some(first) = raw_args.first().map(String::as_str) else {
        return Err(serve_usage152());
    };
    ...
    if raw_args.len() > 1 {
        return Err(format!(
            "serve {first}: unexpected argument {}",
            raw_args[1]
        ));
    }
    Ok(action)
...
fn messages_serve152(
    detach: bool,
    engine: &str,
    port: u16,
    host: &impl MessagesLifecycleHost152,
) -> CliOutput {
    ...
    let _ = writeln!(
        out,
        "upstream: built-in maw serve /api/message-ledger (requested port {port})"
    );
    let _ = writeln!(
        out,
        "serve: {}",
...
fn messages_status152(engine: &str, host: &impl MessagesLifecycleHost152) -> CliOutput {
    ...
    let serve = host.messages_serve_status();
    let registered = if serve.alive() {
        format!("{MESSAGES_ENGINE_PREFIX_152} → built-in maw serve")
    } else {
        "no".to_owned()
    };
    let mut out = String::new();
    ...
    let _ = writeln!(out, "db: {}", host.messages_db_path().display());
    let _ = writeln!(out, "log: {}", host.messages_log_path().display());
    if !alive && pid.is_some() && pid_path.exists() {
        out.push_str("note: stale pid file present\n");
    }
    CliOutput {
        code: 0,
        stdout: out,
...
fn messages_stop152(engine: &str, host: &mut impl MessagesLifecycleHost152) -> CliOutput {
    ...
    if let Some(pid) = pid.filter(|pid| host.messages_pid_alive(*pid)) {
        if let Err(error) = host.messages_stop_pid(pid) {
            return messages_lifecycle_error152(1, &format!("messages stop: {error}"));
        }
        lines.push(format!("sent SIGTERM to PID {pid}"));
        if let Err(error) = host.messages_remove_pid(&pid_path) {
            return messages_lifecycle_error152(1, &format!("messages stop: {error}"));
        }
    ...
    if host.messages_serve_status().alive() {
        lines.push(format!(
            "native route remains served by maw serve at {MESSAGES_ENGINE_PREFIX_152}"
        ));
    }
    CliOutput {
        code: 0,
...
fn serve_stop152(host: &mut impl ServeLifecycleHost152) -> CliOutput {
    ...
    let pid = if status.alive() {
        status
            .listener_pid
            .or(status.pid.filter(|_| status.pid_process_alive))
    } else {
        status.pid
    };
    let Some(pid) = pid else {
        return CliOutput {
            code: 0,
            stdout: "maw serve: already stopped\n".to_owned(),
            stderr: String::new(),
        };
    };
    if !status.alive() {
        if status.stale_pid {
            if let Err(error) = host.serve_remove_pid(&status.file) {
                return messages_lifecycle_error152(1, &format!("serve stop: {error}"));
            }
        }
        return CliOutput {
            code: 0,
            stdout: format!("maw serve: removed stale PID {pid}\n"),
            stderr: String::new(),
        };
    }
    if let Err(error) = host.serve_stop_pid(pid) {
        return messages_lifecycle_error152(1, &format!("serve stop: {error}"));
    }
    ...
    CliOutput {
        code: 0,
        stdout: format!("maw serve: stopped PID {pid}\n"),
        stderr: String::new(),
    }
}
...
fn serve_status_with_host152(host: &impl ServeLifecycleHost152) -> ServeLifecycleStatus152 {
    ...
    let listener_pid = port_responding
        .then(|| host.serve_listener_pid(port))
        .flatten();
    let pid_process_alive =
        pid.is_some_and(|pid| host.serve_pid_alive(pid) && host.serve_process_is_maw(pid));
    let summary = pid
        .filter(|_| pid_process_alive)
        .map(|pid| host.serve_process_summary(pid));
...
fn serve_render_status152(status: &ServeLifecycleStatus152) -> String {
    let stale_note = if status.stale_pid_removed {
        Some("stale pidfile removed")
    } else {
        None
    };
    if status.alive() {
    ...
    let mut evidence = vec![serve_probe_stopped_evidence152(
        &status.port_probe,
        status.port,
    )];
    if let Some(note) = stale_note {
...
fn serve_status_evidence_pid152(status: &ServeLifecycleStatus152) -> Option<u32> {
    status
        .listener_pid
        .or_else(|| status.pid.filter(|_| status.pid_process_alive))
}
...
fn messages_default_engine_url152() -> String {
    std::env::var("MAW_ENGINE_URL")
        .unwrap_or_else(|_| {
            format!(
                "http://127.0.0.1:{}",
                std::env::var("MAW_PORT").unwrap_or_else(|_| "3456".to_owned())
            )
        })
        .trim_end_matches('/')
        .to_owned()
...
fn messages_default_port152() -> u16 {
    std::env::var("MAW_MESSAGES_PORT")
        .ok()
        .and_then(|value| value.parse::<u16>().ok())
        .unwrap_or(0)
}

fn messages_trim_url152(value: &str) -> String {
...
fn messages_validate_engine_url152(value: &str) -> Result<(), String> {
    ...
    if !(value.starts_with("http://") || value.starts_with("https://")) {
        return Err(format!("messages: invalid --engine: {value}"));
    }
    Ok(())
}
...
fn messages_validate_token152(value: &str, label: &str) -> Result<(), String> {
    ...
    {
        return Err(format!("messages: rejected {label} value"));
    }
    Ok(())
}
...
fn serve_usage152() -> String {
    "usage: maw-rs serve [--host 0.0.0.0] [--port <port>] [--cached-pubkey <key>] | maw-rs serve status|--status|stop".to_owned()
}

fn messages_lifecycle_error152(code: i32, message: &str) -> CliOutput {
    CliOutput {
        code,
        stdout: String::new(),
...
fn messages_pid_path152() -> std::path::PathBuf {
    maw_state_path(&current_xdg_env(), &["engine-plugins", "messages.pid"])
}
fn messages_log_path152() -> std::path::PathBuf {
    maw_state_path(&current_xdg_env(), &["engine-plugins", "messages.log"])
}
fn messages_db_path152() -> std::path::PathBuf {
    maw_data_path(&current_xdg_env(), &["message-ledger.sqlite"])
}
fn serve_pid_path152() -> std::path::PathBuf {
    maw_runtime_home_dir(&current_xdg_env()).join("maw.pid")
}
...
fn serve_process_summary152(pid: u32) -> String {
    ...
    format!(
        ", uptime {elapsed}, cmd: {}",
        messages_truncate152(&command, 80)
    )
}

fn serve_process_is_maw152(pid: u32) -> bool {
    let Some(command) = serve_process_command152(pid) else {
        return false;
    };
...
mod messages_serve_lifecycle_tests152 {
    ...
    impl MessagesLifecycleHost152 for FakeHost152 {
        fn messages_pid_path(&self) -> std::path::PathBuf {
            std::path::PathBuf::from("/tmp/maw/engine-plugins/messages.pid")
        }
        fn messages_log_path(&self) -> std::path::PathBuf {
            std::path::PathBuf::from("/tmp/maw/engine-plugins/messages.log")
        }
        fn messages_db_path(&self) -> std::path::PathBuf {
            std::path::PathBuf::from("/tmp/maw/message-ledger.sqlite")
        }
        fn messages_read_pid(&self, _path: &std::path::Path) -> Option<u32> {
            self.messages_pid
        }
        fn messages_pid_alive(&self, pid: u32) -> bool {
            self.messages_alive && Some(pid) == self.messages_pid
        }
        fn messages_remove_pid(&mut self, path: &std::path::Path) -> Result<(), String> {
        ...
        fn messages_serve_status(&self) -> ServeLifecycleStatus152 {
            serve_status_with_host152(self)
        }
    }

    impl ServeLifecycleHost152 for FakeHost152 {
        fn serve_pid_path(&self) -> std::path::PathBuf {
            std::path::PathBuf::from("/tmp/maw/maw.pid")
        }
        fn serve_read_pid(&self, _path: &std::path::Path) -> Option<u32> {
            self.serve_pid
        }
        fn serve_pid_alive(&self, pid: u32) -> bool {
            self.serve_alive && Some(pid) == self.serve_pid
        }
        fn serve_process_is_maw(&self, pid: u32) -> bool {
            self.serve_alive && Some(pid) == self.serve_pid
        }
        fn serve_process_summary(&self, _pid: u32) -> String {
            ", uptime 00:01, cmd: maw serve".to_owned()
        }
        fn serve_configured_port(&self) -> u16 {
        ...
        fn serve_probe_port(&self, _port: u16) -> ServePortProbe152 {
            self.serve_port_probe
                .clone()
                .unwrap_or(ServePortProbe152::NoListener)
        }
        fn serve_listener_pid(&self, _port: u16) -> Option<u32> {
            self.serve_listener_pid
        }
        fn serve_remove_pid(&mut self, path: &std::path::Path) -> Result<(), String> {
            self.removed.push(path.display().to_string());
            Ok(())
    ...
    fn messages_status_uses_existing_serve_state() {
        ...
        let out = messages_lifecycle_run152(
            &args(&["status", "--engine", "http://127.0.0.1:3456"]),
            &mut host,
        );
        ...
        assert!(out
            .stdout
            .contains("registered: /api/message-ledger → built-in maw serve"));
    ...
    fn messages_stop_is_idempotent_and_cleans_stale_pid() {
        let mut host = FakeHost152 {
            messages_pid: Some(123),
            messages_alive: false,
            ..FakeHost152::default()
        };
    ...
    fn serve_status_and_stop_follow_pid_file_state() {
        ...
        assert_eq!(
            status.stdout,
            "maw serve: running (pid 999, :3456 responding)\n"
        );
    ...
    fn serve_status_reports_responding_port_without_pidfile() {
        ...
        assert_eq!(
            status.stdout,
            "maw serve: running (pid 31999, :3456 responding)\n"
        );
    ...
    fn lifecycle_rejects_injection_values() {
        ...
        let bad_engine =
            messages_lifecycle_run152(&args(&["status", "--engine", "--bad"]), &mut host);
        assert_eq!(bad_engine.code, 2);
        assert!(bad_engine.stderr.contains("rejected --engine"));
        let bad_port =
            messages_lifecycle_run152(&args(&["serve", "--port", "bad\nport"]), &mut host);
