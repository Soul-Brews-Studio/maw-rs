const DISPATCH_69: &[DispatcherEntry] = &[DispatcherEntry {
    command: "audit",
    handler: Handler::Sync(run_audit_command),
}];
...
fn audit_run(argv: &[String]) -> Result<String, String> {
    ...
    if let Some(limit) = options.limit {
        audit_truncate_tail(&mut rows, limit);
    }
...
fn audit_parse_args(argv: &[String]) -> Result<AuditOptions, String> {
    ...
    while let Some(arg) = argv.get(index) {
        if arg == "--" {
            audit_push_tail(argv, index + 1, &mut positionals)?;
            break;
        }
        if let Some(consumed) = audit_parse_value_arg(argv, index, &mut options)? {
            index += consumed;
            continue;
        }
        if audit_parse_bool_arg(arg, &mut options) {
            index += 1;
            continue;
        }
        if arg == "--help" || arg == "-h" {
            return Err(AUDIT_USAGE.to_owned());
        }
        if arg.starts_with('-') {
            return Err(format!("audit: unknown argument {arg}"));
        }
...
fn audit_parse_value_arg(
    ...
    let consumed = match arg.as_str() {
        "--event" => {
            options.event = Some(audit_take_value(argv, index, "--event")?);
            2
        }
        "--since" => {
            options.since = Some(audit_take_value(argv, index, "--since")?);
            2
        }
...
fn audit_parse_equals_arg(arg: &str, options: &mut AuditOptions) -> Result<Option<usize>, String> {
    if let Some(value) = arg.strip_prefix("--event=") {
        audit_validate_value(value, "--event")?;
        options.event = Some(value.to_owned());
        return Ok(Some(1));
    }
    if let Some(value) = arg.strip_prefix("--since=") {
        audit_validate_value(value, "--since")?;
...
fn audit_finalize_options(
    mut options: AuditOptions,
    positionals: &[String],
) -> Result<AuditOptions, String> {
    if positionals.len() > 1 {
        return Err(AUDIT_USAGE.to_owned());
    }
    if let Some(value) = positionals.first() {
        options.limit = Some(audit_parse_limit(value)?);
    }
    Ok(options)
}
...
fn audit_take_value(argv: &[String], index: usize, flag: &str) -> Result<String, String> {
    let value = argv
        .get(index + 1)
        .ok_or_else(|| format!("audit: missing value for {flag}"))?;
...
fn audit_validate_value(value: &str, label: &str) -> Result<(), String> {
    if value.is_empty() {
        return Err(format!("audit: empty value for {label}"));
    }
    if value.starts_with('-') {
        return Err(format!("audit: {label} value must not start with '-'"));
    }
    if value.bytes().any(|byte| matches!(byte, 0 | b'\n' | b'\r')) {
        return Err(format!("audit: invalid control character in {label}"));
    }
    Ok(())
}
...
fn audit_parse_limit(value: &str) -> Result<usize, String> {
    let limit = value
        .parse::<usize>()
        .map_err(|_| "audit: limit must be a positive integer".to_owned())?;
    if limit == 0 {
        return Err("audit: limit must be a positive integer".to_owned());
    }
    Ok(limit)
}

fn audit_read_count(options: &AuditOptions) -> usize {
    if options.anomalies || options.event.is_some() || options.since.is_some() {
        return 10_000;
    }
...
fn audit_parse_rows(lines: &[String]) -> Vec<AuditRowNative> {
    lines
        .iter()
        .filter_map(|line| audit_parse_row(line))
        .collect()
}
...
fn audit_parse_row(line: &str) -> Option<AuditRowNative> {
    ...
    let args = value
        ...
        .map_or_else(Vec::new, |values| audit_parse_args_array(values));
    Some(AuditRowNative {
        ts,
...
fn audit_apply_filters(rows: &mut Vec<AuditRowNative>, options: &AuditOptions) {
    if options.anomalies {
        rows.retain(|row| row.kind.as_deref() == Some("anomaly"));
    }
    if let Some(event) = &options.event {
        rows.retain(|row| row.event.as_deref() == Some(event.as_str()));
    }
    if let Some(since) = &options.since {
        audit_filter_since(rows, since);
    }
}

fn audit_filter_since(rows: &mut Vec<AuditRowNative>, since: &str) {
    if !audit_valid_since(since) {
        return;
    }
...
fn audit_render(rows: &[AuditRowNative], options: &AuditOptions) -> String {
    if rows.is_empty() {
        return "\x1b[90mNo audit entries yet.\x1b[0m\n".to_owned();
    }
    let label = if options.anomalies {
        "Anomaly Trail"
    } else {
        "Audit Trail"
    };
...
fn audit_render_row(row: &AuditRowNative, out: &mut String) {
    ...
    if row.kind.as_deref() == Some("anomaly") {
        audit_render_anomaly(row, out, &time);
    } else {
        audit_render_command(row, out, &time);
    }
}
...
fn audit_render_anomaly(row: &AuditRowNative, out: &mut String, time: &str) {
    ...
    let _ = writeln!(
        out,
        "  \x1b[90m{time}\x1b[0m  \x1b[33m⚠\x1b[0m \x1b[35m{event}\x1b[0m  input={input}"
    );
}
...
fn audit_render_command(row: &AuditRowNative, out: &mut String, time: &str) {
    ...
    let result = row
        .result
        .as_ref()
        .map_or_else(String::new, |value| format!(" \x1b[90m→ {value}\x1b[0m"));
    let _ = writeln!(
        out,
        "  \x1b[90m{time}\x1b[0m  \x1b[33m{cmd}\x1b[0m{args}{result}"
    );
}
...
mod audit_tests {
    ...
    struct AuditEnvRestore {
        key: &'static str,
        value: Option<std::ffi::OsString>,
    }

    impl AuditEnvRestore {
        fn audit_capture(key: &'static str) -> Self {
            Self {
                key,
    ...
    fn audit_strings(values: &[&str]) -> Vec<String> {
    ...
    fn audit_temp_root(name: &str) -> std::path::PathBuf {
        ...
        let path =
            std::env::temp_dir().join(format!("maw-rs-audit-{name}-{}-{seq}", std::process::id()));
    ...
    fn audit_seed_env(
        ...
        .into_iter()
        .map(AuditEnvRestore::audit_capture)
        .collect::<Vec<_>>();
    ...
    fn audit_write_log(root: &std::path::Path) {
        ...
        fs::write(
            file,
            include_str!("../../tests/fixtures/native-audit/audit.jsonl"),
        )
        .expect("audit log");
    }
    ...
    fn audit_parse_limit_flags_and_rejects_leading_dash_values() {
        let parsed = audit_parse_args(&audit_strings(&[
            "7",
            "--event=bad-input",
            "--since",
    ...
    fn audit_default_tail_is_hermetic_and_matches_golden() {
        ...
        assert_eq!(
            output.stdout,
            include_str!("../../tests/fixtures/native-audit/audit-tail.stdout")
        );
    }
    ...
    fn audit_anomaly_filter_is_hermetic_and_matches_golden() {
        ...
        assert_eq!(
            output.stdout,
            include_str!("../../tests/fixtures/native-audit/audit-anomalies.stdout")
        );
    }
