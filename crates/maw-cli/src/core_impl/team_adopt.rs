...
const TEAM_RELEASE_USAGE: &str =
    "usage: maw team release <team> <member> [--charter <path>] [--dry-run]";
...
fn team_adopt(argv: &[String]) -> Result<String, String> {
    ...
    let text = std::fs::read_to_string(&charter_path)
        .map_err(|error| format!("team adopt: read charter failed: {error}"))?;
    ...
    team_t5_upsert_tool_member(
        &paths.tool_config,
        &member.role,
        member.engine.as_deref().unwrap_or("claude"),
...
fn team_release(argv: &[String]) -> Result<String, String> {
    ...
    let text = std::fs::read_to_string(&charter_path)
        .map_err(|error| format!("team release: read charter failed: {error}"))?;
...
fn team_adopt_parse(argv: &[String]) -> Result<TeamAdoptOptions381, String> {
    ...
    while index < argv.len() {
        match argv[index].as_str() {
            ...
            "--role" => {
                index += 1;
                opts.role = Some(team_adopt_next(argv, index, "--role")?);
            }
            "--engine" | "-e" => {
                index += 1;
                opts.engine = Some(team_adopt_next(argv, index, "--engine")?);
            }
            "--worktree" | "--cwd" => {
                index += 1;
                opts.worktree = Some(team_adopt_next(argv, index, "--worktree")?);
            }
            "--charter" => {
                index += 1;
                opts.charter_path = Some(team_adopt_next(argv, index, "--charter")?);
            }
            value if value.starts_with('-') => {
                return Err(format!("team adopt: unknown argument {value}"))
            }
    ...
    if positional.len() != 2 {
        return Err(TEAM_ADOPT_USAGE.to_owned());
    }
...
fn team_release_parse(argv: &[String]) -> Result<(String, String, Option<String>, bool), String> {
    ...
    while index < argv.len() {
        match argv[index].as_str() {
            ...
            "--charter" => {
                index += 1;
                charter = Some(team_adopt_next(argv, index, "--charter")?);
            }
            value if value.starts_with('-') => {
                return Err(format!("team release: unknown argument {value}"))
            }
            value => positional.push(value.to_owned()),
        }
        index += 1;
...
fn team_adopt_next(argv: &[String], index: usize, flag: &str) -> Result<String, String> {
    let value = argv
        .get(index)
        .ok_or_else(|| format!("team adopt: {flag} requires a value"))?;
    team_t3_validate_token(value, flag)?;
    Ok(value.clone())
}

/// Federation targets (`<node>:<session>`) are roster-only: no tmux, trust required.
fn team_adopt_resolve(
    opts: &TeamAdoptOptions381,
    charter: &TeamCharter122,
) -> Result<TeamCharterMember122, String> {
    ...
    Ok(TeamCharterMember122 {
        ...
        engine: Some(
            opts.engine
                .clone()
                .unwrap_or_else(|| team_adopt_engine_of(&pane.command)),
        ),
        target: Some(format!("{head}:{tail}")),
...
fn team_adopt_is_named_peer(node: &str) -> bool {
    team_invite_load_config()
        .named_peers
        .iter()
        .any(|peer| peer.name == node || peer.node.as_deref() == Some(node))
}
...
fn team_adopt_require_trust(node: &str) -> Result<(), String> {
    ...
    if TeamInviteSystemTrust125.team_invite_is_trusted(&from, node, "team-invite") {
        return Ok(());
    }
    Err(format!(
        "team adopt refuse untrusted federation peer '{node}' — run: maw team invite <team> {node}"
    ))
}
...
fn team_adopt_live_pane(session: &str, window: &str) -> Result<TeamPane124, String> {
    ...
    let mut matches = panes
        .into_iter()
        .filter(|pane| pane.session == session && pane.window == window)
        .collect::<Vec<_>>();
    if matches.is_empty() {
        return Err(format!(
            "team adopt refuse missing live target: {session}:{window}"
        ));
    }
    if matches.len() > 1 {
        return Err(format!(
            "team adopt refuse ambiguous live target: {session}:{window}"
        ));
    }
...
fn team_adopt_worktree(opts: &TeamAdoptOptions381, pane: &TeamPane124) -> Option<String> {
    if let Some(value) = &opts.worktree {
        if team_worktree_literal_is_opt_out(value) {
            return None;
        }
        return Some(value.clone());
    }
    team_t5_canonical_work_path(&pane.path)
        .ok()
...
fn team_adopt_reject_duplicate(
    charter: &TeamCharter122,
    ...
    for existing in &charter.members {
        if existing.role == member.role || existing.name.as_deref() == Some(name) {
            return Err(format!(
                "team adopt refuse duplicate member '{}': already in charter as role '{}'",
...
fn team_adopt_charter_text(text: &str, member: &TeamCharterMember122) -> Result<String, String> {
    if text.trim_start().starts_with('{') {
        return team_adopt_json_charter_text(text, member);
    }
    ...
    let last = blocks
        .last()
        .ok_or_else(|| "team adopt: charter has no members block".to_owned())?;
...
fn team_adopt_yaml_block(member: &TeamCharterMember122, dash_indent: usize) -> Vec<String> {
    ...
    if let Some(name) = &member.name {
        out.push(format!("{field}name: {name}"));
    }
    if let Some(engine) = &member.engine {
        out.push(format!("{field}engine: {engine}"));
    }
    ...
    if let Some(target) = &member.target {
        out.push(format!("{field}target: {target}"));
    }
    out.push(format!("{field}adopted: true"));
    out
}

fn team_adopt_json_charter_text(
    text: &str,
    member: &TeamCharterMember122,
) -> Result<String, String> {
    let mut value: serde_json::Value = serde_json::from_str(text)
        .map_err(|error| format!("team adopt json charter parse failed: {error}"))?;
    let members = value["members"]
        .as_array_mut()
        .ok_or_else(|| "team adopt json charter missing members".to_owned())?;
    let mut entry = serde_json::json!({"role": member.role, "adopted": true});
    if let Some(name) = &member.name {
        entry["name"] = serde_json::json!(name);
    }
    if let Some(engine) = &member.engine {
        entry["engine"] = serde_json::json!(engine);
    }
    if let Some(target) = &member.target {
        entry["target"] = serde_json::json!(target);
    }
    entry["worktree"] = member.worktree.as_ref().map_or_else(
...
mod team_adopt_tests381 {
    ...
    fn adopt_root(tag: &str) -> std::path::PathBuf {
        let root =
            std::env::temp_dir().join(format!("maw-rs-team-adopt-{}-{tag}", std::process::id()));
    ...
    fn team_adopt_appends_live_pane_without_spawning_or_killing() {
        ...
        assert!(
            body.contains("- role: builder"),
            "existing member must survive: {body}"
        );
    ...
    fn team_adopt_refuses_missing_dead_and_ambiguous_targets() {
        ...
        std::env::set_var(
            "MAW_RS_TEAM_TMUX_PANES",
            "alpha|chorus|claude|/nowhere|%1\nalpha|chorus|codex|/nowhere|%2\n",
        );
        let ambiguous = team_adopt(&argv(&[
            "adopt",
            "alpha",
            "alpha:chorus",
            "--charter",
        ...
        std::env::set_var(
            "MAW_RS_TEAM_TMUX_PANES",
            "alpha|builder|claude|/nowhere|%1\n",
        );
        let duplicate = team_adopt(&argv(&[
            "adopt",
            "alpha",
            "alpha:builder",
            "--charter",
    ...
    fn team_adopt_member_that_dies_later_still_releases_and_never_tears_down() {
        ...
        .expect("adopt while live");
        let body = std::fs::read_to_string(&charter).expect("read back");
        assert!(body.contains("adopted: true"), "{body}");

        // The pane dies afterwards: no panes left at all.
        std::env::set_var("MAW_RS_TEAM_TMUX_PANES", "");
        let parsed = team_parse_charter(&std::fs::read_to_string(&charter).expect("read"))
            .expect("charter parses");
        let refused = team_remove_reject_adopted(&parsed, "chorus")
            .expect_err("remove still refuses a dead adopted member");
        assert!(refused.contains("team release"), "{refused}");

        // Release is charter-only, so a dead pane must not make it fail.
        let out = team_release(&argv(&[
            "release",
            "alpha",
            "chorus",
            "--charter",
        ...
        .expect("release after death");
        assert!(out.contains("teardown: none"), "{out}");
        let after = std::fs::read_to_string(&charter).expect("read back");
        assert!(
            !after.contains("chorus"),
            "dead adopted member must be dropped: {after}"
        );
        assert!(
            after.contains("- role: builder"),
            "spawned member must survive: {after}"
        );
        assert!(
            !remove_log.exists(),
            "must not tear down a pane that is already gone"
        );
        assert!(
            !spawn_log.exists(),
            "must not respawn a dead adopted member"
        );
        let _ = std::fs::remove_dir_all(&root);
    }
    ...
    fn team_up_and_down_never_touch_an_adopted_pane() {
        ...
        std::env::set_var(
            "MAW_RS_TEAM_TMUX_PANES",
            "alpha|chorus|claude|/nowhere|%11\n",
        );
        team_adopt(&argv(&[
            "adopt",
            "alpha",
        ...
        let live = team_t3_classify(
            parsed
                .members
                .iter()
                .find(|m| m.role == "chorus")
                .expect("member"),
            &opts,
            "alpha",
            &team_t3_panes(),
        ...
        let member = parsed
            ...
            .expect("member");
        assert_eq!(
            team_down_keep_reason(member, &live, &[], true).as_deref(),
            Some("adopted"),
            "team down --all must still keep an adopted pane"
        );
        let builder = parsed
            .members
            .iter()
            .find(|m| m.role == "builder")
            .expect("builder");
        let builder_item = team_t3_classify(builder, &opts, "alpha", &team_t3_panes());
        assert_eq!(
            team_down_keep_reason(builder, &builder_item, &[], true),
            None,
            "a member we spawned is still torn down normally"
        );
        ...
        assert_eq!(
            gone.state, TEAM_ADOPTED_GONE,
            "a dead adopted pane must not read back as missing"
        );
        assert_eq!(
            team_t3_up_action(&gone, &opts),
            TEAM_ADOPTED_GONE_ACTION,
            "team up must not fresh wake"
        );
        let forced = TeamT3Options124 {
            flags: TEAM_T3_FORCE,
            ..TeamT3Options124::default()
        };
        assert_eq!(
            team_t3_up_action(&gone, &forced),
            TEAM_ADOPTED_GONE_ACTION,
            "--force must not fresh wake either"
        );
        let _ = std::fs::remove_dir_all(&root);
    }
    ...
    fn team_adopt_json_charter_keeps_shape_and_release_drops_without_teardown() {
        ...
        let adopted = parsed
            .members
            .iter()
            .find(|member| member.role == "chorus")
            .expect("adopted member");
        assert!(adopted.adopted, "adopted flag must round-trip");
        assert_eq!(adopted.target.as_deref(), Some("alpha:chorus"));
        assert_eq!(
            adopted.engine.as_deref(),
            Some("codex"),
            "engine derived from pane command"
        );

        // remove must refuse it, release must drop it and never call maw done.
        let refused =
            team_remove_reject_adopted(&parsed, "chorus").expect_err("remove refuses adopted");
        assert!(refused.contains("team release"), "{refused}");
        team_release(&argv(&[
            "release",
            "alpha",
...
fn team_adopt_render(
    ...
    if dry_run {
        let _ = write!(
            out,
            "  charter: would append to {charter_path}\n\nNo changes made\n"
        );
    } else {
        let _ = writeln!(out, "  charter: appended to {charter_path}");
    }
    out
}
