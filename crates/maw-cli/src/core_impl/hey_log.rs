...
fn hey_log_command(raw_args: &[String]) -> CliOutput {
    if wants_help(raw_args, &["--since", "--from", "-n"]) {
        return help_output(HEY_LOG_USAGE);
    }
        ...
        match raw_args[index].as_str() {
            "--since" => {
                options.since = Some(
                    raw_args
                        .get(index + 1)
                        .ok_or_else(|| "hey log: missing --since value".to_owned())?
                        .clone(),
                );
                index += 1;
            }
            value if value.starts_with("--since=") => {
                options.since = Some(value["--since=".len()..].to_owned());
            }
            "--from" => {
                options.from = Some(
                    raw_args
                        .get(index + 1)
                        .ok_or_else(|| "hey log: missing --from value".to_owned())?
                        .clone(),
                );
                index += 1;
            }
            ...
            "-n" => {
                options.limit = hey_log_parse_limit(
                    raw_args
                        .get(index + 1)
                        .ok_or_else(|| "hey log: missing -n value".to_owned())?,
                )?;
                index += 1;
            }
            value if value.starts_with("-n=") => {
                options.limit = hey_log_parse_limit(&value["-n=".len()..])?;
            }
            value if value.starts_with('-') => {
                return Err(format!("hey log: unknown argument {value}"))
            }
...
fn hey_log_parse_limit(value: &str) -> Result<usize, String> {
    let limit = value
        .parse::<usize>()
        .map_err(|_| "hey log: -n must be a positive integer".to_owned())?;
    (limit > 0)
        .then_some(limit)
        .ok_or_else(|| "hey log: -n must be a positive integer".to_owned())
}
...
fn hey_log_render(options: &HeyLogOptions) -> String {
    ...
    rows.retain(|row| {
        options
            .since
            .as_ref()
            .is_none_or(|since| row.event.ts >= *since)
            && options
                .from
                .as_ref()
                .is_none_or(|from| row.event.from == *from)
            && (!options.suspicious || !row.reasons.is_empty())
    });
    if rows.len() > options.limit {
        rows.drain(0..rows.len() - options.limit);
    }
...
fn hey_log_correlate(events: &[HeyLogEvent], audits: &[HeyLogAudit]) -> Vec<HeyLogRow> {
    ...
    for (index, audit) in audits.iter().enumerate() {
        by_ts.entry(audit.ts.clone()).or_default().push(index);
    }
    let mut used = vec![false; audits.len()];
    events
        .iter()
        .map(|event| {
            let audit_index = by_ts
                .get(&event.ts)
                .and_then(|indexes| indexes.iter().copied().find(|index| !used[*index]));
            ...
            if let Some(audit) = audit {
                if !hey_log_from_matches_user(&event.from, &audit.user) {
                    reasons.push("from!=user");
                }
                if hey_log_audit_from_unresolved(&audit.args) {
                    reasons.push("bad --from");
                }
            } else {
                reasons.push("missing audit");
            }
            if event.msg.starts_with('[') {
                reasons.push("prefix-bypass");
            }
            HeyLogRow {
                event: event.clone(),
                user: audit.map(|audit| audit.user.clone()),
                reasons,
            }
        })
        .collect()
...
fn hey_log_from_matches_user(from: &str, user: &str) -> bool {
    from == user || from.split(':').any(|part| part == user)
}
    ...
    while index < args.len() {
        match args[index].as_str() {
            "--from" => {
                return args
                    .get(index + 1)
                    .and_then(|value| explicit_wire_sender_oracle(value))
                    .is_none()
            }
            value if value.starts_with("--from=") => {
                return explicit_wire_sender_oracle(&value["--from=".len()..]).is_none()
            }
            _ => {}
        }
        index += 1;
