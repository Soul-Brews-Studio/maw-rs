const DISPATCH_263: &[DispatcherEntry] = &[DispatcherEntry {
    command: "footer",
    handler: Handler::Sync(footer_run_command263),
}];
const FOOTER_USAGE_263: &str = "usage: maw footer [--via <skill>] [--emoji]";

fn footer_run_command263(argv: &[String]) -> CliOutput {
    if wants_help(argv, &["--via"]) {
        return help_output(FOOTER_USAGE_263);
    }
    ...
    while index < argv.len() {
        match argv[index].as_str() {
            ...
            "--via" => {
                index += 1;
                via = footer_token263(
                    argv.get(index).map(String::as_str).unwrap_or_default(),
                    "via",
                )?;
            }
            value if value.starts_with("--via=") => {
                via = footer_token263(value.strip_prefix("--via=").unwrap_or_default(), "via")?;
            }
            "--" => return Err((2, "footer: -- separator is not supported".to_owned())),
            value if value.starts_with('-') => {
                return Err((
                    2,
                    format!("footer: unknown flag '{value}'\n  {FOOTER_USAGE_263}"),
                ))
            }
            value => {
                return Err((
                    2,
                    format!("footer: unexpected argument '{value}'\n  {FOOTER_USAGE_263}"),
                ))
            }
        }
        index += 1;
...
fn footer_output263(via: &str, emoji: bool) -> Result<String, (i32, String)> {
    let cwd =
        std::env::current_dir().map_err(|error| (1, format!("footer: current dir: {error}")))?;
    let short_time = footer_cmd263("date", &["+%H:%M"])?;
    let long_time = if emoji {
        footer_cmd263("date", &["+%Y-%m-%d %H:%M"])?
    } else {
        String::new()
    };
    ...
    let (org, repo) =
        footer_repo263().unwrap_or_else(|| ("unknown".to_owned(), "unknown".to_owned()));
    let (oracle, session) = (footer_oracle263(&cwd), footer_session263(&cwd));
    Ok(if emoji {
        format!("———\n📍 {oracle} @ {machine}\n🕐 {long_time}\n🔗 session: {session}\n🛤️ via: {via}\n📦 {org}/{repo} @ {commit}\n")
    } else {
        format!(
            "[{short_time}] [{oracle}@{machine}] [{session}] [{via}] [{org}] [{repo}] [{commit}]\n"
        )
    })
}
...
fn footer_cmd263(program: &str, args: &[&str]) -> Result<String, (i32, String)> {
    let output = std::process::Command::new(program)
        .args(args)
        .output()
        .map_err(|error| (1, format!("footer: failed to run {program}: {error}")))?;
...
fn footer_status263(args: &[&str]) -> bool {
...
fn footer_commit263() -> Result<String, (i32, String)> {
    ...
    if !footer_status263(&["diff", "--quiet"])
        || !footer_status263(&["diff", "--cached", "--quiet"])
    {
...
fn footer_repo263() -> Option<(String, String)> {
    footer_cmd263("git", &["remote", "get-url", "origin"])
        .ok()
        .and_then(|remote| footer_parse_remote263(&remote))
        .or_else(|| {
...
fn footer_parse_remote263(remote: &str) -> Option<(String, String)> {
    ...
    if raw.contains('\t') || raw.contains(" (fetch)") {
        raw = raw.split_whitespace().nth(1).unwrap_or(raw);
    }
    let path = raw
        .strip_prefix("git@github.com:")
        .or_else(|| {
            raw.find("github.com")
                .map(|i| raw[i + "github.com".len()..].trim_start_matches([':', '/']))
        })
        .unwrap_or(raw)
    ...
    let parts = path
        ...
        .collect::<Vec<_>>();
    Some((
        parts.get(parts.len().checked_sub(2)?)?.to_string(),
...
fn footer_oracle263(cwd: &std::path::Path) -> String {
    ...
    ["MAW_ORACLE", "CLAUDE_AGENT_NAME", "CLAUDE_TOKEN_NAME"]
        .into_iter()
        .filter_map(|key| std::env::var(key).ok())
        .find_map(|value| footer_clean_oracle263(&value))
        .or_else(|| footer_config_oracle263(&config))
        .or_else(|| footer_claude_oracle263(cwd))
        .or_else(|| {
            current_tmux_window_name().and_then(|window| {
                footer_clean_oracle263(&resolve_sender_oracle(None, Some(&window), None))
            })
        })
...
fn footer_config_oracle263(config: &serde_json::Value) -> Option<String> {
    ...
    .find_map(|path| {
        config
            ...
            .and_then(footer_clean_oracle263)
    })
}
    ...
    loop {
        if let Ok(text) = std::fs::read_to_string(dir.join("CLAUDE.md")) {
            for line in text.lines().take(120) {
                ...
                if trimmed.ends_with("-oracle") {
                    if let Some(value) = footer_clean_oracle263(trimmed) {
                        return Some(value);
                    }
                }
            }
...
fn footer_session263(cwd: &std::path::Path) -> String {
    let root = std::env::var_os("MAW_CLAUDE_PROJECTS_DIR")
    ...
    root.and_then(|root| footer_newest_jsonl263(cwd, &root))
        .or_else(|| {
            [
                "MAW_SESSION_ID",
            ...
            .into_iter()
            .filter_map(|key| std::env::var(key).ok())
            .find_map(|value| footer_session8_263(&value))
        })
        .unwrap_or_else(|| "unknown".to_owned())
}

fn footer_newest_jsonl263(
    cwd: &std::path::Path,
    ...
    for entry in std::fs::read_dir(projects_root.join(encoded.replace('/', "-")))
        .ok()?
        .flatten()
    {
        let path = entry.path();
        ...
        if newest.as_ref().is_none_or(|(_, time)| modified > *time) {
            newest = Some((stem.to_owned(), modified));
        }
    }
...
mod footer_tests263 {
    ...
    fn footer_parse_and_helpers_cover_contract_shapes() {
        ...
        assert_eq!(
            footer_parse263(&["--via=recap".to_owned(), "--emoji".to_owned()]).unwrap(),
            ("recap".to_owned(), true)
        );
        assert!(footer_parse263(&["--via".to_owned(), "bad value".to_owned()]).is_err());
        assert_eq!(
            footer_parse_remote263("git@github.com:laris-co/nexus-oracle.git"),
            Some(("laris-co".to_owned(), "nexus-oracle".to_owned()))
        );
        assert_eq!(
            footer_parse_remote263("https://github.com/Soul-Brews-Studio/maw-rs.git"),
            Some(("Soul-Brews-Studio".to_owned(), "maw-rs".to_owned()))
        );
        assert_eq!(
            footer_clean_oracle263("athena-oracle"),
            Some("athena".to_owned())
        );
        assert_eq!(
            footer_session8_263("019f324c-33e6-7622-9338-5ad573581e8d"),
            Some("019f324c".to_owned())
        );
    }
}
