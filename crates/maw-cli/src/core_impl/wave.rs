const DISPATCH_326: &[DispatcherEntry] = &[DispatcherEntry {
    command: "wave",
    handler: Handler::Sync(wave_run_command),
}];
...
fn wave_start(argv: &[String]) -> Result<String, String> {
    ...
    for member in &state.members {
        if !root.join(&member.worktree).exists() {
            worktree_run_with(
                &wave_strings(&["add", &member.worktree_name(), "--base", WAVE_BASE_REF]),
                &mut WorktreeSystemRuntime,
            )
    ...
    wave_at_root(&root, || {
        let opts = TeamT3Options124 {
            team: Some(team.clone()),
            session: Some(state.session.clone()),
...
fn wave_dispatch(argv: &[String]) -> Result<String, String> {
    let team = argv
        .get(1)
        .ok_or_else(|| "usage: maw wave dispatch <team> <task-description>".to_owned())?;
    team_validate_name(team)?;
    let task = argv
        .iter()
        .skip(2)
    ...
    let state = wave_read_state(team)?;
    let rows = wave_rows(&state);
    let Some((member, _, Some(pane))) = rows
        .into_iter()
        .find(|(_, health, pane)| *health == "idle" && pane.is_some())
    else {
    ...
    let pointer = format!(
        "Mission for {}: {} — ACK starting/blocked/done.",
        member.role,
        mission.display()
    );
...
fn wave_teardown(argv: &[String]) -> Result<String, String> {
    let team = argv
        .get(1)
        .ok_or_else(|| "usage: maw wave teardown <team>".to_owned())?;
    team_validate_name(team)?;
    let state = wave_read_state(team)?;
    let root = wave_primary_root()?;
    let mut out = format!("wave teardown: {team}\n");
    for member in &state.members {
        if let Some(window) = wave_member_window(&state.session, &member.role) {
            wave_kill_window(&state.session, &window, &mut out);
        }
        ...
        if src.exists() {
            ...
            match std::fs::rename(&src, &dst) {
                Ok(()) => {
                    let _ = writeln!(out, "moved\t{}\t{}", member.worktree, dst.display());
                }
                Err(error) => {
                    let _ = writeln!(out, "kept\t{}\tmove failed: {error}", member.worktree);
                }
            }
        }
    }
    ...
    for member in &state.members {
        match wave_git(&root, &["branch", "-d", &member.branch]) {
            Ok(_) => {
                let _ = writeln!(out, "branch-deleted\t{}", member.branch);
            }
            Err(error) => {
                let _ = writeln!(out, "branch-kept\t{}\t{}", member.branch, error.trim());
            }
        }
    }
    Ok(out)
...
fn wave_parse_start(argv: &[String]) -> Result<(String, usize), String> {
    let team = argv
        .get(1)
        .ok_or_else(|| "usage: maw wave start <team> [--coders N]".to_owned())?
        .clone();
    team_validate_name(&team)?;
    ...
    while index < argv.len() {
        match argv[index].as_str() {
            "--coders" => {
                ...
                coders = argv
                    .get(index)
                    .ok_or_else(|| "wave start: --coders requires a value".to_owned())?
                    .parse()
                    .map_err(|_| "wave start: --coders must be a positive integer".to_owned())?;
            }
    ...
    if coders == 0 {
        return Err("wave start: --coders must be positive".to_owned());
    }
    Ok((team, coders))
}
...
fn wave_primary_root() -> Result<std::path::PathBuf, String> {
    ...
    worktree_list_records(&mut runtime)?
        .first()
        .map(|record| record.path.clone())
        .ok_or_else(|| "wave: no git worktrees found".to_owned())
}
...
fn wave_at_root<T>(
    ...
    std::env::set_current_dir(root)
        .map_err(|error| format!("wave: chdir {}: {error}", root.display()))?;
    let result = run();
    match (
        result,
        std::env::set_current_dir(previous).map_err(|error| format!("wave: restore cwd: {error}")),
    ) {
...
fn wave_state_for(team: &str, coders: usize, root: &std::path::Path) -> WaveState325 {
    let members = (1..=coders)
        .map(|number| {
            let role = format!("coder-{number}");
            let slug = format!("{team}-{role}");
            WaveMember325 {
                role,
                worktree: format!("agents/{slug}"),
                branch: format!("agents/{slug}"),
            }
        })
        .collect();
    WaveState325 {
        team: team.to_owned(),
        session: team.to_owned(),
        mission_dir: root
            .join("ψ")
            .join("missions")
            .join(team)
...
fn wave_charter(state: &WaveState325) -> TeamCharter122 {
    TeamCharter122 {
        name: state.team.clone(),
        session: Some(state.session.clone()),
        members: state
            .members
            .iter()
            .map(|member| TeamCharterMember122 {
                role: member.role.clone(),
                name: Some(member.role.clone()),
                engine: Some("codex".to_owned()),
                worktree: Some(member.worktree.clone()),
...
fn wave_state_path(team: &str) -> std::path::PathBuf {
    team_state_dir().join("waves").join(format!("{team}.json"))
}
fn wave_write_state(state: &WaveState325) -> Result<(), String> {
    team_write_json_atomic_0600(&wave_state_path(&state.team), state)
}
fn wave_read_state(team: &str) -> Result<WaveState325, String> {
    team_read_json(&wave_state_path(team))
        .ok_or_else(|| format!("wave '{team}' not found; run maw wave start {team}"))
}

fn wave_list() -> String {
    let dir = team_state_dir().join("waves");
    let mut names = std::fs::read_dir(dir).map_or_else(
        |_| Vec::new(),
        |entries| {
            entries
                .flatten()
...
fn wave_ensure_session(session: &str, root: &std::path::Path) -> Result<(), String> {
    if std::env::var_os("MAW_RS_TEAM_FAKE_TMUX_LOG").is_some() {
        return Ok(());
    }
...
fn wave_heal(state: &WaveState325, charter: &TeamCharter122) -> Result<Vec<String>, String> {
    ...
    let roster: Vec<_> = charter
        .members
        .iter()
        .map(|member| {
            team_t3_classify(member, &TeamT3Options124::default(), &state.session, &panes)
        })
        .collect();
    let mut healed = Vec::new();
    for item in roster {
        let Some(pane) = item.pane else {
            continue;
        };
        if !team_t3_is_live_command(&pane.command) {
            wave_send_raw(
                &pane.pane_id,
                &format!(
                    "maw wake {} --no-attach --session {} -e codex --repo-path {}",
                    item.identity, state.session, item.worktree
                ),
            )?;
            healed.push(format!("{} relaunch", item.role));
        }
        if wave_capture(&pane.pane_id)
            .is_some_and(|text| text.contains("[Y/n]") || text.contains("Update available"))
        {
...
fn wave_rows(state: &WaveState325) -> Vec<(WaveMember325, &'static str, Option<String>)> {
    ...
    state
        ...
        .map(|member| {
            let pane = panes
                .iter()
                .find(|pane| pane.session == state.session && pane.window == member.role);
            let health = pane.map_or("stalled", |pane| {
                if !team_t3_is_live_command(&pane.command) {
                    "stalled"
                } else if wave_capture(&pane.pane_id)
                    .is_some_and(|text| text.contains("Working") || text.contains("tokens"))
                {
                    "working"
                } else {
                    "idle"
            ...
            (
                member.clone(),
                health,
                pane.map(|pane| pane.pane_id.clone()),
            )
        })
        .collect()
}
...
fn wave_member_window(session: &str, role: &str) -> Option<String> {
    team_down_panes()
        .ok()?
        .into_iter()
        .find(|pane| pane.session == session && pane.window == role)
        .map(|pane| pane.window)
}

fn wave_send_text(target: &str, text: &str) -> Result<(), String> {
    let mut runner = maw_tmux::CommandTmuxRunner::new();
    sendtext_send_text(&mut runner, target, text, std::thread::sleep)
        .map(|_| ())
        .map_err(|error| error.message)
}

fn wave_send_raw(target: &str, text: &str) -> Result<(), String> {
    wave_send_text(target, text)
}
fn wave_send_enter(target: &str) -> Result<(), String> {
    let mut runner = maw_tmux::CommandTmuxRunner::new();
    maw_tmux::TmuxRunner::run(
        &mut runner,
        "send-keys",
        &maw_tmux::tmux_send_enter_args(target),
    )
    .map(|_| ())
    .map_err(|error| error.message)
}
...
fn wave_kill_window(session: &str, window: &str, out: &mut String) {
    let mut runner = maw_tmux::CommandTmuxRunner::new();
    match maw_tmux::TmuxRunner::run(
        &mut runner,
        "kill-window",
        &["-t".to_owned(), format!("{session}:{window}")],
    ) {
        Ok(_) => {
            let _ = writeln!(out, "killed\t{session}:{window}");
        }
        Err(error) => {
            let _ = writeln!(out, "kept-window\t{session}:{window}\t{}", error.message);
        }
    }
}
...
fn wave_strings(values: &[&str]) -> Vec<String> {
