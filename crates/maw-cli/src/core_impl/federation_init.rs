...
fn parse_init_options(argv: &[String]) -> Result<InitOptions, String> {
    ...
    while index < argv.len() {
        match argv[index].as_str() {
            "--non-interactive" => {}
            "--node" => {
                node = required_value(argv, &mut index, "--node")?;
            }
            "--ghq-root" => {
                ghq_root = Some(expand_home(&required_value(
                    argv,
                    &mut index,
                    "--ghq-root",
                )?));
            ...
            "--token" => {
                token = Some(required_value(argv, &mut index, "--token")?);
            }
            "--federate" => federate = true,
            "--peer" => {
                peer_urls.push(required_value(argv, &mut index, "--peer")?);
            }
            "--peer-name" => {
                peer_names.push(required_value(argv, &mut index, "--peer-name")?);
            }
            "--federation-token" => {
                federation_token = Some(required_value(argv, &mut index, "--federation-token")?);
            }
            "--force" => force = true,
            "--backup" => backup = true,
...
fn write_init_config(opts: &InitOptions, interactive: bool) -> CliOutput {
    ...
    let federation_token = opts.federate.then(|| {
        opts.federation_token
            .clone()
            .unwrap_or_else(generate_federation_token)
    });
    ...
    if let Some(token) = &opts.token {
        config["env"]["CLAUDE_CODE_OAUTH_TOKEN"] = serde_json::json!(token);
    }
    if let Some(root) = &opts.ghq_root {
        config["ghqRoot"] = serde_json::json!(root);
    }
    if opts.federate {
        config["federationToken"] = serde_json::json!(federation_token.clone().unwrap_or_default());
        config["namedPeers"] = serde_json::Value::Array(
            opts.peers
                .iter()
    ...
    if let Err(error) = write_json_atomic(&path, &config) {
    ...
    CliOutput {
        code: 0,
        stdout,
        stderr: init_token_warning(opts),
    }
}
...
#[allow(dead_code)]
fn init_error(message: &str) -> CliOutput {
    CliOutput {
        code: 1,
        stdout: String::new(),
