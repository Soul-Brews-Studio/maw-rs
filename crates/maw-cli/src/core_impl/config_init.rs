const DISPATCH_105: &[DispatcherEntry] = &[DispatcherEntry {
    command: "init",
    handler: Handler::Sync(init_run_command),
}];
...
fn init_parse_options(argv: &[String]) -> Result<InitNativeOptions, String> {
    ...
    while index < argv.len() {
        match argv[index].as_str() {
            "--non-interactive" => {}
            "--node" => {
                node = init_required_value(argv, &mut index, "--node")?;
            }
            "--ghq-root" => {
                ghq_root = Some(init_expand_home(&init_required_value(
                    argv,
                    &mut index,
                    "--ghq-root",
                )?));
            ...
            "--token" => {
                token = Some(init_required_value(argv, &mut index, "--token")?);
            }
            "--federate" => federate = true,
            "--peer" => {
                peer_urls.push(init_required_value(argv, &mut index, "--peer")?);
            }
            "--peer-name" => {
                peer_names.push(init_required_value(argv, &mut index, "--peer-name")?);
            }
            "--federation-token" => {
                federation_token =
                    Some(init_required_value(argv, &mut index, "--federation-token")?);
            }
            "--force" => force = true,
            "--backup" => backup = true,
            other if other.starts_with('-') => {
                return Err(format!("maw init: unknown flag {other}"))
            }
...
fn init_collect_answers_from_tty() -> Result<InitNativeOptions, String> {
    ...
    if federate {
        for idx in 1..=32 {
            ...
            if url.is_empty() || url == "done" {
                break;
            }
            init_validate_peer_url(&url)?;
            let name = init_ask_tty(
                &format!("Peer {idx} name (short label)"),
                &format!("peer-{idx}"),
            )?;
...
fn init_ask_tty(question: &str, default_value: &str) -> Result<String, String> {
    ...
    std::io::Write::write_all(&mut tty, prompt.as_bytes())
    ...
    std::io::BufRead::read_line(&mut reader, &mut input)
        .map_err(|error| format!("read /dev/tty: {error}"))?;
...
fn init_write_config(opts: &InitNativeOptions, interactive: bool) -> CliOutput {
    ...
    if path.exists() && !opts.force && !opts.backup {
        return init_port_error(&format!(
            "Config exists at {}. Use --force to overwrite or --backup to preserve + overwrite.",
            path.display()
        ));
    }
    ...
    let federation_token = opts.federate.then(|| {
        opts.federation_token
            .clone()
            .unwrap_or_else(init_generate_federation_token)
    });
    ...
    if let Some(token) = &opts.token {
        updated["env"]["CLAUDE_CODE_OAUTH_TOKEN"] = serde_json::json!(token);
    }
    if let Some(root) = &opts.ghq_root {
        updated["ghqRoot"] = serde_json::json!(root);
    }
    if opts.federate {
        updated["federationToken"] =
            serde_json::json!(federation_token.clone().unwrap_or_default());
        updated["namedPeers"] = serde_json::Value::Array(
            opts.peers
                .iter()
    ...
    if let Err(error) = init_write_json_atomic(&path, &config) {
        return init_port_error(&error);
    }
    config_audit_write(&path, &before, &config);
    if interactive {
        stdout.push_str("\x1b[1mmaw init\x1b[0m — first-run setup\n\n");
    }
    let _ = writeln!(stdout, "\x1b[32m✓\x1b[0m Wrote {}", path.display());
    if opts.federate {
        let _ = writeln!(
            stdout,
            "\x1b[36mfederation token\x1b[0m: {}",
            federation_token.unwrap_or_default()
        );
    }
    CliOutput {
        code: 0,
        stdout,
        stderr: init_port_token_warning(opts),
    }
}
...
fn init_read_config(path: &std::path::Path) -> Result<serde_json::Value, String> {
    ...
    let raw = std::fs::read_to_string(path)
        .map_err(|error| format!("init: read {}: {error}", path.display()))?;
    let value = serde_json::from_str::<serde_json::Value>(&raw)
        .map_err(|error| format!("init: parse {}: {error}", path.display()))?;
    if value.is_object() {
        Ok(value)
    } else {
        Err(format!(
            "init: config root must be an object: {}",
            path.display()
        ))
    }
}
...
fn init_write_json_atomic(path: &std::path::Path, value: &serde_json::Value) -> Result<(), String> {
    ...
    let body = serde_json::to_string_pretty(value)
        .map_err(|error| format!("init: render config: {error}"))?
        + "\n";
...
fn init_backup_config(path: &std::path::Path) -> Result<std::path::PathBuf, String> {
    ...
    std::fs::copy(path, &backup)
        .map_err(|error| format!("init: backup {}: {error}", path.display()))?;
...
fn init_validate_node_name(name: &str) -> Result<(), String> {
    init_validate_name_re(
        name,
        63,
        "Node name must be 1-63 chars, letters/digits/hyphens only",
    )
}
fn init_validate_peer_name(name: &str) -> Result<(), String> {
    init_validate_name_re(
        name,
        31,
        "Name must be 1-31 chars, letters/digits/hyphens only",
    )
}
...
fn init_port_error(message: &str) -> CliOutput {
    CliOutput {
        code: 1,
        stdout: String::new(),
...
mod config_init_federate_tests {
    ...
    fn init_federate_rerun_keeps_peers_an_earlier_run_defined() {
        ...
        let third =
            init_run_non_interactive(&init_federate_argv("alpha", "https://alpha.wg:3457", true));
