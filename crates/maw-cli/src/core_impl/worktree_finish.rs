const DISPATCH_57: &[DispatcherEntry] = &[
    DispatcherEntry {
        command: "done",
        handler: Handler::Sync(run_done_command),
    },
    DispatcherEntry {
        command: "finish",
        handler: Handler::Sync(run_done_command),
...
fn done_run_with_context(
    argv: &[String],
    local: &mut impl DoneRuntime,
    context: &DoneContext,
) -> Result<String, String> {
    let options = done_parse_args(argv)?;
    if options.all && options.worktree.is_some() {
        return Err("done: --worktree cannot be used with --all".to_owned());
    }
    if options.all {
        return Ok(done_run_all(&options, local, context));
    }
    let target = options
        .target
        .clone()
        .ok_or_else(|| DONE_USAGE.to_owned())?;
...
fn done_parse_args(argv: &[String]) -> Result<DoneOptions, String> {
    ...
    while index < argv.len() {
        ...
        match arg.as_str() {
            ...
            "--worktree" => {
                let value = argv
                    .get(index + 1)
                    .ok_or_else(|| "done: missing --worktree value".to_owned())?;
            ...
            value if value.starts_with('-') => {
                return Err(format!("done: unknown argument {value}"))
            }
    ...
    if options.all && positionals.len() > 1 {
        return Err(format!(
            "unexpected extra positional arg(s) for maw done --all: {}\n  {DONE_ALL_USAGE}",
            positionals[1..].join(" ")
        ));
    }
    if !options.all && positionals.len() > 1 {
        let hint = if positionals
            .first()
        ...
        return Err(format!(
            "unexpected extra positional arg(s) for maw done: {}{hint}\n  {DONE_USAGE}",
            positionals[1..].join(" ")
        ));
    }
    if let Some(target) = positionals.first() {
        done_validate_target_arg(target, "target")?;
        options.target = Some(done_normalize_target(target));
    }
    if !options.all && options.target.is_none() {
        return Err(DONE_USAGE.to_owned());
    }
    Ok(options)
}
...
fn done_set_worktree_option(options: &mut DoneOptions, value: &str) -> Result<(), String> {
    if options.worktree.is_some() {
        return Err("done: --worktree specified more than once".to_owned());
    }
...
fn done_run_one_with_context(
    ...
    if let Some(window) = &matched {
        done_assert_may_target_lead(window, &sessions, local, &mut stdout)?;
    }
    let pane_info = matched
        .as_ref()
        .and_then(|window| done_live_pane_info(window, local));
    let selected_worktree = done_select_worktree(
        target,
        &target_lower,
        options,
        pane_info.as_ref(),
    ...
    if let Some(window) = &matched {
        ...
        let _ = writeln!(
            stdout,
            "  \x1b[36m⬡\x1b[0m [dry-run] window '{target}' not running — nothing to auto-save"
        );
    }
    if let Some(window) = &matched {
    ...
    if !removed_worktree {
        stdout.push_str("  \x1b[90m○\x1b[0m no worktree to remove (may be a main window)\n");
    }
    if options.dry_run {
        if matched.is_none() && !removed_worktree {
            done_fail_missing_target(target)?;
        }
    ...
    if !removed_config {
        stdout.push_str("  \x1b[90m○\x1b[0m not in any fleet config\n");
    }
    if matched.is_none() && !removed_worktree && !removed_config {
        done_fail_missing_target(target)?;
    }
    stdout.push('\n');
    Ok(stdout)
...
fn done_run_all(
    ...
    let Some(session_name) = session_name else {
        let reason = if let Some(oracle) = &options.target {
            format!("no tmux session found for oracle '{oracle}'")
        } else if sessions.is_empty() {
            "no tmux sessions to clean".to_owned()
        } else {
            "could not identify current tmux session; run inside tmux".to_owned()
        };
    ...
    if targets.is_empty() {
        let _ = writeln!(
            stdout,
            "  \x1b[90m○\x1b[0m no non-lead windows in {session_name}"
        );
        return stdout;
    }
    ...
    let _ = writeln!(
        stdout,
        "  \x1b[36m⬡\x1b[0m {mode} {} non-lead window(s) in {session_name}",
        targets.len()
    );
    ...
    for window in targets {
        let _ = writeln!(
            stdout,
            "\n\x1b[36m→\x1b[0m done {session_name}:{}",
            window.name
        );
        match done_run_one_with_context(&window.name, options, Some(&session_name), local, context)
        {
            Ok(text) => {
    ...
    let _ = writeln!(
        stdout,
        "  \x1b[32m✓\x1b[0m done --all {verb} {processed} window(s){suffix}"
    );
    stdout
}
...
fn done_find_window(
    ...
    windows
        .iter()
        .find(|window| {
            session_filter.is_none_or(|session| session == window.session)
                && window.name.eq_ignore_ascii_case(target_lower)
        })
        .cloned()
}

fn done_assert_may_target_lead(
    window: &DoneWindow,
    windows: &[DoneWindow],
    local: &mut impl DoneRuntime,
    stdout: &mut String,
    ...
    if lead.index != window.index {
        return Ok(());
    }
    if let Some((current_session, current_index)) = local.done_current_identity() {
        if current_session == window.session && current_index == lead.index {
            return Ok(());
        }
    }
    let message = format!(
        "refusing to done lead window '{}' in session '{}' from a non-lead context",
        window.name, window.session
    );
    let _ = writeln!(stdout, "  \x1b[31m✗\x1b[0m {message}");
    stdout.push_str(
        "  \x1b[90m  run from the lead window, or target a non-lead agent window\x1b[0m\n",
    );
    Err(message)
}
...
fn done_non_lead_windows(windows: &[DoneWindow], session: &str) -> Vec<DoneWindow> {
    let Some(lead) = done_lead_window(windows, session) else {
        return Vec::new();
    };
    let mut out = windows
        .iter()
        .filter(|window| window.session == session && window.index != lead.index)
        .cloned()
        .collect::<Vec<_>>();
...
fn done_current_session_name(
    ...
    if let Some(oracle) = oracle {
        ...
        if let Some(name) = sessions
            .iter()
            .find(|name| done_session_stem(name) == wanted)
        {
            return Some(name.clone());
        }
        let matches = sessions
            .iter()
            .filter(|name| done_compact_stem(name) == done_compact_stem(oracle))
            .cloned()
            .collect::<Vec<_>>();
        if matches.len() == 1 {
    ...
    if let Some((session, _)) = local.done_current_identity() {
...
fn done_session_stem(value: &str) -> String {
    value
        .trim()
        .to_lowercase()
...
fn done_compact_stem(value: &str) -> String {
    done_session_stem(value)
        .chars()
...
fn done_tmux_target(window: &DoneWindow) -> String {
    format!("{}:{}", window.session, window.name)
}
...
fn done_live_pane_info(window: &DoneWindow, local: &mut impl DoneRuntime) -> Option<DonePaneInfo> {
    ...
    match local.done_pane_info(&done_tmux_target(window)) {
        ...
        None if !listed_cwd.is_empty() => Some(DonePaneInfo {
            command: String::new(),
            cwd: listed_cwd.to_owned(),
        }),
        None => None,
    }
}
...
fn done_auto_save(
    ...
    options: &DoneOptions,
    local: &mut impl DoneRuntime,
    pane_info: Option<&DonePaneInfo>,
    worktree: Option<&DoneWorktree>,
    stdout: &mut String,
) {
    ...
    if options.dry_run {
        if let Some(retro) = retro {
            let _ = writeln!(
                stdout,
                "  \x1b[36m⬡\x1b[0m [dry-run] would send {retro} to {target} and wait 10s"
            );
        } else {
            stdout.push_str("  \x1b[36m⬡\x1b[0m [dry-run] would skip retro (no retrospective command for this engine)\n");
        }
        if let Some(worktree) = worktree {
            let _ = writeln!(
                stdout,
                "  \x1b[36m⬡\x1b[0m [dry-run] would git add + commit + push in {}",
                worktree.full_path.display()
            );
        }
        return;
    }
    if let Some(retro) = retro {
        let _ = local.done_send_text(&target, retro);
    }
    if let Some(worktree) = worktree {
        let cwd = worktree.full_path.display().to_string();
        let _ = local.done_git(&[
            "-C".to_owned(),
            cwd.clone(),
            "add".to_owned(),
            "--".to_owned(),
        ...
        let _ = local.done_git(&[
            ...
            "commit".to_owned(),
            "-m".to_owned(),
            "chore: auto-save before done".to_owned(),
        ]);
...
fn done_kill_window(
    ...
    if options.dry_run {
        let _ = writeln!(
            stdout,
            "  \x1b[36m⬡\x1b[0m [dry-run] would kill window {target}"
        );
        return;
    }
    match local.done_tmux("kill-window", &["-t".to_owned(), target.clone()]) {
        Ok(_) => {
            let _ = writeln!(stdout, "  \x1b[32m✓\x1b[0m killed window {target}");
        }
        Err(_) => {
            stdout.push_str("  \x1b[33m⚠\x1b[0m could not kill window (may already be closed)\n");
        }
    }
}
...
fn done_retrospective_command(command: &str) -> Option<&'static str> {
    ...
    if lower.contains("omx") || lower.contains("oh-my-codex") {
        Some("$rrr")
...
fn done_select_worktree(
    ...
    context: &DoneContext,
    stdout: &mut String,
) -> Result<Option<DoneWorktree>, String> {
    if let Some(path) = &options.worktree {
        let Some(worktree) = done_resolve_registered_worktree(local, path, context)? else {
            return Err(format!(
                "done: --worktree path is not a registered git worktree: {}",
                path.display()
            ));
        };
        let _ = writeln!(
            stdout,
            "  worktree: using explicit --worktree {}",
            worktree.full_path.display()
        );
    ...
    if let Some(info) = pane_info {
        ...
        if let Some(live) =
            done_resolve_registered_worktree(local, std::path::Path::new(&info.cwd), context)?
        {
            if let Some(registry) = done_worktree_from_config(window_lower, context) {
                if !done_same_path(&registry.full_path, &live.full_path) {
                    let _ = writeln!(
                        stdout,
                        "  worktree: using live pane cwd {} (registry said {}, stale)",
                        live.full_path.display(),
                        registry.full_path.display()
                    );
                }
            }
            return Ok(Some(live));
    ...
    if let Some(worktree) = done_worktree_from_config(window_lower, context) {
        return Ok(Some(worktree));
    }
...
fn done_worktree_from_config(window_lower: &str, context: &DoneContext) -> Option<DoneWorktree> {
    for file in done_fleet_config_files(context) {
        ...
        let Some(windows) = json.get("windows").and_then(serde_json::Value::as_array) else {
            continue;
        };
        let Some(repo) = windows
            .iter()
            .find(|item| {
                item.get("name")
                    .and_then(serde_json::Value::as_str)
                    .is_some_and(|name| name.eq_ignore_ascii_case(window_lower))
            })
        ...
        let Some(worktree) =
            done_parse_worktree_path(&done_config_repo_path(repo, context), &context.repos_root)
        else {
            break;
        };
...
fn done_config_repo_path(repo: &str, context: &DoneContext) -> std::path::PathBuf {
    ...
    if repo.starts_with("github.com/") {
        if let Some(parent) = context.repos_root.parent() {
            return parent.join(repo);
        }
    }
    context.repos_root.join(repo)
}

fn done_resolve_registered_worktree(
    local: &mut impl DoneRuntime,
    path: &std::path::Path,
    context: &DoneContext,
) -> Result<Option<DoneWorktree>, String> {
    done_validate_exec_path(path)?;
    let top_level = match local.done_git(&[
        "-C".to_owned(),
    ...
    let Some(worktree) = done_parse_live_worktree_path(&top_level, context) else {
        return Ok(None);
    };
    if done_is_registered_worktree(local, &worktree)? {
        Ok(Some(worktree))
    } else {
        Ok(None)
    }
...
fn done_parse_live_worktree_path(
...
fn done_is_registered_worktree(
    local: &mut impl DoneRuntime,
    ...
    let Ok(raw) = local.done_git(&[
        "-C".to_owned(),
        worktree.main_path.display().to_string(),
        "worktree".to_owned(),
        "list".to_owned(),
        "--porcelain".to_owned(),
    ]) else {
    ...
    Ok(raw
        ...
        .any(|path| done_same_path(std::path::Path::new(path), &worktree.full_path)))
}
...
fn done_run_process(command: &str, args: &[&str], cwd: Option<&std::path::Path>) -> String {
    let mut process = if command == "git" {
        std::process::Command::new(done_git_executable())
    } else {
    ...
    let output = process
        .output()
        .unwrap_or_else(|error| panic!("failed to run {process:?}: {error}"));
    assert!(
...
fn done_worktree_by_scan(
    ...
    if matches.len() > 1 {
        let _ = writeln!(
            stdout,
            "  \x1b[31m✗\x1b[0m refusing to remove worktree '{}' — matches {} repos",
            target,
            matches.len()
        );
        return None;
...
fn done_remove_selected_worktree(
    ...
    if options.dry_run {
        let _ = writeln!(
            stdout,
            "  \x1b[36m⬡\x1b[0m [dry-run] would remove worktree {}",
            worktree.label
        );
        return Ok(());
    }
...
fn done_remove_worktree(
    ...
    let branch = local
        .done_git(&[
            "-C".to_owned(),
            worktree.full_path.display().to_string(),
            "rev-parse".to_owned(),
            "--abbrev-ref".to_owned(),
            "HEAD".to_owned(),
        ])
    ...
    let _ = writeln!(
        stdout,
        "  \x1b[32m✓\x1b[0m removed worktree {}",
        worktree.label
    );
...
fn done_cleanup_branch(
    ...
    if branch.is_empty() || branch == "main" || branch == "HEAD" {
        return;
    }
    if options.clean_branch {
        ...
        let _ = writeln!(stdout, "  \x1b[32m✓\x1b[0m deleted branch {branch}");
    } else {
        let _ = writeln!(
            stdout,
            "  \x1b[90m○\x1b[0m branch {branch} retained (use --clean-branch to delete)"
        );
    }
}
...
fn done_find_worktree_paths(target: &str, repos_root: &std::path::Path) -> Vec<DoneWorktree> {
    ...
    for org in orgs.flatten().filter(|entry| entry.path().is_dir()) {
        ...
        for repo in repos.flatten().filter(|entry| entry.path().is_dir()) {
            done_scan_repo_worktrees(&repo.path(), repos_root, &target_lower, &mut out);
        }
    }
...
fn done_scan_repo_worktrees(
    repo_path: &std::path::Path,
    repos_root: &std::path::Path,
    target_lower: &str,
    out: &mut Vec<DoneWorktree>,
) {
    let Some(name) = repo_path.file_name().and_then(std::ffi::OsStr::to_str) else {
        return;
    };
    if name
        .to_lowercase()
        .ends_with(&format!(".wt-{target_lower}"))
    {
        if let Some(worktree) = done_parse_worktree_path(repo_path, repos_root) {
            out.push(worktree);
        }
    }
    ...
    for entry in entries.flatten().filter(|entry| entry.path().is_dir()) {
        ...
        {
            if let Some(worktree) = done_parse_worktree_path(&entry.path(), repos_root) {
                out.push(worktree);
            }
        }
    }
...
fn done_remove_from_fleet_config(
    window_lower: &str,
    context: &DoneContext,
    stdout: &mut String,
) -> bool {
...
fn done_normalize_target(value: &str) -> String {
    value.trim().to_owned()
}

fn done_validate_target_arg(value: &str, label: &str) -> Result<(), String> {
    let trimmed = value.trim();
    if trimmed.is_empty() || trimmed.starts_with('-') || trimmed != value {
        return Err(format!("done: invalid {label} '{value}'"));
    }
    Ok(())
}
...
fn done_validate_worktree_arg(value: &str) -> Result<(), String> {
    let trimmed = value.trim();
    if trimmed.is_empty() || trimmed.starts_with('-') || trimmed != value {
        return Err(format!("done: invalid --worktree path '{value}'"));
    }
    Ok(())
}

fn done_validate_tmux_target(value: &str) -> Result<(), String> {
    if value.trim().is_empty() || value.starts_with('-') {
        Err(format!("done: invalid tmux target '{value}'"))
    } else {
        Ok(())
    }
}
...
fn done_validate_exec_path(path: &std::path::Path) -> Result<(), String> {
    ...
    {
        return Err(format!(
            "done: refusing leading-dash path '{}'",
            path.display()
        ));
    }
    Ok(())
...
mod done_tests {
    ...
    impl DoneFakeRuntime {
        fn register_worktree(&mut self, main: &std::path::Path, worktree: &std::path::Path) {
            self.top_levels
                .insert(worktree.to_path_buf(), worktree.to_path_buf());
            self.registered
                .entry(main.to_path_buf())
                .or_default()
                .push(worktree.to_path_buf());
            self.branches
                .insert(worktree.to_path_buf(), "agent/task".to_owned());
        }

        fn git_cwd(args: &[String]) -> Option<std::path::PathBuf> {
            args.windows(2)
                .find_map(|pair| (pair[0] == "-C").then(|| std::path::PathBuf::from(&pair[1])))
        }
    ...
    impl DoneRuntime for DoneFakeRuntime {
        ...
        fn done_pane_info(&mut self, target: &str) -> Option<(String, String)> {
            self.pane_info.get(target).cloned()
        }
        ...
        fn done_git(&mut self, args: &[String]) -> Result<String, String> {
            ...
            if args.iter().any(|arg| arg == "remove") {
                let worktree = Self::arg_after_separator(args)
                    .ok_or_else(|| "missing worktree path".to_owned())?;
    ...
    impl DoneRuntime for DoneRealGitRuntime {
        ...
        fn done_pane_info(&mut self, _target: &str) -> Option<(String, String)> {
            None
        }

        fn done_tmux(&mut self, _command: &str, _args: &[String]) -> Result<String, String> {
            Err("tmux unavailable in real-git test runtime".to_owned())
        }

        fn done_send_text(&mut self, _target: &str, _text: &str) -> Result<(), String> {
            Err("tmux unavailable in real-git test runtime".to_owned())
        }
    ...
    impl DoneTempRoot {
        fn new(name: &str) -> Self {
            ...
            let path = std::env::temp_dir()
                .join(format!("maw-rs-done-{name}-{}-{seq}", std::process::id()));
            let _ = std::fs::remove_dir_all(&path);
            std::fs::create_dir_all(&path).expect("temp root");
            Self { path }
        }

        fn repos_root(&self) -> std::path::PathBuf {
            self.path.join("github.com")
        }

        fn fleet_dir(&self) -> std::path::PathBuf {
            self.path.join("fleet")
        }
    ...
    fn done_write_fleet(root: &DoneTempRoot, window: &str, repo: &str) {
        ...
        std::fs::write(
            fleet_dir.join("s.json"),
            format!(r#"{{"name":"s","windows":[{{"name":"{window}","repo":"{repo}"}}]}}"#),
        )
        .expect("fleet");
    }

    fn done_args(args: &[&str]) -> Vec<String> {
    ...
    fn done_parse_rejects_leading_dash_positionals() {
        assert_eq!(
            done_parse_args(&["-Sbad".to_owned()]).unwrap_err(),
    ...
    fn done_removes_session_window_without_mutating_squad_roster() {
        ...
        let roster_body =
            r#"{"name":"01-core","windows":[{"name":"worker","repo":"acme/roster"}],"members":[]}"#;
        std::fs::write(&roster, roster_body).expect("roster");

        assert!(done_remove_from_fleet_config(
            "worker",
            &root.context(),
            &mut String::new()
        ));
        assert_eq!(
            std::fs::read_to_string(roster).expect("roster remains"),
            roster_body
        );
        let session = std::fs::read_to_string(root.fleet_dir().join("s.json")).expect("session");
        assert_eq!(
            serde_json::from_str::<serde_json::Value>(&session).expect("json")["windows"],
            serde_json::json!([])
        );
    }
    ...
    fn done_worktree_path_parses_agents_and_dot_wt() {
        ...
        let agents = done_parse_worktree_path(
            std::path::Path::new("/tmp/ghq/github.com/org/repo/agents/task"),
            root,
        )
        .unwrap();
        assert_eq!(
            agents.main_path,
            std::path::PathBuf::from("/tmp/ghq/github.com/org/repo")
        );
        let dot = done_parse_worktree_path(
            std::path::Path::new("/tmp/ghq/github.com/org/repo.wt-task"),
            root,
        )
        .unwrap();
        assert_eq!(
    ...
    fn done_live_cwd_differs_from_registry_live_wins_with_warning() {
        ...
        runtime.pane_info.insert(
            "s:worker".to_owned(),
            ("codex".to_owned(), live.display().to_string()),
        );
        runtime.register_worktree(&main, &live);
        runtime.register_worktree(&main, &stale);

        let out =
            done_run_with_context(&done_args(&["worker", "--dry-run"]), &mut runtime, &context)
                .expect("done");
        assert!(
            out.contains(&format!(
                "worktree: using live pane cwd {} (registry said {}, stale)",
        ...
        assert!(
            out.contains("would remove worktree acme/app/agents/live-task"),
            "{out}"
        );
        assert!(
            !out.contains("would remove worktree acme/app/agents/stale-task"),
            "{out}"
        );
    }
    ...
    fn done_cd_redispatched_window_resolves_listed_live_cwd_not_stale_registry() {
        ...
        let mut runtime = DoneFakeRuntime {
            windows: vec![
                done_test_window("lead"),
                done_test_window_with_cwd("worker", &live),
            ],
            ..DoneFakeRuntime::default()
        };
        runtime.register_worktree(&main, &live);
        ...
        assert!(
            out.contains("would remove worktree acme/app/agents/new-task"),
            "{out}"
        );
        assert!(
            !out.contains("would remove worktree acme/app/agents/old-task"),
            "{out}"
        );
    }
    ...
    fn done_real_git_worktree_resolves_when_context_repos_root_differs() {
        ...
        done_run_process(
            "git",
            &["worktree", "add", "-b", "agents/live-task", &live_path],
            Some(&main),
        );
        ...
        let resolved = done_resolve_registered_worktree(&mut runtime, &live, &wrong_context)
            .expect("resolve")
            .expect("registered worktree");

        assert!(
            done_same_path(&resolved.main_path, &main),
            "{} != {}",
            resolved.main_path.display(),
            main.display()
        );
        assert!(
            done_same_path(&resolved.full_path, &live),
            "{} != {}",
            resolved.full_path.display(),
            live.display()
        );
    ...
    fn done_dead_pane_falls_back_to_registry() {
        ...
        assert!(
            out.contains(&stale.display().to_string())
                || out.contains("acme/app/agents/stale-task"),
            "{out}"
        );
    }
    ...
    fn done_worktree_override_wins_over_live_and_registry() {
        ...
        assert!(
            out.contains(&format!(
                "would git add + commit + push in {}",
                override_path.display()
            )),
            "{out}"
        );
        assert!(
            out.contains("would remove worktree acme/app/agents/override-task"),
            "{out}"
        );
        assert!(
            !out.contains("would remove worktree acme/app/agents/live-task"),
    ...
    fn done_worktree_override_rejects_non_worktree_path() {
        ...
        .expect_err("reject");
        assert!(
            err.contains("--worktree path is not a registered git worktree"),
            "{err}"
        );
    }
    ...
    fn done_dirty_worktree_removal_is_refused_without_force() {
        ...
        let err = done_run_with_context(&done_args(&["worker"]), &mut runtime, &context)
            .expect_err("dirty");
        assert!(
            err.contains("contains modified or untracked files"),
            "{err}"
        );
        assert!(
            runtime
