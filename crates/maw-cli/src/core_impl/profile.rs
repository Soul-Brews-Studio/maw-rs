...
fn profile_dispatch(argv: &[String]) -> Result<String, ProfileError> {
    ...
    let Some(sub) = positional.first().copied() else {
        return Ok(format!("{PROFILE_USAGE}\n"));
    };
    match sub {
        ...
        _ => Err(ProfileError {
            message: format!(
                "maw profile: unknown subcommand \"{sub}\" (expected list|use|show|current)"
            ),
...
fn profile_required_name<'a>(
    ...
    if let Some(raw) = argv.get(1).filter(|value| value.starts_with('-')) {
        return match profile_validate_name(raw) {
            Ok(()) => Err(profile_error(format!(
                "maw profile: invalid profile name \"{raw}\""
            ))),
            Err(error) => Err(profile_error(error)),
        };
    }
    positional.get(1).copied().ok_or_else(|| ProfileError {
...
fn profile_use(name: &str) -> Result<String, ProfileError> {
    ...
    let Some(profile) = profile_load(name)? else {
        return Err(ProfileError {
            message: format!("profile \"{name}\" not found — see \"maw profile list\""),
            stdout: String::new(),
        });
    };
...
fn profile_load(name: &str) -> Result<Option<ProfileRecord>, ProfileError> {
    if profile_validate_name(name).is_err() {
        return Ok(None);
    }
    if name == "all" {
...
fn profile_load_all() -> Result<Vec<ProfileRecord>, ProfileError> {
    ...
    for entry in entries.flatten() {
        ...
        if let Some(profile) = profile_load(name)? {
            rows.push(profile);
        }
    }
...
fn profile_format_list(rows: &[ProfileRecord], active: &str) -> String {
    ...
    for row in rows {
        table.push(vec![
            ...
            row.name.clone(),
            profile_array_len(&row.value, "plugins")
                .map_or_else(|| "-".to_owned(), |len| len.to_string()),
            profile_string_array(&row.value, "tiers")
...
fn profile_get_active() -> String {
    ...
    let active = raw.trim();
    if active.is_empty() || profile_validate_name(active).is_err() {
        "all".to_owned()
    } else {
        active.to_owned()
    }
...
fn profile_set_active(name: &str) -> Result<(), ProfileError> {
    ...
    std::fs::create_dir_all(&dir).map_err(|error| {
        profile_error(format!("maw profile: failed to create config dir: {error}"))
    })?;
...
fn profile_ensure_default() -> Result<(), ProfileError> {
    ...
    std::fs::create_dir_all(profile_profiles_dir()).map_err(|error| {
        profile_error(format!(
            "maw profile: failed to create profiles dir: {error}"
        ))
    })?;
...
fn profile_atomic_write(path: &std::path::Path, body: &str) -> Result<(), ProfileError> {
    if let Some(parent) = path.parent() {
        std::fs::create_dir_all(parent).map_err(|error| {
            profile_error(format!("maw profile: failed to create parent dir: {error}"))
        })?;
    }
    let file_name = path
        .file_name()
    ...
    std::fs::write(&tmp, body).map_err(|error| {
        profile_error(format!("maw profile: failed to write temp file: {error}"))
    })?;
    std::fs::rename(&tmp, path)
        .map_err(|error| profile_error(format!("maw profile: failed to replace file: {error}")))
}

fn profile_config_dir() -> std::path::PathBuf {
    maw_config_dir(&current_xdg_env())
}

fn profile_profiles_dir() -> std::path::PathBuf {
    profile_config_dir().join("profiles")
}

fn profile_path(name: &str) -> std::path::PathBuf {
    profile_profiles_dir().join(format!("{name}.json"))
}

fn profile_active_path() -> std::path::PathBuf {
    profile_config_dir().join("profile-active")
}
...
fn profile_error(message: String) -> ProfileError {
    ProfileError {
        message,
        stdout: String::new(),
    }
...
mod profile_tests {
    ...
    fn profile_rejects_bad_names_before_write() {
        let error =
            profile_dispatch(&["use".to_owned(), "--bad".to_owned()]).expect_err("flag-like name");
        assert!(error.message.contains("invalid profile name"));
        let error =
            profile_dispatch(&["show".to_owned(), "BadName".to_owned()]).expect_err("uppercase");
        assert!(error.message.contains("invalid profile name"));
    }
    ...
    fn profile_use_writes_active_pointer_atomically() {
        ...
        assert_eq!(
            std::fs::read_to_string(root.join("config/profile-active")).expect("active"),
            "lean\n"
        );
