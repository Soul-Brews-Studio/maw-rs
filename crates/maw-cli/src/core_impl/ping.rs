const DISPATCH_138: &[DispatcherEntry] = &[DispatcherEntry {
    command: "ping",
    handler: Handler::Sync(ping_run_command),
}];
...
#[derive(Debug, Clone, PartialEq, Eq)]
struct PingProbe133 {
    target: PingTarget133,
    status: PingStatus133,
    ms: u128,
}

#[derive(Debug, Clone, PartialEq, Eq)]
enum PingStatus133 {
    Ok { auth: String, token: String },
    Http(u16),
    Unreachable,
}

trait PingTransport133 {
...
impl PingTransport133 for PingCurlTransport133 {
    fn ping_auth_status(&mut self, target: &PingTarget133) -> PingStatus133 {
        let Ok(argv) = ping_curl_argv(&target.url) else {
            return PingStatus133::Unreachable;
        };
        let Ok(raw) = ping_spawn_curl(&argv) else {
            return PingStatus133::Unreachable;
        };
...
fn ping_run_command(argv: &[String]) -> CliOutput {
    ping_run_command_with(
        argv,
        &load_hey_config(),
        &mut PingCurlTransport133,
        ping_now_millis,
    )
}
...
fn ping_run(
    ...
    if targets.is_empty() {
        return Ok("\x1b[90mno peers configured\x1b[0m\n".to_owned());
    }
...
fn ping_targets(config: &HeyConfig, node: Option<&str>) -> Result<Vec<PingTarget133>, String> {
    ...
    let legacy = config
        ...
        .peers
        .iter()
        .filter(|url| !named.iter().any(|peer| peer.url == **url))
        .map(|url| PingTarget133 {
            name: url.clone(),
            url: url.clone(),
        })
        .collect::<Vec<_>>();
    match node {
        Some(value) => {
            ...
            if let Some(peer) = named.iter().find(|peer| peer.name == value) {
                return Ok(vec![peer.clone()]);
            }
            if let Some(peer) = legacy.iter().find(|peer| peer.url.contains(value)) {
                return Ok(vec![PingTarget133 {
                    name: value.to_owned(),
                    url: peer.url.clone(),
                }]);
            }
            let known = if named.is_empty() {
                "(none)".to_owned()
            } else {
                named
                    .iter()
...
struct PingPeersStore133 {
    #[serde(default)]
    peers: BTreeMap<String, PingPeerEntry133>,
}

#[derive(Debug, serde::Deserialize, Default)]
struct PingPeerEntry133 {
    url: Option<String>,
    node: Option<String>,
}

fn ping_targets_from_peer_store() -> Result<Vec<PingTarget133>, String> {
    let Some(raw) = ping_read_peers_json()? else {
        return Ok(Vec::new());
    };
    ...
    for (alias, entry) in store.peers {
        ...
        if let Some(url) = entry.url {
            ...
            if let Some(node) = entry.node.as_deref() {
                ping_validate_node(node).map_err(|_| format!("invalid peer node for {alias}"))?;
            }
...
fn ping_read_peers_json() -> Result<Option<String>, String> {
    ...
    if path.exists() {
        return std::fs::read_to_string(&path)
            .map(Some)
            .map_err(|error| format!("peers: read {}: {error}", path.display()));
    }
    Ok(None)
}
...
fn ping_spawn_curl(argv: &[String]) -> Result<String, String> {
    ...
    if !output.status.success() {
        return Err("curl failed".to_owned());
    }
...
fn ping_parse_curl_output(raw: &str) -> PingStatus133 {
    let Some((body, status_raw)) = raw.rsplit_once(PING_HTTP_STATUS_MARKER) else {
        return PingStatus133::Unreachable;
    };
    let Ok(status) = status_raw.trim().parse::<u16>() else {
        return PingStatus133::Unreachable;
    };
    if !(200..300).contains(&status) {
        return PingStatus133::Http(status);
    }
    let Ok(value) = serde_json::from_str::<serde_json::Value>(body.trim()) else {
        return PingStatus133::Unreachable;
    };
    let auth = if value.get("enabled").and_then(serde_json::Value::as_bool) == Some(true) {
        "auth: ok"
    } else {
        "auth: off"
    };
...
fn ping_validate_node(value: &str) -> Result<(), String> {
    ...
    if value.chars().any(|ch| {
        ...
        return Err(
            "node must not contain whitespace, control, or shell metacharacters".to_owned(),
        );
    }
    Ok(())
}
    ...
    for probe in probes {
        match &probe.status {
            PingStatus133::Ok { auth, token } => {
                ...
                let _ = writeln!(
                    out,
                    "\x1b[32m✅\x1b[0m {} \x1b[90m({})\x1b[0m — {}ms, {auth}{suffix}",
                    probe.target.name, probe.target.url, probe.ms
                );
            }
            PingStatus133::Http(status) => {
                let _ = writeln!(
                    out,
                    "\x1b[31m❌\x1b[0m {} \x1b[90m({})\x1b[0m — {}ms, {status}",
                    probe.target.name, probe.target.url, probe.ms
                );
            }
            PingStatus133::Unreachable => {
                let _ = writeln!(
                    out,
                    "\x1b[31m❌\x1b[0m {} \x1b[90m({})\x1b[0m — {}ms, unreachable",
                    probe.target.name, probe.target.url, probe.ms
                );
            }
        }
...
mod ping_tests133 {
    ...
    struct PingFakeTransport133 {
        statuses: VecDeque<PingStatus133>,
        targets: Vec<PingTarget133>,
    }
    ...
    impl PingTransport133 for PingFakeTransport133 {
        fn ping_auth_status(&mut self, target: &PingTarget133) -> PingStatus133 {
            ...
            self.statuses
                .pop_front()
                .unwrap_or(PingStatus133::Unreachable)
        }
    }
        ...
        HeyConfig {
            ...
            route: RouteConfig {
                ...
                named_peers: vec![RouteNamedPeer {
                    name: "alpha".to_owned(),
                    url: "http://alpha.invalid:31745".to_owned(),
                }],
                peers: vec!["http://legacy.invalid:31745".to_owned()],
                agents: HashMap::new(),
            },
        }
    }
    ...
    #[test]
    fn ping_dispatch_registers_native() {
        assert_eq!(dispatcher_status("ping"), DispatchKind::Native);
    }

    #[test]
    fn ping_all_renders_named_and_legacy_without_real_network() {
        let _guard = ping_no_peer_store_env();
        let mut transport = PingFakeTransport133 {
            statuses: VecDeque::from([
                PingStatus133::Ok {
                    auth: "auth: ok".to_owned(),
                    token: "tokn****".to_owned(),
                },
                PingStatus133::Http(503),
            ]),
            ..PingFakeTransport133::default()
        };
    ...
    fn ping_specific_unknown_lists_known_and_errors_before_transport() {
        ...
        let error =
            ping_run(&args(&["missing"]), &cfg(), &mut transport, now).expect_err("unknown");
    ...
    fn ping_curl_argv_is_no_shell_and_targets_auth_status() {
        ...
        assert!(argv
            .iter()
            .any(|arg| arg == "http://peer.invalid:31745/api/auth/status"));
        assert!(!argv
            .windows(2)
    ...
    fn ping_parse_curl_output_maps_status_and_auth() {
        assert_eq!(
            ping_parse_curl_output(
                r#"{"enabled":true,"tokenPreview":"abcd****"}__MAW_HTTP_STATUS__:200"#
            ),
            PingStatus133::Ok {
                auth: "auth: ok".to_owned(),
                token: "abcd****".to_owned()
        ...
        assert_eq!(
            ping_parse_curl_output(r#"{"error":"no"}__MAW_HTTP_STATUS__:404"#),
            PingStatus133::Http(404)
        );
    }
}
