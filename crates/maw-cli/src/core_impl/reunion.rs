...
fn reunion_run(argv: &[String]) -> Result<String, String> {
    ...
    if !cwd.join("ψ").is_dir() {
        return Ok(format!(
            "  \x1b[90m○\x1b[0m reunion: no ψ/ in {}, skipping\n",
            cwd.display()
        ));
    }
    let Some(main_root) = reunion_resolve_main_root(&cwd, options.git_common_dir.as_deref()) else {
        return Ok(
            "  \x1b[90m○\x1b[0m reunion: not a worktree (already main), skipping\n".to_owned(),
        );
    };
...
fn reunion_parse_args(argv: &[String]) -> Result<ReunionArgs, String> {
    ...
    {
        return Err(REUNION_USAGE.to_owned());
    }
    ...
    while index < argv.len() {
        ...
        match arg.as_str() {
            ...
            value if value.starts_with("--git-common-dir=") => {
                parsed.git_common_dir = Some(reunion_validate_common_dir(
                    &value["--git-common-dir=".len()..],
                )?);
            }
            value if value.starts_with('-') => {
                return Err(format!("reunion: unknown argument {value}"))
            }
            value => {
                if parsed.window.is_some() {
                    return Err(format!("reunion: unexpected argument {value}"));
                }
...
fn reunion_parse_common_dir(value: Option<&String>) -> Result<std::path::PathBuf, String> {
    let Some(value) = value else {
        return Err("reunion: missing value for --git-common-dir".to_owned());
    };
...
fn reunion_resolve_cwd(window: Option<&str>) -> Result<ReunionCwd, String> {
    if let Some(window) = window {
        return reunion_resolve_window_cwd(window);
    }
    match reunion_tmux_display(&[]) {
        Ok(raw) => Ok(ReunionCwd::Found(std::path::PathBuf::from(raw.trim()))),
        Err(_) => Ok(ReunionCwd::Skipped(
            "  \x1b[33m⚠\x1b[0m reunion: not in tmux, cannot determine cwd".to_owned(),
        )),
    }
}
    ...
    let target = tmux
        ...
        .find_map(|session| {
            session
                .windows
                .into_iter()
                .find(|item| item.name.to_ascii_lowercase() == wanted)
                .map(|item| format!("{}:{}", session.name, item.name))
        });
    let Some(target) = target else {
        return Ok(ReunionCwd::Skipped(format!(
            "  \x1b[33m⚠\x1b[0m reunion: window '{window}' not found, skipping"
        )));
    };
    reunion_validate_tmux_target(&target)?;
    match reunion_tmux_display(&["-t", &target]) {
        Ok(raw) => Ok(ReunionCwd::Found(std::path::PathBuf::from(raw.trim()))),
        Err(_) => Ok(ReunionCwd::Skipped(format!(
            "  \x1b[33m⚠\x1b[0m reunion: could not get cwd for {target}"
        ))),
    }
}
...
fn reunion_resolve_main_root(
    cwd: &std::path::Path,
    override_common: Option<&std::path::Path>,
) -> Option<std::path::PathBuf> {
    let common = if let Some(common) = override_common {
        common.to_path_buf()
    } else {
    ...
    if common.as_os_str().is_empty() || common == std::path::Path::new(".git") {
        return None;
    }
    let main_git = if common.is_absolute() {
        common
    } else {
        cwd.join(common)
    };
...
fn reunion_run_output(mut command: std::process::Command, label: &str) -> Result<String, String> {
    let output = command
        .output()
        .map_err(|error| format!("reunion: {label}: {error}"))?;
    if !output.status.success() {
        return Err(format!("reunion: {label} failed"));
    }
    Ok(String::from_utf8_lossy(&output.stdout).to_string())
}

fn reunion_sync_all(
    wt_vault: &std::path::Path,
    main_vault: &std::path::Path,
) -> Vec<(String, usize)> {
    REUNION_SYNC_DIRS
        .iter()
        .filter_map(|dir| {
...
fn reunion_walk_copy(src: &std::path::Path, dst: &std::path::Path, count: &mut usize) {
    ...
    for entry in entries.flatten() {
        ...
        if src_path.is_dir() {
            reunion_walk_copy(&src_path, &dst_path, count);
        } else if !dst_path.exists() && reunion_copy_file(&src_path, &dst_path) {
            *count += 1;
        }
    }
}
...
fn reunion_render_result(main_root: &std::path::Path, synced: &[(String, usize)]) -> String {
    let repo = main_root
        ...
        .unwrap_or("main");
    if synced.is_empty() {
        return format!("  \x1b[90m○\x1b[0m reunion: nothing new to sync to main ({repo})\n");
    }
    let parts = synced
        .iter()
        .map(|(dir, count)| format!("{count} {}", reunion_label(dir)))
        .collect::<Vec<_>>();
    format!(
        "  \x1b[32m✓\x1b[0m reunion: synced {} → {repo}/ψ/\n",
        parts.join(", ")
    )
}

fn reunion_label(dir: &str) -> &str {
...
mod reunion_tests {
    ...
    fn reunion_strings(values: &[&str]) -> Vec<String> {
        values.iter().map(|value| (*value).to_owned()).collect()
    }

    #[test]
    fn reunion_parse_accepts_window_and_common_dir() {
        let args = reunion_parse_args(&reunion_strings(&[
            "Work",
            "--git-common-dir",
            "/tmp/main/.git",
        ]))
        ...
        assert_eq!(
            args.git_common_dir.as_deref(),
            Some(std::path::Path::new("/tmp/main/.git"))
        );
    }

    #[test]
    fn reunion_parse_rejects_option_injection() {
        assert!(reunion_parse_args(&reunion_strings(&["--", "main"]))
            .unwrap_err()
            .contains("separator"));
        assert!(reunion_parse_args(&reunion_strings(&["-main"]))
            .unwrap_err()
        ...
        assert!(
            reunion_parse_args(&reunion_strings(&["--git-common-dir", "--bad"]))
                .unwrap_err()
