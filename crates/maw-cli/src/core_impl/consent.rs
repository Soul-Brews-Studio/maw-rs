const DISPATCH_142: &[DispatcherEntry] = &[DispatcherEntry {
    command: "consent",
    handler: Handler::Sync(run_consent_command_135),
}];
...
fn consent_approve_native_135(argv: &[String]) -> Result<String, String> {
    let (Some(id), Some(pin)) = (argv.get(1), argv.get(2)) else {
        return Err(format!(
            "consent approve: expected <id> <pin>\n\n{}",
            consent_help_135()
        ));
    };
    ...
    if !result.ok {
        return Err(format!(
            "consent approve: {}",
            result.error.unwrap_or_else(|| "approval failed".to_owned())
        ));
    }
    let entry = result
        .entry
        .ok_or_else(|| "consent approve: missing trust entry".to_owned())?;
    consent_append_trust_135(&entry)?;
    consent_persist_status_135(&path, &mut value, "approved")?;
    Ok(format!(
        "approved {id}: trust {} → {} action={}\n",
        entry.from,
        entry.to,
        entry.action.as_str()
    ))
...
fn consent_reject_native_135(argv: &[String]) -> Result<String, String> {
    ...
    if !result.ok {
        return Err(format!(
            "consent reject: {}",
            result.error.unwrap_or_else(|| "reject failed".to_owned())
        ));
    }
...
fn consent_load_store_135(
    id: &str,
) -> Result<(std::path::PathBuf, serde_json::Value, ConsentStore), String> {
...
fn consent_pending_from_value_135(value: &serde_json::Value) -> Result<PendingRequest, String> {
    ...
    let action = ConsentAction::parse(&action_raw)
        .ok_or_else(|| format!("consent: unknown action {action_raw}"))?;
...
fn consent_persist_status_135(
    ...
    let object = value
        .as_object_mut()
        .ok_or_else(|| "consent: pending request is not a JSON object".to_owned())?;
    object.insert(
        "status".to_owned(),
        serde_json::Value::String(status.to_owned()),
    );
    let bytes = serde_json::to_vec_pretty(value)
        .map_err(|error| format!("consent: serialize pending request: {error}"))?;
    std::fs::write(path, bytes)
        .map_err(|error| format!("consent: write {}: {error}", path.display()))
}

fn consent_append_trust_135(entry: &TrustEntry) -> Result<(), String> {
    let path = std::env::var_os("CONSENT_TRUST_FILE").map_or_else(
        || maw_state_path(&current_xdg_env(), &["trust.json"]),
    ...
    let approved_by = match entry.approved_by {
    ...
    let object = root
        .as_object_mut()
        .ok_or_else(|| "consent: trust store is not a JSON object".to_owned())?;
    let trust = object
        .entry("trust".to_owned())
        .or_insert_with(|| serde_json::json!({}));
    let trust = trust
        .as_object_mut()
        .ok_or_else(|| "consent: trust map is not a JSON object".to_owned())?;
    trust.insert(trust_key(&entry.from, &entry.to, entry.action), record);
    if let Some(parent) = path.parent() {
        std::fs::create_dir_all(parent)
            .map_err(|error| format!("consent: create {}: {error}", parent.display()))?;
    }
    let bytes = serde_json::to_vec_pretty(&root)
        .map_err(|error| format!("consent: serialize trust store: {error}"))?;
    std::fs::write(&path, bytes)
        .map_err(|error| format!("consent: write {}: {error}", path.display()))
}
...
fn consent_expect_no_extra_args_135(
    label: &str,
    argv: &[String],
    allowed: usize,
) -> Result<(), String> {
    ...
    if extra.starts_with('-') {
        Err(format!("consent {label}: unknown argument {extra}"))
    } else {
        Err(format!("consent {label}: unexpected argument {extra}"))
    }
}
...
fn consent_read_pending_135() -> Vec<ConsentPendingRow135> {
    ...
    for dir in consent_pending_dirs_135() {
        ...
        for path in paths {
            ...
            let Ok(mut row) = serde_json::from_str::<ConsentPendingRow135>(&text) else {
                continue;
            };
...
fn consent_read_trust_135() -> Vec<ConsentTrustRow135> {
    let Some(path) = consent_readable_trust_path_135() else {
        return Vec::new();
    };
    let Ok(text) = std::fs::read_to_string(path) else {
        return Vec::new();
    };
    let Ok(file) = serde_json::from_str::<ConsentTrustFile135>(&text) else {
        return Vec::new();
    };
...
fn consent_pending_dirs_135() -> Vec<std::path::PathBuf> {
    if let Some(value) = std::env::var_os("CONSENT_PENDING_DIR") {
        return vec![std::path::PathBuf::from(value)];
    }
...
fn consent_readable_trust_path_135() -> Option<std::path::PathBuf> {
    if let Some(value) = std::env::var_os("CONSENT_TRUST_FILE") {
        return Some(std::path::PathBuf::from(value)).filter(|path| path.exists());
    }
    let env = current_xdg_env();
...
fn consent_apply_expiry_135(row: &mut ConsentPendingRow135) {
    ...
    let Some(expires_ms) = consent_parse_iso_millis_135(&row.expires_at) else {
        return;
    };
    if now.as_millis() > u128::from(expires_ms) {
        "expired".clone_into(&mut row.status);
    }
}
...
fn consent_parse_iso_millis_135(value: &str) -> Option<u64> {
    ...
    let (whole_seconds, millis_raw) = second_segment
        .split_once('.')
        .unwrap_or((second_segment, "0"));
    let second = whole_seconds.parse::<u32>().ok()?;
    let millis = millis_raw
        .get(..millis_raw.len().min(3))?
        .parse::<u32>()
        .ok()?;
...
fn consent_pad_135(value: &str, width: usize) -> String {
    ...
    if chars >= width {
        ...
        format!("{value}{}", " ".repeat(width - chars))
    }
}
