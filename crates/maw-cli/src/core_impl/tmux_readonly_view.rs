const DISPATCH_260: &[DispatcherEntry] = &[DispatcherEntry {
    command: "view",
    handler: Handler::Sync(view_run_command),
}];
...
fn view_run_with_runner<R: maw_tmux::TmuxRunner>(
    ...
    if parsed.zombie_agents {
        return view_cleanup_zombie_agents(runner, parsed.yes)
            .map(|stdout| CliOutput {
                code: 0,
                stdout,
                stderr: String::new(),
            })
            .map_err(|message| command_target_error("view", &message));
    }

    let sessions = if parsed.alive.is_empty() {
        view_list_sessions(runner)
    } else {
        parsed.alive.iter().cloned().collect()
    };
    ...
    view_validate_tmux_token(&session_name, "resolved session")
        .map_err(|message| command_target_error("view", &message))?;
    ...
    view_validate_tmux_token(&view_name, "view session")
        .map_err(|message| command_target_error("view", &message))?;
    ...
    if session_name.ends_with("-view") {
        ...
        view_select_window_if_requested(
            runner,
            &mut stdout,
            &session_name,
            &session_name,
            parsed.window_hint.as_deref(),
            &windows,
        )?;
        ...
        stdout.push_str(&view_render_attach_line(
            &session_name,
            parsed.clean,
            parsed.readonly,
        ));
    ...
    if view_has_session(runner, &view_name) {
        stdout.push_str(&format!(
            "\x1b[36mreuse\x1b[0m   → {view_name} (existing grouped session — {session_name})\n"
        ));
    } else {
        view_new_grouped_session(runner, &session_name, &view_name)?;
        stdout.push_str(&format!(
            "\x1b[36mcreated\x1b[0m → {view_name} (grouped with {session_name})\n"
        ));
    }
    view_select_window_if_requested(
        runner,
...
fn view_parse_args(argv: &[String]) -> Result<ViewArgs, String> {
        ...
        match argv[index].as_str() {
            ...
            "--alive" => {
                let Some(value) = argv.get(index + 1) else {
                    return Err("view: missing --alive value".to_owned());
                };
                alive.insert(value.clone());
                index += 1;
            }
            arg if arg.starts_with("--split=") => {
                split_anchor = Some(ViewSplitAnchor::Target(arg["--split=".len()..].to_owned()));
            }
    ...
    if positional.is_empty() {
        return Err(VIEW_USAGE.to_owned());
    }
    if positional.len() > 2 {
        return Err("view: too many positional arguments".to_owned());
    }
    Ok(ViewArgs {
        agent: positional[0].clone(),
        window_hint: positional.get(1).cloned(),
        clean,
        kill,
        readonly,
        wake,
...
fn view_validate_args(args: &ViewArgs) -> Result<(), String> {
    if !args.zombie_agents {
        view_validate_user_value(&args.agent, "agent")?;
    }
    if let Some(window) = &args.window_hint {
        view_validate_user_value(window, "window")?;
    }
    if args.wake && args.no_wake {
        return Err("--wake and --no-wake cannot be combined".to_owned());
    }
    for alive in &args.alive {
        view_validate_tmux_token(alive, "alive session")?;
    }
    if let Some(ViewSplitAnchor::Target(anchor)) = &args.split_anchor {
        view_validate_user_value(anchor, "split anchor")?;
    }
    Ok(())
}
...
fn view_validate_tmux_token(value: &str, label: &str) -> Result<(), String> {
    ...
    {
        return Err(format!(
            "view {label} contains unsafe shell/tmux characters"
        ));
    }
    Ok(())
}
...
fn view_resolve_session(
    agent: &str,
    sessions: &BTreeSet<String>,
    windows: &[ViewWindowRef],
) -> Result<Option<String>, CliOutput> {
    let candidates: Vec<String> = sessions
        .iter()
        .filter(|name| !name.ends_with("-view-view"))
        .cloned()
        .collect();
    match resolve_session_target(agent, &candidates) {
        ...
        ResolveResult::None { .. } => {
            ...
            Ok(windows
                .iter()
                .find(|window| {
                    window.name.to_lowercase().contains(&agent_lower)
                        && !window.session.ends_with("-view-view")
                })
...
fn view_view_name(session_name: &str, window_hint: Option<&str>) -> String {
    ...
    match window_hint {
        Some(window) => format!("{base}-view-{window}"),
        None => format!("{base}-view"),
    }
}
...
fn view_new_grouped_session<R: maw_tmux::TmuxRunner>(
    runner: &mut R,
    parent: &str,
    name: &str,
) -> Result<(), CliOutput> {
    runner
        ...
        .map_err(|error| {
            command_target_error(
                "view",
                &format!("failed to create grouped session {name}: {}", error.message),
            )
        })?;
    runner
        .run(
...
#[allow(clippy::format_push_string)]
fn view_select_window_if_requested<R: maw_tmux::TmuxRunner>(
    runner: &mut R,
    stdout: &mut String,
    source_session: &str,
    view_name: &str,
    ...
    let Some(hint) = window_hint else {
        return Ok(());
    };
    if let Some(window) = windows.iter().find(|window| {
        window.session == source_session
            && (window.name == hint || window.name.contains(hint) || window.index == hint)
    }) {
        let target = format!("{view_name}:{}", window.index);
        view_validate_tmux_token(&target, "window target")
            .map_err(|message| command_target_error("view", &message))?;
        runner.run("select-window", &["-t".to_owned(), target]).ok();
        stdout.push_str(&format!(
            "\x1b[36mwindow\x1b[0m  → {} ({})\n",
            window.name, window.index
        ));
    } else {
        stdout.push_str(&format!(
            "\x1b[33mwarn\x1b[0m: window '{hint}' not found, using default\n"
        ));
    }
    Ok(())
}

fn view_tmux_set<R: maw_tmux::TmuxRunner>(
    runner: &mut R,
    target: &str,
    option: &str,
    value: &str,
) -> Result<(), CliOutput> {
    view_validate_tmux_token(target, "set target")
        .map_err(|message| command_target_error("view", &message))?;
    view_validate_tmux_token(option, "set option")
        .map_err(|message| command_target_error("view", &message))?;
    view_validate_tmux_token(value, "set value")
        .map_err(|message| command_target_error("view", &message))?;
    runner
        .run(
            "set",
        ...
        .map(|_| ())
        .map_err(|error| command_target_error("view", &error.message))
}

fn view_resolve_split_anchor<R: maw_tmux::TmuxRunner>(
    runner: &mut R,
    anchor: &ViewSplitAnchor,
) -> Result<Option<String>, CliOutput> {
    match anchor {
        ViewSplitAnchor::Active => Ok(None),
        ViewSplitAnchor::Target(value) if value.contains(':') => {
            view_validate_tmux_token(value, "split anchor")
                .map_err(|message| command_target_error("view", &message))?;
            Ok(Some(value.clone()))
        }
        ViewSplitAnchor::Target(value) => {
            let view_name = format!("{}-view", value.trim_end_matches("-view"));
            view_validate_tmux_token(&view_name, "split anchor view")
                .map_err(|message| command_target_error("view", &message))?;
            if !view_has_session(runner, &view_name) {
                let sessions = view_list_sessions(runner);
                let windows = view_list_windows(runner);
                let Some(parent) = view_resolve_session(value, &sessions, &windows)?
                    .filter(|name| !name.ends_with("-view"))
                else {
                    return Err(command_target_error(
                        "view",
                        &format!("--split={value}: no matching session or existing view"),
                    ));
                };
...
fn view_split_view<R: maw_tmux::TmuxRunner>(
    runner: &mut R,
    view_name: &str,
    anchor_pane: Option<&str>,
) -> Result<(), CliOutput> {
    let target = anchor_pane.unwrap_or(view_name);
    view_validate_tmux_token(target, "split target")
        .map_err(|message| command_target_error("view", &message))?;
...
fn view_attach_or_switch<R: maw_tmux::TmuxRunner>(
    runner: &mut R,
    session: &str,
    readonly: bool,
) -> Result<(), CliOutput> {
    view_validate_tmux_token(session, "attach session")
        .map_err(|message| command_target_error("view", &message))?;
...
fn view_kill_session_guarded<R: maw_tmux::TmuxRunner>(
    runner: &mut R,
    view_name: &str,
    parent_session: &str,
) -> Result<(), CliOutput> {
    view_validate_tmux_token(view_name, "kill session")
        .map_err(|message| command_target_error("view", &message))?;
    if !view_name.ends_with("-view") && !view_name.contains("-view-") {
        return Err(command_target_error(
            "view",
            &format!("refusing to kill non-view session {view_name}"),
        ));
    }
    ...
    if !fresh_sessions.contains(view_name) {
        return Err(command_target_error(
            "view",
            &format!("refusing to kill {view_name}: session disappeared before kill"),
        ));
    }
    if !parent_session.ends_with("-view") && !fresh_sessions.contains(parent_session) {
        return Err(command_target_error(
            "view",
            &format!("refusing to kill {view_name}: parent session {parent_session} is not live"),
        ));
    }
    runner
        .run("kill-session", &["-t".to_owned(), view_name.to_owned()])
        .map(|_| ())
        .map_err(|error| {
            command_target_error(
                "view",
                &format!("kill failed for {view_name}: {}", error.message),
            )
        })
}

#[allow(clippy::format_push_string)]
fn view_cleanup_zombie_agents<R: maw_tmux::TmuxRunner>(
    runner: &mut R,
    yes: bool,
    ...
    for zombie in &zombies {
        stdout.push_str(&format!(
            "  \x1b[33m{}\x1b[0m  {}  \x1b[90m(team: unknown — DELETED)\x1b[0m\n",
...
fn view_find_zombie_panes(panes: &[ViewPaneRef]) -> Vec<ViewZombiePane> {
    let safe_pane_ids: BTreeSet<String> = panes
        .iter()
        .filter(|pane| {
            view_is_fleet_or_view_target(&pane.target) || view_is_primary_oracle_pane(&pane.target)
        })
    ...
    panes
        .iter()
        .filter(|pane| {
            pane.command.contains("claude")
                && !safe_pane_ids.contains(&pane.id)
                && !view_is_fleet_or_view_target(&pane.target)
                && !view_is_primary_oracle_pane(&pane.target)
        })
        .map(|pane| ViewZombiePane {
            pane_id: pane.id.clone(),
            info: format!(
                "{}  \"{}\"",
                pane.target,
                pane.title.chars().take(50).collect::<String>()
            ),
        })
        .collect()
}
...
fn view_is_fleet_or_view_target(target: &str) -> bool {
    ...
    session == "maw-view"
        || session.ends_with("-view")
        || session
            .strip_prefix(|c: char| c.is_ascii_digit() || c == '-')
            .is_some_and(|stem| {
                !stem.is_empty() && target.starts_with(session) && !stem.contains("team")
            })
}
...
fn view_is_primary_oracle_pane(target: &str) -> bool {
    target
        ...
        .is_some_and(|(window, pane)| window == "1" && pane == "0")
}

fn view_kill_pane_guarded<R: maw_tmux::TmuxRunner>(
    runner: &mut R,
    pane_id: &str,
) -> Result<(), String> {
    ...
    if !zombies.iter().any(|zombie| zombie.pane_id == pane_id) {
        return Err(format!(
            "refusing to kill {pane_id}: pane ownership changed before kill"
        ));
    }
    runner
        .run("kill-pane", &["-t".to_owned(), pane_id.to_owned()])
        .map(|_| ())
...
fn view_stdout_is_terminal() -> bool {
    std::io::IsTerminal::is_terminal(&std::io::stdout())
}
fn view_stdin_is_terminal() -> bool {
...
mod view_tests_260 {
    ...
    impl maw_tmux::TmuxRunner for ViewFakeRunner {
        fn run(
            ...
            match subcommand {
                "list-sessions" => {
                    Ok(self.sessions.iter().cloned().collect::<Vec<_>>().join("\n") + "\n")
                }
                "list-windows" => Ok(self.windows.clone()),
                "list-panes" => Ok(self.panes.clone()),
                "has-session" => {
                    if args.get(1).is_some_and(|name| self.sessions.contains(name)) {
                        Ok(String::new())
                    } else {
                        Err(maw_tmux::TmuxError::new("missing"))
                    }
                }
                "new-session" => {
                    if let Some(pos) = args.iter().position(|arg| arg == "-s") {
                        if let Some(name) = args.get(pos + 1) {
                            self.sessions.insert(name.clone());
                        }
                    }
                    Ok(String::new())
                }
    ...
    fn view_strings(values: &[&str]) -> Vec<String> {
        values.iter().map(|value| (*value).to_owned()).collect()
    }

    #[test]
    fn view_dispatch_fragment_owns_view_in_part260() {
        assert_eq!(DISPATCH_260[0].command, "view");
    }

    #[test]
    fn view_clean_kill_relists_before_kill_session() {
        let mut runner = ViewFakeRunner {
            sessions: ["50-mawjs".to_owned()].into_iter().collect(),
            windows: "50-mawjs|||0|||main\n".to_owned(),
            ..ViewFakeRunner::default()
        };
        let out = view_run_with_runner(
            &view_strings(&["mawjs", "--clean", "--kill", "--print"]),
            &mut runner,
        )
        .unwrap();
        ...
        assert!(calls
            .windows(2)
            .any(|w| w == ["list-sessions", "kill-session"]));
        assert!(runner.calls.iter().any(|(cmd, args)| cmd == "set"
            && args
                == &vec![
                    "-t".to_owned(),
                    "mawjs-view".to_owned(),
    ...
    fn view_split_anchor_bootstraps_anchor_view_and_splits() {
        let mut runner = ViewFakeRunner {
            sessions: ["50-mawjs".to_owned(), "51-anchor".to_owned()]
                .into_iter()
                .collect(),
            windows: "50-mawjs|||0|||main\n51-anchor|||0|||main\n".to_owned(),
            ..ViewFakeRunner::default()
        ...
        let out = view_run_with_runner(
            &view_strings(&["mawjs", "--split=anchor", "--print"]),
            &mut runner,
        )
        .unwrap();
        assert_eq!(out.code, 0);
        ...
        assert!(runner
            .calls
            .iter()
            .any(|(cmd, args)| cmd == "split-window"
                && args.iter().any(|arg| arg == "anchor-view:0")));
    }

    #[test]
    fn view_zombie_agents_requires_yes_and_fresh_relist_before_kill() {
        let panes =
            "%9|||claude|||team-old:2.1|||stale\n%1|||claude|||50-mawjs:1.0|||live\n".to_owned();
        let mut runner = ViewFakeRunner {
            panes,
            ..ViewFakeRunner::default()
        };
        let preview =
            view_run_with_runner(&view_strings(&["--zombie-agents"]), &mut runner).unwrap();
        assert!(preview.stdout.contains("Run with"));
        assert!(!runner.calls.iter().any(|(cmd, _)| cmd == "kill-pane"));
        runner.calls.clear();
        let killed =
            view_run_with_runner(&view_strings(&["--zombie-agents", "--yes"]), &mut runner)
                .unwrap();
