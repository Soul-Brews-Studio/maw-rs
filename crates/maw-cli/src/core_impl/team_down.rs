...
fn team_down(argv: &[String]) -> Result<String, String> {
    ...
    Ok(team_down_render(
        &opts.team, &session, &charter, &actions, false,
    ))
}
...
fn team_down_parse(argv: &[String]) -> Result<TeamDownOptions126, String> {
    ...
    while index < argv.len() {
        match argv[index].as_str() {
            ...
            value if value.starts_with('-') => {
                return Err(format!("team down: unknown argument {value}"))
            }
    ...
    opts.team = team.ok_or_else(|| {
        "usage: maw team down <team> [--all] [--keep <a,b>] [--dry-run] [--status]".to_owned()
    })?;
...
fn team_down_panes() -> Result<Vec<TeamPane124>, String> {
    ...
    let raw = maw_tmux::TmuxRunner::run(&mut runner, "list-panes", &args)
...
fn team_down_plan(
    ...
    opts: &TeamDownOptions126,
    session: &str,
    panes: &[TeamPane124],
) -> Result<Vec<TeamDownAction126>, String> {
    let mut actions = Vec::new();
    let roster: Vec<_> = charter
        .members
        .iter()
        .map(|member| team_t3_classify(member, &TeamT3Options124::default(), session, panes))
        .collect();
    ...
    for (member, item) in charter.members.iter().zip(roster.iter()) {
        if let Some(reason) = team_down_keep_reason(member, item, &opts.keep, opts.all) {
            actions.push(TeamDownAction126 {
                role: item.role.clone(),
                state: item.state.clone(),
                action: format!("keep ({reason})"),
                target: None,
            });
            continue;
        }
        if item.state == "missing" && !opts.status && !opts.dry_run {
            return Err(format!(
                "team down refuse missing target before teardown: {session}:{}",
                item.role
            ));
        }
        if item.state != "live" {
        ...
        let target = item
            ...
            .ok_or_else(|| {
                format!(
                    "team down validation failed for {}: live member has no pane",
                    item.role
                )
            })?;
        team_down_validate_target(session, &target, panes)?;
        killable.push(target.clone());
        let action = if opts.status || opts.dry_run {
            format!("would maw done {target}")
        } else {
            format!("maw done {target}")
        };
        actions.push(TeamDownAction126 {
            role: item.role.clone(),
            state: item.state.clone(),
    ...
    if !opts.status && !opts.dry_run && team_down_would_kill_last_window(session, panes, &killable)
    {
        actions.insert(
            ...
            TeamDownAction126 {
                ...
                state: "guard".to_owned(),
                action: format!("create {TEAM_DOWN_GUARD_WINDOW}"),
                target: Some(TEAM_DOWN_GUARD_WINDOW.to_owned()),
            },
        );
    }
    Ok(actions)
...
fn team_down_matches_keep(member: &TeamCharterMember122, keep: &[String]) -> bool {
    keep.iter().any(|selector| {
        selector == &member.role
            || member.name.as_ref().is_some_and(|name| selector == name)
            || member.cwd.as_ref().is_some_and(|cwd| selector == cwd)
    })
}

fn team_down_validate_target(
    session: &str,
    target: &str,
    panes: &[TeamPane124],
) -> Result<(), String> {
    ...
    let matches = panes
        .iter()
        .filter(|pane| pane.session == session && pane.window == target)
        .count();
    match matches {
        1 => Ok(()),
        0 => Err(format!(
            "team down refuse missing target before teardown: {session}:{target}"
        )),
        _ => Err(format!(
            "team down refuse ambiguous target before teardown: {session}:{target}"
        )),
    }
}

fn team_down_would_kill_last_window(
    session: &str,
    panes: &[TeamPane124],
    killable: &[String],
) -> bool {
    let windows = panes
        .iter()
        .filter(|pane| pane.session == session && pane.window != TEAM_DOWN_GUARD_WINDOW)
        .map(|pane| pane.window.as_str())
        .collect::<std::collections::BTreeSet<_>>();
    !windows.is_empty()
        && windows
            .iter()
            .all(|window| killable.iter().any(|target| target == window))
}

fn team_down_execute(
    team: &str,
    session: &str,
    actions: &[TeamDownAction126],
) -> Result<(), String> {
    ...
    for action in actions
        .iter()
        .filter(|action| action.target.is_some() && action.role != "session")
    {
...
fn team_down_create_guard(session: &str) -> Result<(), String> {
    ...
    let args = vec![
        ...
        format!("{session}:"),
        "-n".to_owned(),
        TEAM_DOWN_GUARD_WINDOW.to_owned(),
    ];
    maw_tmux::TmuxRunner::run(&mut runner, "new-window", &args)
        .map(|_| ())
        .map_err(|error| error.message)
}
...
fn team_down_archive_before_done(team: &str, member: &str) -> Result<(), String> {
    let archive_dir = team_psi_dir()
        ...
        .join("mailbox")
        .join(member)
        .join(format!("team-{team}-archive"));
    std::fs::create_dir_all(&archive_dir)
        .map_err(|error| format!("team down archive mkdir failed: {error}"))?;
    ...
    if inbox.exists() {
        std::fs::copy(&inbox, archive_dir.join("inbox.json"))
            .map_err(|error| format!("team down archive inbox failed: {error}"))?;
    }
    ...
    if let Ok(entries) = std::fs::read_dir(&member_dir) {
        for entry in entries.flatten() {
            ...
            if name.ends_with("_findings.md") {
                std::fs::copy(&path, archive_dir.join(name))
                    .map_err(|error| format!("team down archive findings failed: {error}"))?;
            }
        }
    }
    if paths.tool_config.exists() {
        ...
        std::fs::create_dir_all(&team_archive)
            .map_err(|error| format!("team down archive team mkdir failed: {error}"))?;
        std::fs::copy(&paths.tool_config, team_archive.join("manifest.json"))
            .map_err(|error| format!("team down archive manifest failed: {error}"))?;
    }
...
fn team_down_record_fake(kind: &str, value: &str) -> Result<(), String> {
    ...
    let Some(path) = std::env::var_os("MAW_RS_TEAM_DOWN_FAKE_LOG") else {
        return Ok(());
    };
    let path = std::path::PathBuf::from(path);
    if let Some(parent) = path.parent() {
        std::fs::create_dir_all(parent)
            .map_err(|error| format!("team down fake log mkdir failed: {error}"))?;
    }
    let mut file = std::fs::OpenOptions::new()
        .create(true)
        .append(true)
        .open(&path)
        .map_err(|error| format!("team down fake log open failed: {error}"))?;
    writeln!(file, "{kind}\t{value}")
        .map_err(|error| format!("team down fake log write failed: {error}"))
}

fn team_down_render(
    team: &str,
    session: &str,
    _charter: &TeamCharter122,
    actions: &[TeamDownAction126],
    dry_run: bool,
) -> String {
