const DISPATCH_115: &[DispatcherEntry] = &[DispatcherEntry {
    command: "attach-ssh",
    handler: Handler::Sync(attachssh_run_command),
}];
...
struct AttachsshOptions {
    dry_run: bool,
    plan_json: bool,
}

#[derive(Debug, Clone, PartialEq, Eq)]
struct AttachsshStatus {
    success: bool,
    code: Option<i32>,
}
...
fn attachssh_run_command(argv: &[String]) -> CliOutput {
    attachssh_run_with_runner(argv, &mut AttachsshSystemRunner)
}

fn attachssh_run_with_runner<R: AttachsshCommandRunner>(
    argv: &[String],
    runner: &mut R,
) -> CliOutput {
    ...
    if let Err(message) = attachssh_validate_target(&target) {
    ...
    if opts.plan_json {
        return attachssh_plan_json(&target, &probe_args, &attach_args);
    }
    if opts.dry_run {
        return attachssh_dry_run(&probe_args, &attach_args);
    }
    ...
    match runner.attachssh_status("ssh", &attach_args, true) {
        ...
        Ok(status) => command_target_error(
            "attach-ssh",
            &format!(
                "ssh attach to {} ({}) failed with exit {}",
                target.node,
                target.ssh_alias,
        ...
        Err(error) => {
            command_target_error("attach-ssh", &format!("failed to execute ssh: {error}"))
        }
    }
}
        ...
        match argv[index].as_str() {
            ...
            "--node" => {
                let Some(value) = argv.get(index + 1) else {
                    return Err("attach-ssh: missing --node value".to_owned());
                };
            ...
            "--session" | "--session-name" => {
                let Some(value) = argv.get(index + 1) else {
                    return Err("attach-ssh: missing --session value".to_owned());
                };
            ...
            "--ssh-alias" => {
                let Some(value) = argv.get(index + 1) else {
                    return Err("attach-ssh: missing --ssh-alias value".to_owned());
                };
            ...
            arg if arg.starts_with("--session=") => {
                session_name = Some(arg["--session=".len()..].to_owned());
            }
            arg if arg.starts_with("--session-name=") => {
                session_name = Some(arg["--session-name=".len()..].to_owned());
            }
            arg if arg.starts_with("--ssh-alias=") => {
                ssh_alias = Some(arg["--ssh-alias=".len()..].to_owned());
            }
            arg if arg.starts_with('-') => {
                return Err(format!("attach-ssh: unknown argument {arg}"))
            }
    ...
    Ok((
        AttachsshTarget {
            ...
            session_name: session_name
                .ok_or_else(|| "attach-ssh: --session required".to_owned())?,
...
fn attachssh_usage_text() -> String {
...
fn attachssh_validate_target(target: &AttachsshTarget) -> Result<(), String> {
    ...
    if !attachssh_safe_token(&target.session_name) {
        return Err(format!(
...
fn attachssh_validate_alias(node: &str, alias: &str) -> Result<(), String> {
    ...
    if !attachssh_safe_token(alias) {
        return Err(format!(
            "cannot attach to {node}: unsafe ssh alias '{alias}'"
        ));
    }
    Ok(())
}
...
fn attachssh_probe_args(alias: &str) -> Vec<String> {
    vec![
        "-o".to_owned(),
...
fn attachssh_exec_args(alias: &str, session_name: &str) -> Vec<String> {
    vec![
        "-tt".to_owned(),
        alias.to_owned(),
...
fn attachssh_preflight<R: AttachsshCommandRunner>(
    runner: &mut R,
    alias: &str,
) -> Result<(), String> {
...
fn attachssh_plan_json(
    target: &AttachsshTarget,
    probe_args: &[String],
    attach_args: &[String],
) -> CliOutput {
...
mod attachssh_tests {
    ...
    impl AttachsshCommandRunner for AttachsshFakeRunner {
        fn attachssh_status(
            ...
            self.calls
                .push((program.to_owned(), args.to_vec(), interactive));
            Ok(AttachsshStatus {
                success: true,
                code: Some(0),
            })
    ...
    fn attachssh_strings(values: &[&str]) -> Vec<String> {
        values.iter().map(|value| (*value).to_owned()).collect()
    }

    #[test]
    fn attachssh_dispatch_fragment_owns_attach_ssh() {
        assert_eq!(DISPATCH_115[0].command, "attach-ssh");
    }

    #[test]
    fn attachssh_live_path_uses_ssh_argv_vec_without_shell() {
        let mut runner = AttachsshFakeRunner::default();
        let out = attachssh_run_with_runner(
            &attachssh_strings(&[
                "--node",
                "peer-one",
                "--session",
        ...
        assert_eq!(
            runner.calls[0],
            ("ssh".to_owned(), attachssh_probe_args("peer-one"), false)
        );
        assert_eq!(
            runner.calls[1],
            (
                "ssh".to_owned(),
                attachssh_exec_args("peer-one", "50-mawjs"),
                true
            )
        );
    }
