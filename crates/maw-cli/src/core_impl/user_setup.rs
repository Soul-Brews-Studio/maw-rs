const DISPATCH_68: &[DispatcherEntry] = &[DispatcherEntry {
    command: "user-setup",
    handler: Handler::Sync(run_usersetup_command),
}];
...
#[derive(Debug, Clone, serde::Serialize, PartialEq, Eq)]
struct UsersetupWorktreeFinding {
    detected: bool,
    #[serde(skip_serializing_if = "Option::is_none")]
    alive: Option<bool>,
    evidence: Vec<String>,
...
struct UsersetupAuditResult {
    ...
    entries: Vec<UsersetupAuditEntry>,
    warnings: Vec<String>,
}

#[derive(Debug, Clone, serde::Serialize, PartialEq, Eq)]
struct UsersetupDuplicateFinding {
    inferred: String,
    encoded: Vec<String>,
}
...
struct UsersetupPrunePlan {
    ...
    #[serde(rename = "dryRun")]
    dry_run: bool,
    #[serde(rename = "staleProjectDirs")]
    stale_project_dirs: Vec<UsersetupAuditEntry>,
    #[serde(rename = "orphanSessionFiles")]
    orphan_session_files: Vec<UsersetupFileFinding>,
    #[serde(rename = "duplicateEncodedPaths")]
    duplicate_encoded_paths: Vec<UsersetupDuplicateFinding>,
    warnings: Vec<String>,
}

#[derive(Debug, Clone, Default)]
struct UsersetupScanSummary {
    size_bytes: u64,
...
fn usersetup_run(argv: &[String]) -> Result<(i32, String), (i32, String)> {
    ...
    if opts.scope.as_deref() == Some("projects") || opts.action.as_deref() == Some("audit") {
        return usersetup_run_audit(&opts);
    }
    let plan = usersetup_plan_prune(opts.dry_run);
    if opts.json {
        return usersetup_json(&plan)
            .map(|out| (0, out))
            .map_err(|e| (1, e));
    }
    if opts.porcelain {
        return Ok((0, usersetup_render_porcelain(&plan)));
    }
...
fn usersetup_parse_args(argv: &[String]) -> Result<UsersetupOptions, (i32, String)> {
    ...
    for arg in argv {
        match arg.as_str() {
            ...
            value if value.starts_with('-') => {
                return Err((2, format!("user-setup: unknown argument {value}")))
            }
            value => {
                usersetup_validate_positional(value)?;
    ...
    match positionals.as_slice() {
        [] => Ok(opts),
        [scope, action] if scope == "projects" && action == "audit" => {
            opts.scope = Some(scope.clone());
...
fn usersetup_validate_positional(value: &str) -> Result<(), (i32, String)> {
    if value.is_empty()
        ...
        || value.contains('\\')
    {
        return Err((2, format!("user-setup: invalid argument {value}")));
    }
    Ok(())
}
    ...
    let dirents = match std::fs::read_dir(&root) {
        ...
        Err(error) => {
            return UsersetupAuditResult {
                ...
                entries,
                warnings: vec![format!(
                    "cannot read Claude projects dir {}: {error}",
                    root.display()
                )],
            }
        }
    };
    let mut dirs = dirents
        .into_iter()
        .filter(|entry| {
            entry
                .file_type()
...
fn usersetup_audit_entry(dir: &std::fs::DirEntry) -> UsersetupAuditEntry {
    ...
    if matches!(path.confidence.as_str(), "ambiguous" | "unknown") {
        warnings.push(format!("path confidence is {}", path.confidence));
    }
    if scan.session_count == 0 {
        warnings.push("no JSONL sessions found".to_owned());
    }
    let source_exists = path
        .inferred
        .as_ref()
        .map(|inferred| std::path::Path::new(inferred).exists());
    ...
    UsersetupAuditEntry {
        ...
        last_modified: scan.last_modified_ms.map(usersetup_iso_from_ms),
        source_exists,
        repo,
        worktree,
...
fn usersetup_projects_root() -> std::path::PathBuf {
    std::env::var_os("MAW_CLAUDE_PROJECTS_DIR").map_or_else(
        || usersetup_home_dir().join(".claude/projects"),
...
fn usersetup_infer_path(encoded: &str) -> UsersetupPathFinding {
    ...
    if !encoded.starts_with('-') {
        return UsersetupPathFinding {
            encoded: encoded.to_owned(),
            inferred: None,
            confidence: "unknown".to_owned(),
            evidence: vec![
                "encoded-name-does-not-look-like-absolute-claude-project-dir".to_owned(),
            ],
        };
    }
    ...
    if std::path::Path::new(&inferred).exists() {
        evidence.push("inferred-path-exists".to_owned());
    }
    if let Ok(cwd) = std::env::current_dir() {
        if usersetup_encode_project_path(&cwd) == encoded {
            evidence.push("current-working-directory-reencodes-to-project-dir".to_owned());
            return UsersetupPathFinding {
                encoded: encoded.to_owned(),
                inferred: Some(cwd.display().to_string()),
                confidence: "exact".to_owned(),
                evidence,
            };
        }
    }
    let confidence = if std::path::Path::new(&inferred).exists() {
        "probable"
    } else {
        "ambiguous"
    };
    UsersetupPathFinding {
        encoded: encoded.to_owned(),
...
fn usersetup_scan_project_dir(dir: &std::path::Path) -> UsersetupScanSummary {
    ...
    summary
        .largest_files
        .sort_by(|a, b| b.size_bytes.cmp(&a.size_bytes).then(a.name.cmp(&b.name)));
    summary.largest_files.truncate(5);
    summary
}

fn usersetup_visit_project_dir(
    root: &std::path::Path,
    path: &std::path::Path,
    summary: &mut UsersetupScanSummary,
) {
    ...
    if let Some(ms) = mtime {
        summary.last_modified_ms = Some(summary.last_modified_ms.map_or(ms, |old| old.max(ms)));
    }
    if meta.is_file() {
        usersetup_add_file(root, path, &meta, mtime, summary);
        return;
    }
    if !meta.is_dir() {
        return;
    ...
    for entry in entries {
        usersetup_visit_project_dir(root, &entry.path(), summary);
    }
}

fn usersetup_add_file(
    root: &std::path::Path,
    path: &std::path::Path,
    ...
    summary.largest_files.push(UsersetupFileFinding {
        name,
        size_bytes,
        last_modified: mtime.map(usersetup_iso_from_ms),
    });
}
...
fn usersetup_detect_repo(path: &UsersetupPathFinding) -> Option<UsersetupRepoFinding> {
    ...
    if path.confidence != "exact" && path.confidence != "probable" {
        return None;
    }
    let repo_path = std::path::Path::new(inferred);
    if !usersetup_safe_path(repo_path) || !repo_path.exists() {
        return None;
    }
...
fn usersetup_detect_worktree(
    encoded: &str,
    path: &UsersetupPathFinding,
) -> UsersetupWorktreeFinding {
    let mut evidence = Vec::new();
    let inferred = path.inferred.as_deref().unwrap_or_default();
    if encoded.contains("-wt-")
        || encoded.contains("-agents-")
        || inferred.contains("/agents/")
        || inferred.contains("worktree")
    {
        evidence.push("path-shape-suggests-worktree".to_owned());
    }
...
fn usersetup_worktree_alive(
    ...
    evidence.push(
        if listed {
            "git-worktree-list-confirms-path"
        } else {
            "git-worktree-list-did-not-include-path"
        }
        .to_owned(),
    );
    Some(listed)
...
fn usersetup_plan_prune(dry_run: bool) -> UsersetupPrunePlan {
    let audit = usersetup_audit_projects();
    let stale_project_dirs = audit
        .entries
        .iter()
        .filter(|entry| usersetup_is_safe_prune_candidate(entry))
        .cloned()
        .collect::<Vec<_>>();
    ...
    UsersetupPrunePlan {
        ...
        duplicate_encoded_paths,
        warnings: audit.warnings,
    }
}

fn usersetup_is_safe_prune_candidate(entry: &UsersetupAuditEntry) -> bool {
    usersetup_is_worktree_derived(&entry.encoded)
        && entry.session_count == 0
        && entry.source_exists == Some(false)
}

fn usersetup_is_worktree_derived(encoded: &str) -> bool {
    encoded.contains("-wt-") || encoded.contains("-agents-")
}
...
fn usersetup_root_session_files(root: &std::path::Path) -> Vec<UsersetupFileFinding> {
    ...
    for entry in entries.flatten() {
        ...
        files.push(UsersetupFileFinding {
            name: entry.file_name().to_string_lossy().to_string(),
            size_bytes: meta.as_ref().map_or(0, std::fs::Metadata::len),
            last_modified: meta
                .and_then(|m| m.modified().ok())
                .and_then(usersetup_ms_since_epoch)
                .map(usersetup_iso_from_ms),
        });
    }
...
fn usersetup_duplicate_paths(entries: &[UsersetupAuditEntry]) -> Vec<UsersetupDuplicateFinding> {
    ...
    for entry in entries {
        if let Some(inferred) = &entry.path.inferred {
            groups
                .entry(inferred.clone())
                .or_default()
                .push(entry.encoded.clone());
        }
    }
    groups
        .into_iter()
        .filter_map(|(inferred, mut encoded)| {
            encoded.sort();
            (encoded.len() > 1).then_some(UsersetupDuplicateFinding { inferred, encoded })
        })
        .collect()
}
...
fn usersetup_render_plan(plan: &UsersetupPrunePlan) -> String {
    let mut lines = vec![
        format!(
            "maw user-setup {}audit",
            if plan.dry_run { "dry-run " } else { "" }
        ),
        format!("root: {}", plan.root),
        format!("safe prune candidates: {}", plan.stale_project_dirs.len()),
        format!("orphan session files: {}", plan.orphan_session_files.len()),
        format!(
            "duplicate encoded paths: {}",
            plan.duplicate_encoded_paths.len()
        ),
    ];
...
fn usersetup_push_plan_sections(lines: &mut Vec<String>, plan: &UsersetupPrunePlan) {
    if !plan.stale_project_dirs.is_empty() {
        lines.push(String::new());
        lines.push("safe prune candidates:".to_owned());
        for entry in &plan.stale_project_dirs {
            lines.push(format!(
                "  - {}{}",
                entry.encoded,
                entry
            ...
            lines.push("    why: worktree-derived name (-wt-/-agents-), 0 JSONL sessions, decoded path missing".to_owned());
        }
    }
    if !plan.orphan_session_files.is_empty() {
        lines.push(String::new());
        lines.push("orphan session files:".to_owned());
        for file in &plan.orphan_session_files {
            lines.push(format!("  - {} ({} bytes)", file.name, file.size_bytes));
        }
    }
    if !plan.duplicate_encoded_paths.is_empty() {
        lines.push(String::new());
        lines.push("duplicate encoded paths:".to_owned());
        for group in &plan.duplicate_encoded_paths {
            lines.push(format!(
                "  - {}: {}",
                group.inferred,
                group.encoded.join(", ")
...
fn usersetup_render_porcelain(plan: &UsersetupPrunePlan) -> String {
    let mut lines = vec![
        format!("root\t{}", plan.root),
        format!("stale\t{}", plan.stale_project_dirs.len()),
        format!("orphan\t{}", plan.orphan_session_files.len()),
        format!("duplicate\t{}", plan.duplicate_encoded_paths.len()),
    ];
    for entry in &plan.stale_project_dirs {
        lines.push(format!(
            "candidate\t{}\t{}",
            entry.encoded,
            entry.path.inferred.as_deref().unwrap_or("")
        ));
    }
    lines.join("\n") + "\n"
}
...
fn usersetup_now_iso() -> String {
    std::env::var("MAW_USERSETUP_NOW").unwrap_or_else(|_| "1970-01-01T00:00:00.000Z".to_owned())
}

fn usersetup_iso_from_ms(ms: u128) -> String {
    format!("epoch-ms:{ms}")
}
...
mod usersetup_tests {
    ...
    fn usersetup_parser_rejects_leading_dash_positionals() {
        ...
        assert_eq!(
            usersetup_parse_args(&argv).unwrap_err(),
            (2, "user-setup: unknown argument -audit".to_owned())
        );
    }
