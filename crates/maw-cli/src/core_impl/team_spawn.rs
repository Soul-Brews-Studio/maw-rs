...
#[derive(Debug, Clone, Default)]
struct TeamT5SpawnFromOptions127 {
    path: String,
    approve: bool,
    exec: bool,
}
...
fn team_t5_spawn_from(argv: &[String]) -> Result<String, String> {
    ...
    let unsupported = charter
        .members
        .iter()
        .filter(|m| m.target.as_deref().unwrap_or("auto") != "auto")
        .map(|m| format!("{}={}", m.role, m.target.as_deref().unwrap_or("auto")))
        .collect::<Vec<_>>();
    if !unsupported.is_empty() {
        return Err(format!(
            "charter spawn currently supports only target:auto; blocked {}",
            unsupported.join(", ")
        ));
    }
...
fn team_t5_parse_spawn(argv: &[String]) -> Result<TeamT5SpawnOptions127, String> {
    ...
    let mut opts = TeamT5SpawnOptions127 {
        team,
        role,
        ..Default::default()
    };
    ...
    while i < argv.len() {
        match argv[i].as_str() {
            "--engine" | "-e" => {
                i += 1;
                opts.engine = Some(team_t5_safe_token(
                    team_t5_next(argv, i, "--engine")?,
                    "engine",
                )?);
            }
            "--model" => {
                i += 1;
                opts.model = Some(team_t5_safe_token(
                    team_t5_next(argv, i, "--model")?,
                    "model",
                )?);
            }
            "--cwd" | "--worktree" => {
                i += 1;
                opts.cwd = Some(team_t5_next(argv, i, "--cwd")?);
            }
            "--parent" | "--parent-session-id" => {
                i += 1;
                opts.parent_session_id = Some(team_t5_safe_token(
                    team_t5_next(argv, i, "--parent-session-id")?,
                    "parent-session-id",
                )?);
            }
            "--session-id" => {
                i += 1;
                opts.session_id = Some(team_t5_safe_token(
                    team_t5_next(argv, i, "--session-id")?,
                    "session-id",
                )?);
            }
            "--exec" => opts.exec = true,
            "--prompt" => {
                opts.prompt = Some(team_t5_collect_prompt(argv, i + 1)?);
                break;
            }
            other if other.starts_with('-') => {
                return Err(format!("team spawn: unknown argument {other}"))
            }
...
fn team_t5_parse_spawn_from(argv: &[String]) -> Result<TeamT5SpawnFromOptions127, String> {
    let path = argv
        .get(1)
        .ok_or_else(|| {
            "usage: maw team spawn-from <team.yaml|team.json> [--approve] [--exec]".to_owned()
        })?
        .clone();
    team_validate_path_arg(&path)?;
    let mut opts = TeamT5SpawnFromOptions127 {
        path,
        ..Default::default()
    };
    for arg in argv.iter().skip(2) {
        match arg.as_str() {
            ...
            "--exec" => opts.exec = true,
            other if other.starts_with('-') => {
                return Err(format!("team spawn-from: unknown argument {other}"))
            }
...
fn team_t5_spawn_one(opts: &TeamT5SpawnOptions127) -> Result<String, String> {
    ...
    if !paths.vault_manifest.exists() {
        return Err(format!(
            "team '{}' not found — run: maw team create {}",
            opts.team, opts.team
        ));
    }
    ...
    if let Some(model) = &opts.model {
        let _ = writeln!(out, "  \x1b[90mmodel: {model}\x1b[0m");
    }
    let _ = write!(
        out,
        "  \x1b[90mprompt: {}\x1b[0m\n\n",
        prompt_path.display()
    );
    ...
    if opts.exec {
        ...
        let _ = writeln!(
            out,
            "  \x1b[32m✓ --exec\x1b[0m spawned {} with controlled maw invocation",
            opts.role
        );
    } else {
        let _ = writeln!(
            out,
            "  \x1b[36mRun:\x1b[0m {}",
            team_t5_render_command(&invocation)
        );
    }
    Ok(out)
}
...
fn team_t5_next(argv: &[String], index: usize, flag: &str) -> Result<String, String> {
    argv.get(index)
        .cloned()
        .ok_or_else(|| format!("team spawn: {flag} requires a value"))
}
...
fn team_t5_collect_prompt(argv: &[String], start: usize) -> Result<String, String> {
    ...
    if prompt.chars().any(|ch| ch == '\0') {
        return Err("invalid team prompt: NUL rejected".to_owned());
    }
    if prompt.is_empty() {
        Err("team spawn: --prompt requires text".to_owned())
    } else {
        Ok(prompt)
    }
}
...
fn team_t5_safe_token(value: impl AsRef<str>, label: &str) -> Result<String, String> {
    ...
    if value.starts_with('-') {
        return Err(format!(
            "invalid team {label} '{value}': leading dash rejected"
        ));
    }
    if value.contains("..") || value.contains('/') || value.contains('\\') {
        return Err(format!(
            "invalid team {label} '{value}': path traversal rejected"
        ));
    }
    if value.chars().any(|ch| ch.is_control() || ch == '\0') {
        return Err(format!("invalid team {label}: control character rejected"));
    }
    Ok(value.to_owned())
}
...
fn team_t5_validate_work_path(path: &str) -> Result<(), String> {
    ...
    if path.split(['/', '\\']).any(|part| part == "..") {
        return Err(format!(
            "invalid worktree path {path:?}: traversal rejected"
        ));
    }
    Ok(())
}
...
fn team_t5_canonical_work_path(path: &str) -> Result<std::path::PathBuf, String> {
    ...
    let canonical = full.canonicalize().map_err(|error| {
        format!(
            "team spawn: canonicalize {} failed: {error}",
            full.display()
        )
    })?;
    let root = team_t5_repo_root(&std::env::current_dir().map_err(|error| error.to_string())?)?
        .canonicalize()
        .map_err(|error| format!("team spawn: canonicalize repo root failed: {error}"))?;
    if !canonical.starts_with(&root) {
        return Err(format!(
            "invalid worktree path {}: outside repo root {}",
            canonical.display(),
            root.display()
        ));
    }
...
fn team_t5_repo_root(start: &std::path::Path) -> Result<std::path::PathBuf, String> {
    ...
    loop {
        ...
        let Some(parent) = dir.parent() else {
            return Err("team spawn: repo root not found (.git)".to_owned());
        };
        if parent == dir {
            return Err("team spawn: repo root not found (.git)".to_owned());
        }
...
fn team_t5_spawn_prompt(team: &str, role: &str, prompt: Option<&str>, fallback: &str) -> String {
    ...
    if parts.len() == 1 && !fallback.is_empty() {
        let _ = fallback;
    }
    parts.join("\n\n")
}

fn team_t5_member_prompt(
    charter: &TeamCharter122,
    member: &TeamCharterMember122,
) -> Option<String> {
    let mut parts = Vec::new();
    if !charter.goal.trim().is_empty() {
        parts.push(format!("## Team goal\n{}", charter.goal));
    }
    if let Some(prompt) = member.prompt.as_ref().filter(|p| !p.trim().is_empty()) {
        parts.push(format!("## Role prompt\n{prompt}"));
    }
    if parts.is_empty() {
        None
    } else {
...
fn team_t5_upsert_manifest_member(
    path: &std::path::Path,
    role: &str,
    engine: Option<&str>,
) -> Result<(), String> {
    let mut value: serde_json::Value = team_read_json(path)
        .ok_or_else(|| format!("team spawn: invalid manifest {}", path.display()))?;
    let members = value["members"]
        .as_array_mut()
        .ok_or_else(|| "team spawn: manifest members must be an array".to_owned())?;
    if !members.iter().any(|item| item.as_str() == Some(role)) {
        members.push(serde_json::json!(role));
    }
    let Some(engine) = engine else {
        return team_write_json_atomic_0600(path, &value);
    };
...
fn team_t5_upsert_tool_member(
    ...
    if !config.members.iter().any(|member| member.name == role) {
        config.members.push(TeamMember122 {
            name: role.to_owned(),
            model: model.map(str::to_owned),
            backend_type: (engine != "claude").then(|| engine.to_owned()),
            ..Default::default()
        });
...
fn team_t5_controlled_maw_invocation(
    opts: &TeamT5SpawnOptions127,
    engine: &str,
    ...
    if let Some(command) = &opts.engine_command {
        args.extend(["--engine-cmd".to_owned(), command.clone()]);
    }
    if let Some(cwd) = cwd {
        args.extend(["--repo-path".to_owned(), cwd.display().to_string()]);
    }
    if let Some(id) = &opts.parent_session_id {
...
fn team_t5_spawn_controlled(args: &[String], cwd: Option<&std::path::Path>) -> Result<(), String> {
    ...
    let mut child = std::process::Command::new(team_t5_self_bin()?)
        .args(args)
        .stdin(Stdio::null())
        .stdout(Stdio::piped())
        .stderr(Stdio::piped())
        ...
        .spawn()
        .map_err(|error| format!("team spawn: controlled maw spawn failed: {error}"))?;
    let deadline = std::time::Instant::now() + std::time::Duration::from_secs(5);
    loop {
        if let Some(status) = child
            .try_wait()
            .map_err(|error| format!("team spawn: wait failed: {error}"))?
        {
            return if status.success() {
                Ok(())
            } else {
                Err(format!("team spawn: controlled maw exited with {status}"))
            };
        }
        if std::time::Instant::now() >= deadline {
            let _ = child.kill();
            return Err("team spawn: controlled maw timed out".to_owned());
        }
...
fn team_t5_self_bin() -> Result<std::path::PathBuf, String> {
    std::env::var_os("MAW_RS_SELF_BIN")
        ...
        .map_or_else(
            || {
                std::env::current_exe()
                    .map_err(|error| format!("team spawn: current_exe failed: {error}"))
            },
            Ok,
        )
}
...
fn team_t5_render_command(args: &[String]) -> String {
    let mut parts =
        vec![team_t5_self_bin().map_or_else(|_| "maw".to_owned(), |p| p.display().to_string())];
    parts.extend(args.iter().map(|arg| {
            ...
            .all(|ch| ch.is_ascii_alphanumeric() || matches!(ch, '-' | '_' | '.' | '/' | ':'))
        {
            ...
            team_t5_shell_quote(arg)
        }
    }));
    parts.join(" ")
}

fn team_t5_shell_quote(value: &str) -> String {
    format!("'{}'", value.replace('\'', "'\\''"))
}

fn team_t5_format_spawn_from(
    charter: &TeamCharter122,
    roles: &[String],
    opts: &TeamT5SpawnFromOptions127,
) -> String {
    use std::fmt::Write as _;
    let mut out = format!(
        "team charter spawn complete: {}\n\nroles ({}):\n",
        charter.name,
        roles.len()
    );
    for role in roles {
    ...
    out.push_str("\nspawn safety:\n  - preflight passed\n");
    out.push_str(if opts.approve {
        "  - governance approval flag present\n"
    } else {
        "  - no governance approval required\n"
    });
    out.push_str(if opts.exec {
        "  - --exec passed through to local cmdTeamSpawn\n"
    } else {
        "  - spawn prompts written; no tmux panes spawned without --exec\n"
    });
...
mod team_spawn_tests127 {
    ...
    fn team_spawn_without_prompt_preserves_existing_prompt_and_records_engine() {
        with_spawn_fixture("preserve", |_| {
            ...
            assert_eq!(
                std::fs::read_to_string(&prompt_path).expect("prompt"),
                "line one\nline two\n"
            );
            assert!(out.contains("engine: codex"), "{out}");
            let manifest: serde_json::Value =
                team_read_json(&paths.vault_manifest).expect("manifest");
