fn team_send(argv: &[String]) -> Result<String, String> {
    let team = argv
        .get(1)
        .ok_or_else(|| "usage: maw team send <team> <message>".to_owned())?;
...
fn team_broadcast(argv: &[String]) -> Result<String, String> {
    let team = argv
        .get(1)
        .ok_or_else(|| "usage: maw team broadcast <team> <message>".to_owned())?;
    team_validate_name(team)?;
    let message = argv
        .iter()
        .skip(2)
...
fn team_inbox(argv: &[String]) -> Result<String, String> {
    let team = argv
        .get(1)
        .ok_or_else(|| "usage: maw team inbox <team> <agent>".to_owned())?;
    let agent = argv
        .get(2)
        .ok_or_else(|| "usage: maw team inbox <team> <agent>".to_owned())?;
    team_validate_name(team)?;
...
fn team_resolve_send_mode(args: &[String], targets: &[String]) -> Result<TeamSendMode122, String> {
    if args.is_empty() {
        return Err("usage: maw team send <team> <message>".to_owned());
    }
    if args.len() == 1 {
        team_validate_message(&args[0])?;
        return Ok(TeamSendMode122::Broadcast {
    ...
    if targets.is_empty() || targets.iter().any(|target| target == &first) {
        ...
        team_validate_message(&message)?;
        return Ok(TeamSendMode122::Single {
            agent: first,
            message,
        });
    }
...
fn team_send_single(team: &str, agent: &str, message: &str) -> Result<String, String> {
    if team_read_json::<TeamConfig122>(&team_paths(team).tool_config).is_some() {
        ...
        return Ok(format!(
            "\x1b[32m✓\x1b[0m message sent to {agent} in live team '{team}'\n"
        ));
    }
    team_write_vault_message(team, agent, message)?;
    Ok(format!(
        "\x1b[32m✓\x1b[0m message written to ψ/memory/mailbox/{agent}/ (team not live)\n"
    ))
}
...
fn team_send_broadcast(team: &str, message: &str) -> Result<String, String> {
    ...
    if targets.is_empty() {
        return Err(format!(
            "no members in team '{team}' — add members before broadcasting"
        ));
    }
    let mut out = format!(
        "\x1b[36m⚡\x1b[0m broadcast to {} member(s) in team '{team}':\n",
        targets.len()
    );
    for target in &targets {
        team_send_single_quiet(team, target, message)?;
    }
    let _ = writeln!(
        out,
        "\x1b[32m✓\x1b[0m broadcast delivered to {} member(s)",
        targets.len()
    );
    Ok(out)
}
...
fn team_message_targets(team: &str) -> Vec<String> {
    ...
    if let Some(registry) = team_read_oracle_registry(team) {
        for member in registry.members {
            team_push_unique_target(&mut seen, &mut out, &member.oracle);
        }
    }
    if let Some(config) = team_read_json::<TeamConfig122>(&team_paths(team).tool_config) {
        ...
        {
            team_push_unique_target(&mut seen, &mut out, &member.name);
        }
    }
    out
}

fn team_push_unique_target(
...
fn team_render_inbox(team: &str, agent: &str) -> String {
    let live = team_paths(team)
        .tool_dir
        .join("inboxes")
        .join(format!("{agent}.json"));
    if let Some(messages) = team_read_json::<Vec<TeamInboxMessage122>>(&live) {
        return team_render_live_inbox(team, agent, &messages);
    }
...
fn team_render_live_inbox(team: &str, agent: &str, messages: &[TeamInboxMessage122]) -> String {
    use std::fmt::Write as _;
    let mut out = format!(
        "\x1b[36mℹ\x1b[0m inbox for {agent} in team '{team}' ({}):\n",
        messages.len()
    );
    for message in messages {
        let _ = writeln!(out, "  - {}: {}", message.from, message.summary);
    }
    out
}
...
fn team_render_vault_inbox(team: &str, agent: &str, dir: &std::path::Path) -> String {
    ...
    if let Ok(files) = std::fs::read_dir(dir) {
        for file in files.flatten() {
            if let Some(message) =
                team_read_json::<TeamVaultMessage122>(&file.path()).filter(|m| m.team == team)
            {
    ...
    let mut out = format!(
        "\x1b[36mℹ\x1b[0m vault inbox for {agent} from team '{team}' ({}):\n",
        entries.len()
    );
    for message in entries {
        let _ = writeln!(out, "  - {}: {}", message.from, team_summary(&message.text));
    }
    out
}
...
fn team_validate_target(target: &str) -> Result<(), String> {
    if target.is_empty() {
        return Err("team target is empty".to_owned());
    }
    if target.starts_with('-') {
        return Err(format!(
            "unsafe team target '{target}': leading dash rejected"
        ));
    }
    if target.chars().any(|ch| ch.is_control() || ch == '\0') {
        return Err("unsafe team target: control character rejected".to_owned());
    }
    Ok(())
}
...
fn team_validate_message(message: &str) -> Result<(), String> {
    if message.is_empty() {
        return Err("team message is empty".to_owned());
    }
    if message.starts_with('-') {
        return Err("unsafe team message: leading dash rejected".to_owned());
    }
    if message.chars().any(|ch| ch.is_control() || ch == '\0') {
        return Err("unsafe team message: control character rejected".to_owned());
    }
    Ok(())
}
