...
const DISPATCH_332: &[DispatcherEntry] = &[DispatcherEntry {
    command: "oracle-recruit",
    handler: Handler::Sync(run_oracle_recruit_command),
}];
...
fn oracle_recruit_parse(argv: &[String]) -> Result<(String, String, RecruitOptions), String> {
    ...
    while index < argv.len() {
        ...
        match argv[index].as_str() {
            ...
            "--pin" => {
                options.pin = Some(take_value(index)?);
                index += 1;
            }
            "--from" => {
                options.from = Some(take_value(index)?);
                index += 1;
            }
            "--now" => {
                options.now_ms = Some(
                    take_value(index)?
                        .parse()
                        .map_err(|_| "oracle recruit: --now must be unix millis".to_owned())?,
                );
                index += 1;
            }
            value if value.starts_with('-') => {
                return Err(format!(
                    "oracle recruit: unknown argument {value}\n{ORACLE_RECRUIT_USAGE}"
                ))
            }
    ...
    let (Some(fleet), Some(oracle), None) =
...
fn oracle_recruit_run(argv: &[String]) -> Result<String, String> {
    ...
    if !entries
        .iter()
        .any(|entry| fleet_roster_entry_matches(entry, &fleet))
    {
        return Err(format!(
            "oracle recruit: no squad named {fleet} — try: maw fleet create {fleet}"
        ));
    }
    let cached = locate_load_registry_cache()
        ...
        .ok_or_else(|| {
            format!(
                "oracle recruit: {oracle} not found in oracles.json cache — run maw oracle scan"
            )
        })?;
    let org_repo = Some(format!("{}/{}", cached.org, cached.repo));
    let node = cached.federation_node.clone();
    let now_ms = match options.now_ms {
        Some(value) => value,
        None => consent_now_ms_135()?,
    };
    let code = maw_auth::normalize_pair_code(&team_invite_generate_pin());
    let pin = options.pin.map_or_else(team_invite_generate_pin, |pin| {
        maw_auth::normalize_pair_code(&pin)
    });
    ...
    let result = maw_auth::request_consent_plan(
        &mut store,
        maw_auth::ConsentRequestArgs {
            from: from.clone(),
            to: oracle.clone(),
    ...
    if !result.ok {
        return Err(format!(
            ...
            result
                .error
                .unwrap_or_else(|| "consent request failed".to_owned())
        ));
    }
    let pending = store
        .read_pending(&request_id)
        .ok_or_else(|| "oracle recruit: missing pending request".to_owned())?;
    ...
    let _ = writeln!(
        out,
        "  consent: requestId={request_id} action=fleet-recruit pin={pin}"
    );
    let _ = writeln!(
        out,
        "  deliver: maw hey {deliver_target} {deliver_message:?}"
    );
    if options.dry_run {
...
fn recruit_invites_dir(env: &MawXdgEnv) -> std::path::PathBuf {
    maw_state_path(env, &["fleet-invites"])
}

fn recruit_write_invite(env: &MawXdgEnv, invite: &RecruitInvite) -> Result<(), String> {
    let dir = recruit_invites_dir(env);
    std::fs::create_dir_all(&dir)
        .map_err(|error| format!("oracle recruit: create {}: {error}", dir.display()))?;
...
fn recruit_write_pending(pending: &maw_auth::PendingRequest) -> Result<(), String> {
    let dir = consent_pending_dirs_135()
        .into_iter()
        .next()
        .ok_or_else(|| "oracle recruit: no consent-pending dir".to_owned())?;
    std::fs::create_dir_all(&dir)
        .map_err(|error| format!("oracle recruit: create {}: {error}", dir.display()))?;
    let value = serde_json::json!({
...
fn fleet_join_run(argv: &[String]) -> Result<(i32, String), String> {
        ...
        match argv[index].as_str() {
            "--code" => {
                code = Some(
                    argv.get(index + 1)
                        .ok_or_else(|| {
                            format!("fleet join: missing --code value\n{FLEET_JOIN_USAGE}")
                        })?
                        .clone(),
                );
                index += 1;
            ...
            value if value.starts_with('-') => {
                return Err(format!(
                    "fleet join: unknown argument {value}\n{FLEET_JOIN_USAGE}"
                ))
            }
            value => positional.push(value.to_owned()),
        }
    ...
    let (Some(sub), Some(fleet), None) = (positional.first(), positional.get(1), positional.get(2))
    else {
    ...
    let text = std::fs::read_to_string(&path).map_err(|_| {
        format!(
            "fleet join: unknown code {}",
            maw_auth::redact_pair_code(&code)
        )
    })?;
    let mut invite: RecruitInvite = serde_json::from_str(&text)
        .map_err(|error| format!("fleet join: parse {}: {error}", path.display()))?;
    if invite.squad != *fleet {
        return Err(format!(
            "fleet join: code is for squad {}, not {fleet}",
            invite.squad
        ));
    }
    ...
    let now_ms = u64::try_from(consent_now_ms_135()?)
        .map_err(|_| "fleet join: clock before epoch".to_owned())?;
    let expires_ms = consent_parse_iso_millis_135(&invite.expires_at)
        .ok_or_else(|| "fleet join: invite has invalid expiresAt".to_owned())?;
    if now_ms > expires_ms {
        return Err(format!("fleet join: code expired at {}", invite.expires_at));
    }
    let consent = consent_read_pending_135()
        .into_iter()
        .find(|row| row.id == invite.request_id);
    match consent.as_ref().map(|row| row.status.as_str()) {
        Some("approved") => {}
        Some(status) => {
            return Err(format!(
                "fleet join: consent {} is {status} — run maw consent approve {} <pin>",
                invite.request_id, invite.request_id
            ))
        }
        None => {
            return Err(format!(
                "fleet join: consent request {} not found",
                invite.request_id
            ))
        }
    }
    ...
    Ok((
        0,
        format!(
            "fleet join {fleet}: {} joined ({member_count} members)\n",
            invite.oracle
        ),
    ))
}

fn fleet_join_append_member(
    env: &MawXdgEnv,
    fleet: &str,
    invite: &RecruitInvite,
) -> Result<usize, String> {
    let entries = fleet_load_entries_result_for_env(env, "fleet join")?;
    let entry = entries
        .into_iter()
        .find(|entry| fleet_roster_entry_matches(entry, fleet))
        .ok_or_else(|| format!("fleet join: no squad named {fleet}"))?;
    if entry
        .session
        .members
        .as_ref()
        .is_some_and(|members| members.iter().any(|member| member.handle == invite.oracle))
    {
        return Err(format!(
            "fleet join: {} is already a member of {fleet}",
            invite.oracle
        ));
    }
    let text = std::fs::read_to_string(&entry.path)
        .map_err(|error| format!("fleet join: read {}: {error}", entry.path.display()))?;
    let mut value: serde_json::Value = serde_json::from_str(&text)
        .map_err(|error| format!("fleet join: parse {}: {error}", entry.path.display()))?;
    ...
    let object = value
        .as_object_mut()
        .ok_or_else(|| "fleet join: squad file is not a JSON object".to_owned())?;
    let members = object
        .entry("members".to_owned())
        .or_insert_with(|| serde_json::json!([]));
    let members = members
        .as_array_mut()
        .ok_or_else(|| "fleet join: members is not a JSON array".to_owned())?;
    members.push(member);
...
mod oracle_recruit_tests {
    ...
    fn recruit_args(values: &[&str]) -> Vec<String> {
        values.iter().map(|value| (*value).to_owned()).collect()
    }

    fn recruit_env(name: &str) -> (std::path::PathBuf, Vec<EnvVarRestore>) {
        let root = std::env::temp_dir().join(format!(
            "maw-rs-oracle-recruit-{name}-{}",
            std::process::id()
        ));
        let _ = std::fs::remove_dir_all(&root);
        std::fs::create_dir_all(&root).expect("temp root");
        let guards: Vec<EnvVarRestore> = [
            "HOME",
            "MAW_HOME",
            "MAW_CONFIG_DIR",
            "MAW_STATE_DIR",
        ...
        .into_iter()
        .collect();
        for key in ["MAW_HOME", "CONSENT_PENDING_DIR", "CONSENT_TRUST_FILE"] {
    ...
    fn oracle_recruit_dry_run_plan_shape_and_no_writes() {
        ...
        assert_eq!(
            plan["deliver"]["message"],
            format!("maw fleet join 3e --code {code}")
        );
        assert!(
            !root.join("state/fleet-invites").exists(),
            "dry-run writes nothing"
        );
        let missing = run_fleet_command(&recruit_args(&["join", "3e", "--code", code]));
        assert_eq!(missing.code, 1);
        assert!(
            missing.stderr.contains("unknown code"),
            "{}",
            missing.stderr
        );
    }
    ...
    fn fleet_join_round_trip_appends_member_and_consumes_code() {
        ...
        assert!(
            joined.stdout.contains("fireman joined (1 members)"),
            "{}",
            joined.stdout
        );
        let shown: serde_json::Value = serde_json::from_str(
            &run_fleet_command(&recruit_args(&["show", "3e", "--json"])).stdout,
        )
        .expect("roster json");
        ...
        assert!(
            replay.stderr.contains("already consumed"),
            "{}",
            replay.stderr
        );
    }
    ...
    fn fleet_join_rejects_expired_code() {
        ...
        assert!(
            expired.stderr.contains("code expired"),
            "{}",
            expired.stderr
        );
    }
