const DISPATCH_327: &[DispatcherEntry] = &[DispatcherEntry {
    command: "zai",
    handler: Handler::Async(zai_run_async),
}];
...
async fn zai_dispatch(args: &[String]) -> CliOutput {
    let (fleet, rest) = match zai_split_fleet(args) {
        Ok(parts) => parts,
        Err(e) => return zai_err(&e),
    };
    let fleet = zai_fleet_scope(fleet);
    match rest.first().map_or("status", String::as_str) {
        "help" | "--help" | "-h" => zai_ok(ZAI_USAGE.to_owned()),
        "status" | "ls" | "list" => match zai_load_pool(fleet.as_deref()) {
            Ok((_, keys)) => zai_ok(zai_format_status(&keys)),
            Err(e) => zai_err(&e),
        },
        "mon" => match zai_load_pool(fleet.as_deref()) {
            Ok((_, keys)) => zai_ok(zai_format_mon(&keys)),
            Err(e) => zai_err(&e),
        },
        "test" => match zai_test_pool().await {
            Ok(out) => out,
            Err(e) => zai_err(&e),
        },
        _ => zai_err(ZAI_USAGE),
...
fn zai_fleet_token(args: &[String]) -> CliOutput {
    let Some(group) = args.first().filter(|value| !value.starts_with('-')) else {
        return zai_err(ZAI_FLEET_TOKEN_USAGE);
    };
    if !zai_safe_group(group) {
        return zai_err(&format!("fleet token: invalid group name {group}"));
    }
    match args.get(1).map_or("status", String::as_str) {
        "status" | "ls" | "list" => match zai_load_pool(Some(group)) {
            Ok((_, keys)) => zai_ok(zai_format_status(&keys)),
            Err(e) => zai_err(&e),
        },
...
fn zai_split_fleet(args: &[String]) -> Result<(Option<String>, Vec<String>), String> {
    ...
    while let Some(arg) = iter.next() {
        if arg == "--fleet" {
            let value = iter
                .next()
                .ok_or_else(|| "zai: --fleet needs a group name".to_owned())?;
            if !zai_safe_group(value) {
                return Err(format!("zai: invalid fleet squad name {value}"));
            }
...
fn zai_fleet_scope(cli: Option<String>) -> Option<String> {
    cli.or_else(|| std::env::var("MAW_ZAI_POOL").ok())
        .filter(|group| zai_safe_group(group))
}
...
fn zai_pool_pointers(fleet: Option<&str>) -> Vec<String> {
    ...
    if let Some(group) = fleet.filter(|group| *group != "zai") {
        pointers.push(format!("/tokenPool/{group}"));
    }
...
fn zai_ok(stdout: String) -> CliOutput {
    CliOutput {
        code: 0,
        stdout,
        stderr: String::new(),
...
fn zai_err(message: &str) -> CliOutput {
    CliOutput {
        code: 1,
        stdout: String::new(),
...
fn zai_auth_paths() -> Vec<std::path::PathBuf> {
    if let Some(path) = std::env::var_os("MAW_ZAI_AUTH_JSON").map(std::path::PathBuf::from) {
        return vec![path];
    }
    let home = zai_home();
    vec![
...
fn zai_load_pool(fleet: Option<&str>) -> Result<(std::path::PathBuf, Vec<ZaiKey>), String> {
    ...
    for path in zai_auth_paths() {
        match zai_load_pool_at(&path, fleet) {
            Ok(keys) => return Ok((path, keys)),
            Err(e) => last = e,
        }
    }
    Err(if last.is_empty() {
        "zai: no token pool found".to_owned()
    } else {
        last
    })
}
...
fn zai_load_pool_at(path: &std::path::Path, fleet: Option<&str>) -> Result<Vec<ZaiKey>, String> {
    let raw = std::fs::read_to_string(path)
        .map_err(|_| format!("zai: no token pool at {}", path.display()))?;
    let json: serde_json::Value = serde_json::from_str(&raw)
        .map_err(|_| format!("zai: invalid json at {}", path.display()))?;
    let pointers = zai_pool_pointers(fleet);
    let pool = pointers
        .iter()
        .find_map(|pointer| json.pointer(pointer).and_then(serde_json::Value::as_array))
        .ok_or_else(|| {
            format!(
                "zai: missing tokenPool.zai or credential_pool.zai in {}",
                path.display()
            )
        })?;
    ...
    if keys.is_empty() {
        return Err("zai: credential_pool.zai is empty".to_owned());
    }
    Ok(keys)
}
...
fn zai_format_status(keys: &[ZaiKey]) -> String {
    ...
    for key in keys {
        let ready = if key.token.is_some() {
            "ready"
        } else {
            "missing-secret"
        };
        let err = if key.error.is_empty() {
            String::new()
        } else {
            format!(" — {}", zai_short(&key.error, 80))
        };
...
fn zai_format_mon(keys: &[ZaiKey]) -> String {
    ...
    let action = if ready == 0 {
        "set GLM_API_KEY or add manual access_token to ~/.hermes/auth.json"
    } else if exhausted == ready {
        "all ready keys look exhausted; wait/reset or add another key"
    } else {
        "run `maw zai test` before dispatching a large fleet"
    };
    let _ = std::fmt::Write::write_fmt(
        &mut out,
        format_args!(
...
async fn zai_test_pool() -> Result<CliOutput, String> {
    ...
    let client = reqwest::Client::builder()
        ...
        .build()
        .map_err(|e| format!("zai: http client: {e}"))?;
    let mut ok = 0usize;
...
async fn zai_probe_key(client: &reqwest::Client, key: &ZaiKey) -> (bool, String) {
    let Some(token) = key.token.as_deref() else {
        return (false, "missing secret".to_owned());
    };
    ...
    match client.post(url).bearer_auth(token).json(&body).send().await {
        Ok(resp) => {
            ...
            if resp.status().is_success() {
                (true, format!("{}ms", started.elapsed().as_millis()))
            } else {
                (
                    false,
                    format!(
                        "http {code}: {}",
                        zai_short(&resp.text().await.unwrap_or_default(), 120)
                    ),
                )
            }
        }
...
fn zai_update_statuses(path: &std::path::Path, results: &[(bool, String)]) -> Result<(), String> {
    ...
    let mut json: serde_json::Value =
        serde_json::from_str(&raw).map_err(|_| "parse failed".to_owned())?;
    let Some(pool) = json
        .pointer_mut("/credential_pool/zai")
    ...
    for (entry, (ok, msg)) in pool.iter_mut().zip(results) {
        ...
        entry["last_error_message"] = if *ok {
    ...
    std::fs::write(
        path,
        serde_json::to_string_pretty(&json).map_err(|_| "encode failed".to_owned())?,
    )
...
mod zai_tests {
    ...
    fn zai_pool_pointer_order_puts_fleet_scope_first() {
        assert_eq!(
            zai_pool_pointers(Some("3e")),
            vec!["/tokenPool/3e", "/tokenPool/zai", "/credential_pool/zai"]
        );
        assert_eq!(
            zai_pool_pointers(None),
            vec!["/tokenPool/zai", "/credential_pool/zai"]
        );
        assert_eq!(
            zai_pool_pointers(Some("zai")),
            vec!["/tokenPool/zai", "/credential_pool/zai"]
        );
    }
    ...
    fn zai_scoped_pool_reads_fleet_group_and_falls_back_to_global() {
        let dir =
            std::env::temp_dir().join(format!("maw-rs-zai-293-scoped-{}", std::process::id()));
        ...
        assert_eq!(
            zai_load_pool_at(&path, Some("missing-group"))
                .unwrap()
                .len(),
            4
        );
