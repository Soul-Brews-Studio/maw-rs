const DISPATCH_83: &[DispatcherEntry] = &[DispatcherEntry {
    command: "notify",
    handler: Handler::Async(run_notify_async),
}];

const NOTIFY_USAGE: &str =
    "usage: maw notify [--from <node:oracle>] <target> <message> [--approve] [--trust]";
...
fn notify_parse_args(argv: &[String]) -> Result<NotifyArgs, String> {
    ...
    while index < argv.len() {
        ...
        match arg.as_str() {
            ...
            "--from" => {
                index += 1;
                parsed.from = Some(notify_take_from(argv, index)?);
            }
            value if value.starts_with("--from=") => {
                parsed.from = Some(notify_validate_from(value.trim_start_matches("--from="))?);
            }
            value if value.starts_with('-') => {
                return Err(format!("notify: unknown argument {value}"))
            }
...
fn notify_finish_args(mut parsed: NotifyArgs, positional: &[String]) -> Result<NotifyArgs, String> {
    if positional.is_empty() {
        return Err("missing target and message".to_owned());
    }
    if positional.len() < 2 {
        return Err(format!("missing message for '{}'", positional[0]));
    }
...
fn notify_validate_message(value: &str) -> Result<String, String> {
    if value.trim().is_empty() || value.bytes().any(|byte| byte == 0) {
        return Err("notify: message cannot be empty".to_owned());
    }
    if value
        .bytes()
        .any(|byte| matches!(byte, 0x01..=0x08 | 0x0b | 0x0c | 0x0e..=0x1f | 0x7f))
    {
        return Err("notify: message contains control characters".to_owned());
    }
    Ok(value.to_owned())
}
...
fn notify_local_with(
    ...
    match inbox_write_file(&inbox_dir, &from, &to, &args.text, inbox_now_ms()) {
        Ok(filename) => CliOutput {
            code: 0,
            stdout: notify_success(&to, &filename, args.force),
            stderr: String::new(),
        },
        Err(message) => CliOutput {
            code: 1,
            stdout: String::new(),
            stderr: format!("notify: {message}\n"),
        },
    }
}
...
async fn notify_peer(
    ...
    let mut output = gated_send_peer_message(
        "notify",
        peer_url,
        target,
        &send_args,
    ...
    .await;
    if output.code == 0 && args.force {
        output.stderr.push_str("\x1b[90mnote: --force is not meaningful for notify (delivery is always inbox-only).\x1b[0m\n");
    }
    output
}
...
fn notify_route_error(detail: &str, hint: Option<&str>) -> CliOutput {
    CliOutput {
        ...
        stderr: if let Some(hint) = hint {
            format!("notify: {detail}; {hint}\n")
        } else {
            format!("notify: {detail}\n")
        },
    }
}
...
fn notify_inbox_to(requested: &str, resolved: &str) -> String {
    if requested.starts_with("local:") {
        requested.trim_start_matches("local:").to_owned()
    } else {
        requested.to_owned()
    }
    .chars()
    ...
    .trim()
    .to_owned()
    .notify_if_empty_then(resolved)
}

trait NotifyEmptyFallback {
    fn notify_if_empty_then(self, fallback: &str) -> String;
}
...
fn notify_success(to: &str, filename: &str, force: bool) -> String {
    ...
    if force {
        out.push_str("\x1b[90mnote: --force is not meaningful for notify (delivery is always inbox-only).\x1b[0m\n");
    }
    out
}
...
mod notify_tests {
    ...
    fn notify_strings(values: &[&str]) -> Vec<String> {
        values.iter().map(|value| (*value).to_owned()).collect()
    }

    #[test]
    fn notify_parser_accepts_flags_and_joins_message() {
        let args = notify_parse_args(&notify_strings(&[
            "--from",
            "node:oracle",
            "--approve",
            "--trust",
    ...
    fn notify_parser_rejects_option_injection_and_missing_message() {
        assert!(
            notify_parse_args(&notify_strings(&["--", "local:nova", "msg"]))
                .unwrap_err()
                .contains("-- separator")
        );
        assert!(notify_parse_args(&notify_strings(&["-target", "msg"]))
            .unwrap_err()
            .contains("unknown argument"));
        assert!(notify_parse_args(&notify_strings(&["local:nova"]))
            .unwrap_err()
            .contains("missing message"));
        assert!(
            notify_parse_args(&notify_strings(&["--from", "--bad", "local:nova", "msg"]))
                .unwrap_err()
    ...
    fn notify_local_writes_to_the_receiver_inbox_not_the_sender() {
        ...
        let receiver =
            std::env::temp_dir().join(format!("maw-rs-notify-recv-{}", std::process::id()));
