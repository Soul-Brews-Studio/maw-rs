...
fn team_task_ops241(argv: &[String]) -> Result<String, String> {
    match team_task_parse241(argv)? {
        TeamTaskOp241::Add {
            ...
            description,
            assign,
        } => team_task_add241(&team, &subject, description.as_deref(), assign.as_deref()),
...
fn team_task_parse241(argv: &[String]) -> Result<TeamTaskOp241, String> {
    ...
    match sub {
        "add" | "task" => {
            let parsed =
                team_task_parse_flags241(&argv[1..], &["--team", "--assign", "--description"])?;
            ...
            if let Some(assign) = &parsed.assign {
                team_validate_task_member241(assign)?;
            }
            if let Some(description) = &parsed.description {
                team_validate_task_text241(description, "description")?;
            }
            Ok(TeamTaskOp241::Add {
                team,
                subject,
        ...
        "tasks" => {
            ...
            let team = parsed
                ...
                .unwrap_or_else(team_task_context_team241);
            team_validate_name(&team)?;
        ...
        "done" => {
            ...
            let id = parsed
                .positionals
                .first()
                .ok_or_else(|| "usage: maw team done <task-id> [--team <name>]".to_owned())
                .and_then(|raw| team_parse_task_id241(raw))?;
            let team = parsed.team.unwrap_or_else(team_task_context_team241);
            team_validate_name(&team)?;
        ...
        "assign" => {
            ...
            let id = parsed
                ...
                .first()
                .ok_or_else(|| {
                    "usage: maw team assign <task-id> <agent> [--team <name>]".to_owned()
                })
                .and_then(|raw| team_parse_task_id241(raw))?;
            let agent = parsed
                .positionals
    ...
    while index < args.len() {
        ...
        if let Some((flag, value)) = arg.split_once('=') {
            if !allowed.contains(&flag) {
                return Err(format!("team: unknown argument {flag}"));
            }
            ...
            return Err(format!(
                "invalid team argument {arg}: leading dash rejected"
            ));
        } else {
...
fn team_task_set_flag241(
    parsed: &mut TeamTaskFlags241,
    flag: &str,
    value: &str,
) -> Result<(), String> {
    match flag {
        "--team" => {
            team_validate_name(value)?;
            parsed.team = Some(value.to_owned());
        }
        "--assign" => {
            team_validate_task_member241(value)?;
            parsed.assign = Some(value.to_owned());
        }
        "--description" => {
            team_validate_task_text241(value, "description")?;
...
fn team_task_add241(
    ...
    if let Some(description) = description {
        task.insert("description".to_owned(), serde_json::json!(description));
    }
    task.insert("status".to_owned(), serde_json::json!("pending"));
    if let Some(assign) = assign {
        task.insert("assignee".to_owned(), serde_json::json!(assign));
    }
    task.insert("createdAt".to_owned(), serde_json::json!(now));
    task.insert("updatedAt".to_owned(), serde_json::json!(now));
    team_task_write_value241(
        &team_task_path241(team, id),
...
fn team_task_list241(team: &str) -> String {
    ...
    if tasks.is_empty() {
        return format!("\x1b[36mℹ\x1b[0m no tasks for team \"{team}\"\n");
    }
    let mut out = format!(
        "\x1b[36mℹ\x1b[0m tasks for team \"{team}\" ({}):\n",
        tasks.len()
    );
    for task in tasks {
        ...
        let _ = writeln!(
            out,
            "  #{id}  [{}]  {subject}{assignee}",
            team_task_status_color241(status)
        );
    }
    out
}
...
fn team_task_done241(team: &str, id: u64) -> Result<String, String> {
    team_task_ensure_dir241(team)?;
    let Some(path) = team_task_existing_path241(team, id) else {
        return Ok(format!(
            "\x1b[33m⚠\x1b[0m task #{id} not found in team \"{team}\"\n"
        ));
    };
    let mut task = team_task_read_value241(&path)
        .ok_or_else(|| format!("\x1b[33m⚠\x1b[0m task #{id} not found in team \"{team}\""))?;
    team_task_set_field241(&mut task, "status", serde_json::json!("completed"));
    team_task_set_field241(
        &mut task,
        "updatedAt",
        serde_json::json!(team_task_timestamp241()),
    );
...
fn team_task_set_field241(task: &mut serde_json::Value, key: &str, value: serde_json::Value) {
    if !task.is_object() {
        *task = serde_json::Value::Object(serde_json::Map::new());
    }
...
fn team_task_read_all241(team: &str) -> Vec<serde_json::Value> {
    ...
    for dir in dirs.into_iter().rev() {
        ...
        for entry in entries.flatten() {
            ...
            let Some(task) = team_task_read_value241(&path) else {
                continue;
            };
            if let Some(id) = team_task_id_from_value241(&task) {
...
fn team_task_id_from_value241(task: &serde_json::Value) -> Option<u64> {
    task.get("id").and_then(serde_json::Value::as_u64)
}
...
fn team_task_next_id241(team: &str) -> Result<u64, String> {
    ...
    let read_path = if primary.exists() {
        ...
        team_task_legacy_counter_path241(team)
    };
    let mut counter = if read_path.exists() {
        team_task_read_value241(&read_path).unwrap_or_else(|| serde_json::json!({"next":1}))
    } else {
        serde_json::json!({"next":1})
    };
    let id = counter
        .get("next")
    ...
    team_task_set_field241(
        &mut counter,
        "next",
        serde_json::json!(id.saturating_add(1)),
    );
...
fn team_task_ensure_dir241(team: &str) -> Result<(), String> {
    ...
    std::fs::create_dir_all(&dir)
        .map_err(|error| format!("team task: create {} failed: {error}", dir.display()))
}
...
fn team_task_read_value241(path: &std::path::Path) -> Option<serde_json::Value> {
    std::fs::read_to_string(path)
        .ok()
        .and_then(|text| serde_json::from_str(&text).ok())
}

fn team_task_write_value241(
    path: &std::path::Path,
    value: &serde_json::Value,
) -> Result<(), String> {
    let body = serde_json::to_string_pretty(value)
        .map_err(|error| format!("team task: encode json failed: {error}"))?
        + "\n";
...
fn team_task_atomic_write_0600_241(path: &std::path::Path, body: &str) -> Result<(), String> {
    ...
    std::fs::create_dir_all(parent)
        .map_err(|error| format!("team task: create {} failed: {error}", parent.display()))?;
    ...
    let mut file = opts
        .open(&tmp)
        .map_err(|error| format!("team task: open {} failed: {error}", tmp.display()))?;
    file.write_all(body.as_bytes())
        .map_err(|error| format!("team task: write {} failed: {error}", tmp.display()))?;
    file.sync_all()
        .map_err(|error| format!("team task: sync {} failed: {error}", tmp.display()))?;
    drop(file);
    std::fs::rename(&tmp, path).map_err(|error| {
        format!(
            "team task: rename {} -> {} failed: {error}",
            tmp.display(),
            path.display()
        )
    })
...
fn team_task_dir241(team: &str) -> std::path::PathBuf {
    maw_xdg::maw_state_dir(&current_xdg_env())
        .join("teams")
        .join(team)
        .join("tasks")
}
fn team_task_legacy_dir241(team: &str) -> std::path::PathBuf {
    maw_xdg::maw_config_dir(&current_xdg_env())
        .join("teams")
        .join(team)
        .join("tasks")
}
fn team_task_counter_path241(team: &str) -> std::path::PathBuf {
    team_task_dir241(team).join("_counter.json")
}
fn team_task_legacy_counter_path241(team: &str) -> std::path::PathBuf {
    team_task_legacy_dir241(team).join("_counter.json")
}
fn team_task_path241(team: &str, id: u64) -> std::path::PathBuf {
    team_task_dir241(team).join(format!("{id}.json"))
}
fn team_task_legacy_path241(team: &str, id: u64) -> std::path::PathBuf {
    team_task_legacy_dir241(team).join(format!("{id}.json"))
}
...
fn team_parse_task_id241(raw: &str) -> Result<u64, String> {
    ...
    let id = raw
        .parse::<u64>()
        .map_err(|_| "team task id must be numeric".to_owned())?;
    if id == 0 {
        return Err("team task id must be numeric".to_owned());
    }
    Ok(id)
}
...
fn team_validate_task_member241(value: &str) -> Result<(), String> {
    ...
    if value.contains("..") || value.contains('/') || value.contains('\\') {
        return Err(format!(
            "invalid team member '{value}': path traversal rejected"
        ));
    }
    Ok(())
}
...
fn team_validate_task_text241(value: &str, label: &str) -> Result<(), String> {
    ...
    if value.starts_with('-') {
        return Err(format!("invalid team {label}: leading dash rejected"));
    }
...
fn team_task_timestamp241() -> String {
...
mod team_task_tests241 {
    ...
    fn team_task_parse_rejects_injection_values() {
        assert!(team_task_parse241(&team_task_strings241(&["add", "-bad"]))
            .expect_err("subject")
            .contains("leading dash"));
        assert!(
            team_task_parse241(&team_task_strings241(&["assign", "1", "../bad"]))
                .expect_err("member")
                .contains("path traversal")
        );
        assert!(team_task_parse241(&team_task_strings241(&["done", "-1"]))
            .expect_err("id")
            .contains("leading dash"));
    }

    fn team_task_strings241(args: &[&str]) -> Vec<String> {
