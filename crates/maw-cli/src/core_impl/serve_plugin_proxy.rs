    ...
    for route in plugin_routes {
        if route.command.is_some() {
            router = router
                ...
                .route(
                    &format!("{}/*path", route.prefix),
                    any(api_plugin_serve_proxy),
                );
        }
...
async fn api_plugin_serve_health(State(state): State<Arc<ServeState>>, uri: Uri) -> Response {
    let Some(route) = serve_plugin_route_for_path(&state, uri.path()) else {
        return (
            StatusCode::NOT_FOUND,
            Json(json!({"ok": false, "error": "plugin route not found"})),
        )
            .into_response();
    };
    if route.command.is_some() && serve_plugin_process_running(route) {
        return serve_proxy_to_plugin(route, Method::GET, &uri, HeaderMap::new(), Bytes::new())
            .await;
    }
...
async fn api_plugin_serve_proxy(
    ...
    if let Some(ws) = ws {
        ...
        return ws
            .on_upgrade(move |socket| serve_proxy_websocket(route, uri, headers, socket))
            .into_response();
    }
...
async fn api_plugin_serve_events(
    ...
    serve_plugin_route_for_path(&state, uri.path()).map_or_else(
        ...
        |route| {
            (
                StatusCode::OK,
                Json(json!({"ok": true, "plugin": route.name, "events": route.events})),
            )
        },
    )
}

fn serve_plugin_route_for_path<'a>(
    state: &'a ServeState,
    path: &str,
) -> Option<&'a ServePluginRoute> {
    state
        .plugin_serve_routes
        .iter()
...
fn serve_plugin_route_for_prefix<'a>(
    state: &'a ServeState,
    path: &str,
) -> Option<&'a ServePluginRoute> {
    state
...
fn serve_plugin_process_port(route: &ServePluginRoute) -> Result<u16, String> {
    ...
    let command = route
        .command
        .as_deref()
        .ok_or_else(|| "missing engine.serve.command".to_owned())?;
    let argv = command
        .split_whitespace()
        .map(str::to_owned)
        .collect::<Vec<_>>();
    let (program, command_args) = argv
        .split_first()
        .ok_or_else(|| "empty engine.serve.command".to_owned())?;
    let child = Command::new(program)
        .args(command_args)
        .current_dir(&route.dir)
        .env("PORT", port.to_string())
        .env("MAW_ENGINE_SERVE_PORT", port.to_string())
        .env("MAW_ENGINE_SERVE_PREFIX", &route.prefix)
        .stdin(Stdio::null())
        .stdout(Stdio::null())
        .stderr(Stdio::null())
        .spawn()
...
fn serve_allocate_loopback_port() -> Result<u16, String> {
    TcpListener::bind((std::net::Ipv4Addr::LOCALHOST, 0))
        ...
        .map(|addr| addr.port())
        .map_err(|error| format!("allocate plugin port: {error}"))
}
...
async fn serve_proxy_to_plugin(
    route: &ServePluginRoute,
    method: Method,
    uri: &Uri,
    headers: HeaderMap,
    body: Bytes,
    ...
    let port = match serve_plugin_process_port(route) {
        ...
        Err(error) => {
            return (
                ...
                Json(json!({"ok": false, "error": error, "plugin": route.name})),
            )
                .into_response()
        }
    };
    ...
    let req_method = match reqwest::Method::from_bytes(method.as_str().as_bytes()) {
        ...
        Err(error) => {
            return (
                ...
                Json(json!({"ok": false, "error": format!("bad method: {error}")})),
            )
                .into_response()
        }
    };
...
async fn serve_proxy_websocket(
    route: ServePluginRoute,
    uri: Uri,
    headers: HeaderMap,
    socket: axum::extract::ws::WebSocket,
) {
    let Ok(port) = serve_plugin_process_port(&route) else {
        return;
    };
    let target = format!(
        "ws://127.0.0.1:{port}{}",
...
fn serve_ws_to_client(
    ...
    match message {
        ...
        tokio_tungstenite::tungstenite::Message::Ping(bytes) => {
            Some(axum::extract::ws::Message::Ping(bytes))
        }
        tokio_tungstenite::tungstenite::Message::Pong(bytes) => {
            Some(axum::extract::ws::Message::Pong(bytes))
        }
