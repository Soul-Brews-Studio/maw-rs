const DISPATCH_76: &[DispatcherEntry] = &[DispatcherEntry {
    command: "panes",
    handler: Handler::Sync(run_panes_command),
}];

const PANES_USAGE: &str =
...
fn panes_run_with_runner<R: maw_tmux::TmuxRunner>(
    argv: &[String],
    runner: &mut R,
...
fn panes_parse_args(argv: &[String]) -> Result<PanesOptions, String> {
    ...
    for arg in argv {
        match arg.as_str() {
            ...
            value => {
                if target.is_some() {
                    return Err(PANES_USAGE.to_owned());
                }
...
fn panes_filter<R: maw_tmux::TmuxRunner>(
    options: &PanesOptions,
    runner: &mut R,
) -> Result<(PanesFilter, String), String> {
    if options.all {
        let warning = options.target.as_ref().map_or_else(String::new, |_| {
            "  \x1b[90m⚠ --all ignores target argument\x1b[0m\n".to_owned()
        });
        return Ok((PanesFilter::All, warning));
    }
    let Some(target) = &options.target else {
        return Ok((PanesFilter::Current, String::new()));
    };
    if let Some((session, rest)) = target.split_once(':') {
        let matched = panes_resolve_session(session, runner)?;
        panes_validate_target(rest)?;
        return Ok((
            PanesFilter::Target(format!("{matched}:{rest}")),
            String::new(),
        ));
    }
...
fn panes_resolve_session<R: maw_tmux::TmuxRunner>(
    target: &str,
    runner: &mut R,
) -> Result<String, String> {
...
fn panes_match_session(target: &str, sessions: &[String]) -> PanesResolve {
    if let Some(exact) = sessions.iter().find(|session| session.as_str() == target) {
        return PanesResolve::Match(exact.clone());
    }
    let candidates = sessions
        .iter()
        .filter(|session| session.contains(target))
        .cloned()
        .collect::<Vec<_>>();
    match candidates.len() {
        0 => PanesResolve::None {
            hints: panes_hints(target, sessions),
        },
...
fn panes_hints(target: &str, sessions: &[String]) -> Vec<String> {
    ...
    sessions
        .iter()
        .filter(|session| {
            session.to_ascii_lowercase().contains(&lower)
                || lower.contains(&session.to_ascii_lowercase())
        })
        .take(5)
        .cloned()
        .collect()
...
fn panes_missing_error(target: &str, hints: &[String]) -> String {
    ...
    if hints.is_empty() {
        out.push_str("  \x1b[90m  try: maw ls\x1b[0m\n");
    } else {
...
fn panes_fetch_rows<R: maw_tmux::TmuxRunner>(
    filter: &PanesFilter,
    pid: bool,
    runner: &mut R,
) -> Result<Vec<PanesRow>, String> {
    let args = panes_list_args(filter, pid)?;
    let raw = runner
        .run("list-panes", &args)
        .map_err(|error| format!("list-panes failed: {}", error.message))?;
    Ok(raw
        .lines()
        .filter(|line| !line.is_empty())
        .map(|line| panes_parse_row(line, pid))
        .collect())
}
...
fn panes_widths(rows: &[PanesRow], pid: bool) -> (usize, usize, usize, usize) {
    ...
    let command = rows
        ...
        .unwrap_or(0)
        .max(7);
    let pid_width = if pid {
        rows.iter()
            .map(|row| row.pid.as_deref().unwrap_or_default().len())
            .max()
            .unwrap_or(0)
            .max(3)
    } else {
...
fn panes_push_header(out: &mut String, widths: &(usize, usize, usize, usize), pid: bool) {
    ...
    if pid {
        let _ = writeln!(
            ...
            panes_pad("TARGET", target),
            panes_pad("SIZE", dims),
            panes_pad("PID", pid_width),
            panes_pad("COMMAND", command)
        );
    } else {
        let _ = writeln!(
            out,
...
fn panes_push_row(
    ...
    if pid {
        let _ = writeln!(
            out,
            "  {}  {}  {}  {}  \x1b[90m{}\x1b[0m",
            panes_pad(&row.target, target),
            panes_pad(&row.dims, dims),
            panes_pad(row.pid.as_deref().unwrap_or_default(), pid_width),
            panes_pad(&row.command, command),
            row.title
        );
    } else {
        let _ = writeln!(
...
fn panes_pad(value: &str, width: usize) -> String {
    format!("{value:<width$}")
}
...
mod panes_tests {
    ...
    fn panes_parser_accepts_flags_and_rejects_leading_dash_target() {
        let opts =
            panes_parse_args(&["alpha".to_owned(), "--pid".to_owned(), "-a".to_owned()]).unwrap();
        assert_eq!(opts.target.as_deref(), Some("alpha"));
        assert!(opts.pid);
        assert!(opts.all);
        assert!(panes_parse_args(&["-bad".to_owned()])
            .unwrap_err()
