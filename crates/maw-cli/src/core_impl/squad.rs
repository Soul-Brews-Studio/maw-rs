const DISPATCH_333: &[DispatcherEntry] = &[DispatcherEntry {
    command: "squad",
    handler: Handler::Sync(run_squad_command),
}];
...
fn squad_run(argv: &[String]) -> Result<String, String> {
    match argv.first().map(String::as_str) {
        ...
        _ => Err(
            "usage: maw squad token set <squad> <name> [--member M] | maw squad import <seed.json>"
                .to_owned(),
        ),
    }
}
        ...
        match argv[idx].as_str() {
            "--member" => {
                ...
                let Some(value) = argv.get(idx) else {
                    return Err("maw squad token set: --member requires a value".to_owned());
                };
...
fn squad_set_token(
    ...
    let text = std::fs::read_to_string(&entry.path)
        .map_err(|error| format!("squad token set: read {}: {error}", entry.path.display()))?;
    let mut value: serde_json::Value = serde_json::from_str(&text)
        .map_err(|error| format!("squad token set: parse {}: {error}", entry.path.display()))?;
    let object = value
        .as_object_mut()
        .ok_or_else(|| "squad token set: squad file is not a JSON object".to_owned())?;
    if let Some(member) = member {
        let members = object
            .get_mut("members")
            .and_then(serde_json::Value::as_array_mut)
            .ok_or_else(|| format!("squad token set: squad {group} has no members roster"))?;
        let target = members
            .iter_mut()
            .find(|item| item.get("handle").and_then(serde_json::Value::as_str) == Some(member))
            .ok_or_else(|| format!("squad token set: unknown member {member} in {group}"))?;
...
fn squad_import(argv: &[String]) -> Result<String, String> {
    let (Some(path), None) = (argv.get(1), argv.get(2)) else {
        return Err("usage: maw squad import <seed.json>".to_owned());
    };
    let text = std::fs::read_to_string(path)
        .map_err(|error| format!("squad import: read {path}: {error}"))?;
    let seed: serde_json::Value = serde_json::from_str(&text)
        .map_err(|error| format!("squad import: parse {path}: {error}"))?;
    let squads = seed
        .get("squads")
        .and_then(serde_json::Value::as_array)
        .ok_or_else(|| "squad import: seed must contain squads[]".to_owned())?;
    ...
    for squad in squads {
        let name = squad
            ...
            .ok_or_else(|| "squad import: squad missing name".to_owned())?;
        let token = squad
            .get("token")
            .and_then(serde_json::Value::as_str)
            .ok_or_else(|| format!("squad import: squad {name} missing token"))?;
        fleet_validate_session_name(name)?;
        token_validate_name("token name", token)?;
        let members = squad
            .get("members")
            .and_then(serde_json::Value::as_array)
            .ok_or_else(|| format!("squad import: squad {name} missing members[]"))?;
        let entry = if let Ok(entry) = fleet_roster_find_entry(&env, name, "import") {
            entry
        } else {
        ...
        let object = value
            .as_object_mut()
            .ok_or_else(|| "squad import: squad file is not a JSON object".to_owned())?;
...
mod squad_tests {
    ...
    fn squad_token_set_preserves_unknown_fields_and_sets_member_override() {
        ...
        assert_eq!(
            squad_run(&args(&["token", "set", "core", "duo"])).expect("set squad"),
            "squad token set core: token -> duo\n"
        );
        assert_eq!(
            squad_run(&args(&["token", "set", "core", "dd2", "--member", "atlas"]))
                .expect("set member"),
            "squad token set core: atlas -> dd2\n"
        );
        let saved: serde_json::Value =
            serde_json::from_str(&std::fs::read_to_string(dir.join("squad.json")).expect("saved"))
                .expect("json");
    ...
    fn squad_import_creates_seven_fleet_seed() {
        ...
        assert_eq!(
            squad_run(&args(&["import", &seed.display().to_string()])).expect("import"),
            "squad import: 7 squad(s) imported\n"
        );
        let core = root.join("state/fleet/squads/06-core/squad.json");
        let saved: serde_json::Value =
            serde_json::from_str(&std::fs::read_to_string(core).expect("core squad"))
                .expect("json");
        assert_eq!(saved["token"], "duo");
