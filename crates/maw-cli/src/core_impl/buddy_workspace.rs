const DISPATCH_121: &[DispatcherEntry] = &[
    DispatcherEntry {
        command: "bud",
        handler: Handler::Sync(bud_run_command),
    },
    DispatcherEntry {
        command: "buddy",
        handler: Handler::Sync(bud_run_command),
...
fn bud_run_with(
    ...
    fs: &mut impl BudFs,
    wake: &mut impl BudWakeRunner,
    http: &mut impl BudHttpClient,
) -> CliOutput {
    match bud_parse(argv).and_then(|options| bud_run_options(&options, gh, fs, wake, http)) {
        ...
        Err(message) if message == BUD_USAGE => CliOutput {
            code: 0,
            stdout: format!("{BUD_USAGE}\n"),
            stderr: String::new(),
        },
...
fn bud_parse(argv: &[String]) -> Result<BudOptions, String> {
    ...
    while index < argv.len() {
        match argv[index].as_str() {
            ...
            | "--session-id"
            | "--session") => {
                bud_assign_value(&mut options, flag, &bud_take_value(argv, &mut index, flag)?)?;
            }
...
fn bud_assign_value(options: &mut BudOptions, flag: &str, value: &str) -> Result<(), String> {
    match flag {
        ...
        "--parent" | "--parent-session-id" => {
            options.parent_session_id = Some(bud_validate_id(value, flag)?);
        }
...
fn bud_assign_slug(options: &mut BudOptions, flag: &str, value: &str) -> Result<(), String> {
    ...
    if flag == "--repo" {
        options.repo = Some(slug);
    } else {
        options.issue_repo = Some(slug);
    }
    Ok(())
}
...
fn bud_take_value(argv: &[String], index: &mut usize, flag: &str) -> Result<String, String> {
    *index += 1;
    let Some(value) = argv.get(*index) else {
        return Err(format!("bud: {flag} requires a value"));
    };
    if value.starts_with('-') {
        return Err(format!("bud: {flag} value must not start with '-'"));
    }
    Ok(value.to_owned())
}
...
fn bud_set_name(options: &mut BudOptions, value: &str) -> Result<(), String> {
    if options.name.is_some() {
        return Err(format!("bud: unexpected argument {value}"));
    }
...
fn bud_validate_options(options: BudOptions) -> Result<BudOptions, String> {
    if options.from_repo.is_some() {
        if options.stem.is_none() {
            return Err("--from-repo requires --stem <stem>".to_owned());
        }
        if options.no_suffix || options.repo_name.is_some() || options.prefix.is_some() {
            return Err(
                "bud: --no-suffix, --repo-name, and --prefix apply only to fresh-created repos"
                    .to_owned(),
            );
        }
        return Ok(options);
...
fn bud_run_options(
    ...
    if options.from_repo.is_some() {
        return bud_from_repo(options, gh, fs);
    }
    let ctx = bud_context(options, gh)?;
    if options.dry_run {
        return Ok(bud_dry_run(&ctx, options));
    }
    ...
    if options.scaffold_only {
        ...
        return Ok(format!(
            "{}{env_ready}{}",
            extraction_warning.unwrap_or_default(),
            bud_scaffold_summary(&ctx)
        ));
    }
...
fn bud_context(options: &BudOptions, gh: &mut impl BudGhGitRunner) -> Result<BudContext, String> {
    ...
    let org = options.org.clone().unwrap_or_else(|| {
        std::env::var("MAW_BUD_OWNER").unwrap_or_else(|_| "Soul-Brews-Studio".to_owned())
    });
    ...
    Ok(BudContext {
        stem,
        org,
        parent: options.from.clone().filter(|_| !options.root),
        repo_name,
        slug,
        repo_path,
    })
...
fn bud_from_repo(
    ...
    let stem = options
        .stem
        .clone()
        .ok_or_else(|| "--from-repo requires --stem <stem>".to_owned())?;
    let target = options
        .from_repo
        .clone()
        .ok_or_else(|| "--from-repo requires a target".to_owned())?;
    if bud_looks_like_url(&target) {
        return bud_from_repo_url(options, &stem, &target, gh, fs);
    }
    ...
    if !options.force && fs.bud_exists(&path.join("ψ")) {
        return Err(format!(
            "ψ/ already present at {} — pass --force to merge",
            path.display()
        ));
    }
    bud_write_from_repo_scaffold(&stem, &path, options, fs)?;
    if options.sync_peers {
        let _ = bud_sync_peers(&path, fs);
    }
...
fn bud_from_repo_url(
    ...
    if !gh.bud_run("git", &clone_args).ok {
        return Err("bud: git clone failed".to_owned());
    }
    let env_ready = bud_prepare_env(&tmp, options, gh, fs)?;
    Ok(format!(
        "{plan}  \x1b[32m✓\x1b[0m cloned → {}\n{env_ready}",
        tmp.display()
    ))
}
...
fn bud_dry_run(ctx: &BudContext, options: &BudOptions) -> String {
    ...
    let label = ctx.parent.as_deref().map_or_else(
        || ctx.stem.clone(),
        |parent| format!("{parent} → {}", ctx.stem),
    );
    let _ = writeln!(
        out,
    ...
    let _ = writeln!(
        out,
        "  \x1b[36m⬡\x1b[0m [dry-run] would create repo: {}",
        ctx.slug
    );
    if let Some(prefix) = &options.prefix {
        let _ = writeln!(
            out,
            "  \x1b[36m⬡\x1b[0m [dry-run] would run git subtree split --prefix={prefix}"
        );
        let _ = writeln!(
            out,
            "  \x1b[36m⬡\x1b[0m [dry-run] would push split history as {}/main",
            ctx.slug
        );
    }
    let _ = writeln!(
        out,
        "  \x1b[36m⬡\x1b[0m [dry-run] would init ψ/ vault at: {}",
        ctx.repo_path.display()
    );
    let _ = writeln!(out, "  \x1b[36m⬡\x1b[0m [dry-run] would generate CLAUDE.md");
    let _ = writeln!(
        out,
        "  \x1b[36m⬡\x1b[0m [dry-run] would create fleet config"
    );
    if options.scaffold_only {
        ...
        let _ = writeln!(
            out,
            "  \x1b[36m⬡\x1b[0m [dry-run] would wake {} with engine {engine}",
            ctx.stem
        );
    } else {
...
fn bud_create_repo(
    ...
    if fs.bud_exists(&ctx.repo_path) {
        return Ok(());
    }
    let view = gh.bud_run(
        "gh",
    ...
    let get = gh.bud_run(
        "ghq",
        &["get".to_owned(), format!("github.com/{}", ctx.slug)],
    );
    if !get.ok {
        return Err(format!("bud: ghq get failed: {}", get.stderr.trim()));
    }
    Ok(())
}
...
fn bud_extract_prefix(
    ...
    let cwd =
        std::env::current_dir().map_err(|error| format!("bud: current directory: {error}"))?;
    let root = gh.bud_run(
        "git",
        &[
            "-C".to_owned(),
    ...
    if !fs.bud_exists(&source.join(prefix)) {
        return Err(format!(
            "bud: --prefix subdirectory does not exist: {prefix}"
        ));
    }
    ...
    if !status.ok {
        return Err(format!(
            "bud: cannot inspect source working tree: {}",
            status.stderr.trim()
        ));
    }
    ...
    if !tracked.ok || tracked.stdout.trim().is_empty() {
        return Err(format!(
            "bud: --prefix subdirectory has no tracked files: {prefix}"
        ));
    }
    ...
    if !create.ok {
        return Err(bud_prefix_failure(
            gh,
            &source,
            &branch,
            format!("bud: gh repo create failed: {}", create.stderr.trim()),
        ));
    }
    let push = gh.bud_run(
        "git",
    ...
    if !push.ok {
        return Err(bud_prefix_failure(
            ...
            &source,
            &branch,
            format!("bud: split history push failed: {}", push.stderr.trim()),
        ));
    }
    let get = gh.bud_run(
        "ghq",
    ...
    if !get.ok {
        return Err(bud_prefix_failure(
            ...
            &source,
            &branch,
            format!("bud: ghq get failed: {}", get.stderr.trim()),
        ));
    }
...
fn bud_write_skeleton(
    ctx: &BudContext,
    options: &BudOptions,
    fs: &mut impl BudFs,
) -> Result<(), String> {
    if fs.bud_exists(&ctx.repo_path.join("CLAUDE.md")) {
        return Err(format!(
            "bud: refusing to overwrite existing oracle at {}",
            ctx.repo_path.display()
        ));
    }
    bud_create_vault(&ctx.repo_path, fs)?;
    fs.bud_write_atomic(
        &ctx.repo_path.join("CLAUDE.md"),
        bud_claude_md(ctx).as_bytes(),
    )?;
    fs.bud_write_atomic(
        &ctx.repo_path.join(".claude/settings.json"),
        bud_settings().as_bytes(),
    )?;
    if let Some(nickname) = &options.nickname {
        fs.bud_write_atomic(
            &ctx.repo_path.join("ψ/nickname"),
            format!("{nickname}\n").as_bytes(),
        )?;
    }
    if let Some(note) = &options.note {
        fs.bud_write_atomic(
            &ctx.repo_path.join("ψ/memory/birth-note.md"),
            format!("# Birth note\n\n{note}\n").as_bytes(),
        )?;
    }
...
fn bud_create_vault(root: &std::path::Path, fs: &mut impl BudFs) -> Result<(), String> {
    for dir in [
        ...
        fs.bud_create_dir_all(&root.join(dir))?;
    }
    Ok(())
}
...
fn bud_validate_session_name(value: &str) -> Result<String, String> {
    let Some((prefix, stem)) = value.split_once('-') else {
        return Err("bud: --session must look like <N>-<name>".to_owned());
    };
    ...
    let number = prefix
        .parse::<u32>()
        .map_err(|_| "bud: --session prefix must be numeric".to_owned())?;
    if !(1..=99).contains(&number) {
        return Err("bud: --session prefix must be between 1 and 99".to_owned());
    }
    let stem =
        bud_validate_oracle_stem(stem).map_err(|_| "bud: --session name is invalid".to_owned())?;
...
fn bud_finalize(
    ...
    let _ = writeln!(
        ...
        ctx.parent
            .as_deref()
            .map_or("root oracle — no parent".to_owned(), |p| format!(
                "born blank — pull memory when ready: maw soul-sync {p} --from"
            ))
    );
    ...
    if options.signal_on_birth {
        let _ = bud_birth_signal(ctx, gh, fs, &mut out);
    }
    let _ = writeln!(
        out,
...
fn bud_prepare_env(
    ...
    if options
        .engine
        .as_deref()
        .is_some_and(|engine| engine != "claude")
    {
        return Ok(String::new());
    }
    let target_envrc = target.join(".envrc");
    let existing = if fs.bud_exists(&target_envrc) {
        String::from_utf8(fs.bud_read(&target_envrc)?)
            .map_err(|_| format!("bud: {} is not UTF-8", target_envrc.display()))?
    } else {
        String::new()
    };
    if token_detect_active_token(&existing).is_none() {
        ...
        fs.bud_write_atomic(
            &target_envrc,
            token_build_envrc_content(&existing, &name, no_team).as_bytes(),
        )?;
    }
    ...
    Ok(format!(
        "  \x1b[32m✓\x1b[0m token environment allowed: {}\n",
        target.display()
    ))
}
...
fn bud_git_commit(ctx: &BudContext, gh: &mut impl BudGhGitRunner, out: &mut String) {
    ...
    let msg = ctx
        .parent
        .as_deref()
        .map_or("feat: birth — root oracle".to_owned(), |parent| {
            format!("feat: birth — budded from {parent}")
        });
    let ok =
        gh.bud_run(
            "git",
        ...
        .ok && gh
            ...
            && gh
                ...
                .ok;
    if ok {
        let _ = writeln!(out, "  \x1b[32m✓\x1b[0m initial commit pushed");
    } else {
        let _ = writeln!(
            out,
            "  \x1b[33m⚠\x1b[0m git push failed (may need manual setup)"
        );
    }
}

fn bud_update_parent_peers(
    ctx: &BudContext,
    fs: &mut impl BudFs,
    out: &mut String,
) -> Result<(), String> {
    ...
    if !fs.bud_exists(&parent_file) {
        return Ok(());
    }
    let raw = fs.bud_read(&parent_file)?;
    let mut cfg: serde_json::Value =
        serde_json::from_slice(&raw).map_err(|error| error.to_string())?;
    let peers = cfg["sync_peers"]
        .as_array_mut()
        .ok_or_else(|| "bud: invalid sync_peers".to_owned())?;
    if !peers.iter().any(|value| value.as_str() == Some(&ctx.stem)) {
        peers.push(serde_json::json!(ctx.stem));
        fs.bud_write_atomic(
            &parent_file,
            format!(
                "{}\n",
                serde_json::to_string_pretty(&cfg).map_err(|error| error.to_string())?
            )
            .as_bytes(),
        )?;
        let _ = writeln!(
            out,
            "  \x1b[32m✓\x1b[0m added {} to {parent}'s sync_peers",
            ctx.stem
        );
    }
    Ok(())
...
fn bud_wake(
    ...
    let mut args = vec![
        ...
        "--no-attach".to_owned(),
        "--repo-path".to_owned(),
        path_string(&ctx.repo_path),
    ];
    if let Some(engine) = &options.engine {
        args.extend(["-e".to_owned(), engine.clone()]);
    }
    if let Some(id) = &options.parent_session_id {
        args.extend(["--parent-session-id".to_owned(), id.clone()]);
    }
    if let Some(id) = &options.session_id {
        args.extend(["--session-id".to_owned(), id.clone()]);
    }
    match wake.bud_wake(&args) {
        Ok(_) => {
            let _ = writeln!(out, "  \x1b[32m✓\x1b[0m {} is alive", ctx.stem);
        }
        Err(error) => {
            let _ = writeln!(out, "  \x1b[33m⚠\x1b[0m wake failed: {}", error.trim());
        }
    }
    if options.split && std::env::var_os("TMUX").is_some() {
        let _ = wake.bud_split(&ctx.stem);
    }
}
...
fn bud_birth_signal(
    ...
    let path = bud_repos_root(gh).join(&ctx.org).join(format!(
        "{parent}-oracle/ψ/memory/signals/{}_{}_birth.json",
        bud_date(),
        ctx.stem
    ));
    ...
    let _ = writeln!(
        out,
        "  \x1b[36m⬡\x1b[0m signal dropped → {parent}'s ψ/memory/signals/"
    );
    Ok(())
}
...
fn bud_from_repo_plan(stem: &str, path: &std::path::Path, options: &BudOptions) -> String {
    ...
    if !options.track_vault {
        let _ = writeln!(out, "  append .gitignore ψ/");
    }
    if options.sync_peers {
        let _ = writeln!(out, "  copy peers.json snapshot (redacted output)");
    }
    out
}

fn bud_write_from_repo_scaffold(
    stem: &str,
    path: &std::path::Path,
    options: &BudOptions,
    fs: &mut impl BudFs,
    ...
    if !options.track_vault {
        fs.bud_append_atomic(&path.join(".gitignore"), "ψ/\n")?;
    }
    Ok(())
}
...
fn bud_sync_peers(target: &std::path::Path, fs: &mut impl BudFs) -> Result<(), String> {
    ...
    if !fs.bud_exists(&src) {
        return Ok(());
    }
    let dst = target.join("ψ/peers.json");
    if let Some(parent) = dst.parent() {
        fs.bud_create_dir_all(parent)?;
    }
...
fn bud_validate_oracle_stem(value: &str) -> Result<String, String> {
    ...
    {
        return Err(format!("invalid oracle name: {value:?}"));
    }
    Ok(value.to_owned())
}

fn bud_validate_stem_like(value: &str, label: &str) -> Result<String, String> {
    bud_validate_oracle_stem(value).map_err(|_| format!("bud: invalid {label} value"))
}
...
fn bud_validate_org(value: &str) -> Result<String, String> {
    ...
    {
        return Err(format!("bud: invalid org {value:?}"));
    }
    Ok(value.to_owned())
}

fn bud_validate_repo_name(value: &str) -> Result<(), String> {
    let stem = value
        .strip_suffix("-oracle")
        .ok_or_else(|| "bud: repo must end with -oracle".to_owned())?;
    bud_validate_oracle_stem(stem).map(|_| ())
}
...
fn bud_validate_slug(value: &str, label: &str) -> Result<String, String> {
    let Some((org, repo)) = value.split_once('/') else {
        return Err(format!("bud: {label} must be owner/repo"));
    };
...
fn bud_validate_issue(value: &str) -> Result<u32, String> {
    value
        .parse::<u32>()
        .ok()
        .filter(|n| *n > 0)
        .ok_or_else(|| "bud: --issue must be a positive integer".to_owned())
}
...
fn bud_validate_text(value: &str, label: &str) -> Result<String, String> {
    ...
    {
        Err(format!("bud: invalid {label} value"))
    } else {
        Ok(value.to_owned())
    }
}
...
fn bud_flag_like(value: &str) -> String {
    format!("\"{value}\" looks like a flag, not an oracle name.\n  {BUD_USAGE}")
}
...
fn bud_looks_like_url(value: &str) -> bool {
    value.starts_with("http://")
        || value.starts_with("https://")
        || value.starts_with("git@")
        || (!value.starts_with('/') && value.matches('/').count() == 1)
}
fn bud_claude_md(ctx: &BudContext) -> String {
...
fn bud_now_iso() -> String {
    format!("epoch-seconds:{}", bud_epoch_seconds())
}
fn bud_date() -> String {
    "1970-01-01".to_owned()
}
fn bud_epoch_seconds() -> u64 {
    SystemTime::now()
...
mod bud_tests {
        ...
        fn bud_run(&mut self, program: &str, args: &[String]) -> BudCmdOutput {
            ...
            if program == "ghq" && args.len() == 1 && args[0] == "root" {
                ...
                return BudCmdOutput {
                    ok,
                    stdout: if ok {
                        self.ghq_root.clone().unwrap_or_default()
                    } else {
                        String::new()
                    },
                    stderr: if ok {
            ...
            let ok = if program == "gh" && args.get(1).is_some_and(|arg| arg == "view") {
                self.ok_view
            } else {
    ...
    impl BudFs for FakeFs {
        ...
        fn bud_copy_atomic(
            ...
            let data = self.bud_read(from)?;
            self.bud_write_atomic(to, &data)
        }
        fn bud_archive_existing(
            &mut self,
            path: &std::path::Path,
        ) -> Result<Option<std::path::PathBuf>, String> {
            if self.bud_exists(path) {
                let archive = path.with_extension("json.archive-test");
    ...
    impl BudWakeRunner for FakeWake {
        fn bud_wake(&mut self, args: &[String]) -> Result<String, String> {
            self.calls.push(args.to_vec());
            Ok(String::new())
        }
        fn bud_split(&mut self, stem: &str) -> Result<String, String> {
            self.split.push(stem.to_owned());
            Ok(String::new())
        }
    }
    #[derive(Default)]
    ...
    impl BudHttpClient for FakeHttp {
        fn bud_reload(&mut self, port: u16) -> Result<(), String> {
    ...
    fn bud_args(values: &[&str]) -> Vec<String> {
        values.iter().map(|value| (*value).to_owned()).collect()
    }

    #[test]
    fn bud_dispatch_has_alias() {
        assert_eq!(
            DISPATCH_121
                .iter()
    ...
    fn bud_dry_run_matches_golden_without_ref() {
        ...
        let out = bud_run_with(
            &bud_args(&["sprout", "--from", "nova", "--dry-run"]),
            &mut gh,
            &mut fs,
            &mut wake,
            &mut http,
        ...
        assert_eq!(
            out.stdout,
            include_str!("../../tests/fixtures/native-bud/bud-dry-run.stdout")
        );
    ...
    fn bud_prefix_parser_and_dry_run_render_history_plan_without_writes() {
        ...
        let options =
            bud_parse(&bud_args(&["sprout", "--prefix", "packages/ui"])).expect("prefix parse");
        ...
        assert!(output
            .stdout
            .contains("git subtree split --prefix=packages/ui"));
        assert!(output
            .stdout
            .contains("Soul-Brews-Studio/sprout-oracle/main"));
        assert!(
            gh.calls.is_empty(),
            "dry-run must not run git or gh: {:?}",
            gh.calls
        );
    }
    ...
    fn bud_prefix_guards_missing_subdir_and_dirty_source_before_split() {
        ...
        assert!(dirty_gh
            .calls
            .iter()
            .all(|(_, args)| !args.contains(&"subtree".to_owned())));
    }
    ...
    fn bud_gh_is_idempotent_and_argv_only() {
        ...
        let ctx = bud_context(
            &bud_parse(&bud_args(&["sprout", "--org", "Soul-Brews-Studio"])).unwrap(),
            &mut gh,
        )
        .unwrap();
        bud_create_repo(&ctx, &mut gh, &fs).unwrap();
        assert!(gh.calls.iter().any(|(p, a)| p == "gh"
            && a == &[
                "repo",
                "view",
                "Soul-Brews-Studio/sprout-oracle",
                "--json",
                "name"
            ]
            .map(str::to_owned)));
        assert!(!gh
            .calls
            .iter()
            .any(|(_, a)| a.iter().any(|arg| arg.contains(';'))));
        assert!(!gh
            .calls
            .iter()
            .any(|(_, a)| a.get(1).is_some_and(|arg| arg == "create")));
    }
    ...
    fn bud_session_default_scans_lowest_free_prefix() {
        ...
        assert_eq!(
            bud_resolve_session_name(dir, "sprout", None, &fs).unwrap(),
            "03-sprout"
        );
    }
    ...
    fn bud_session_override_validates_and_normalizes_prefix() {
        ...
        let fs = FakeFs::default();
        assert_eq!(
            bud_resolve_session_name(
                std::path::Path::new("/fleet"),
                "sprout",
        ...
        assert!(bud_parse(&bud_args(&["sprout", "--session", "custom"]))
            .unwrap_err()
            .contains("<N>-<name>"));
    }
    ...
    fn bud_scaffold_only_skips_git_and_wake() {
        ...
        let mut http = FakeHttp::default();
        fs.files.insert(
            std::env::current_dir().unwrap().join(".envrc"),
            b"export CLAUDE_TOKEN_NAME=\"duo\"\n".to_vec(),
        );
        let out = bud_run_with(
            &bud_args(&["sprout", "--org", "Org", "--scaffold-only"]),
            &mut gh,
            &mut fs,
            &mut wake,
            &mut http,
        ...
        assert_eq!(out.code, 0);
        assert!(out.stdout.contains("Scaffold complete"));
        assert!(!gh.calls.iter().any(|(p, _)| p == "git"));
        assert!(wake.calls.is_empty());
        assert_eq!(http.ports, [3456]);
    }
    ...
    fn bud_flow_bootstraps_env_and_allows_before_wake() {
        ...
        assert_eq!(out.code, 0);
        assert!(String::from_utf8(fs.files[&repo.join(".envrc")].clone())
            .unwrap()
            .contains("CLAUDE_TOKEN_NAME=\"duo\""));
        assert!(gh.calls.iter().any(|(program, args)| program == "direnv"
            && args == &["allow".to_owned(), path_string(repo)]));
    ...
    fn bud_flow_hard_fails_direnv_before_wake() {
        ...
        std::env::set_var("GHQ_ROOT", "/repos");
        let mut gh = FakeGh {
            fail: ["direnv allow".to_owned()].into(),
            ..Default::default()
        };
        let mut fs = FakeFs::default();
        let mut wake = FakeWake::default();
        ...
        assert!(out
            .stderr
            .contains("run: cd /repos/github.com/Org/sprout-oracle && direnv allow ."));
        assert!(wake.calls.is_empty());
    }
    ...
    fn bud_sync_peers_archives_before_overwrite_and_redacts() {
        ...
        fs.files.insert(
            src,
            br#"{"peers":{"a":{"pubkey":"SECRET-PUBKEY"}}}"#.to_vec(),
        );
        fs.files.insert(dst.clone(), b"old".to_vec());
        bud_sync_peers(std::path::Path::new("/tmp/target"), &mut fs).unwrap();
        assert_eq!(fs.archives.len(), 1);
        assert_eq!(
            fs.files.get(&dst).unwrap(),
            br#"{"peers":{"a":{"pubkey":"SECRET-PUBKEY"}}}"#
        );
    }
    ...
    fn bud_repos_root_uses_ghq_root_before_home_fallback() {
        ...
        assert_eq!(
            ctx.repo_path,
            std::path::PathBuf::from("/opt/Code/github.com/Org/sprout-oracle")
        );
        assert!(gh
            .calls
            .iter()
    ...
    fn bud_repos_root_env_wins_over_ghq_root_command() {
        ...
        let mut gh = FakeGh {
            ghq_root: Some("/opt/Code".to_owned()),
            ..Default::default()
        };
        let options = bud_parse(&bud_args(&["sprout", "--org", "Org"])).unwrap();
        let ctx = bud_context(&options, &mut gh).unwrap();
        ...
        assert_eq!(
            ctx.repo_path,
            std::path::PathBuf::from("/env/Code/github.com/Org/sprout-oracle")
        );
        assert!(!gh
            .calls
            .iter()
    ...
    fn bud_repos_root_falls_back_to_home_when_ghq_root_fails() {
        ...
        assert_eq!(
            ctx.repo_path,
            std::path::PathBuf::from("/home/fallback/Code/github.com/Org/sprout-oracle")
        );
    }
    ...
    fn bud_wake_uses_self_argv_shape() {
        let mut wake = FakeWake::default();
        let ctx = BudContext {
            stem: "sprout".to_owned(),
            org: "Org".to_owned(),
            parent: None,
            repo_name: "sprout-oracle".to_owned(),
            slug: "Org/sprout-oracle".to_owned(),
            repo_path: "/tmp/sprout".into(),
        };
        bud_wake(&ctx, &BudOptions::default(), &mut wake, &mut String::new());
        assert_eq!(wake.calls[0][0], "wake");
        assert!(wake.calls[0].contains(&"--repo-path".to_owned()));
