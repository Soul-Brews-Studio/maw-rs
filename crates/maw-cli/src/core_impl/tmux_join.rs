const DISPATCH_264: &[DispatcherEntry] = &[DispatcherEntry {
    command: "join",
    handler: Handler::Sync(join_run_command),
}];
...
fn join_with_runner<R: maw_tmux::TmuxRunner>(
    argv: &[String],
    runner: &mut R,
    ...
    runner
        .run("join-pane", &tmux_args)
        .map_err(|error| (1, format!("join: join-pane failed: {}", error.message)))?;
...
fn join_parse(argv: &[String]) -> Result<JoinOptions, (i32, String)> {
    ...
    while let Some(arg) = iter.next() {
        match arg.as_str() {
            ...
            "--to" => {
                target = Some(
                    iter.next()
                        .ok_or_else(|| (2, "join: --to requires a target".to_owned()))?
                        .clone(),
                );
            }
            value if value.starts_with("--to=") => target = Some(value[5..].to_owned()),
            value if value.starts_with('-') => {
                return Err((2, format!("join: unknown argument {value}")))
            }
            value => {
                if source.is_some() {
                    return Err((2, "join: source already provided".to_owned()));
                }
    ...
    Ok(JoinOptions {
        source: source.ok_or_else(|| (2, JOIN_USAGE.to_owned()))?,
        target: target.ok_or_else(|| (2, "join: --to <session:window> is required".to_owned()))?,
        vertical,
    })
}

fn join_resolve_source<R: maw_tmux::TmuxRunner>(
    runner: &mut R,
    source: &str,
    ...
    if source.starts_with('%') || source.contains(':') {
        ...
        resolve_local_tmux_runner_target(runner, source, "join")
    }
}
...
fn join_validate_target(value: &str, label: &str) -> Result<(), String> {
    if value.is_empty() || value.trim() != value || value == "--" || value.starts_with('-') {
        return Err(format!(
            "join: {label} must be non-empty, unpadded, not '--', and not start with '-'"
        ));
    }
    if value
        .chars()
    ...
    {
        return Err(format!(
            "join: {label} must not contain whitespace, NUL, or control characters"
        ));
    }
...
mod tmux_join_tests {
    ...
    #[test]
    fn join_dispatches_and_resolves_oracle_window_to_join_pane() {
        assert_eq!(DISPATCH_264[0].command, "join");
        let mut runner = JoinFakeRunner {
            windows: "team|||3|||coder|||1|||/tmp\n".to_owned(),
            ..JoinFakeRunner::default()
        };
        assert_eq!(
            join_with_runner(&strings(&["coder", "--to", "lead:main"]), &mut runner).expect("join"),
            "joined team:3 → lead:main\n"
        );
        assert_eq!(
            runner.calls[1],
            (
                "join-pane".to_owned(),
                strings(&["-s", "team:3", "-t", "lead:main", "-h"])
            )
        );
    }
    #[test]
    fn join_direct_pane_skips_resolution_vertical_and_guards_injection() {
        ...
        assert_eq!(
            ...
            vec![(
                "join-pane".to_owned(),
                strings(&["-s", "%42", "-t", "lead:main", "-v"])
            )]
        );
        let mut runner = JoinFakeRunner::default();
        assert_eq!(
            join_with_runner(&strings(&["bad;pane", "--to", "lead:main"]), &mut runner)
                .expect_err("guard")
                .0,
            1
        );
