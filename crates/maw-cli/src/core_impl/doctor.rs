const DISPATCH_65: &[DispatcherEntry] = &[DispatcherEntry {
    command: "doctor",
    handler: Handler::Sync(run_doctor_command),
}];
...
fn doctor_run(argv: &[String]) -> Result<(i32, String), String> {
    ...
    if let Some(forward) = doctor_forward_check(&options, &checks, &comparison) {
        checks.push(forward);
    }
    let stdout = if options.json {
        doctor_render_json(&checks, ok, &comparison)?
    } else {
        doctor_render_text(&checks, ok, &comparison)
    };
...
fn doctor_parse_args(argv: &[String]) -> Result<DoctorOptions, String> {
    ...
    while let Some(arg) = argv.get(index) {
        if arg == "--" {
            doctor_push_tail(argv, index + 1, &mut positionals)?;
            break;
        }
        if let Some(consumed) = doctor_parse_value_arg(argv, index, &mut options)? {
            index += consumed;
            continue;
        }
        if doctor_parse_bool_arg(arg, &mut options) {
            index += 1;
            continue;
        }
        if arg.starts_with('-') {
            return Err(format!("doctor: unknown argument {arg}"));
        }
...
fn doctor_parse_value_arg(
    argv: &[String],
    index: usize,
    options: &mut DoctorOptions,
) -> Result<Option<usize>, String> {
    let arg = &argv[index];
    let consumed = match arg.as_str() {
        "--forward" => {
            options.forward = Some(doctor_take_value(
                argv,
                index,
                "--forward",
                doctor_validate_target,
            )?);
            2
        }
        "--gateway" => {
            options.gateway = Some(doctor_take_value(
                argv,
                index,
                "--gateway",
                doctor_validate_gateway,
            )?);
            2
        }
        "--manifest-path" => {
            options.manifest_path = Some(doctor_take_value(
                argv,
                index,
                "--manifest-path",
                doctor_validate_path_arg,
            )?);
            2
        }
        "--port" => {
            options.port = Some(doctor_take_port(argv, index, "--port")?);
            2
        }
        "--backend" => {
            options.backend = Some(doctor_take_port(argv, index, "--backend")?);
            2
        }
...
fn doctor_parse_equals_arg(
    arg: &str,
    options: &mut DoctorOptions,
) -> Result<Option<usize>, String> {
    if let Some(value) = arg.strip_prefix("--forward=") {
        doctor_validate_target(value, "--forward")?;
        options.forward = Some(value.to_owned());
        return Ok(Some(1));
    }
    if let Some(value) = arg.strip_prefix("--gateway=") {
        doctor_validate_gateway(value, "--gateway")?;
        options.gateway = Some(value.to_owned());
        return Ok(Some(1));
    }
    if let Some(value) = arg.strip_prefix("--manifest-path=") {
        doctor_validate_path_arg(value, "--manifest-path")?;
        options.manifest_path = Some(value.to_owned());
        return Ok(Some(1));
    }
    if let Some(value) = arg.strip_prefix("--port=") {
        options.port = Some(doctor_parse_port(value, "--port")?);
        return Ok(Some(1));
    }
    if let Some(value) = arg.strip_prefix("--backend=") {
        options.backend = Some(doctor_parse_port(value, "--backend")?);
        return Ok(Some(1));
    }
    Ok(None)
}
...
fn doctor_finalize_options(
    mut options: DoctorOptions,
    positionals: &[String],
) -> Result<DoctorOptions, String> {
    if positionals.len() > 1 {
        return Err("doctor: expected at most one check name".to_owned());
    }
    if let Some(only) = positionals.first() {
        options.only = Some(only.clone());
    }
    if options.version && options.only.is_none() {
        options.only = Some("version".to_owned());
    }
    if options.smoke && options.only.is_none() {
        options.only = Some("smoke".to_owned());
    }
    Ok(options)
}

fn doctor_take_value(
    argv: &[String],
    index: usize,
    flag: &str,
    ...
    let value = argv
        .get(index + 1)
        .ok_or_else(|| format!("doctor: missing value for {flag}"))?;
    validate(value, flag)?;
    Ok(value.clone())
}
...
fn doctor_parse_port(value: &str, flag: &str) -> Result<u16, String> {
    ...
    let port = value
        .parse::<u16>()
        .map_err(|_| format!("doctor: {flag} must be a tcp port"))?;
    if port == 0 {
        return Err(format!("doctor: {flag} must be a tcp port"));
    }
    Ok(port)
}
...
fn doctor_validate_arg_value(value: &str, label: &str) -> Result<(), String> {
    if value.is_empty() {
        return Err(format!("doctor: empty value for {label}"));
    }
    if value.starts_with('-') {
        return Err(format!("doctor: {label} value must not start with '-'"));
    }
    if value.bytes().any(|byte| matches!(byte, 0 | b'\n' | b'\r')) {
        return Err(format!("doctor: invalid control character in {label}"));
    }
    Ok(())
}

fn doctor_validate_target(value: &str, flag: &str) -> Result<(), String> {
    doctor_validate_arg_value(value, flag)
}

fn doctor_validate_gateway(value: &str, flag: &str) -> Result<(), String> {
    doctor_validate_arg_value(value, flag)?;
    if matches!(value, "bun" | "rust") {
        Ok(())
    } else {
        Err(format!("doctor: {flag} must be bun or rust"))
    }
}

fn doctor_validate_path_arg(value: &str, flag: &str) -> Result<(), String> {
    doctor_validate_arg_value(value, flag)
}
...
fn doctor_collect_checks(options: &DoctorOptions) -> Vec<DoctorCheckNative> {
    ...
    if only == Some("smoke") {
        return doctor_smoke_checks();
    }
    if doctor_should_run(only, &["serve"]) {
        checks.push(doctor_check_serve(options));
    }
    if doctor_should_run(only, &["gateway", "all"]) {
        checks.push(doctor_check_gateway(options));
    }
    if doctor_should_run(only, &["install", "all"]) {
        checks.push(doctor_check_install(options));
    }
    if only.is_some_and(|value| matches!(value, "xdg" | "all")) {
        checks.push(doctor_check_xdg(options));
    }
    if doctor_should_run(only, &["version", "all"]) {
        checks.push(doctor_check_version(options));
    }
    ...
    if doctor_should_run(only, &["peers", "all"]) {
        checks.push(doctor_check_peer_duplicates());
        checks.push(doctor_check_stale_peers());
    }
    if doctor_should_run(only, &["hub", "all"]) {
        checks.push(doctor_check_hub());
    }
    if doctor_should_run(only, &["hub", "all"]) {
        if let Some(check) = doctor_check_config_shadow() {
            checks.push(check);
        }
        if let Some(check) = doctor_check_config_backup_loss() {
            checks.push(check);
        }
    }
    if doctor_should_run(only, &["scout"]) {
        checks.push(doctor_ok("scout", "scout check is native-noop on maw-rs"));
    }
    if doctor_should_run(only, &["federation"]) {
        checks.push(doctor_ok(
            "federation",
            "federation reachability skipped without configured peers",
        ));
    }
    if doctor_should_run(only, &["disk"]) {
        checks.push(doctor_check_disk());
    }
    if doctor_should_run(only, &["manifest", "all"]) {
        checks.push(doctor_check_manifest(options));
    }
    if doctor_should_run(only, &["maw-js", "all"]) {
        checks.push(doctor_check_maw_js());
    }
    if doctor_should_run(only, &["worktrees", "all"]) {
        checks.push(doctor_check_worktrees());
    }
    checks
}
...
fn doctor_check_serve(options: &DoctorOptions) -> DoctorCheckNative {
    ...
    if doctor_tcp_reachable(port) {
        return doctor_ok("serve", &format!("running on :{port}"));
    }
    doctor_warn(
        "serve",
        &format!("not reachable on :{port}"),
        &["maw serve"],
    )
}
...
fn doctor_check_gateway(options: &DoctorOptions) -> DoctorCheckNative {
    ...
    if selected != "rust" {
        return doctor_info(
            "gateway",
            &format!("gateway {selected} selected — rust probe skipped"),
        );
    }
    let binary = doctor_find_on_path("maw-gateway").or_else(|| doctor_find_on_path("maw-rs"));
    match binary {
        Some(path) => doctor_info(
            "gateway:rust",
            &format!("binary found at {}", path.display()),
        ),
        None => doctor_error(
            "gateway:rust",
            "binary not found on PATH — build maw gateway",
            &["cargo build --release"],
        ),
    }
}
...
fn doctor_check_install(options: &DoctorOptions) -> DoctorCheckNative {
    ...
    match exe {
        Some(path) => doctor_info(
            "install",
            &format!("maw-rs executable present ({mode}) at {}", path.display()),
        ),
        None => doctor_error(
            "install",
...
fn doctor_check_xdg(options: &DoctorOptions) -> DoctorCheckNative {
    if options.migrate || options.fix_xdg {
        return doctor_migrate_xdg(options.dry_run || options.plan);
    }
    ...
    doctor_info(
        "xdg",
        &format!(
            "enabled={enabled}; config={}; data={}; state={}",
            config.display(),
            data.display(),
            state.display()
        ),
...
fn doctor_check_version(_options: &DoctorOptions) -> DoctorCheckNative {
    doctor_info(
        "version:source",
        &format!("maw-rs {MAW_RS_BUILD_VERSION} (no running maw probe in native doctor)"),
    )
}
...
fn doctor_check_plugins() -> DoctorCheckNative {
    ...
    match entries {
        Ok(values) => doctor_info(
            "plugins",
            &format!("{} loaded (broken symlink scan native)", values.len()),
        ),
...
fn doctor_check_wasm_host() -> DoctorCheckNative {
    ...
    if wasm_plugins.is_empty() {
        return doctor_info(
            "plugins:wasm-host",
            "no installed wasm plugins require the wasm-host runtime",
        );
    }
    ...
    if cfg!(feature = "wasm-host") {
        return doctor_ok(
            "plugins:wasm-host",
            &format!(
                "wasm-host runtime compiled in ({count} wasm plugin{})",
                doctor_plural(count)
            ),
        );
    }
...
fn doctor_check_plugin_stale_artifacts() -> DoctorCheckNative {
    ...
    if issues.is_empty() {
        return doctor_info(
            ...
            "no stale or provenance-mismatched plugin artifacts detected",
        );
    }
    doctor_warn(
        "plugins:stale-artifacts",
...
fn doctor_stale_plugin_artifacts(dir: &std::path::Path) -> Vec<String> {
    ...
    for entry in entries.flatten() {
        let path = entry.path();
        let Ok(meta) = doctor_read_json(&path.join("registry.meta.json")) else {
            continue;
        };
        let name = meta
            .get("name")
        ...
        let manifest_sha = doctor_read_json(&path.join("plugin.json"))
            .ok()
            .and_then(|manifest| {
                manifest
                    .get("artifact")
        ...
        if let (Some(lock), Some(manifest_sha)) = (
            read_plugin_lock_entry_full(&name).ok().flatten(),
            &manifest_sha,
        ) {
...
fn doctor_check_plugin_sdk_floor() -> DoctorCheckNative {
    ...
    let report = maw_plugin_manifest::discover_packages(
        &maw_plugin_manifest::DiscoverPackagesOptions::default(),
    );
    ...
    if refusals.is_empty() {
        return doctor_info(
            "plugins:sdk-floor",
            &format!(
                "all installed plugins accept host ABI {abi} ({} host fns)",
                maw_plugin_manifest::HOST_FN_NAMES.len()
            ),
        );
    }
    let names = refusals
...
fn doctor_check_peer_duplicates() -> DoctorCheckNative {
    ...
    if duplicates.is_empty() {
        return doctor_info(
            "peers:duplicates",
            &format!(
                "no <oracle>:<node> collisions across {total} peer{}",
                doctor_plural(total)
            ),
        );
    }
...
fn doctor_check_hub() -> DoctorCheckNative {
    ...
    let sources = maw_xdg::discover_config_layers(
        &env,
    ...
    if let Some(source) = sources.first() {
        doctor_info(
            "hub",
            &format!("config readable at {}", source.path.display()),
        )
    } else if cfg.exists() {
        ...
        doctor_warn(
            "hub",
            &format!("config missing at {}", cfg.display()),
            &["maw init"],
        )
    }
}
...
fn doctor_check_config_backup_loss() -> Option<DoctorCheckNative> {
    ...
    for entry in entries.flatten() {
        ...
        let Some((current_name, _)) = name.split_once(".bak-pre-") else {
            continue;
        };
        if current_name != "maw.config.json" && !doctor_is_weighted_config_name(current_name) {
            continue;
        }
        let Ok(backup) = doctor_read_json(&entry.path()) else {
            continue;
        };
        let current_path = config_dir.join(current_name);
        let Ok(current) = doctor_read_json(&current_path) else {
            continue;
        };
    ...
    (!losses.is_empty()).then(|| {
        doctor_warn(
            ...
            &format!(
                "backup contains config keys absent from current file: {}",
                losses.into_iter().collect::<Vec<_>>().join("; ")
            ),
...
fn doctor_collect_missing_json_paths(
    ...
    if let Some(object) = backup.as_object() {
        for (key, value) in object {
            ...
            doctor_collect_missing_json_paths(
                &path,
                value,
                current.and_then(|item| item.get(key)),
                missing,
            );
        }
...
fn doctor_has_weighted_config_layer(config_dir: &std::path::Path) -> bool {
    ...
    entries.flatten().any(|entry| {
        entry
            .file_name()
            .to_str()
            .is_some_and(doctor_is_weighted_config_name)
    })
}
...
fn doctor_check_disk() -> DoctorCheckNative {
    ...
    if dir.exists() {
        doctor_info("disk", &format!("state dir reachable at {}", dir.display()))
    } else {
        doctor_warn(
            "disk",
            &format!("state dir missing at {}", dir.display()),
            &["maw init"],
        )
    }
}
...
fn doctor_check_manifest(options: &DoctorOptions) -> DoctorCheckNative {
    let path = options.manifest_path.as_ref().map_or_else(
        || maw_config_path(&doctor_xdg_env(), &["oracles.json"]),
    ...
    if !path.exists() {
        return doctor_info(
            ...
            &format!(
                "manifest unreadable ({}) — skipping cross-source check",
                path.display()
            ),
        );
    }
    match std::fs::read_to_string(&path).and_then(|raw| {
        ...
        Ok(_) => doctor_info(
            "manifest:cross-source",
            "manifest parsed; no native cross-source gaps detected",
        ),
        Err(error) => doctor_warn(
            "manifest:cross-source",
            &format!("manifest unreadable ({error}) — skipping cross-source check"),
            &[],
        ),
    }
}
...
fn doctor_check_maw_js() -> DoctorCheckNative {
    ...
    let path = ghq
        .join("github.com")
        .join("Soul-Brews-Studio")
        .join("maw-js");
    if path.join("package.json").exists() {
        doctor_info("maw-js", &format!("checkout present at {}", path.display()))
    } else {
        doctor_warn("maw-js", "maw-js checkout not found under GHQ_ROOT", &[])
    }
}
...
fn doctor_check_worktrees() -> DoctorCheckNative {
    ...
    doctor_info(
        "worktrees:stillborn",
        &format!(
            "scanned {count} candidate worktree dir{}",
            doctor_plural(count)
        ),
    )
}
...
fn doctor_fix_stale(options: &DoctorOptions) -> Result<(i32, String), String> {
    let result = if options.dry_run || options.plan {
        maw_peer::stale_peer_check(&doctor_peer_env(), doctor_now_ms())
    } else {
        maw_peer::remove_stale_peers(&doctor_peer_env(), doctor_now_ms())
    ...
    let stdout = if options.json {
        doctor_render_json(&checks, ok, &[])?
    } else {
        doctor_render_text(&checks, ok, &[])
    };
    Ok((i32::from(!ok), stdout))
}
...
fn doctor_fix_sessions(options: &DoctorOptions) -> Result<(i32, String), String> {
    ...
    let check = if dry {
        doctor_info(
            "sessions:fix-doubled",
            "dry-run: no doubled github.com/github.com dirs scanned by native doctor",
        )
    } else {
        doctor_warn(
            "sessions:fix-doubled",
            "native doctor refuses automatic session rewrites; rerun maw-js doctor if needed",
            &[],
        )
    };
    let ok = check.ok;
...
fn doctor_migrate_xdg(dry_run: bool) -> DoctorCheckNative {
    ...
    if errors == 0 {
        doctor_info("xdg:migrate", &message)
    } else {
        doctor_error("xdg:migrate", &message, &[])
    }
}
...
fn doctor_copy_xdg_item(
    ...
    if source.is_dir() {
        doctor_copy_dir(source, destination).map_or("error", |()| "copied")
    } else {
        std::fs::copy(source, destination).map_or("error", |_| "copied")
    }
}
...
fn doctor_copy_dir(source: &std::path::Path, destination: &std::path::Path) -> std::io::Result<()> {
    ...
    for entry in std::fs::read_dir(source)? {
        ...
        if entry.file_type()?.is_dir() {
            doctor_copy_dir(&entry.path(), &target)?;
        } else {
...
fn doctor_forward_check(
    options: &DoctorOptions,
    checks: &[DoctorCheckNative],
    comparison: &[DoctorComparisonNative],
) -> Option<DoctorCheckNative> {
    let target = options.forward.as_ref()?;
    let message = doctor_format_forward_message(checks, comparison, options.capture);
    if message.is_empty() {
        return Some(doctor_info(
            "forward",
            &format!("no doctor issues to forward to {target}"),
        ));
    }
    Some(doctor_warn(
        "forward",
        &format!("native doctor prepared report for {target}; send manually"),
        &["maw hey <target> '<doctor report>'"],
    ))
}

fn doctor_format_forward_message(
    checks: &[DoctorCheckNative],
    comparison: &[DoctorComparisonNative],
    capture: bool,
) -> String {
    ...
    let mut lines = vec![format!(
        "maw doctor report: {} issue{}",
        issues.len(),
        doctor_plural(issues.len())
    )];
    lines.extend(
        issues
            .iter()
        ...
        .take(6)
    {
        lines.push(format!(
            "- {}: {} -> {} ({})",
            item.name, item.before, item.after, item.status
        ));
    }
    if capture {
        lines.push("capture: native doctor capture disabled for hermetic safety".to_owned());
    }
    lines.join("\n")
}

fn doctor_render_text(
    checks: &[DoctorCheckNative],
    ok: bool,
    comparison: &[DoctorComparisonNative],
) -> String {
    ...
    for check in checks {
        doctor_write_check(&mut out, check);
    }
    ...
    if remaining > 0 {
        let _ = writeln!(
            out,
            "\n  {remaining} issue{} remaining. Run suggested commands above to resolve.",
            doctor_plural(remaining)
        );
    }
    let _ = writeln!(out);
    out
...
fn doctor_write_comparison(out: &mut String, comparison: &[DoctorComparisonNative]) {
    let changed: Vec<&DoctorComparisonNative> = comparison
        .iter()
        .filter(|item| item.status != "unchanged")
        .take(12)
        .collect();
...
fn doctor_render_json(
    ...
    serde_json::to_string_pretty(
        &serde_json::json!({"ok": ok, "checks": checks_json, "comparison": comparison_json}),
    )
    .map(|mut value| {
        value.push('\n');
        value
...
fn doctor_load_last_run() -> Option<BTreeMap<String, String>> {
    ...
    Some(
        checks
            .iter()
            .map(|(key, value)| (key.clone(), value.as_str().unwrap_or("issue").to_owned()))
            .collect(),
    )
}
...
fn doctor_compare_runs(
    previous: Option<&BTreeMap<String, String>>,
    checks: &[DoctorCheckNative],
) -> Vec<DoctorComparisonNative> {
    let current: BTreeMap<String, String> = checks
        .iter()
        .map(|check| (check.name.clone(), doctor_status(check)))
        .collect();
    ...
    names
        .into_iter()
        .map(|name| doctor_compare_one(&name, previous, &current))
        .collect()
}

fn doctor_compare_one(
    name: &str,
    previous: &BTreeMap<String, String>,
    current: &BTreeMap<String, String>,
) -> DoctorComparisonNative {
    let before = previous
        .get(name)
        .cloned()
...
fn doctor_last_path() -> std::path::PathBuf {
    maw_state_path(&doctor_xdg_env(), &["doctor-last.json"])
}

fn doctor_from_peer_check(result: &maw_peer::PeerDoctorCheck, fix: &str) -> DoctorCheckNative {
    if result.ok {
        doctor_info(&result.name, &result.message)
    } else {
        doctor_warn(&result.name, &result.message, &[fix])
    }
}

fn doctor_info(name: &str, message: &str) -> DoctorCheckNative {
    DoctorCheckNative {
        name: name.to_owned(),
        ok: true,
        severity: "info",
...
fn doctor_ok(name: &str, message: &str) -> DoctorCheckNative {
    doctor_info(name, message)
}

fn doctor_warn(name: &str, message: &str, fixes: &[&str]) -> DoctorCheckNative {
    DoctorCheckNative {
        name: name.to_owned(),
        ok: false,
        severity: "warn",
        message: message.to_owned(),
        fixes: fixes.iter().map(|value| (*value).to_owned()).collect(),
    }
}

fn doctor_error(name: &str, message: &str, fixes: &[&str]) -> DoctorCheckNative {
    DoctorCheckNative {
        name: name.to_owned(),
        ok: false,
        severity: "error",
...
fn doctor_gateway_kind(options: &DoctorOptions) -> String {
    options
        .gateway
        .clone()
        .or_else(|| std::env::var("MAW_GATEWAY").ok())
        .or_else(doctor_config_gateway)
...
fn doctor_count_duplicate_peer_identities(store: &maw_peer::PeerStoreFile) -> (usize, Vec<String>) {
    ...
    for (alias, peer) in &store.peers {
        ...
        if let Some(first) = seen.insert(key.clone(), alias.clone()) {
            duplicates.push(format!("{key} shared by {first} and {alias}"));
        }
    }
...
fn doctor_peer_env() -> maw_peer::PeerStoreEnv {
    ...
    let vars = [
        ...
        "XDG_STATE_HOME",
        "PEERS_FILE",
        "MAW_PEER_STALE_TTL_MS",
    ];
    maw_peer::PeerStoreEnv::with_vars(
        home,
        vars.into_iter()
            .filter_map(|key| std::env::var(key).ok().map(|value| (key, value))),
    )
...
fn doctor_count_worktree_dirs(ghq: &std::path::Path) -> usize {
    ...
    for part in ["github.com", "gitlab.com"] {
        ...
        if let Ok(orgs) = std::fs::read_dir(root) {
            count += orgs.flatten().filter(|entry| entry.path().is_dir()).count();
        }
    }
    count
}
...
fn doctor_plural(count: usize) -> &'static str {
    if count == 1 {
        ""
    } else {
        "s"
...
mod doctor_tests {
    ...
    struct DoctorEnvRestore {
        key: &'static str,
        value: Option<std::ffi::OsString>,
    }

    impl DoctorEnvRestore {
    ...
    fn doctor_strings(values: &[&str]) -> Vec<String> {
    ...
    fn doctor_temp_root(name: &str) -> std::path::PathBuf {
        ...
        let path =
            std::env::temp_dir().join(format!("maw-rs-doctor-{name}-{}-{seq}", std::process::id()));
    ...
    fn doctor_seed_env(
        ...
        fs::write(
            config.join("maw/maw.config.json"),
            "{\"node\":\"doctor-test\",\"gateway\":\"bun\"}\n",
        )
        .expect("config write");
    ...
    fn doctor_parse_flags_and_rejects_leading_dash_values() {
        let parsed = doctor_parse_args(&doctor_strings(&[
            "--json",
            "--gateway=rust",
            "--port",
            "3457",
    ...
    fn doctor_json_uses_seeded_xdg_without_real_home() {
        ...
        assert!(parsed["checks"][0]["message"]
            .as_str()
            .expect("message")
            .contains("json-xdg"));
    }
    ...
    fn doctor_reported_version(message: &str) -> &str {
        message
            ...
            .unwrap_or_else(|| {
                panic!("doctor message must lead with `maw-rs <version>`: {message}")
            })
    }
    ...
    fn doctor_version_checks_agree_with_top_level_version() {
        ...
        for (args, check_index, check_name) in [
            ...
            let message = parsed["checks"][check_index]["message"]
                .as_str()
                .expect("message");
            ...
            assert_eq!(
                reported,
                super::MAW_RS_BUILD_VERSION,
                "{check_name} must report the embedded build version"
            );
            assert!(
            ...
            if super::MAW_RS_BUILD_VERSION != env!("CARGO_PKG_VERSION") {
                assert!(
                    ...
                    "{check_name} leaked CARGO_PKG_VERSION: {message}"
                );
            }
        }
    }
    ...
    fn doctor_warns_when_weighted_config_shadows_legacy_keys() {
        ...
        fs::write(
            ...
            r#"{"hooks":{"postWake":["echo hi"]},"node":"legacy"}"#,
        )
        .expect("legacy");
        fs::write(
            config.join("maw.config.50.json"),
            r#"{"commands":{"default":"codex"}}"#,
        )
        .expect("weighted");
    ...
    fn doctor_warns_when_backup_has_lost_post_wake_hook() {
        ...
        assert_eq!(output.code, 1);
        let parsed: serde_json::Value = serde_json::from_str(&output.stdout).expect("json");
        let loss = parsed["checks"]
            .as_array()
            .expect("checks")
            .iter()
            .find(|check| check["name"] == "config:backup-key-loss")
            .expect("backup warning");
        assert!(loss["message"]
            .as_str()
            .expect("message")
            .contains("hooks.postWake"));
    }
    ...
    fn doctor_xdg_migrate_dry_run_has_golden_shape() {
        ...
        assert_eq!(
            output.stdout,
            include_str!("../../tests/fixtures/native-doctor/xdg-migrate-dry-run.json")
        );
    }
    ...
    fn doctor_plugin_env(temp: &std::path::Path, name: &str) -> Vec<DoctorEnvRestore> {
        ...
        std::env::set_var(
            "MAW_PLUGINS_LOCK",
            temp.join(format!("{name}-plugins.lock")),
        );
    ...
    fn doctor_flags_stale_registry_meta_artifacts() {
        ...
        let plugin_dir = temp.join("stale-artifacts/xdg-data/maw/plugins/broadcast");
        ...
        assert!(
            message.contains("no longer matches the manifest artifact pin"),
            "{message}"
        );
