const DISPATCH_71: &[DispatcherEntry] = &[
    DispatcherEntry {
        command: "artifacts",
        handler: Handler::Sync(run_artifacts_command),
    },
    DispatcherEntry {
        command: "artifact",
        handler: Handler::Sync(run_artifacts_command),
    ...
    for arg in argv {
        match arg.as_str() {
            ...
            value if value.starts_with('-') => {
                return Err(format!("artifacts: unknown argument {value}"))
            }
            value => {
                artifacts_validate_value(value, "argument")?;
...
fn artifacts_options_from_positionals(
    json: bool,
    positionals: &[String],
) -> Result<ArtifactsOptions, String> {
    match positionals {
        [] => Ok(ArtifactsOptions {
            action: ArtifactsAction::List,
            json,
            team: None,
            task_id: None,
        }),
        [sub] if matches!(sub.as_str(), "ls" | "list") => Ok(ArtifactsOptions {
            action: ArtifactsAction::List,
            json,
            team: None,
            task_id: None,
        }),
        [sub, team] if matches!(sub.as_str(), "ls" | "list") => Ok(ArtifactsOptions {
            action: ArtifactsAction::List,
            json,
            team: Some(team.clone()),
            task_id: None,
        }),
        [sub, team, task] if matches!(sub.as_str(), "get" | "show") => Ok(ArtifactsOptions {
            action: ArtifactsAction::Get,
            json,
            team: Some(team.clone()),
            task_id: Some(task.clone()),
        }),
        [sub, ..] if matches!(sub.as_str(), "get" | "show") => {
            Err("usage: maw artifacts get <team> <task-id>".to_owned())
        }
    ...
    if items.is_empty() {
        return Ok(format!(
            ...
            options
                .team
                .as_ref()
                .map_or(String::new(), |team| format!(" (team: {team})"))
        ));
    }
...
fn artifacts_get(options: &ArtifactsOptions) -> Result<String, String> {
    let team = options
        .team
        .as_ref()
        .ok_or_else(|| "usage: maw artifacts get <team> <task-id>".to_owned())?;
    let task_id = options
        .task_id
        .as_ref()
        .ok_or_else(|| "usage: maw artifacts get <team> <task-id>".to_owned())?;
    let artifact = artifacts_read_full(team, task_id)?
        .ok_or_else(|| format!("artifact not found: {team}/{task_id}"))?;
    if options.json {
        return artifacts_json(&artifact);
    }
...
fn artifacts_list_items(team_filter: Option<&str>) -> Result<Vec<ArtifactsSummary>, String> {
    ...
    for root in artifacts_roots_for_read() {
        ...
        for team in teams {
            artifacts_collect_team(&root, &team, &mut seen, &mut results)?;
        }
    }
...
fn artifacts_collect_team(
    root: &std::path::Path,
    team: &str,
    seen: &mut std::collections::BTreeSet<String>,
    results: &mut Vec<ArtifactsSummary>,
) -> Result<(), String> {
    let team_dir = root.join(team);
    let Ok(entries) = std::fs::read_dir(&team_dir) else {
        return Ok(());
    };
    ...
    for task in tasks {
        artifacts_collect_task(team, &task.path(), seen, results)?;
    }
    Ok(())
}

fn artifacts_collect_task(
    team: &str,
    task_dir: &std::path::Path,
    seen: &mut std::collections::BTreeSet<String>,
    results: &mut Vec<ArtifactsSummary>,
) -> Result<(), String> {
    let Some(task_id) = task_dir.file_name().and_then(std::ffi::OsStr::to_str) else {
        return Ok(());
    };
    ...
    results.push(ArtifactsSummary {
        team: team.to_owned(),
        task_id: task_id.to_owned(),
        subject: meta.subject,
        status: meta.status,
        owner: meta.owner,
        files: artifacts_file_count(task_dir),
        has_result: task_dir.join("result.md").is_file(),
...
fn artifacts_read_full(team: &str, task_id: &str) -> Result<Option<ArtifactsFull>, String> {
    let Some(dir) = artifacts_existing_dir(team, task_id) else {
        return Ok(None);
    };
...
fn artifacts_roots_for_read() -> Vec<std::path::PathBuf> {
    ...
    if let Some(legacy) = artifacts_legacy_root() {
        if legacy != roots[0] {
            roots.push(legacy);
        }
    }
...
fn artifacts_root() -> std::path::PathBuf {
    maw_cache_path(&current_xdg_env(), &["artifacts"])
}

fn artifacts_legacy_root() -> Option<std::path::PathBuf> {
    if std::env::var_os("MAW_HOME").is_some() || std::env::var_os("MAW_CACHE_DIR").is_some() {
        return None;
    }
    Some(std::env::var_os("HOME").map_or_else(
        || std::path::PathBuf::from(".maw/artifacts"),
        |home| std::path::PathBuf::from(home).join(".maw/artifacts"),
    ))
}

fn artifacts_read_meta(path: &std::path::Path) -> Result<ArtifactsMeta, String> {
    let text = std::fs::read_to_string(path)
        .map_err(|error| format!("artifacts: cannot read {}: {error}", path.display()))?;
    serde_json::from_str(&text)
        .map_err(|error| format!("artifacts: invalid json {}: {error}", path.display()))
}
...
fn artifacts_file_count(task_dir: &std::path::Path) -> usize {
    ...
    let attachments = std::fs::read_dir(task_dir.join("attachments"))
        .map_or(0, |entries| entries.flatten().count());
    direct + attachments
}

fn artifacts_attachments(dir: &std::path::Path) -> Vec<String> {
    let Ok(entries) = std::fs::read_dir(dir.join("attachments")) else {
        return Vec::new();
    };
...
fn artifacts_render_table(items: &[ArtifactsSummary]) -> String {
    ...
    artifacts_push_padded(&mut out, "TEAM", 18);
    artifacts_push_padded(&mut out, "TASK", 6);
    artifacts_push_padded(&mut out, "STATUS", 12);
    artifacts_push_padded(&mut out, "OWNER", 16);
    artifacts_push_padded(&mut out, "FILES", 6);
    artifacts_push_padded(&mut out, "RESULT", 8);
    out.push_str("SUBJECT\n");
    out.push_str(&"-".repeat(90));
    out.push('\n');
    for item in items {
        artifacts_push_row(&mut out, item);
    }
    out
}
...
fn artifacts_render_full(artifact: &ArtifactsFull) -> String {
    ...
    let _ = writeln!(
        ...
        artifacts_color_status(&artifact.meta.status)
    );
    let _ = writeln!(
        out,
        "Owner: {} | Created: {}",
        artifact.meta.owner.as_deref().unwrap_or("-"),
    ...
    out.push_str(artifact.spec.trim());
    out.push_str("\n\n");
...
fn artifacts_push_result(out: &mut String, artifact: &ArtifactsFull) {
    if let Some(result) = &artifact.result {
        out.push_str("\x1b[32m── result.md ──\x1b[0m\n");
        out.push_str(result.trim());
        out.push_str("\n\n");
    } else {
        out.push_str("\x1b[90m(no result.md yet)\x1b[0m\n\n");
    }
}
...
fn artifacts_color_status(status: &str) -> String {
    match status {
        "completed" => "\x1b[32mcompleted\x1b[0m".to_owned(),
        "in_progress" => "\x1b[33min_progress\x1b[0m".to_owned(),
        _ => status.to_owned(),
    }
}
...
mod artifacts_tests {
    ...
    fn artifacts_parser_rejects_leading_dash_values() {
        ...
        assert_eq!(
            artifacts_parse_args(&argv).unwrap_err(),
